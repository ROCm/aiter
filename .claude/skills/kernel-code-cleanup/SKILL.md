...
description: >
  ...
  reviewing, cleaning, or migrating existing kernels in aiter.
allowed-tools: Read Edit Bash Grep Glob Agent
---

# FlyDSL Kernel Code Cleanup (aiter)
...
wrappers like `moe_kernels.py` still carry a local `_run_compiled(exe, args)` with
a list arg; new code should import from `tensor_shim`.)
...
- **`flydsl-internal-types-cleanup` is superseded** for kernel migration — it still
  recommends deprecated `ArithValue` / `fx.Index`; follow this skill instead.
...
- aiter legacy code may still use `_to_raw` from `tensor_shim` at `scf.IfOp`
  boundaries — replace with `.ir_value()` when touching those call sites.
...
| per-call `@flyc.jit` on a hot path | `_run_compiled(exe, *args)` via `tensor_shim` |
