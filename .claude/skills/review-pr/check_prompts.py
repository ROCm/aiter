...
# (prompt file, start anchor, end anchor) — anchors are unique lines in SKILL.md.
CHECKS = [
    (
        "prompts/refuter.md",
...
def main():
    ...
    for prompt, start, end in CHECKS:
        ...
        if block is None:
            ...
            print(
                f"✅ {prompt}: quoted block matches SKILL.md verbatim ({len(block)} chars)"
            )
        else:
            print(
                f"❌ {prompt}: quoted block DRIFTED from SKILL.md — re-copy the section verbatim"
            )
            bad += 1
