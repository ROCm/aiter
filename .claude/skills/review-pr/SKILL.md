...
**Read `stages.perf.measurements[]` before deciding.** The stage can now time more than one target — the bench the repository already owns and the bench the PR shipped — and the fields at the top mirror whichever of them gated. So a top-level `skip` means *every* target skipped, and a top-level `pass`/`fail` can sit beside an entry that skipped. Report the number from the gating entry, name which file produced it (`target`) and on whose authority it was chosen (`target_basis`), and do not fire P6 because a second target went unmeasured.

**Do not re-file what the validator already filed.** A python kernel changed in code with neither path finding anything to time it now carries its own `should-fix` from the deterministic layer, with the changed modules named. Carry that finding into the card; firing P6 alongside it reports one gap twice. What is left for P6 is the cases the layer deliberately declines to charge anyone for:

- **discovery found several candidates and refused to choose** — `stages.perf.candidates` lists them and the report says a choice was owed. This is a caller-side gap, not the author's: name the candidates and say the run should be repeated with `--perf-target`.
- **only `csrc/` changed** — every validator target runs under `AITER_TRITON_ONLY=1`, so nothing it can time reaches that code. Fire P6 and say the timing has to come from outside this tool; do not write it up as a gap in the PR.
...
- **the PR adds the target and no control column was named** — the cross-tree transplant is only attributable when a column the patch does not touch reproduces across it. Fire P6 asking for the re-run with `--perf-control-column`, not for a benchmark.
...
- **the run never reached both phases** — no idle GPU, or a base tree that would not come clean. The report is `INCONCLUSIVE` and the missing number is the least of it; fire P6 only after the blocking problem is named.
...
FP self-check: do NOT fire when the triage says NOT REQUIRED (no runtime surface), when `stages.perf` is `pass`/`fail`, when the validator already filed its own `should-fix` for the same gap, when a manual measurement was taken and reported, or when the PR's own harness is the thing under test and has no steady state to measure. A single sample on a shared box is weak evidence — report the spread or the sample count rather than one number.
