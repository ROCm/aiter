...
def _token():
    ...
    if tf:
        try:
            m = re.findall(r'(?:ghp_|github_pat_)[A-Za-z0-9_]+',
    ...
    try:
        for line in open(os.path.expanduser('~/.git-credentials'), encoding='utf-8'):
            if 'github.com' in line and ':' in line:
                return line.split('://', 1)[1].split('@')[0].split(':', 1)[1].strip()
    except OSError:
        pass
    return ''
...
def _api(path, method='GET', body=None):
    ...
    try:
        return json.loads(subprocess.run(cmd, capture_output=True, text=True).stdout or 'null')
...
def head_sha(pr):
    try:
        d = json.loads((REPORTS / f'PR-{pr}' / 'pr_meta.json').read_text(encoding='utf-8'))
...
def decide(pr):
    ...
    card = (d / 'card.md')
    if not card.is_file():
        return 'SKIP', 'reports/PR-%s/card.md does not exist (run run_one first)' % pr
    pull = _api(f'pulls/{pr}')
    if isinstance(pull, dict) and (pull.get('merged_at') or pull.get('state') != 'open'):
        return 'SKIP', 'PR already %s (post-merge follow-up, should not go as a PR review)' % (
            'merged' if pull.get('merged_at') else pull.get('state'))
    sha = head_sha(pr)
    if sha and already_posted(pr, sha):
        return 'SKIP', 'same head %s already posted (marker hit)' % sha[:9]
...
def main(argv):
    ...
    ap.add_argument('--post', action='store_true', help='actually post; default dry-run prints the decision only')
    a = ap.parse_args(argv)
    print(f'=== publish gate  [{"POST" if a.post else "DRY-RUN"}]  identity={BOT}  repo={REPO} ===')
    held, posted = [], []
    for pr in a.prs:
