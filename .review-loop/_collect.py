...
if not pr:
    sys.exit(f"cannot read PR number ({W}/pr_meta.json missing or corrupt); pass it explicitly: collect.sh {W} <PR_number>")
...
if missing:
    ...
    sys.exit(f"refusing to collect: {W} is missing required artifacts {', '.join(missing)}.\n"
...
if _lib.nonempty(os.path.join(W, 'independent.txt')):
    ...
    if 'not independently refuted' in _t:
        _t2 = _t.replace(' — not independently refuted', '').replace(' -- not independently refuted', '')
        _card.write_text(_t2, encoding='utf-8')
        print("  removed 'not independently refuted' from the card (independent.txt exists)")

copied = 0
for f in _lib.REQUIRED + _lib.OPTIONAL:
    src = os.path.join(W, f)
    if _lib.nonempty(src):
        shutil.copy2(src, D / f); copied += 1
...
for name, rc, first, full in res:
    lines.append(f"  {'✅' if rc == 0 else '❌'} {name:<12} "
                 + (first if rc == 0 else f"exit={rc}\n" + '\n'.join('       ' + l for l in full.split('\n'))))
...
if npass != 7:
    print(f"  ⚠️ this report has {7-npass} gate(s) unpassed — the index marks it; do not treat it as a conclusion")
...
print(f"  ↳ not landed yet: git commit reports/PR-{pr}/ to finish. run ./pending.sh anytime to find unstaged reports.")
