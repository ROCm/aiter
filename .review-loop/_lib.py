...
REQUIRED = ['card.md', 'verdicts.txt', 'answers.txt', 'ai_diagnostic.txt',
            'core_files.txt', 'refutations.txt', 'independent.txt', 'rules.txt', 'pr_meta.json']
...
def card_findings(work):
    return [l for l in read(os.path.join(work, 'card.md')).split('\n') if FINDING_RE.match(l)]
...
def run_gates(work, proj):
    ...
    for f in ['answers.txt', 'ai_diagnostic.txt', 'core_files.txt', 'verdicts.txt',
              'refutations.txt', 'independent.txt', 'late_findings.txt']:
    ...
    specs = [
        ...
        ('card',        ['card', W('card.md'), W('verdicts.txt'), W('ai_diagnostic.txt'),
                         W('answers.txt'), W('pr.diff'), W('late_findings.txt')]),
        ('refutations', ['refutations', W('refutations.txt'), W('pr.diff'), W('card.md')]),
        ('independent', ['independent', W('independent.txt'), W('card.md')]),
    ]
    env = dict(os.environ, PYTHONUTF8='1')
    res = []
    for name, args in specs:
        p = subprocess.run([sys.executable, sk] + args, cwd=proj, env=env,
                           capture_output=True, text=True)
        out = (p.stdout + p.stderr).strip().split('\n')
        res.append((name, p.returncode, out[0] if out and out[0] else '(no output)',
                    '\n'.join(out[:10])))
    return res
