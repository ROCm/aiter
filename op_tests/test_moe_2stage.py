...
def test_bm16_tiled_scale_boundary():
    ...
    w1_scale = filled((experts, inter_dim * 2, model_dim // 32), 127, dtypes.fp8_e8m0)
    ...
    def assert_match(actual, expected):
        for actual_stage, expected_stage, tolerance in zip(actual, expected, (0, 0.02)):
    ...
    def run_stage1_scale_boundary(block_m):
        boundary_token = 33
        boundary_topk_ids = torch.zeros((boundary_token, 1), dtype=torch.int32)
        boundary_topk_weights = torch.ones((boundary_token, 1), dtype=torch.float32)
        boundary_activation = (
            (
                torch.linspace(0.25, 1.0, 32, dtype=torch.float32)
                * torch.exp2(
                    (torch.arange(boundary_token * model_dim // 32) % 8 - 8).view(
                        boundary_token, -1, 1
                    )
                )
            )
            .reshape(boundary_token, model_dim)
            .to(torch.bfloat16)
        )
        boundary_w1 = filled((1, inter_dim * 2, model_dim // 2), 0x11, dtypes.fp4x2)
