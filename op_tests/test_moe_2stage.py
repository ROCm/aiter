...
def test_fmoe(
    ...
    if use_g1u1:
        ...
        exp_bias1 = (
            torch.clamp(torch.randn((E, inter_dim * 2), dtype=dtype), -1.0, 1.0)
            if bias
            else None
        )
    else:
        ...
        exp_bias1 = (
            torch.clamp(torch.randn((E * inter_dim), dtype=dtype), -1.0, 1.0)
            if bias
            else None
        )
    ...
    if hidden_pad != 0:
        w2[:, -hidden_pad:, :] = 0
    exp_bias2 = (
        torch.clamp(torch.randn((E, model_dim), dtype=dtype), -1.0, 1.0)
        if bias
        else None
    )
