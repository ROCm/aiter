...
from aiter.ops.triton.fused_routing_from_topk import fused_routing_from_topk
...
def test_fused_routing_from_topk_rejects_oversize():
    """NK above the single-CTA budget must raise ValueError so callers
    can dispatch to a multi-kernel reference path."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    n_tokens, n_expts_act, n_expts_tot = 1024, 8, 384  # NK = 8192 > 4096
    topk_ids, topk_weights = _make_inputs(
        n_tokens, n_expts_act, n_expts_tot, torch.float32, DEVICE, seed=0
    )
    with pytest.raises(ValueError, match="single-CTA budget"):
        fused_routing_from_topk(topk_weights, topk_ids, n_expts_tot)
