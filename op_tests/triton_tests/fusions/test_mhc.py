...
def _make_hres_op_config(hres_op: int, num_ksplit: int = 1) -> dict:
    """
    Helper to create config with specified HRES_OP value.
    
    Args:
        hres_op: 0 for FMA loop, 1 for dot product
        num_ksplit: Number of K-splits (default: 1 for non-split-K path)
    
    Returns:
        Config dict with HRES_OP and required kernel parameters
    """
    return {
        "waves_per_eu": 1,
        "num_stages": 1,
...
@pytest.mark.parametrize("M, n, C", get_lite_test_shapes())
@pytest.mark.parametrize("hres_op", [0, 1])
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_hres_op_correctness(M, n, C, hres_op, dtype):
    """
    Test that both HRES_OP modes match PyTorch reference implementation.
    
    HRES_OP controls the computation strategy in _compute_hres_mhc_lite:
    - HRES_OP=0: FMA loop with explicit iteration over permutations
    - HRES_OP=1: Dot product with tl.dot for matrix multiplication
    
    Both should produce results matching mhc_lite_torch reference.
    """
    torch.cuda.empty_cache()
    torch.cuda.synchronize()
    ...
    # Reference: PyTorch implementation
    H_pre_ref, H_post_ref, H_res_ref = mhc_lite_torch(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams
    )

    # Test: Triton kernel with specified HRES_OP
    H_pre_triton, H_post_triton, H_res_triton = mhc(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams,
        hres_mode="lite",
        config=_make_hres_op_config(hres_op)
    )
    ...
    # H_res should match - both produce exact doubly stochastic
    torch.testing.assert_close(
        H_res_triton.to(torch.float32),
        H_res_ref.to(torch.float32),
        atol=5e-2,
...
@pytest.mark.parametrize("M, n, C", [(16, 4, 512), (32, 4, 1024), (64, 2, 1024)])
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_hres_op_mode_equivalence(M, n, C, dtype):
    """
    Test that HRES_OP=0 and HRES_OP=1 produce identical outputs.
    
    Both modes implement the same mathematical operation:
    H_res = softmax(logits) @ P
    
    They differ only in implementation strategy:
    - HRES_OP=0: FMA loop with direct exp-normalize softmax
    - HRES_OP=1: Dot product with log-domain softmax
    ...
    # HRES_OP=0: FMA loop
    H_pre_fma, H_post_fma, H_res_fma = mhc(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams,
        hres_mode="lite",
        config=_make_hres_op_config(0)
    )

    # HRES_OP=1: Dot product
    H_pre_dot, H_post_dot, H_res_dot = mhc(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams,
        hres_mode="lite",
        config=_make_hres_op_config(1)
    )

    # Pre/post streams should be identical (HRES_OP only affects res stream)
    torch.testing.assert_close(
        H_pre_fma.to(torch.float32),
        H_pre_dot.to(torch.float32),
        atol=1e-5,
        rtol=1e-5,
    )
    torch.testing.assert_close(
        H_post_fma.to(torch.float32),
        H_post_dot.to(torch.float32),
        atol=1e-5,
        rtol=1e-5,
    )
    # H_res should be numerically close (different softmax implementations)
    torch.testing.assert_close(
        H_res_fma.to(torch.float32),
        H_res_dot.to(torch.float32),
        atol=5e-2,
        rtol=5e-2,
    )
...
@pytest.mark.parametrize("M, n, C", get_lite_test_shapes())
@pytest.mark.parametrize("hres_op", [0, 1])
def test_hres_op_doubly_stochastic(M, n, C, hres_op):
    """
    Test that both HRES_OP modes produce exact doubly stochastic matrices.
    
    mHC-lite guarantees exact doubly stochasticity by construction via
    the Birkhoff-von Neumann theorem (convex combination of permutations).
    ...
    assert is_doubly_stochastic(H_res_3d, tol=5e-3), (
        f"HRES_OP={hres_op}: H_res is not doubly stochastic! "
        f"Row sums mean: {H_res_3d.sum(dim=-1).mean():.6f}, "
        f"Col sums mean: {H_res_3d.sum(dim=-2).mean():.6f}"
    )


@pytest.mark.parametrize("hres_op", [0, 1])
@pytest.mark.parametrize("value_scale", [0.01, 1.0, 10.0])
def test_hres_op_numerical_stability(hres_op, value_scale):
    """
    Test numerical stability of both HRES_OP modes with extreme input values.
    
    Tests:
    - Small values (scale=0.01): Near-uniform softmax weights
    - Normal values (scale=1.0): Typical operating range
    - Large values (scale=10.0): Peaked softmax (near-permutation matrices)
    
    Validates no NaN/Inf and doubly stochastic output.
    """
    torch.cuda.empty_cache()

    M, n, C = 32, 4, 1024
    x, phi_pre, phi_post, phi_res, _, _, _, bias, n_streams = generate_mhc_inputs(M, n, C, hres_mode="lite")
    
    # Scale inputs to create extreme scenarios
    x = x * value_scale
    alpha_pre = alpha_post = alpha_res = 1.0

    H_pre, H_post, H_res = mhc(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams,
        hres_mode="lite",
        config=_make_hres_op_config(hres_op)
    )
    ...
    assert not torch.isnan(H_res).any(), f"HRES_OP={hres_op}, scale={value_scale}: H_res contains NaN"
    assert not torch.isinf(H_res).any(), f"HRES_OP={hres_op}, scale={value_scale}: H_res contains Inf"
    
    # Check valid probability range
    H_res_f32 = H_res.to(torch.float32)
    assert torch.all(H_res_f32 >= -1e-6), f"HRES_OP={hres_op}, scale={value_scale}: H_res has negative values"
    assert torch.all(H_res_f32 <= 1.0 + 1e-6), f"HRES_OP={hres_op}, scale={value_scale}: H_res has values > 1"
    
    # Verify doubly stochastic (relaxed tolerance for extreme scales)
    H_res_3d = H_res.view(M, n, n).to(torch.float32)
    tol = 0.1 if value_scale >= 10.0 else 5e-3
    assert is_doubly_stochastic(H_res_3d, tol=tol), (
        f"HRES_OP={hres_op}, scale={value_scale}: H_res is not doubly stochastic"
    )
...
@pytest.mark.parametrize("hres_op", [0, 1])
@pytest.mark.parametrize("num_ksplit", [2, 4])
def test_hres_op_split_k(M, n, C, hres_op, num_ksplit):
    """
    Test that both HRES_OP modes work correctly with split-K kernels.
    
    Split-K divides the K (input feature) dimension across multiple programs,
    then reduces partial results in a separate kernel. The HRES_OP parameter
    is used in the reduce kernel (_mhc_fused_reduce_kernel).
    
    Validates that split-K with both HRES_OP modes matches PyTorch reference.
    """
    torch.cuda.empty_cache()
    torch.cuda.synchronize()
    ...
    # Test: split-K with specified HRES_OP
    H_pre_split, H_post_split, H_res_split = fused_mhc(
        x, phi_pre, phi_post, phi_res, alpha_pre, alpha_post, alpha_res, bias, n_streams,
        hres_mode="lite",
        config=_make_hres_op_config(hres_op, num_ksplit=num_ksplit)
    )
...
@pytest.mark.parametrize("hres_op", [0, 1])
def test_hres_op_output_range(hres_op):
    """
    Test that both HRES_OP modes produce H_res values in valid range [0, 1].
    ...
    # Verify pre-stream bounds (sigmoid: [0, 1])
    assert torch.all(H_pre >= 0.0), f"HRES_OP={hres_op}: Pre-stream has values < 0"
    assert torch.all(H_pre <= 1.0), f"HRES_OP={hres_op}: Pre-stream has values > 1"

    # Verify post-stream bounds (2*sigmoid: [0, 2])
    assert torch.all(H_post >= 0.0), f"HRES_OP={hres_op}: Post-stream has values < 0"
    assert torch.all(H_post <= 2.0), f"HRES_OP={hres_op}: Post-stream has values > 2"

    # Verify res-stream bounds (convex combination: [0, 1])
    H_res_f32 = H_res.to(torch.float32)
    assert torch.all(H_res_f32 >= -1e-6), f"HRES_OP={hres_op}: H_res has values < 0: min={H_res_f32.min()}"
    assert torch.all(H_res_f32 <= 1.0 + 1e-6), f"HRES_OP={hres_op}: H_res has values > 1: max={H_res_f32.max()}"
