...
from aiter.ops.triton.attention.unified_attention import (
    ...
    unified_attention,
    use_gfx950_fp8_prefill_fast_path,
)
...
@pytest.mark.parametrize(
    "max_seqlen_q,sliding_window,all_decode,head_size,q_dtype,kv_dtype,shuffled,expected",
    [
        (255, 0, False, 128, e4m3_dtype, e4m3_dtype, False, False),
        (256, 0, False, 128, e4m3_dtype, e4m3_dtype, False, True),
        (256, 256, False, 128, e4m3_dtype, e4m3_dtype, False, False),
        (256, 0, True, 128, e4m3_dtype, e4m3_dtype, False, False),
        (256, 0, False, 64, e4m3_dtype, e4m3_dtype, False, False),
        (256, 0, False, 128, torch.bfloat16, e4m3_dtype, False, False),
        (256, 0, False, 128, e4m3_dtype, torch.bfloat16, False, False),
        (256, 0, False, 128, e4m3_dtype, e4m3_dtype, True, False),
    ],
)
def test_gfx950_fp8_prefill_fast_path_guard(
    max_seqlen_q,
    sliding_window,
    all_decode,
    head_size,
    ...
    if DEVICE_ARCH != "gfx950":
        pytest.skip("gfx950-only prefill fast path")
...
@pytest.mark.parametrize(
    "seq_lens,q_dtype,kv_dtype,block_size,sliding_window",
    [
        ([(255, 512)], e4m3_dtype, e4m3_dtype, 16, None),
        ([(256, 512)], e4m3_dtype, e4m3_dtype, 16, None),
        ([(257, 1024)], e4m3_dtype, e4m3_dtype, 16, None),
        ([(512, 1024)], e4m3_dtype, e4m3_dtype, 64, None),
        ([(512, 8192)], e4m3_dtype, e4m3_dtype, 16, None),
        (
            [(128, 1024), (384, 1024)],
            torch.bfloat16,
            e4m3_dtype,
        ...
        ([(512, 1024)], e4m3_dtype, e4m3_dtype, 16, 256),
    ],
)
@torch.inference_mode()
def test_gfx950_prefill_fast_path(
    seq_lens,
    q_dtype,
    kv_dtype,
    block_size,
    ...
    kv_lens_list = [kv_len for _, kv_len in seq_lens]
    (
        query,
        key_cache,
        value_cache,
    ...
    assert (
        checkAllclose(
            ...
            msg="gfx950 unified attention prefill fast path",
        )
        <= tol_err_ratio
    )
