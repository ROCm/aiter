...
_GFX942_GRAPH_HEAD_DIM_CKV = 512
_GFX942_GRAPH_HEAD_DIM_KPE = 64
_GFX942_GRAPH_NHEAD = 12
_GFX942_GRAPH_BLOCK_SIZE = 768
_GFX942_GRAPH_SM_SCALE = 1.0 / math.sqrt(
    _GFX942_GRAPH_HEAD_DIM_CKV + _GFX942_GRAPH_HEAD_DIM_KPE
)


if _GLUON_AVAILABLE and DEVICE_ARCH == "gfx942":

    @dataclass
    class _Gfx942GraphMlaCase:
        q_nope: torch.Tensor
        q_pe: torch.Tensor
        kv_buffer: torch.Tensor
        output: torch.Tensor
    ...
    def _gfx942_graph_physical_indices(
        seq_lens: list[int], num_physical_blocks: int
    ) -> list[int]:
        logical_blocks = sum(
            math.ceil(seq_len / _GFX942_GRAPH_BLOCK_SIZE) for seq_len in seq_lens
        )
        ...
        for seq_len in seq_lens:
            ...
            while remaining:
                ...
                block_tokens = min(remaining, _GFX942_GRAPH_BLOCK_SIZE)
                block_start = physical_block * _GFX942_GRAPH_BLOCK_SIZE
                indices.extend(range(block_start, block_start + block_tokens))
                remaining -= block_tokens
                logical_block += 1
        return indices

    def _make_gfx942_graph_case(
        batch_size: int,
        qlen: int,
        seq_lens: list[int],
        seed: int,
        ...
        kv_buffer = (
            torch.randn(
                (
                    num_physical_blocks * _GFX942_GRAPH_BLOCK_SIZE,
                    _GFX942_GRAPH_HEAD_DIM_CKV + _GFX942_GRAPH_HEAD_DIM_KPE,
                ),
                device="cuda",
                dtype=torch.bfloat16,
        ...
        q_shape = (
            (batch_size, _GFX942_GRAPH_NHEAD, _GFX942_GRAPH_HEAD_DIM_CKV)
            if qlen == 1
            else (batch_size, qlen, _GFX942_GRAPH_NHEAD, _GFX942_GRAPH_HEAD_DIM_CKV)
        )
        q_pe_shape = (
            (batch_size, _GFX942_GRAPH_NHEAD, _GFX942_GRAPH_HEAD_DIM_KPE)
            if qlen == 1
            else (
                batch_size,
                qlen,
                _GFX942_GRAPH_NHEAD,
                _GFX942_GRAPH_HEAD_DIM_KPE,
            )
        )
        q_nope = (
            torch.randn(
                q_shape, device="cuda", dtype=torch.bfloat16, generator=generator
            )
            * 0.2
        )
        q_pe = (
        ...
        indices = _gfx942_graph_physical_indices(seq_lens, num_physical_blocks)
        indptr = [0]
        for seq_len in seq_lens:
            indptr.append(indptr[-1] + seq_len)

        return _Gfx942GraphMlaCase(
            q_nope=q_nope,
            q_pe=q_pe,
            kv_buffer=kv_buffer,
            output=output,
    ...
    def _gfx942_graph_reference(case: _Gfx942GraphMlaCase) -> torch.Tensor:
        ...
        for batch_idx in range(batch_size):
            ...
            for q_pos in range(qlen):
                ...
                scores = torch.einsum(
                    "hd,td->ht",
                    q_nope[batch_idx, q_pos].float(),
                    k_nope[:causal_end],
                )
                scores += torch.einsum(
                    "hd,td->ht",
                    q_pe[batch_idx, q_pos].float(),
                    k_pe[:causal_end],
                )
    ...
    def _assert_gfx942_graph_matches_reference(case: _Gfx942GraphMlaCase) -> None:
        assert torch.isfinite(case.output).all().item()
        torch.testing.assert_close(
            case.output,
            _gfx942_graph_reference(case),
            atol=1.5e-2,
            rtol=1.0e-2,
        )

    def _run_gfx942_graph_decode(case: _Gfx942GraphMlaCase) -> None:
        mla_decode_fwd(
            None,
            case.kv_buffer,
            case.output,
            ...
            0,
            None,
            _GFX942_GRAPH_SM_SCALE,
            _GFX942_GRAPH_HEAD_DIM_CKV,
            _GFX942_GRAPH_HEAD_DIM_KPE,
            True,
            None,
            None,
            q_nope=case.q_nope,
            q_pe=case.q_pe,
            page_table=case.kv_indices,
            seq_info=case.kv_indptr,
        )
    ...
    def test_gfx942_graph_combined_physical_cache_ragged(
        batch_size: int,
        qlen: int,
        seq_lens: list[int],
    ) -> None:
        case = _make_gfx942_graph_case(
            batch_size, qlen, seq_lens, seed=17 + batch_size + qlen
        )
        _run_gfx942_graph_decode(case)
        _assert_gfx942_graph_matches_reference(case)

    def test_gfx942_graph_low_level_split_reduction_forced_64bit_load() -> None:
        from aiter.ops.triton._gluon_kernels.gfx942.attention.mla import (
            _mla_gluon_gfx942,
        )

        case = _make_gfx942_graph_case(2, 1, [97, 121], seed=41)
        _mla_gluon_gfx942(
            case.q_nope,
            case.q_pe,
    ...
    def test_gfx942_graph_q_pe_validation(invalid_q_pe: str) -> None:
        from aiter.ops.triton._gluon_kernels.gfx942.attention.mla import (
            _mla_gluon_gfx942,
        )

        case = _make_gfx942_graph_case(1, 4, [25], seed=47)
    ...
    def test_gfx942_graph_cuda_graph_replay_with_metadata_mutation() -> None:
        case = _make_gfx942_graph_case(4, 4, [101, 113, 127, 139], seed=53)
        alternate = _make_gfx942_graph_case(4, 4, [139, 127, 113, 101], seed=59)
