...
def assert_scales_close(ref_scale, tri_scale, rtol=1e-4, atol=1e-6, verbose=True):
    ...
    if verbose:
        print(
            f"Scale comparison - max_diff: {max_diff:.6e}, mean_diff: {mean_diff:.6e}"
        )

    assert torch.allclose(
        ref_scale, tri_scale, rtol=rtol, atol=atol
...
def assert_int8_close(ref_int8, tri_int8, max_diff_allowed=1, verbose=True):
    ...
    if verbose:
        print(
            f"Int8 comparison - max_diff: {max_diff}, num_different: {num_different}/{ref_int8.numel()}, "
            f"num_large_diff (>{max_diff_allowed}): {num_large_diff}"
        )

    assert (
        max_diff <= max_diff_allowed
    ), f"Int8 values differ by more than {max_diff_allowed}: max_diff={max_diff}"


def dequantize_and_compare(
    x_int8, x_scale, x_original, smooth_scale, rtol=0.15, verbose=True
):
    ...
    if verbose:
        print(
            f"Dequant comparison - max_rel_error: {max_rel_error:.4f}, mean_rel_error: {mean_rel_error:.4f}"
        )

    # Allow reasonable quantization error for int8 (typically 10-15% mean error)
    assert (
        mean_rel_error < rtol
    ), f"Mean relative error {mean_rel_error} exceeds tolerance {rtol}"
...
def test_smoothquant_deterministic(device="cuda"):
    ...
    for i in range(1, len(results)):
        ...
        ), f"Int8 results differ at run {i}"
        assert torch.equal(
            results[0][1], results[i][1]
        ), f"Scale results differ at run {i}"
