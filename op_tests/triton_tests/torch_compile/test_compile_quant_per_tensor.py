...
def torch_dynamic_per_tensor_quant_fp8(x):
    x_float = x.to(torch.float32)
    amax = x_float.abs().amax().clamp(min=1e-12)
    scale = amax / FP8_MAX
    qx = (x_float / scale).clamp(-FP8_MAX, FP8_MAX).to(torch.float8_e4m3fnuz)
    return qx, scale.unsqueeze(0)


@pytest.mark.parametrize("M, N", [(64, 128), (128, 256), (256, 512)])
def test_compile_quant_per_tensor(M, N):
    torch.manual_seed(42)
    torch.cuda.empty_cache()
    x = torch.randn(M, N, device="cuda", dtype=torch.float16)

    qx_eager, scale_eager = torch_dynamic_per_tensor_quant_fp8(x)

    compiled_fn = _get_compiled(torch_dynamic_per_tensor_quant_fp8)
