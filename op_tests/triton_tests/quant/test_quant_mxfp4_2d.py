...
from aiter.ops.triton.quant import dynamic_mxfp4_quant_2d
...
def _torch_dynamic_mxfp4_quant_2d(
    x: torch.Tensor,
...
def test_dynamic_mxfp4_quant_2d_matches_reference(M: int, N: int, dtype):
    torch.manual_seed(20)
    x = torch.randn((M, N), dtype=dtype, device="cuda") * 4.0

    actual_packed, actual_scales = dynamic_mxfp4_quant_2d(x)
    expected_packed, expected_scales = _torch_dynamic_mxfp4_quant_2d(x)
...
def test_dynamic_mxfp4_quant_2d_even_scale_tie():
    below_tie = torch.nextafter(torch.tensor(1.75), torch.tensor(0.0)).item()
    x = torch.zeros((64, 32), dtype=torch.float32, device="cuda")
    x[0, 0] = below_tie
    x[32, 0] = 1.75

    _, scales = dynamic_mxfp4_quant_2d(x, scaling_mode="even")

    # EVEN uses the 1.75x threshold when rounding amax to a power of two.
    torch.testing.assert_close(
        scales.cpu(), torch.tensor([[125], [126]], dtype=torch.uint8), atol=0, rtol=0
    )


def test_dynamic_mxfp4_quant_2d_payload_rne_ties():
    ...
    packed, scales = dynamic_mxfp4_quant_2d(x)
...
def test_dynamic_mxfp4_quant_2d_is_transpose_invariant():
    torch.manual_seed(37)
    x = torch.randn((64, 96), dtype=torch.float32, device="cuda")

    packed, scales = dynamic_mxfp4_quant_2d(x)
    packed_t, scales_t = dynamic_mxfp4_quant_2d(x.t().contiguous())
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float32])
def test_dynamic_mxfp4_quant_2d_e8m0_endpoints(dtype):
    zeros = torch.zeros((32, 32), dtype=dtype, device="cuda")
    zero_packed, zero_scale = dynamic_mxfp4_quant_2d(zeros)
    ...
    smallest_packed, smallest_scale = dynamic_mxfp4_quant_2d(smallest)
    ...
    largest_packed, largest_scale = dynamic_mxfp4_quant_2d(largest)
...
def test_dynamic_mxfp4_quant_2d_rejects_empty_dimensions(shape):
    x = torch.empty(shape, dtype=torch.bfloat16, device="cuda")
...
def test_dynamic_mxfp4_quant_2d_rejects_ragged_tiles(shape):
    x = torch.empty(shape, dtype=torch.bfloat16, device="cuda")
    with pytest.raises(ValueError, match="divisible by 32"):
        dynamic_mxfp4_quant_2d(x)


def test_dynamic_mxfp4_quant_2d_rejects_noncontiguous_input():
...
def test_dynamic_mxfp4_quant_2d_rejects_cpu_input():
    x = torch.empty((32, 32), dtype=torch.bfloat16)
    with pytest.raises(ValueError, match="must be a CUDA tensor"):
        dynamic_mxfp4_quant_2d(x)


def test_dynamic_mxfp4_quant_2d_rejects_unknown_scaling_mode():
    x = torch.empty((32, 32), dtype=torch.bfloat16, device="cuda")
    with pytest.raises(ValueError, match="scaling_mode must be 'even'"):
        dynamic_mxfp4_quant_2d(x, scaling_mode="ceil")
