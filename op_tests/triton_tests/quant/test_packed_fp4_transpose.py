...
"""Correctness tests for packed FP4 logical transpose."""
...
def _reference_transpose(data_fp4: torch.Tensor) -> torch.Tensor:
...
def test_transpose_packed_fp4_matches_reference(shape):
    ...
    output = transpose_packed_fp4(data_fp4)
    expected = _reference_transpose(data_fp4)
...
def test_transpose_packed_fp4_supports_strided_input():
    ...
    torch.testing.assert_close(output, _reference_transpose(data_fp4), atol=0, rtol=0)


def test_transpose_packed_fp4_uses_int64_strides():
    huge_stride = (1 << 32) + 1
    input_storage = torch.tensor([0x21, 0x43], dtype=torch.uint8, device="cuda")
    output_storage = torch.full((2,), 0xFF, dtype=torch.uint8, device="cuda")
    data_fp4 = torch.as_strided(input_storage, (2, 1), (1, huge_stride))
    output = torch.as_strided(output_storage, (2, 1), (1, huge_stride))

    _transpose_packed_fp4_kernel[(1, 1)](
        data_fp4,
        output,
        2,
        1,
...
def test_transpose_packed_fp4_preserves_native_dtype():
    ...
    torch.testing.assert_close(
        output.view(torch.uint8), _reference_transpose(data_bytes), atol=0, rtol=0
    )
...
def test_transpose_packed_fp4_validates_input():
    with pytest.raises(ValueError, match="must be 2-D"):
        transpose_packed_fp4(torch.zeros((2, 2, 2), dtype=torch.uint8))
    with pytest.raises(TypeError, match="torch.uint8"):
        transpose_packed_fp4(torch.zeros((2, 2), dtype=torch.float32))
    with pytest.raises(ValueError, match="CUDA tensor"):
        transpose_packed_fp4(torch.zeros((2, 2), dtype=torch.uint8))

    if not torch.cuda.is_available():
        return

    with pytest.raises(ValueError, match="non-zero"):
        transpose_packed_fp4(torch.empty((2, 0), dtype=torch.uint8, device="cuda"))
    with pytest.raises(ValueError, match="rows must be even"):
        transpose_packed_fp4(torch.zeros((3, 2), dtype=torch.uint8, device="cuda"))
