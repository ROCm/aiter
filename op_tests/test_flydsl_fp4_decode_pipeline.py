...
def make_case(heads, dim, next_n, page, seed=43):
    ...
    q_dense = torch.randn(batch, next_n, heads, dim, dtype=torch.bfloat16, device="cuda")
    kv_dense = torch.randn(batch, padded, dim, dtype=torch.bfloat16, device="cuda")
    weights = torch.randn(batch * next_n, heads, dtype=torch.bfloat16, device="cuda") * 0.1
    ...
    table_storage = torch.full((batch, padded // page + 3), -1, dtype=torch.int32, device="cuda")
    ...
    def expected():
        return reference.ref_mqa_logits_mixed(
            q, q_scale, dense_fp4, dense_scales, weights, context,
            next_n=next_n, weight_scale=1.5,
        )[:, :maximum]

    return dict(
        q_fp4=q, q_scale=shuffled, kv_cache=kv, kv_scale=scales,
        block_tables=table, weights=weights, context_lens=context,
        max_seq_len=maximum, next_n=next_n, kv_block_size=page,
        weight_scale=1.5, out=output,
    ), expected, storage


SHAPES = [
    (16, 128, 1, 64), (32, 128, 2, 64), (48, 128, 3, 64),
    (64, 128, 1, 64), (64, 128, 2, 64), (64, 128, 4, 64),
    (64, 128, 5, 64), (64, 128, 8, 64), (80, 128, 2, 64),
    (96, 128, 3, 64), (112, 128, 1, 64), (128, 128, 2, 64),
    (64, 256, 1, 64), (128, 256, 2, 64),
    (32, 128, 2, 128), (64, 128, 1, 128), (64, 128, 3, 128),
    (128, 256, 4, 128),
]
...
def test_decode_graph_updates_context(next_n):
    ...
    for lengths in ([129, 0, 257], [0, 0, 0], [513, 1, 65]):
        data["context_lens"].copy_(torch.tensor(lengths, dtype=torch.int32, device="cuda"))
        graph.replay()
