...
def _json_table(line):
    """A validated one-line JSON table, or None for ordinary log output."""
    try:
...
def _json_tables(lines):
    ...
    return [json.dumps(table) for line in lines if (table := _json_table(line))]
...
def _profile_kernel_blocks(lines):
    """CUDA kernel names from each ``get_trace_perf`` DataFrame in order."""
    profiles = []
    current = None
    ...
    def finish_profile():
        ...
        if current is None:
            return
        names = _kernel_names(current)
        if names:
            profiles.append(names)
        current = None

    for line in lines:
        if "[aiter INFO]" in line and line.rstrip().endswith(" - get_trace_perf"):
            finish_profile()
            current = []
            continue
...
def _case_kernel_blocks(lines):
    """GPU kernels grouped by the ``@benchmark`` call that produced one row."""
    cases = []
    current = None
    ...
    def finish_case():
        ...
        if current is None:
            return
        cases.append(_kernel_names(current))
        current = None

    for line in lines:
        if line.lstrip().startswith("calling "):
            finish_case()
            current = []
            continue
        if current is not None:
...
def _profiled_json_tables(lines):
    """JSON tables paired with profiler blocks emitted since the prior table.

    Child UTs append result rows in the same order as their ``run_perftest``
    calls, then print the accumulated JSON table. Keep those profile blocks
    private and hand each batch to the next JSON table. Unrelated INFO,
    compiler output and warnings are never forwarded.
    """
    tables = []
    segment_start = 0
    for index, line in enumerate(lines):
        table = _json_table(line)
        if table is None:
            continue
        segment = lines[segment_start:index]
        # @benchmark logs one ``calling ...`` block for each result row. Use
        # that row boundary first, so a case may own any number of perftest
        # profiles without per-op rules or equal-size assumptions.
        profiles = _case_kernel_blocks(segment)
        if len(profiles) != len(table["rows"]):
            profiles = _profile_kernel_blocks(segment)
        # A few standalone benches do not use @benchmark. Their combo runner
        # invokes one result case per child, so all GPU rows in the segment are
        # unambiguous. Never apply this fallback to a multi-row table.
        if not profiles and len(table["rows"]) == 1:
            names = _kernel_names(segment)
            if names:
                profiles = [names]
...
def _attach_kernel_profiles(table, profiles):
    """Attach ordered profiler kernels to the corresponding result rows.

    Some rows benchmark more than one backend. When every row owns the same
    number of profiler blocks, combine that row's blocks in execution order.
    Refuse to guess if the counts do not divide evenly.
    """
    rows = table["rows"]
    if not rows:
        return
    if not profiles:
        for row in rows:
            if isinstance(row, dict):
                row.setdefault("kernel", None)
        return
    if len(profiles) % len(rows):
        reason = f"{len(profiles)} profiler blocks for {len(rows)} result rows"
        for row in rows:
            if isinstance(row, dict):
                row.setdefault("kernel", None)
                row.setdefault("kernel_trace_error", reason)
        return

    profiles_per_row = len(profiles) // len(rows)
    for index, row in enumerate(rows):
        if not isinstance(row, dict):
            continue
        names = []
        start = index * profiles_per_row
        for profile in profiles[start : start + profiles_per_row]:
            for name in profile:
                if name not in names:
                    names.append(name)
        row.setdefault("kernel", " + ".join(names) or None)
...
def _pin_arch(env):
    ...
    the caller wins.
    """
    if "GPU_ARCHS" not in env:
        env["GPU_ARCHS"] = get_gfx()
    if "CU_NUM" not in env:
        env["CU_NUM"] = str(get_cu_num())
    return env
...
def _run_child(
    ...
    # `results` decides whether the op reported anything; profiler kernels are
    # annotations and must not stand in for a result table, or an extractor that
    # stops matching turns into a silent hole instead of a failure.
    results = extract(lines)
    if structured:
        tables = [json.loads(result) for result in results]
        if kernels:
            profiled = _profiled_json_tables(lines)
            for index, table in enumerate(tables):
                profiles = []
                if index < len(profiled):
                    profiled_table, candidate_profiles = profiled[index]
                    if table["name"] == profiled_table["name"] and len(
                        table["rows"]
                    ) == len(profiled_table["rows"]):
        ...
        for init in inits:
            for head_dim, seqlen, causal in _MHA_SHAPES:
                with _smi_case(
                    ...
                    row = mha_mod.test_fmha_fwd_with_sink_asm_perf(
...
# alongside the row's kernel column, which says whether anything reached the GPU.
...
def main():
    ...
    # get_gfx() lazily imports module_aiter_core and logs that import at INFO.
    # Silence only this probe; benchmark captures still need INFO profiler rows
    # so kernel names can be extracted from them.
    with _silence():
        gfx = get_gfx()
        _pin_arch(os.environ)
    if gfx not in SUPPORTED_GFX:
        print(f"combo bench targets {SUPPORTED_GFX} only; current {gfx} — skipping")
        return
    # Already pinned inside the silenced probe above. Children that inherit our
    # environ (env=None) get these too, not just ones handed an explicit env.
    ...
    if _FAILURES:
        if _JSON_OUTPUT:
            _print_table(
                "combo failures",
                [{"op": label, "err_msg": why} for label, why in _FAILURES],
            )
        else:
            print(
