...
1. Detailed args — drive one exact shape from the CLI:
     AITER_ENABLE_EXPERIMENTAL=1 python op_tests/op_benchmarks/flydsl/bench_mha_fwd.py \\
         --layout thd -b 2 -sq 4095 -sk 4095 -nh 16 -nhkv 4 -c

2. Predefined cases — replay curated shapes by id:
     AITER_ENABLE_EXPERIMENTAL=1 python op_tests/op_benchmarks/flydsl/bench_mha_fwd.py --case 0 4
     AITER_ENABLE_EXPERIMENTAL=1 python op_tests/op_benchmarks/flydsl/bench_mha_fwd.py --list   # show, don't run

seqlen ranges 0..2^17; the cases sprinkle in odd (non-power-of-two) lengths. Each case
shape is replayed at all three supported hdims: 128/128, 192/128 and 256/128.
Defaults: --warmup 2 --repeat 5.
"""
...
def run_perf(case, warmup, repeat, fix_init=None):
    ...
    # Always measure OUR m32x8 kernel, so experimental is always ON: the PR3039
    # gfx1250 varlen ASM gate in mha.py owns 128/128 and 192/128 unless experimental
    # yields them to FlyDSL, and the FlyDSL router only takes 256 when experimental.
    os.environ["AITER_ENABLE_EXPERIMENTAL"] = "1"
...
def main():
    ...
    # The m32x8 path is gated behind experimental (forced on at module top); without it
    # the public wrappers silently run CK, and the varlen router hands d128/d192 to the
    # prefill ASM kernel instead. Assert the invariant held in case the env was clobbered
    # after import.
    assert (
        is_experimental_enabled()
    ), "AITER_ENABLE_EXPERIMENTAL must be enabled for the m32x8 path"
