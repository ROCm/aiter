...
Usage::
    ...
    python op_tests/op_benchmarks/flydsl/qwenimage_vae_conv/plot_conv_vs_hipblaslt.py
"""
...
OUT = HERE / "figures" / "conv_implicit_vs_hipblaslt.png"
...
for yc, r in zip(centers, rows):
    ...
    # pale overlay: the same ratio once the NCHW->NHWC transpose is charged too
    ax.barh(
...
ax.set_title(
    "aiter conv3d_implicit 对标 hipBLASLt（条宽 = 调用次数；同 M/N/K，同 FLOP；gfx950）"
)
...
ax.legend(
    handles=[
        ...
        Patch(facecolor=WIN_PALE, label="计入 NCHW→NHWC 转置后（赢）"),
...
fig.text(
    0.01,
    0.012,
    f"深色为 conv 主 kernel（与参考图同口径）：{len(rows)} 个 shape 中 {nwin} 个跑赢，"
    f"按调用次数加权 {weighted:.3f}×。浅色叠加了 NCHW→NHWC 转置——kernel 内部是"
    " channels-last，NCHW 输入需先转置，通道窄、空间大的层代价最重。",
    color=NEUTRAL,
    fontsize=8,
)
fig.text(
    0.01,
    -0.004,
    "非同类对比：hipBLASLt 读已物化的 M×K 矩阵，conv 从约 9 倍小的原张量 gather，"
    "物化代价未计入 hipBLASLt；条宽为该 shape 在 VAE 中的调用次数。"
    ' 若上游已是 NHWC（input_layout="NHWC"），转置那部分不发生。',
    color=NEUTRAL,
    fontsize=8,
)
