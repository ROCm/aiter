...
def make_kernel_runner(
    ...
    if args.kernel == "aiter_fp8_flydsl":
        from aiter.ops.flydsl.kernels.flash_attn_fp8_pingpong import (
        ...
        if k_bshd.shape[2] != H:
            raise ValueError(
                "aiter_fp8_flydsl: GQA (hk != hq) not supported yet; run with hk == hq"
            )
...
def metric_lines(args: argparse.Namespace, include_sparse_metric: bool) -> List[str]:
    ...
    if args.compare_to_ref:
        return ["time(ms)", "throughput(TFLOPS)"]
