...
import tempfile
from aiter.ops.triton.quant.quant_moe import (
...
def quantize(x, dtype):
    if dtype == "bf16":
        ...
        x, scale = downcast_to_mxfp(x.to(torch.bfloat16), fp8e4_dtype, axis=1)
        return x, scale
    else:
...
def bench_mlp(
    ...
    if w_dtype_str == "fp8":
        ...
        w1 = downcast_to_static_fp8_3d(w1, w_static_scale)
        w2 = downcast_to_static_fp8_3d(w2, w_static_scale)
        swizzle_mx_scale1 = "CDNA4_SCALE"
        swizzle_mx_scale2 = "CDNA4_SCALE"
    else:
        wg, _ = quantize(wg, "bf16")
    ...
    for i in range(reps):
        ...
        if x_dtype_str == "fp8" and w_dtype_str == "fp8":
            x = downcast_to_static_fp8(x, x_static_scale)
            x = moe_gemm_a8w8(
                x,
                w1,
                None,
        ...
        elif x_dtype_str == "fp8" and w_dtype_str == "mx8":
            x = downcast_to_static_fp8(x, x_static_scale)
            x = moe_gemm_a8w8(
                x,
                w1,
        ...
        elif x_dtype_str == "mx8" and w_dtype_str == "fp8":
