...
def bench_mlp(
    batch, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, per_row_act_quant, TP, op_regex
):
    rank = 0
    dev = f"cuda:{rank}"
    ...
    w1 = (torch.randn((n_expts_tot, dim1, dim2 // TP), dtype=torch.bfloat16, device=dev) / 10).to(torch.float8_e4m3fn)
    w2 = (torch.randn((n_expts_tot, dim2 // TP // 2, dim1), dtype=torch.bfloat16, device=dev) / 10).to(torch.float8_e4m3fn)
    # biases
    ...
    reps = 100
    x = (torch.randn((batch, dim1), dtype=torch.bfloat16, device=dev) / 10).to(torch.float8_e4m3fn)
    ...
    if w_dtype_str == "fp8":
        ...
        w1_scale = torch.rand((n_expts_tot, k_blocks_w1, n_blocks_w1), dtype=torch.float32,device=dev)
        w2_scale = torch.rand((n_expts_tot, k_blocks_w2, n_blocks_w2), dtype=torch.float32, device=dev)
