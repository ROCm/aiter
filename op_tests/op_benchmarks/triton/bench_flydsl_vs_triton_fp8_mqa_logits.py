...
def _triton_logits_fallback(
    Q, KV, kv_scales, weights, cu_starts, cu_ends, clean_logits=True, block_kv=64
):
...
def calculate_tflops(start_inds, end_inds, num_heads_q, head_dim, time_ms):
    ...
    for i in range(len(start_inds)):
        total_flops += (
            2.0 * num_heads_q * head_dim * max(0, end_inds[i] - start_inds[i])
        )
...
def _time_modes(closure, modes, cfg, quiet_probe=False):
    ...
    for mode in modes:
        try:
            ...
            print(
                "[warn] graph capture failed (e.g. NULL-stream launch or allocation during capture)"
            )
            out[mode] = "GRAPH-FAIL"
...
PRESET_SHAPES = [
    ...
    (8, 2048, 2048, 32, 128),  # rows 16,384
    (2, 4096, 4096, 32, 128),  # rows 8,192
    (4, 4096, 4096, 32, 128),  # rows 16,384
    (2, 8192, 8192, 32, 128),  # rows 16,384
    # -- H = 64 (DSV4) --
    ...
    (2, 4096, 4096, 64, 128),  # rows 8,192
    (4, 4096, 4096, 64, 128),  # rows 16,384
    (2, 8192, 8192, 64, 128),  # rows 16,384
]
...
def _run_one(idx, impls, shape, args, q_fp8_dtype=None, kv_fp8_dtype=None):
    ...
    q, kv, q_fp8, kv_fp8, scales, weights, ks, ke = _make_inputs(
        ...
        shape.num_heads_q,
        shape.head_dim,
        q_fp8_dtype=q_fp8_dtype,
        kv_fp8_dtype=kv_fp8_dtype,
    )
    ...
    for name, fn in impls.items():
        closure = _make_closure(name, fn, shape, q_fp8, kv_fp8, scales, weights, ks, ke)
    ...
    if args.verification == "reference":
        ...
        verify = _verify_vs_triton(impls, q_fp8, kv_fp8, scales, weights, ks, ke, shape)
    else:
...
def _verify_vs_triton(impls, q_fp8, kv_fp8, scales, weights, ks, ke, shape):
    ...
    for name, fn in impls.items():
        ...
        if fin.any():
            rel = (
                ((out[fin] - ref[fin]).abs() / ref[fin].abs().clamp_min(1e-3))
                .max()
                .item()
            )
        else:
...
def _gpu_info():
    ...
    if torch.cuda.is_available():
        ...
        info.append(("VRAM", f"{props.total_memory / (1024 ** 3):.1f} GiB"))
    else:
...
def _write_markdown(path, rows, impls, args):
    ...
    parts = [
        ...
        (
            "- Time cells: `median(\u00b1std) us`. `GRAPH-FAIL` = not "
            "graph-capturable (e.g. Triton allocates its output internally)."
        ),
        "",
        "## Environment",
        "",
...
def _print_table(rows, impls, args):
    ...
    for row in rows:
        ...
        widths = [
            max(len(h), max((len(dr[i]) for dr in data_rows), default=0)) + 2
...
def _default_dtype_combo() -> str:
    ...
    if arch == "gfx942":
        return "fnuz/fnuz"
    raise SystemExit(
        f"[error] unknown GPU arch {arch!r}; cannot select default dtype combo."
    )
...
def main():
    ...
    if args.list_variants:
        if avail is None:
            ...
            print(
                "    auto  (shape-adaptive: picks a variant per shape via "
                "_auto_variant)"
            )
        return
