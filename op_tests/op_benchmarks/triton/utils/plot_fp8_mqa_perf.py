...
def parse_md(path: Path) -> list[dict]:
    ...
    for i in range(1, len(sections), 2):
        ...
        for line in body.splitlines():
            ...
            if len(cells) < 5:
                continue
            impl, _time_us_raw, tflops_raw, verify, _vs_triton_raw = cells[:5]
        ...
        def _speedup(entry, triton_row=triton_row):
