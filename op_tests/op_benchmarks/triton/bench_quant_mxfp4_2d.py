...
def _torch_mxfp4_quant_2d(x: torch.Tensor):
...
def run_benchmark(args):
    ...
    def bench_quant_mxfp4_2d(
        ...
        fn = (
            (lambda: dynamic_mxfp4_quant_2d(x))
            if provider == "aiter"
            else (lambda: _torch_mxfp4_quant_2d(x))
        )
...
def parse_args(args: list[str] | None = None):
    parser = argparse.ArgumentParser(
        prog="Benchmark 2-D MXFP4 Quant",
