...
def main(args=None):
    ...
    print("dynamic_mxfp8_quant calls:")
    for m, n in get_default_shapes():
        if n % 32 != 0:
            raise ValueError(f"N must be a multiple of 32 for MXFP8 blockscaling, got N={n}")

        x = torch.randn((m, n), dtype=dtype, device="cuda")
        x_fp8, x_scale = dynamic_mxfp8_quant(x)

        print(f"shape=({m}, {n})")
        print(f"  input  : {tuple(x.shape)}, {x.dtype}, {x.device}")
        print(f"  x_fp8  : {tuple(x_fp8.shape)}, {x_fp8.dtype}, {x_fp8.device}")
