...
def test_out_of_scope_is_refused_rather_than_mishandled():
    ...
    # A weight or bias in the *other* narrow dtype, which the fp32 case above
    # does not reach: one `dtype_str` specializes the whole kernel, so a
    # mismatch is reinterpreted rather than converted. The accepted call is the
    # control -- without it, a predicate that refused everything would pass.
    other = torch.float16 if DTYPE is torch.bfloat16 else torch.bfloat16
    for fn in (
        _causal_conv1d_update_flydsl_supported,
        _causal_conv1d_update_sglang_flydsl_supported,
    ):
        assert fn(t["x"], t["conv_state"], t["weight"], bias=t["bias"])
        assert not fn(
            t["x"], t["conv_state"], t["weight"].to(other)
        ), f"{fn.__name__} accepted a weight the kernel would read as {DTYPE}"
        assert not fn(
