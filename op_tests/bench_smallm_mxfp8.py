...
@triton.jit
def _triton_mxfp8_linear(x_ptr, xs_ptr, w_ptr, ws_ptr, out_ptr, M, N, K,
                         sxm, sxk, sxsm, sxsk, swn, swk, swsn, swsk, som, son,
...
def triton_dense(xq, xs, wq, ws):
    ...
    _triton_mxfp8_linear[grid](
        xq, xs, wq, ws, out, M, N, K,
        xq.stride(0), xq.stride(1), xs.stride(0), xs.stride(1),
        wq.stride(0), wq.stride(1), ws.stride(0), ws.stride(1),
...
def main():
    torch.manual_seed(0)
    shapes = [(6144, 2304, "qkv"), (2048, 6144, "o_proj"),
              (6144, 1536, "gate_up"), (1536, 6144, "mlp_down")]
    Ms = [1, 2, 4, 8, 16, 32, 64]
    arch = torch.cuda.get_device_properties(0).gcnArchName
    print(f"# MX-FP8 dense GEMV: HIP vs stock Triton dot_scaled  ({arch})")
    print(f"{'shape':10} {'K':>5} {'N':>5} {'M':>4} {'HIP us':>8} {'Triton us':>10} {'speedup':>8}")
    for K, N, name in shapes:
        ...
        for M in Ms:
            ...
            print(f"{name:10} {K:>5} {N:>5} {M:>4} {t_hip:>8.2f} {t_tri:>10.2f} "
