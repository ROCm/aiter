...
def load_asm_tuned_configs():
    ...
    if os.path.isdir(model_dir):
        for f in os.listdir(model_dir):
            if f.startswith("a8w8_blockscale_bpreshuffle_tuned_gemm") and f.endswith(".csv"):
...
def test_gemm(dtype, m, n, k, ck_preshuffle=True):
    ...
    if asm_cfg:
        aiter.logger.info(f"asm tuned config hit: M={m}, N={n}, K={k}, splitK={asm_splitK}, kernel={asm_kernel}")
    else:
        aiter.logger.info(f"asm tuned config miss: M={m}, N={n}, K={k}, using heuristic")
    c, avg_c = run_asm(x, weight_asm, x_scale_t, w_scale, dtype,
                       splitK=asm_splitK, kernel_name=asm_kernel)
...
@perftest()
def run_asm(x, weight, x_scale, w_scale, dtype=dtypes.bf16, splitK=None, kernel_name=None):
    m, k = x.shape
    n, _ = weight.shape
    ...
    return aiter.gemm_a8w8_blockscale_bpreshuffle_asm(
        x, weight, out, x_scale, w_scale,
        splitK=splitK, kernelName=kernel_name,
