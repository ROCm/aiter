...
"""Correctness test for the symmetric-compute split-DMA fp8 E4M3 FMHA kernel.

Same methodology as test_flash_attn_fp8_pingpong.py (fp32 SDPA reference on
dequantized fp8 inputs, flat cosine >= 0.997 gate); only the kernel module
differs (flash_attn_fp8_symmetric).

Run:
    PYTHONPATH=/data/work/flydsl:/data/work/aiter HIP_VISIBLE_DEVICES=0 \\
        python op_tests/test_flash_attn_fp8_symmetric.py
"""
...
from aiter.ops.flydsl.kernels.flash_attn_fp8_symmetric import (  # noqa: E402
...
def test_flash_attn_fp8_symmetric_small():
    if not torch.cuda.is_available():
        import pytest
