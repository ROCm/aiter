...
@benchmark()
def test_qsa_family_a_plumbing(m, seq_len, page_size, dtype):
...
@benchmark()
def test_qsa_family_a_vllm_amd(m, seq_len, page_size, dtype):
...
@benchmark()
def test_qsa_family_a_4882_triton(m, seq_len, page_size, dtype):
    ...
    for dtype in args.dtype:
        ...
        for m, seq_len, page_size in itertools.product(
            ...
            if m > seq_len:
                ...
                continue
            rows.append(test_qsa_family_a_plumbing(m, seq_len, page_size, dtype))
        ...
        for m, seq_len, page_size in itertools.product(
            ...
            if m > seq_len:
                continue
            rows.append(test_qsa_family_a_vllm_amd(m, seq_len, page_size, dtype))
        df = pd.DataFrame(rows)
        aiter.logger.info(
        ...
        for m, seq_len, page_size in itertools.product(
            ...
            if m > seq_len:
                continue
            rows.append(test_qsa_family_a_4882_triton(m, seq_len, page_size, dtype))
        df = pd.DataFrame(rows)
        aiter.logger.info(
