...
Purpose:
    Profiles best-performing MOE kernels using rocprofv3 to collect hardware
    performance counters. Uses multi-round profiling to avoid counter limits.
...
Output:
    CSV file with profiling results including hardware performance counters

Usage:
    python profile_kernels.py -i best_kernels.csv -o profiling_output
"""
...
# Import shared utilities (only for print helper)
import moe_utils

# ============================================================================
# COUNTER GROUPS FOR MULTI-ROUND PROFILING
# Split counters into manageable groups (8-10 per round) to avoid rocprofv3 limits
# ============================================================================
COUNTER_GROUPS = {
    'r1': [
...
# CRITICAL: These functions must remain exactly as-is to preserve data
# generation logic, column name handling, and dtype string transformations
# ============================================================================
...
def parse_rocprofv3_output(config_name, output_dir, keep_temp_files, expected_counters):
    ...
    CRITICAL: Column name handling must remain exactly as-is.
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Profile MOE kernels with rocprofv3 - Multi-round mode"
    )
    parser.add_argument(
        '-i', '--input',
        required=True,
        help='Input CSV file with kernel configs from 02_analyze_results.py'
    )
    parser.add_argument(
        '-o', '--output-dir',
    ...
    moe_utils.print_section_header("MULTI-ROUND PROFILING")
