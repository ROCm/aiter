...
def _rmsnorm(x, eps=1e-6):
    ...
    61-layer residual chain -- without it the narrow activation quant (fp4/fp8)
    saturates and the chain diverges to NaN after a few layers. This is the
    reference-side and definitional version; the measured device path runs
    ``aiter.rms_norm``, which is bit-identical to it (see ``_MegaPipeline._norm``)."""
    xf = x.float()
...
class DeviceMoEPipeline:
    ...
    def _layer_step(self, x, layer_idx):
        ...
        xn = self._norm(x)  # keep the quantized activations in range across 61 layers
