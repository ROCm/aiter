...
class MoriFusedMoeBaselinePath:
    ...
    def __init__(
        ...
        *,
        ...
        config = mori.ops.EpDispatchCombineConfig(
            ...
            max_num_inp_token_per_rank=(shape.tokens if exact_capacity else max(128, shape.tokens)),
        ...
        self.rdma_block_num = int(rdma_block_num)
        self._fused_moe = fused_moe
        self._activation = (
            aiter.ActivationType.Situv2
            if shape.activation == "situv2" else aiter.ActivationType.Silu
        )
    ...
    def _forward(
        self, *, validate_recv: bool = False, debug_sync: bool = False,
        reference_coalesce_duplicates: bool = False,
    ) -> torch.Tensor:
        route_ids, route_weights = self.shared.topk_ids, self.shared.route_weights
        if reference_coalesce_duplicates:
            # The Opus reference sorter stores one slot per (token, expert), so
            # exact duplicate IDs lose contributions. Only the eager oracle
            # coalesces their weights; candidate input slots stay untouched.
            # doweight_stage1=False below makes this linear identity valid.
            if torch.cuda.is_current_stream_capturing():
                raise RuntimeError("duplicate coalescing is an eager reference only")
            from scripts.megamoe_tile.stage2_graph_reference import coalesce_reference_routes

            ids, weights, duplicates = coalesce_reference_routes(
                route_ids.cpu().tolist(), route_weights.cpu().tolist(),
                experts_per_rank=self.shape.local_experts,
                num_experts=self.shape.experts,
            )
            route_ids = torch.tensor(ids, dtype=route_ids.dtype, device=route_ids.device)
            route_weights = torch.tensor(weights, dtype=route_weights.dtype, device=route_weights.device)
            if self.rank == 0:
                print("MORI_REFERENCE_COALESCED " + json.dumps({
                    "rank": self.rank, "duplicate_slots": duplicates,
                    "candidate_routes_modified": False,
                }), flush=True)
        # Diagnose the first eager reference only. Synchronizing the phases would
        # change overlap and is illegal during Graph capture, so it is opt-in and
        # the Graph driver excludes these diagnostic runs from performance use.
        def debug_phase(phase: str, *, completed: bool = False) -> None:
            if not debug_sync:
                return
            if completed:
                torch.cuda.synchronize(self.shared.x.device)
            print("MORI_FORWARD_DEBUG " + json.dumps({
                "rank": self.rank, "phase": phase,
                "state": "complete" if completed else "begin",
            }), flush=True)
        ...
        debug_phase("quant", completed=True)
        debug_phase("dispatch")
        ...
        debug_phase("dispatch", completed=True)
        if validate_recv:
        ...
        debug_phase("combine", completed=True)
