...
from aiter.ops.flydsl.kernels.megamoe_tile.rank_push_layout import (
...
TIMELINE_INTERVALS = {
    ...
    "s2_init_clear": ("stage2_entry", "stage2_init_clear_done"),
    "s2_init_count": ("stage2_init_clear_done", "stage2_init_count_done"),
    "s2_init_publish": ("stage2_init_count_done", "stage2_init_gate_done"),
    "s2_entry_to_rank_push_done": ("stage2_entry", "stage2_rank_push_last_done"),
    "s2_entry_to_node_reduce_done": ("stage2_entry", "stage2_node_reduce_last_done"),
    "s2_entry_to_rail_done": ("stage2_entry", "stage2_rail_last_done"),
    "s2_entry_to_final_done": ("stage2_entry", "stage2_final_last_done"),
    "s2_entry_to_all_ctas_done": ("stage2_entry", "stage2_all_ctas_done"),
...
def _shape(tokens: int, hidden: int = 7168, activation: str = "silu") -> BenchmarkShape:
...
class Stage2ModeOperator(MegaMoETileA4W4):
    ...
    def __init__(
        ...
        *args,
        ...
        stage2_node_accumulation_mode: str = "rank_local",
        stage2_node_reduce_blocks: int = 32,
        stage2_node_reduce_token_owner_fastpath: bool = False,
        stage2_node_reduce_token_owner_mask_prefetch: bool = False,
        stage2_node_reduce_expected_popcount: bool = False,
        stage2_node_reduce_vec_bytes: int = 16,
        stage2_node_reduce_schedule: str = "token",
        stage2_node_reduce_load_schedule: str = "load_first",
        ...
        stage2_rank_epilogue_barrier: str = "per_row",
        stage2_rank_accumulation_mode: str = "reduce_push",
        stage2_rank_reduce_blocks: int = 8,
        stage2_rank_push_batch_size: int = 1,
        stage2_rank_push_batch_invariants: bool = False,
        stage2_rank_push_count_prefetch: bool = False,
        stage2_rank_push_count_wave_broadcast: bool = False,
        stage2_rank_push_map_prefetch: bool = False,
        stage2_rank_push_count_from_slot_map: bool = False,
        stage2_rank_push_tile_index_recurrence: bool = False,
        stage2_rank_push_acquire_cohort: int = 1,
        stage2_gemm_use_nt: bool = False,
        stage2_rank_push_use_nt: bool = False,
        ...
        **kwargs,
        ...
        if stage2_rank_accumulation_mode not in ("atomic", "staged_reduce", "staged_ring", "reduce_push"):
            raise ValueError(
                "stage2_rank_accumulation_mode must be atomic, staged_reduce, staged_ring, or reduce_push"
            )
        _validate_rank_epilogue_barrier(
            stage2_rank_epilogue_barrier,
        ...
        if stage2_ready_granularity not in ("token", "tile", "group"):
        ...
        self.stage2_node_reduce_token_owner_fastpath = bool(
            stage2_node_reduce_token_owner_fastpath
        )
        self.stage2_node_reduce_token_owner_mask_prefetch = bool(
            stage2_node_reduce_token_owner_mask_prefetch
        )
        self.stage2_node_reduce_expected_popcount = bool(
            stage2_node_reduce_expected_popcount
        )
        ...
        self.stage2_rank_epilogue_barrier = str(stage2_rank_epilogue_barrier)
        self.stage2_rank_accumulation_mode = str(stage2_rank_accumulation_mode)
        self.stage2_rank_reduce_blocks = int(stage2_rank_reduce_blocks)
        self.stage2_rank_push_batch_size = int(stage2_rank_push_batch_size)
        self.stage2_rank_push_batch_invariants = bool(
            stage2_rank_push_batch_invariants
        )
        self.stage2_rank_push_count_prefetch = bool(
            stage2_rank_push_count_prefetch
        )
        self.stage2_rank_push_count_wave_broadcast = bool(
            stage2_rank_push_count_wave_broadcast
        )
        self.stage2_rank_push_map_prefetch = bool(
            stage2_rank_push_map_prefetch
        )
        self.stage2_rank_push_count_from_slot_map = bool(
            stage2_rank_push_count_from_slot_map
        )
        self.stage2_rank_push_tile_index_recurrence = bool(
            stage2_rank_push_tile_index_recurrence
        )
        self.stage2_rank_push_acquire_cohort = int(
            stage2_rank_push_acquire_cohort
        )
        self.stage2_gemm_use_nt = bool(stage2_gemm_use_nt)
        self.stage2_rank_push_use_nt = bool(stage2_rank_push_use_nt)
        ...
        self.stage2_group_batch = bool(stage2_group_batch)
        ...
        self.timeline_history_depth = int(timeline_history_depth)
        if self.timeline_history_depth and not self.timeline_instrument:
            raise ValueError("timeline history requires device timeline instrumentation")
        ...
        if self.stage2_rank_accumulation_mode == "reduce_push":
            if self.stage2_rank_reduce_blocks not in (8, 16, 32, 48, 56, 64):
                raise ValueError(
                    "rank reduce push CTA count must be 8,16,32,48,56,64"
                )
            if self.stage2_worker_blocks <= (1 + self.stage2_final_combine_blocks
                                             + self.stage2_node_reduce_blocks
                                             + self.stage2_rank_reduce_blocks):
                raise ValueError("reduce_push requires a GEMM CTA after all service roles")
        reduce_role_blocks = (
            self.stage2_node_reduce_blocks
        ...
        # Stage2 has its own runtime grid; Stage1 was resolved independently
        # by the base constructor and remains fixed for this operator instance.
    ...
    def _compile_stage2(self):
        launcher = compile_megamoe_tile_ep16_stage2_a4w4(
            ...
            node_reduce_token_owner_fastpath=(
                self.stage2_node_reduce_token_owner_fastpath
            ),
            node_reduce_token_owner_mask_prefetch=(
                self.stage2_node_reduce_token_owner_mask_prefetch
            ),
            node_reduce_expected_popcount=(
                self.stage2_node_reduce_expected_popcount
            ),
            ...
            rank_epilogue_barrier=self.stage2_rank_epilogue_barrier,
            rank_accumulation_mode=self.stage2_rank_accumulation_mode,
            rank_reduce_blocks=self.stage2_rank_reduce_blocks,
            rank_push_batch_size=self.stage2_rank_push_batch_size,
            rank_push_batch_invariants=self.stage2_rank_push_batch_invariants,
            rank_push_count_prefetch=self.stage2_rank_push_count_prefetch,
            rank_push_count_wave_broadcast=(
                self.stage2_rank_push_count_wave_broadcast
            ),
            rank_push_map_prefetch=self.stage2_rank_push_map_prefetch,
            rank_push_count_from_slot_map=(
                self.stage2_rank_push_count_from_slot_map
            ),
            rank_push_tile_index_recurrence=(
                self.stage2_rank_push_tile_index_recurrence
            ),
            rank_push_acquire_cohort=self.stage2_rank_push_acquire_cohort,
            gemm_use_nt=self.stage2_gemm_use_nt,
            rank_push_use_nt=self.stage2_rank_push_use_nt,
            ...
            group_batch=self.stage2_group_batch,
        ...
        if launcher.gmm_work_swizzle != self.stage2_gmm_work_swizzle:
            raise AssertionError("Stage2 GMM work swizzle manifest mismatch")
        if int(launcher.window_n_groups) != self.stage2_window_n_groups:
            raise AssertionError("Stage2 GMM window size manifest mismatch")
        ...
        if bool(launcher.node_reduce_token_owner_fastpath) != (
            self.stage2_node_reduce_token_owner_fastpath
        ):
            raise AssertionError("Stage2 node-reduce token-owner manifest mismatch")
        if bool(launcher.node_reduce_token_owner_mask_prefetch) != (
            self.stage2_node_reduce_token_owner_mask_prefetch
        ):
            raise AssertionError(
                "Stage2 node-reduce token-owner mask-prefetch manifest mismatch"
            )
        if bool(launcher.node_reduce_expected_popcount) != (
            self.stage2_node_reduce_expected_popcount
        ):
            raise AssertionError(
                "Stage2 node-reduce expected-popcount manifest mismatch"
            )
        ...
        if launcher.rank_epilogue_barrier != self.stage2_rank_epilogue_barrier:
            raise AssertionError("Stage2 rank epilogue barrier mismatch")
        if bool(launcher.rank_push_batch_invariants) != (
            self.stage2_rank_push_batch_invariants
        ):
            raise AssertionError("Stage2 rank-push batch-invariant manifest mismatch")
        if bool(launcher.rank_push_count_prefetch) != (
            self.stage2_rank_push_count_prefetch
        ):
            raise AssertionError("Stage2 rank-push count-prefetch manifest mismatch")
        if bool(launcher.rank_push_count_wave_broadcast) != (
            self.stage2_rank_push_count_wave_broadcast
        ):
            raise AssertionError("Stage2 rank-push count-wave manifest mismatch")
        if bool(launcher.rank_push_map_prefetch) != (
            self.stage2_rank_push_map_prefetch
        ):
            raise AssertionError("Stage2 rank-push map-prefetch manifest mismatch")
        if bool(launcher.rank_push_count_from_slot_map) != (
            self.stage2_rank_push_count_from_slot_map
        ):
            raise AssertionError("Stage2 rank-push map-derived-count manifest mismatch")
        if bool(launcher.rank_push_tile_index_recurrence) != (
            self.stage2_rank_push_tile_index_recurrence
        ):
            raise AssertionError("Stage2 rank-push tile-index manifest mismatch")
        if int(launcher.rank_push_acquire_cohort) != (
            self.stage2_rank_push_acquire_cohort
        ):
            raise AssertionError("Stage2 rank-push acquire-cohort manifest mismatch")
...
class CandidateStage2Path:
    def __init__(self, operator, shared, shape, device, mode: str):
        ...
        self.name = (
            ...
            + (
                "_nrtokenowner"
                if operator.stage2_node_reduce_token_owner_fastpath
                else ""
            )
            + (
                "_nrmaskprefetch"
                if operator.stage2_node_reduce_token_owner_mask_prefetch
                else ""
            )
            + (
                "_nrexpectpopcount"
                if operator.stage2_node_reduce_expected_popcount
                else ""
            )
            + (
                "_rpbinv"
                if operator.stage2_rank_push_batch_invariants
                else ""
            )
            + (
                "_rpcprefetch"
                if operator.stage2_rank_push_count_prefetch
                else ""
            )
            + (
                "_rpcwave"
                if operator.stage2_rank_push_count_wave_broadcast
                else ""
            )
            + (
                "_rpmapprefetch"
                if operator.stage2_rank_push_map_prefetch
                else ""
            )
            + (
                "_rpcmap"
                if operator.stage2_rank_push_count_from_slot_map
                else ""
            )
            + (
                "_rptileidx"
                if operator.stage2_rank_push_tile_index_recurrence
                else ""
            )
            + (
                "_rpacq2"
                if operator.stage2_rank_push_acquire_cohort == 2
                else ""
            )
            ...
            f"_reb{operator.stage2_rank_epilogue_barrier}"
    ...
    def prepare_iteration(self) -> None:
        ...
        if op.timeline_instrument:
            ...
            from aiter.ops.flydsl.kernels.megamoe_tile.window_view import write_window_u64
    ...
    def validate_prime(self, output: torch.Tensor) -> None:
        ...
        if self.operator.stage2_rank_accumulation_mode == "reduce_push" and self.mode == "full":
            # The legacy eager validator assumes balanced route counts and
            # decrementing token counters. Push counts actual routes and uses
            # per-token/group arrivals; apply the same checks as Graph mode.
            from scripts.megamoe_tile.stage2_graph_protocol import validate_protocol_snapshot

            validate_protocol_snapshot(
                snapshot, expected_generation=self.operator._generation,
                readiness=self.operator.stage2_ready_granularity,
                tokens=self.shape.tokens, accumulation_mode="reduce_push",
            )
            return
        if self.mode == "full":
            _validate_direct_tile_debug_snapshot(
            ...
            if (
                ...
                if self.operator.stage2_ready_granularity == "tile":
                    ...
                    return
                if self.operator.stage2_ready_granularity == "group":
                    print(
                        "MEGAMOE_GROUP_READY_DEBUG "
                        + json.dumps(
                            {
                                key: snapshot[key]
                                for key in (
                                    "rank_local_active_tokens",
                                    "rank_group_total",
                                    "rank_group_pending_mismatch",
                                    "rank_group_ready_missing",
                                    "stage2_error_count",
                                )
                            },
                            sort_keys=True,
                    ...
                    if snapshot["rank_group_pending_mismatch"] != 0:
                        raise AssertionError(
                            "Stage2 rank-group pending counters did not match "
                            "rank_group_total"
                        )
                    if snapshot["rank_group_ready_missing"] != 0:
                        raise AssertionError(
                            "Stage2 rank-group readiness did not complete"
                        )
                    return
            ...
            if (
                self.operator.stage2_ready_granularity == "group"
                and self.mode == "gmm2_atomic_only"
            ):
                print(
                    "MEGAMOE_GROUP_REDUCER_DEBUG "
                    + json.dumps(
                        {
                            key: snapshot[key]
                            for key in (
    ...
    def close(self) -> None:
        # 顺序和 bench_mega_moe.py 的 teardown 一样:算子先还自己的
        # window/memory,communicator 最后销毁。
        self.operator.close()
        comm = getattr(self.operator, "_bench_communicator", None)
        if comm is not None:
            self.operator._bench_communicator = None
            comm.destroy()
...
def _build_candidate(
    ...
    device_generation=False,
    group_batch=False,
    timeline_history_depth=0,
    rank_epilogue_barrier="per_row",
    stage1_workers=None,
    rank_reduce_blocks=8,
    rank_push_batch_size=1,
    rank_push_batch_invariants=False,
    rank_push_count_prefetch=False,
    rank_push_count_wave_broadcast=False,
    rank_push_map_prefetch=False,
    rank_push_count_from_slot_map=False,
    rank_push_tile_index_recurrence=False,
    rank_push_acquire_cohort=1,
    gemm_use_nt=False,
    rank_push_use_nt=False,
    node_reduce_token_owner_fastpath=False,
    node_reduce_token_owner_mask_prefetch=False,
    node_reduce_expected_popcount=False,
):
    ...
    # CCO 资源由测试脚本建、由测试脚本销毁,和 bench_mega_moe.py 一致:
    # rank0 生成 unique id -> 广播 -> Communicator.init;teardown 里先
    # operator.close() 还掉它自己的 window/memory,再 comm.destroy()。
    from mori.cco import Communicator, UniqueId

    _uid_payload = [
        bytes(Communicator.get_unique_id()) if rank == 0 else None
    ]
    dist.broadcast_object_list(_uid_payload, src=0)
    _comm = Communicator.init(
        shape.ep_size,
        rank,
        UniqueId.from_bytes(_uid_payload[0]),
    ...
    operator = Stage2ModeOperator(
        ...
        # ä½¿ç”¨chunked transportï¼Œé¿å…æŠŠ128-tokenåè®®è¯¯ç”¨äºŽ512+ã€‚
        stage1_transport="chunked",
        stage1_worker_blocks=stage1_workers,
        stage2_mode=mode,
        ...
        stage2_node_reduce_expected_popcount=node_reduce_expected_popcount,
        ...
        stage2_rank_epilogue_barrier=rank_epilogue_barrier,
        stage2_rank_accumulation_mode=rank_accumulation_mode,
        stage2_rank_reduce_blocks=rank_reduce_blocks,
        stage2_rank_push_batch_size=rank_push_batch_size,
        stage2_rank_push_batch_invariants=rank_push_batch_invariants,
        stage2_rank_push_count_prefetch=rank_push_count_prefetch,
        stage2_rank_push_count_wave_broadcast=rank_push_count_wave_broadcast,
        stage2_rank_push_map_prefetch=rank_push_map_prefetch,
        stage2_rank_push_count_from_slot_map=rank_push_count_from_slot_map,
        stage2_rank_push_tile_index_recurrence=rank_push_tile_index_recurrence,
        stage2_rank_push_acquire_cohort=rank_push_acquire_cohort,
        stage2_gemm_use_nt=gemm_use_nt,
        stage2_rank_push_use_nt=rank_push_use_nt,
        ...
        stage2_group_batch=group_batch,
        ...
        timeline_history_depth=timeline_history_depth,
        stage1_diagnostic_phase=stage1_diagnostic_phase,
    )
    # 这个 Communicator 归测试脚本所有;CandidateStage2Path.close() 会在
    # operator.close() 之后销毁它。
    operator._bench_communicator = _comm
...
def main() -> int:
    ...
    parser.add_argument("--torch-profiler-dir", default="trace_data/stage2_graph")
    parser.add_argument("--graph-rel-l2-threshold", type=float, default=5e-2)
    parser.add_argument("--graph-comparison-statistic",
                        choices=("pooled_min", "rank_mean_of_min", "rank_mean"),
                        default="rank_mean",
                        help="Stage2 span statistic; EPLB runner selects pooled_min per the requested comparison convention")
    parser.add_argument("--graph-check-routing-replay", action="store_true",
                        help="Swap expert nodes and restore routing in the same captured graph before timing")
    parser.add_argument("--graph-debug-reference", action="store_true",
    ...
    parser.add_argument("--candidate-rank-reduce-blocks", type=int,
                        choices=(8,16,32,48,56,64), default=8)
    parser.add_argument("--candidate-rank-push-batch-size", type=int,
                        choices=(1,2,4,8,16,32,64), default=1)
    parser.add_argument(
        "--candidate-rank-push-batch-invariants",
        action="store_true",
        help="Reuse group/rank/peer geometry across each complete token batch",
    )
    parser.add_argument(
        "--candidate-rank-push-count-prefetch",
        action="store_true",
        help="Load one immutable local route count per wave lane and batch",
    )
    parser.add_argument(
        "--candidate-rank-push-count-wave-broadcast",
        action="store_true",
        help="Load one immutable route count from every wave lane",
    )
    parser.add_argument(
        "--candidate-rank-push-map-prefetch",
        action="store_true",
        help="Issue each immutable slot-map load before arrival polling",
    )
    parser.add_argument(
        "--candidate-rank-push-count-from-slot-map",
        action="store_true",
        help="Derive each local arrival target from the immutable slot map",
    )
    parser.add_argument(
        "--candidate-rank-push-tile-index-recurrence",
        action="store_true",
        help="Advance the peer tile index once per complete-batch slot",
    )
    parser.add_argument(
        "--candidate-rank-push-acquire-cohort",
        type=int,
        choices=(1, 2),
        default=1,
        help="Share one agent acquire across this many adjacent tiles",
    )
    parser.add_argument("--candidate-gemm-use-nt", action="store_true")
    parser.add_argument("--candidate-push-use-nt", action="store_true")
    parser.add_argument("--stage1-workers", type=int, default=None,
                        help="Independent Stage1 resident grid; omitted preserves Stage2-grid inheritance")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Publish a token from its sole static reducer wave",
    )
    parser.add_argument(
        "--candidate-node-reduce-token-owner-mask-prefetch",
        action="store_true",
        help="Load each owner wave's immutable token masks once",
    )
    parser.add_argument(
        "--candidate-node-reduce-expected-popcount",
        action="store_true",
        help="Use LLVM ctpop for the per-group contributor count",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--candidate-rank-epilogue-barrier",
        choices=("per_row", "per_tile"),
        default="per_row",
        help="per_tile removes immutable LDS row barriers; tile-exit and group publication remain",
    )
    parser.add_argument(
    ...
    parser.add_argument("--device-timeline-history-depth", type=int, default=0,
                        help="Diagnostic generation ring (power of two, e.g. 128); retains both timing batches without per-replay copies/sync")
    parser.add_argument("--candidate-group-batch", action="store_true",
                        help="experimental batch group reducer and one completion per M tile")
    ...
    if args.device_timeline_history_depth:
        ...
        if not (args.cuda_graph and args.device_timeline and 2 * args.iters <= depth <= 1024
                and depth >= 2 and not depth & (depth - 1)):
            raise ValueError("timeline history needs --cuda-graph --device-timeline and power-of-two depth >= 2*iters, <= 1024")
    if args.warmup < 0 or args.iters < 1:
        raise ValueError("warmup must be non-negative and iters must be positive")
    if args.route_pattern == "skewed-duplicate-topk" and not (
        args.cuda_graph and args.path == "candidate"
        and args.graph_coalesce_reference_duplicates
        and (args.max_routes_per_token_per_rank or 0) >= 16
    ):
        raise ValueError(
            "skewed-duplicate-topk requires a candidate Graph correctness run, "
            "--graph-coalesce-reference-duplicates and route capacity >= 16"
        )
    _validate_rank_epilogue_barrier(
        args.candidate_rank_epilogue_barrier,
    ...
    shape = _shape(args.tokens, args.hidden, args.activation)
    if (
        args.candidate_rank_push_batch_invariants
        and shape.tokens % args.candidate_rank_push_batch_size
    ):
        raise ValueError(
            "rank-push batch invariants require complete source-token batches"
        )
    if (
        args.candidate_rank_push_count_prefetch
        and shape.tokens % args.candidate_rank_push_batch_size
    ):
        raise ValueError(
            "rank-push count prefetch requires complete source-token batches"
        )
    if args.candidate_rank_push_count_from_slot_map:
        if getattr(
            args, "candidate_rank_push_publication", "per_tile_counter"
        ) != "per_tile_counter":
            raise ValueError(
                "rank-push map-derived count requires per_tile_counter publication"
            )
        incompatible = []
        if args.candidate_rank_push_count_prefetch:
            incompatible.append("count prefetch")
        if args.candidate_rank_push_map_prefetch:
            incompatible.append("map prefetch")
        if getattr(args, "candidate_rank_push_single_row_fastpath", False):
    ...
    if args.candidate_rank_push_count_wave_broadcast:
        if getattr(
            args, "candidate_rank_push_publication", "per_tile_counter"
        ) != "per_tile_counter":
            raise ValueError(
        ...
        if args.candidate_rank_push_tile_index_recurrence:
    ...
    if args.candidate_rank_push_tile_index_recurrence:
        if not args.candidate_rank_push_batch_invariants:
            raise ValueError(
    ...
    if args.candidate_rank_push_acquire_cohort == 2:
        if not args.candidate_rank_push_batch_invariants:
            raise ValueError(
        ...
        if (
            args.candidate_rank_push_batch_size < 2
            or args.candidate_rank_push_batch_size % 2
        ):
            raise ValueError(
    ...
    if args.candidate_node_reduce_token_owner_fastpath and not (
        node_reduce_token_owner_supported(
            max_tokens=shape.tokens,
            node_reduce_blocks=args.candidate_node_reduce_blocks,
        )
    ):
        raise ValueError(
            "node-reduce token-owner fast path requires 2*tokens divisible "
            "by 4*candidate-node-reduce-blocks"
        )
    if args.candidate_node_reduce_token_owner_mask_prefetch:
        if not args.candidate_node_reduce_token_owner_fastpath:
            raise ValueError(
        ...
        if not node_reduce_token_owner_mask_prefetch_supported(
            ...
            raise ValueError(
                "node-reduce token-owner mask prefetch requires all owned "
                "token masks to fit in one wave"
            )
    if args.cuda_graph and not args.torch_profiler_dir:
        raise ValueError("graph profiling requires --torch-profiler-dir")
    if args.cuda_graph and (args.candidate_mode != "full" or args.stage1_diagnostic_phase != "full"):
        raise ValueError("graph profiling requires full Stage1/Stage2")
    if args.cuda_graph and args.path == "mori" and args.mori_mode != "gmm2_combine":
        raise ValueError("graph MORI profiling requires --mori-mode gmm2_combine")
    ...
    contract = {
        ...
        "stage1_workers": args.stage1_workers if args.stage1_workers is not None else args.stage2_workers,
        ...
        "candidate_group_batch": args.candidate_group_batch,
        "candidate_group_completion_unit": "m_tile" if args.candidate_group_batch else "route",
        ...
        "candidate_node_reduce_token_owner_fastpath": (
            args.candidate_node_reduce_token_owner_fastpath
        ),
        "candidate_node_reduce_token_owner_mask_prefetch": (
            args.candidate_node_reduce_token_owner_mask_prefetch
        ),
        "candidate_node_reduce_expected_popcount": (
            args.candidate_node_reduce_expected_popcount
        ),
        ...
        "candidate_rank_reduce_blocks": args.candidate_rank_reduce_blocks if args.candidate_rank_accumulation_mode == "reduce_push" else 0,
        "candidate_rank_push_batch_size": args.candidate_rank_push_batch_size,
        "candidate_rank_push_batch_invariants": (
            args.candidate_rank_push_batch_invariants
        ),
        "candidate_rank_push_count_prefetch": (
            args.candidate_rank_push_count_prefetch
        ),
        "candidate_rank_push_count_wave_broadcast": (
            args.candidate_rank_push_count_wave_broadcast
        ),
        "candidate_rank_push_map_prefetch": (
            args.candidate_rank_push_map_prefetch
        ),
        "candidate_rank_push_count_from_slot_map": (
            args.candidate_rank_push_count_from_slot_map
        ),
        "candidate_rank_push_tile_index_recurrence": (
            args.candidate_rank_push_tile_index_recurrence
        ),
        "candidate_rank_push_acquire_cohort": (
            args.candidate_rank_push_acquire_cohort
        ),
        "candidate_gemm_use_nt": args.candidate_gemm_use_nt,
        "candidate_rank_push_use_nt": args.candidate_push_use_nt,
        "candidate_rank_epilogue_barrier": args.candidate_rank_epilogue_barrier,
        ...
        "device_timeline_history_depth": args.device_timeline_history_depth,
    ...
    if args.cuda_graph:
        contract.update({
            ...
            "statistic": args.graph_comparison_statistic,
    ...
    if args.path == "candidate":
        path = _build_candidate(
            ...
            args.cuda_graph,
            args.candidate_group_batch,
            args.device_timeline_history_depth,
            rank_epilogue_barrier=args.candidate_rank_epilogue_barrier,
            stage1_workers=args.stage1_workers,
            rank_reduce_blocks=args.candidate_rank_reduce_blocks,
            rank_push_batch_size=args.candidate_rank_push_batch_size,
            rank_push_batch_invariants=(
                args.candidate_rank_push_batch_invariants
            ),
            rank_push_count_prefetch=(
            ...
            gemm_use_nt=args.candidate_gemm_use_nt,
            rank_push_use_nt=args.candidate_push_use_nt,
            node_reduce_token_owner_fastpath=(
                args.candidate_node_reduce_token_owner_fastpath
            ),
            node_reduce_token_owner_mask_prefetch=(
    ...
    try:
        if args.cuda_graph:
            ...
            run_graph_profile(path, shared, shape, args, contract, rank, world, device)
            return 0
        result = _run(
            path,
            device,
