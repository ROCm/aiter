...
"""Step 4: fused min-expert push + per-expert GEMM1.

Consumers wait ``received[e] == expected[e]`` while producers continue.
Correctness matches packed NCCL GEMM1. Rank 0 delays the second-half
min-expert prefix; expert 0 tiles must finish during that delay.

Run with::

    MORI_SOCKET_IFNAME=lo MORI_SHMEM_HEAP_SIZE=40G PYTHONPATH="$PWD" \\
      torchrun --standalone --nproc-per-node=8 \\
      op_tests/multigpu_tests/test_tp_incremental_stage1.py
"""
...
def test_tp_incremental_fused(
    ...
    want_skew = m_local >= 64 and int(expected[0].item()) > 0
    skew_sleeps = SKEW_SLEEPS if want_skew else 0
    workspace.reset()
    out_fused, os_fused = _alloc_out(num_valid, inter_dim, device)
    run_tp_incremental_fused(
        workspace,
        x_fp8,
        x_scale,
    ...
    gemm_err = checkAllclose(
        ...
        atol=0,
        printLog=(rank == 0),
        msg="fused gemm1 vs packed NCCL A",
    )
    gemm_scale_err = checkAllclose(
        ...
        atol=0,
        printLog=(rank == 0),
        msg="fused gemm1 scale vs packed NCCL A",
    )
    _agree(
        recv_ok and gemm_err == 0 and gemm_scale_err == 0,
        device,
        f"fused mismatch: recv_ok={bool(recv_ok)} out_err={gemm_err} "
        f"scale_err={gemm_scale_err}",
    )

    overlap = int(workspace.overlap.item())
    if want_skew:
        overlap_ok = overlap == 1 if rank == 0 else True
        _agree(
            overlap_ok,
            device,
            f"rank0 expert0 GEMM did not overlap delayed second-half publish "
            f"(overlap={overlap} expert0_done={int(workspace.expert0_done.item())})",
        )
...
def main():
    ...
    try:
        ...
        if gfx not in SUPPORTED_GFX:
            if rank == 0:
                aiter.logger.warning(
                    "tp incremental fused unsupported on %s; skipping", gfx
                )
            return
        if world < 2:
            if rank == 0:
        ...
        for _dtype, m_local, (model_dim, inter_dim), experts, topk in itertools.product(
            ...
            if rank == 0:
                aiter.logger.info(
                    "fused case m_local=%s model_dim=%s experts=%s topk=%s",
                    m_local,
                    model_dim,
                    experts,
                    topk,
        ...
        if rank == 0:
            ...
            aiter.logger.info(
                "tp incremental fused summary (markdown):\n%s",
