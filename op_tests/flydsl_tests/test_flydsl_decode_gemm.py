...
def _fp8_wave_config(m: int, k: int) -> WaveDecodeConfig:
    return WaveDecodeConfig(
        m_per_wave=m,
        n_per_wave=1,
        kvec=8,
        prefetch_depth=0,
        waves_per_eu=4,
        reduction=ReductionMode.DPP if k % 8 == 0 else ReductionMode.BPERMUTE,
        # E4M3 has 3 mantissa bits, so the gfx950 FP8->BF16 convert is exact and
        # the faster dot2 contraction costs no accuracy over the f32 path.
        contraction=(
            ContractionMode.DOT2_BF16
            if ARCH == "gfx950"
...
def _run_fp8_per_tensor(m: int, n: int, k: int) -> None:
    a, b = _inputs(m, n, k)
    aq, x_scale = aiter.per_tensor_quant(a, quant_dtype=dtypes.fp8)
    bq, w_scale = aiter.per_tensor_quant(b, quant_dtype=dtypes.fp8)
    # Compare against the fp8 round-trip, not the bf16 product: the kernel and
    # the oracle see identical quantized operands, so quantization noise cancels
    # and only f32 reassociation plus the bf16 output rounding remain.
    reference = (
        (aq.float() * x_scale.float()) @ (bq.float() * w_scale.float()).T
    ).bfloat16()
    output = _output(m, n)
    returned = gemm_decode_fp8(
        aq,
        bq,
        output,
        x_scale,
        w_scale,
        _fp8_wave_config(m, k),
    )
    torch.cuda.synchronize()
    assert returned is output
    assert torch.isfinite(output).all()
    # bf16 carries 8 mantissa bits, so the store alone costs ~2e-3 relative;
    # the absolute floor tracks the summand magnitude so cancelled entries are
    # not held to a relative bound they cannot meet.
...
def test_wave_fp8_per_tensor_single_row() -> None:
    _run_fp8_per_tensor(1, 64, 1024)


def test_wave_fp8_per_tensor_odd_n_and_k_tail() -> None:
    # K stays a multiple of 4: the FP8 path addresses operands as packed words.
    _run_fp8_per_tensor(2, 65, 260)
