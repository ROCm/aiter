...
from aiter.ops.flydsl.kernels.kimi_k3_kda_input_group64_gfx950 import (
...
def test_group64_projection_matches_dequantized_reference_on_gfx950() -> None:
    ...
    hidden = torch.sin(columns * 0.03125).to(torch.bfloat16).unsqueeze(0)
    stored_weight = (
    ...
    hidden_groups = hidden.float().reshape(groups_per_row, group_size)
    dequantized = (
