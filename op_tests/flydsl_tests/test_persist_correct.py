"""Correctness: jagged_dense_bmm_persist vs torch eager, all 4 headline shapes.
Tests BOTH a UNIFORM Mi (regression check vs the static-grid baseline) and a
SKEWED (power-law, some empty) Mi distribution (where the persistent scheduler
should help). cos > 0.999 required."""
import sys
import torch
import importlib
...
def skewed_mi(B, msl, seed=1234):
    """Power-law-ish skew: a few groups near msl, many short, some empty."""
    g = torch.Generator().manual_seed(seed)
    u = torch.rand(B, generator=g)
    mi = (msl * (u ** 4)).floor().to(torch.int64)  # heavy tail toward 0
    # force a few empties and a couple of long ones
    mi[: max(1, B // 5)] = 0
    mi[-1] = msl
    if B > 1:
        mi[-2] = int(0.9 * msl)
...
print("\n=== SKEWED Mi (power-law, some empty) ===")
for (B, D, Kout) in SHAPES:
    mi = skewed_mi(B, MSL)
