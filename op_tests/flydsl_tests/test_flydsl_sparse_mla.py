...
def test_flydsl_sparse_mla_prefill_ragged(num_heads: int, with_sink: bool) -> None:
    from aiter.ops.flydsl import flydsl_sparse_mla_prefill

    torch.manual_seed(17 + num_heads)
    q = torch.randn(4, num_heads, HEAD_DIM, dtype=torch.bfloat16, device="cuda") * 0.125
    ...
    indices = torch.tensor([0, 2, -1, 8, 1, 3, 6], dtype=torch.int32, device="cuda")
...
def test_flydsl_sparse_mla_prefill_h16_multitile_tail(
    ...
    q = torch.randn(2, 16, HEAD_DIM, dtype=torch.bfloat16, device="cuda") * 0.125
...
def test_flydsl_sparse_mla_prefill_h16_overallocated_ragged_capacity(
    ...
    q = torch.randn(5, 16, HEAD_DIM, dtype=torch.bfloat16, device="cuda") * 0.125
    ...
    indptr = torch.tensor([0, 1, 3, 3, 6, 10], dtype=torch.int32, device="cuda")
    sink = (
        torch.linspace(-0.5, 0.5, 16, dtype=torch.float32, device="cuda")
        if with_sink
        else None
...
def test_flydsl_sparse_mla_prefill_h16_split_boundaries(
    ...
    q = (
        torch.randn(len(lengths), 16, HEAD_DIM, dtype=torch.bfloat16, device="cuda")
        * 0.125
    )
    kv = torch.randn(num_kv, HEAD_DIM, dtype=torch.bfloat16, device="cuda") * 0.125
    rows = []
    for query, length in enumerate(lengths):
        row = (
            torch.arange(length, dtype=torch.int32, device="cuda") * 13 + query * 17
        ) % num_kv
        if length > 3:
            row[3] = -1
        if length > 67:
