...
Run (single rank is enough; world_size=1 => every expert is local, dest_pe=0):
    ...
    AITER_EP_PUSH_GROUP=1 FLYDSL_GPU_ARCH=$(python -c \
...
def main():
    assert os.environ.get("AITER_EP_PUSH_GROUP") in ("1", "true", "True", "yes", "on"), (
        "set AITER_EP_PUSH_GROUP=1"
    )
    ...
    assert cfg.push_group, "config did not pick up AITER_EP_PUSH_GROUP"
