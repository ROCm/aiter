...
# args: <B> <D> <Kout> <Mi> <W> <C> <S>   (D=reduction K, Kout=output N, S=epi_strips)
B=int(sys.argv[1]); D=int(sys.argv[2]); Kout=int(sys.argv[3]); Mi=int(sys.argv[4])
W=int(sys.argv[5]); C=int(sys.argv[6]); S=int(sys.argv[7])
N=Kout; K=D; msl=Mi
...
def launch(): m.jagged_dense_bmm(tC,tA,dt,bf,sod,B,msl,stream=st,xcd_c=C,xcd_w=W,epi_strips=S)
