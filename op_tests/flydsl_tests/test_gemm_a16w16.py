...
@pytest.mark.parametrize(
    "M, N, K", [(64, 64, 2048), (128, 128, 1024), (64, 5120, 2880)]
)
def test_gemm_a16_w16_tile_k256(M, N, K):
    torch.cuda.empty_cache()
    x, w, _, _ = _generate_inputs(M, N, K, torch.bfloat16)
...
def _splitk_configs():
    ...
    return [
        ...
        (128, 128, 1024, 64, 64, 256, 2, 2, 2, 2),  # tile_k=256, 4 tiles, 2/split
        (
            64,
            5120,
            2880,
            ...
            4,
            2,
        ),  # tile_k=256 tuned shape (K pads 2880->3072)
    ]
