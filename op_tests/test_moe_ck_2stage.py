...
def run_2stage_moe(
    ...
    if use_fp8_blockscale:
        ...
        w1_fp32 = torch.randn(
            (num_experts, inter_dim * 2, hidden_dim), dtype=torch.float32, device="cuda"
        )
        w2_fp32 = torch.randn(
            (num_experts, hidden_dim, inter_dim), dtype=torch.float32, device="cuda"
        )
        ...
        w1_scale = (
            torch.ones(
                (num_experts, num_blocks_w1_n, num_blocks_w1_k),
                dtype=torch.float32,
                device="cuda",
            )
            * 0.01
        ...
        w2_scale = (
            torch.ones(
                (num_experts, num_blocks_w2_n, num_blocks_w2_k),
                dtype=torch.float32,
                device="cuda",
            )
            * 0.01
        ...
        a1_scale = torch.ones(
            (num_tokens, (hidden_dim + block_size - 1) // block_size),
            dtype=torch.float32,
            device="cuda",
        )
        a2_scale = torch.ones(
            (num_tokens, topk, (inter_dim + block_size - 1) // block_size),
            dtype=torch.float32,
            device="cuda",
        )
        ...
        # BF16 case
        w1 = 0.1 * torch.randn(
            (num_experts, inter_dim * 2, hidden_dim), dtype=dtype, device="cuda"
        )
        w2 = 0.1 * torch.randn(
            (num_experts, hidden_dim, inter_dim), dtype=dtype, device="cuda"
        )
        w1_scale = None
        w2_scale = None
        a1_scale = None
    ...
    stage1_error = checkAllclose(
        inter_ref.to(dtype), inter_ck, tol_err_ratio=0.1 if use_fp8_blockscale else 0.01
    )
    ...
    output_out_validation = torch.zeros(
    ...
    stage2_error = checkAllclose(
        output_ref.to(dtype),
        output_ck,
        tol_err_ratio=0.1 if use_fp8_blockscale else 0.01,
    )
    ...
    if exclude_activations_from_bw:
        ...
        stage1_read_bytes = (
            effective_experts * inter_dim * 2 * hidden_dim * bytes_per_elem
        )  # w1 only
        stage1_write_bytes = 0
    else:
        stage1_read_bytes = (
            num_tokens * hidden_dim * bytes_per_elem  # input
            + effective_experts * inter_dim * 2 * hidden_dim * bytes_per_elem
        )  # w1 (effective experts)
    ...
    if exclude_activations_from_bw:
        # Only count weight reads (assuming activations stay in cache)
        stage2_read_bytes = (
            effective_experts * hidden_dim * inter_dim * bytes_per_elem
        )  # w2 only
        stage2_write_bytes = 0
    else:
        stage2_read_bytes = (
            num_tokens * topk * inter_dim * bytes_per_elem  # inter
            + effective_experts * hidden_dim * inter_dim * bytes_per_elem
        )  # w2 (effective experts)
...
def test_moe_ck_2stage_correctness(
    num_experts, num_tokens, topk, hidden_dim, inter_dim, dtype_str
):
    ...
    assert (
        result["stage1_error"] <= tol
    ), f"Stage1 error {result['stage1_error']} exceeds tolerance {tol}"
    assert (
        result["stage2_error"] <= tol
    ), f"Stage2 error {result['stage2_error']} exceeds tolerance {tol}"
...
if __name__ == "__main__":
    ...
    collected = []
    for i, (num_tokens, hidden_dim, inter_dim, num_experts, topk) in enumerate(
        configs, 1
    ):
        print(
    ...
    if len(errors) > 0:
        print(
            f"WARNING: {len(errors)} configuration(s) had errors exceeding tolerance {tol}!"
        )
