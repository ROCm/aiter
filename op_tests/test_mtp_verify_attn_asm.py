...
def test_correctness(ctx, num_seqs, num_q_heads, num_kv_heads, k_descale, v_descale):
    caches, bt, q, cu_q, seq_lens, kd, vd, _kvlen = make_inputs(
        ctx, num_seqs, num_q_heads, num_kv_heads, k_descale, v_descale
    )
    k2, v2 = caches[0]
    out = aiter.mtp_verify_attn_fwd_asm(
        q, k2, v2, bt, seq_lens, cu_q, kd, vd, 1.0 / math.sqrt(HD)
    )
...
def test_perf(ctx, num_seqs, num_q_heads, num_kv_heads, replicas=8):
    ...
    def run():
        ...
        state["i"] += 1
        return aiter.mtp_verify_attn_fwd_asm(
            q, k2, v2, bt, seq_lens, cu_q, kd, vd, scale, num_segments=num_segments
        )
...
def main():
    ...
    parser.add_argument(
        "--perf", action="store_true", help="also run the bandwidth benchmark"
    )
    args = parser.parse_args()
    if get_gfx() not in SUPPORTED_GFX:
        print(
            f"skip: mtp_verify_attn asm kernel is only shipped for {SUPPORTED_GFX}, got {get_gfx()}"
        )
        return
    rows = []
