...
def _run_decode(q, kv_buffer_fp8, max_split, device="cuda"):
    ...
    out = torch.empty(
        (BATCH * DECODE_QLEN, NHEAD, V_HEAD_DIM), dtype=torch.bfloat16, device=device
    )
    mla_decode_fwd(
        q,
...
def main():
    gfx = get_gfx()
    print(
        f"[det] gfx={gfx} nhead={NHEAD} fp8/fp8 page_size={PAGE_SIZE} "
    ...
    print(
        f"[det] split=1 run-to-run  max|d| = {d_s1:.3e}   (expect 0: single-split reduce is a no-op)"
    )
    print(f"[det] split={N} run-to-run  max|d| = {d_sN:.3e}")
    print(
        f"[det] split=1 vs split={N} max|d| = {d_1vN:.3e}   "
    ...
    if det_env:
        ...
        print(
            f"[det] deterministic clamp set: split={N} now matches split=1? "
    ...
    ok_baseline = d_s1 == 0
    print(
        f"[det] baseline single-split deterministic: {'PASS' if ok_baseline else 'FAIL'}"
    )
    print(
        "[det] Re-run with AITER_MLA_DECODE_DETERMINISTIC=1 to confirm the "
