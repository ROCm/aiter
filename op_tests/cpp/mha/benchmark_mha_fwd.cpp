...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    // For FP8 quantization, we need descale values
    ...
    // Initialize descale values based on qscale type
    ...
    {
        // For blockscale, fill with uniform distribution
        float qkv_max       = 3.f;
    ...
    const auto init_args = [&, k_paddings_ = seqlen_kpads](auto& args) {
        ...
        else // fmha_fwd_args or fmha_fwd_splitkv_args
        {
            ...
            if constexpr(std::is_same_v<aiter::mha_fwd_args, std::decay_t<decltype(args)>>)
            {
                ...
                args.has_lse             = lse;
                args.qscale_type         = static_cast<int>(qscale.type);
                args.has_sink            = false;
                args.sink_size           = 0;
                args.min_seqlen_q        = 0;
