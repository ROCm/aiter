...
auto create_args(int argc, char *argv[]) {
  ...
  arg_parser
      ...
      .insert(
          ...
          "also with \"-s=s0,s1,s2...\" comma seperated int to set per batch "
          "seqlen(group-mode)")
      .insert("s_k", "-1",
      ...
      .insert("s_kpad", "-1",
              "seqlen_k stride between 2 batches, currently used in group-mode "
              "only\n"
              "for kv-cache case, each batch [1,s,h,d]/[1,h,s,d] can have a "
              "stride\n"
      ...
      .insert("range_q", "16",
              "per-tensor quantization range of q. used if squant=1.")
      .insert("range_k", "16",
              "per-tensor quantization range of k. used if squant=1.")
      .insert("range_v", "16",
              "per-tensor quantization range of v. used if squant=1.")
      .insert("range_p", "1",
              "per-tensor quantization range of p [e^(s-m)]. used if squant=1.")
      .insert("range_o", "16",
              "per-tensor quantization range of o (p*v). used if squant=1.")
      .insert("squant", "auto",
              "if using static quantization fusion or not. auto: fp8 will "
              "default use squant, "
              "other will not\n"
              "0: no static quant(not implemented) 1: apply scale_p and "
              "scale_o with respect to "
              "P and O.\n"
              "calculate scale_s, scale_p, scale_o according to range_q, "
              "range_k, range_v, "
              "range_p, range_o")
      .insert("iperm", "1",
              "permute input\n"
      ...
      .insert("bias", "n",
              "n or 0, no bias\n"
              "e(lementwise) or 1, elementwise bias with 1*1*s*s. e:1, "
              "1*h*s*s. e:2, b*h*s*s\n"
      ...
      .insert(
          ...
          "'t:l,r', top-left sliding window attn(swa) with FA style left right "
          "size\n"
          "'b:l,r', bottom-r sliding window attn(swa) with FA style left right "
          "size\n"
          "'xt:window_size', xformer style masking from top-left, window_size "
          "negative is "
          "causal, positive is swa\n"
          "'xb:window_size', xformer style masking from bottom-r, window_size "
          "negative is "
          "causal, positive is swa\n"
          "'g:y,x', generic attention mask coordinate with y/x size (only "
          "debug purpose for "
          "now)")
      .insert("vlayout", "r",
              "r for row-major(seqlen*hdim), c for col-major(hdim*seqlen)")
      .insert("lse", "0", "0 not store lse, 1 store lse")
      .insert("kname", "0", "if set to 1 will print kernel name")
      .insert("init", "uf",
              "init method. ui, uniform random int, ni, normalized random int\n"
              "uf, uniform random float, nf, normalized random float, tf, trig "
              "float, uf:q, "
              "quantization")
      .insert("seed", "11939",
      ...
      .insert("num_splits", "1",
              "# of splits for key/value. 0 to determine actual number by "
              "heuristic")
      .insert("page_block_size", "0",
      ...
      .insert("fwd_v3", "0",
              "if set to 1, some cases will call the fwd v3 kernel");
...
template <> auto get_elimit<FmhaFwdBf16>(std::string /*init_method*/) {
  double rtol = 1e-2;
  double atol = 1e-2;
  return ck_tile::make_tuple(rtol, atol);
}

template <> auto get_elimit<FmhaFwdFp8>(std::string init_method) {
...
int override_num_splits_if_necessary(int batch, int nhead, int max_seqlen_q,
                                     int hdim_v, float p_drop, int num_splits) {
  int device;
  ...
  if (num_splits < 1 && p_drop == 0.0f) {
    return num_splits_heuristic(batch * nhead * num_m_blocks,
                                props.multiProcessorCount * 2, num_n_blocks,
                                128);
  }
...
bool run(const ck_tile::ArgParser &arg_parser) {
  ...
  if constexpr (!(std::is_same_v<DataTypeConfig, FmhaFwdFp16> ||
    ...
    if (0 < rotary_dim) {
      std::cerr << "rotary embedding is only available for data type=fp16|bf16"
                << std::endl;
      return false;
    }
  ...
  if (!(rotary_dim <= hdim_q)) {
    std::cerr << "rotary_dim should be less than or equal to head dim for q"
              << std::endl;
    return false;
  ...
  if (0 < page_block_size) {
    std::cerr << "paged-kvcache is not supported. ignoring the "
                 "'page_block_size' option"
              << std::endl;
    page_block_size = 0;
  }
#endif
  if (!(page_block_size % 128 == 0)) {
    std::cerr << "only paged-kvcache block size divisible by 128 are currently "
                 "supported"
              << std::endl;
    return false;
  }
  ...
  const bool use_kvcache =
      (need_append_kvcache || use_cache_batch_idx || 0 < page_block_size);

  auto [seqlen_qs, seqlen_ks, seqlen_kpads] = decode_seqlen(
      mode, batch, arg_parser.get_str("s"), arg_parser.get_str("s_k"),
      arg_parser.get_str("s_kpad"),
      /*seqlen_k_min=*/0 < seqlen_knew ? seqlen_knew : 0, need_append_kvcache);
  // compute kvcache seqlen_k (before appending knew/vnew)
  auto cache_seqlen_ks = seqlen_ks;
  std::transform(cache_seqlen_ks.begin(), cache_seqlen_ks.end(),
  ...
  bool i_perm = arg_parser.get_bool(
      "iperm"); // if true, will be batch * nhead * seqlen * hdim
  bool o_perm = arg_parser.get_bool(
      "operm"); // if false, will be batch * seqlen * nhead * hdim
  ...
  mask_info mask =
      mask_info::decode(arg_parser.get_str("mask"), seqlen_qs[0],
                        seqlen_ks[0]); // TODO: we don't need x/y anymore
...
#if !CK_TILE_FMHA_FWD_SPLITKV_API
  if (num_splits != 1) {
    std::cerr << "split-kv is not supported. ignoring the 'num_splits' option"
              << std::endl;
    num_splits = 1;
  }
#endif
  ...
  float q_dtype_max =
      ck_tile::type_convert<float>(ck_tile::numeric<QDataType>::max());
  float k_dtype_max =
      ck_tile::type_convert<float>(ck_tile::numeric<KDataType>::max());
  float v_dtype_max =
      ck_tile::type_convert<float>(ck_tile::numeric<VDataType>::max());
  float p_dtype_max = v_dtype_max; // assume p and v is the same type
  float o_dtype_max =
      ck_tile::type_convert<float>(ck_tile::numeric<ODataType>::max());
  ...
  if (squant) {
    ...
    scale_o = (o_dtype_max / range_o) * (range_p / p_dtype_max) *
              (range_v / v_dtype_max);
  }
  ...
  auto max_seqlen_q =
      std::numeric_limits<int32_t>::min(); // we will use max seqlen to decide
                                           // grid size
  auto max_seqlen_k = std::numeric_limits<int32_t>::min();
  {
    for (ck_tile::index_t wb = 0; wb < batch; ++wb) {
      const int32_t real_seqlen_q =
          seqstart_q_host[wb + 1] - seqstart_q_host[wb];
      const int32_t real_seqlen_k =
          seqstart_k_host[wb + 1] - seqstart_k_host[wb];
      ...
      flop += nhead * (static_cast<std::size_t>(2) * real_seqlen_q *
                           real_seqlen_k * hdim_q +
                       static_cast<std::size_t>(2) * real_seqlen_q * hdim_v *
                           real_seqlen_k);
  ...
  const ck_tile::index_t max_num_page_blocks =
      (0 < page_block_size
           ? batch * std::max(1, ck_tile::integer_divide_ceil(max_seqlen_k,
                                                              page_block_size))
           : 0);

  // legalize num_splits according to other options
  if (num_splits < 1) {
    num_splits = override_num_splits_if_necessary(batch, nhead, max_seqlen_q,
                                                  hdim_v, p_drop, num_splits);
  }
  if (128 < num_splits) {
  ...
  if (0 < p_drop && (1 < num_splits || use_kvcache)) {
    std::cerr << "dropout is not supoprted by split-kv kernels. ignoring the "
                 "'p_drop' option"
              << std::endl;
    p_drop = 0.0f;
  }
  ...
  static const auto get_lengths = [](bool permute, ck_tile::index_t b /*batch*/,
  ...
  ck_tile::HostTensor<KDataType> k_host(
      0 < page_block_size
          ? get_lengths(i_perm, max_num_page_blocks, nhead_k, page_block_size,
                        hdim_q)
          : get_lengths(i_perm, shape_batch, nhead_k, shape_seqlen_k, hdim_q));
  /// NOTICE: always use same shape for knew_host & vnew_host in batch/group
  /// mode
  ck_tile::HostTensor<KDataType> knew_host(
      0 < seqlen_knew ? get_lengths(i_perm, batch, nhead_k, seqlen_knew, hdim_q)
                      : std::array<ck_tile::index_t, 4>{
                            1, 1, 1, 1} /* dummy shape for simplifying code */);
  ck_tile::HostTensor<VDataType> v_host(
      0 < page_block_size
          ? (is_v_rowmajor ? get_lengths(i_perm, max_num_page_blocks, nhead_k,
                                         page_block_size, hdim_v)
                           : get_lengths(i_perm, max_num_page_blocks, nhead_k,
                                         hdim_v, page_block_size))
          : (is_v_rowmajor ? get_lengths(i_perm, shape_batch, nhead_k,
                                         shape_seqlen_k, hdim_v)
                           : get_lengths(i_perm, shape_batch, nhead_k, hdim_v,
                                         shape_seqlen_k)));
  ck_tile::HostTensor<VDataType> vnew_host(
      0 < seqlen_knew
          ? (is_v_rowmajor
                 ? get_lengths(i_perm, batch, nhead_k, seqlen_knew, hdim_v)
  ...
  ck_tile::HostTensor<SaccDataType> alibi_slope_host(
      bias.type == bias_enum::alibi
          ? (bias.rank_info == 0
                 ? std::array<ck_tile::index_t, 2>{1, nhead}
  ...
  ck_tile::HostTensor<LSEDataType> lse_acc_host(
      1 < num_splits || use_kvcache
          ? std::array<ck_tile::index_t, 4>{shape_batch, nhead, num_splits,
                                            shape_seqlen_q}
          : std::array<ck_tile::index_t, 4>{1, 1, 1, 1});
  ck_tile::HostTensor<OaccDataType> o_acc_host(
      1 < num_splits || use_kvcache
          ? std::array<ck_tile::index_t, 5>{shape_batch, nhead, num_splits,
                                            shape_seqlen_q, hdim_v}
  ...
  ck_tile::HostTensor<LSEDataType> lse_host(
      lse ? std::array<ck_tile::index_t, 3>{shape_batch, nhead, shape_seqlen_q}
          : std::array<ck_tile::index_t, 3>{
                1, 1, 1} /* dummy shape for simplifying code */);
  ...
  ck_tile::HostTensor<RandValOutputDataType> randval_host(
      p_drop > 0
          ? get_lengths(true, shape_batch, nhead, shape_seqlen_q, max_seqlen_k)
          : std::array<ck_tile::index_t, 4>{1, 1, 1, 1});

  ck_tile::HostTensor<int32_t> block_table_host(
      0 < page_block_size
          ? std::array<ck_tile::index_t, 2>{batch, max_num_page_blocks / batch}
          : std::array<ck_tile::index_t, 2>{1, 1});

  ck_tile::HostTensor<int32_t> cache_batch_idx_host(
      use_cache_batch_idx ? std::array<ck_tile::index_t, 1>{batch}
                          : std::array<ck_tile::index_t, 1>{1});

  if (init_method == "ui" || init_method == "0") {
    ck_tile::FillUniformDistributionIntegerValue<QDataType>{-3.f, 3.f,
                                                            seed}(q_host);
    ck_tile::FillUniformDistributionIntegerValue<KDataType>{-3.f, 3.f,
                                                            seed}(k_host);
    ck_tile::FillUniformDistributionIntegerValue<KDataType>{-3.f, 3.f,
                                                            seed}(knew_host);
    ck_tile::FillUniformDistributionIntegerValue<VDataType>{-3.f, 3.f,
                                                            seed}(v_host);
    ck_tile::FillUniformDistributionIntegerValue<VDataType>{-3.f, 3.f,
                                                            seed}(vnew_host);
    ck_tile::FillUniformDistributionIntegerValue<BiasDataType>{-3.f, 3.f,
                                                               seed}(bias_host);
  } else if (init_method == "ni") {
    ck_tile::FillNormalDistributionIntegerValue<QDataType>{-3.f, 3.f,
                                                           seed}(q_host);
    ck_tile::FillNormalDistributionIntegerValue<KDataType>{-3.f, 3.f,
                                                           seed}(k_host);
    ck_tile::FillNormalDistributionIntegerValue<KDataType>{-3.f, 3.f,
                                                           seed}(knew_host);
    ck_tile::FillNormalDistributionIntegerValue<VDataType>{-3.f, 3.f,
                                                           seed}(v_host);
    ck_tile::FillNormalDistributionIntegerValue<VDataType>{-3.f, 3.f,
                                                           seed}(vnew_host);
    ck_tile::FillNormalDistributionIntegerValue<BiasDataType>{-3.f, 3.f,
                                                              seed}(bias_host);
  ...
  {
    ck_tile::FillUniformDistribution<QDataType>{-q_dtype_max, q_dtype_max,
                                                seed}(q_host);
    ck_tile::FillUniformDistribution<KDataType>{-k_dtype_max, k_dtype_max,
                                                seed}(k_host);
    ck_tile::FillUniformDistribution<KDataType>{-k_dtype_max, k_dtype_max,
                                                seed}(knew_host);
    ck_tile::FillUniformDistribution<VDataType>{-v_dtype_max, v_dtype_max,
                                                seed}(v_host);
    ck_tile::FillUniformDistribution<VDataType>{-v_dtype_max, v_dtype_max,
                                                seed}(vnew_host);
    ...
    ck_tile::FillUniformDistribution<BiasDataType>{-qscale_bias, qscale_bias,
                                                   seed}(bias_host);
  }
  if (bias.type == bias_enum::alibi) {
    ...
    if (bias.rank_info == 0) {
      ...
      for (auto i_b = 0; i_b < batch; i_b++) {
        std::copy(slopes.begin(), slopes.end(),
                  alibi_slope_host.begin() + i_b * nhead);
      }
    }
  }
  ...
  ck_tile::DeviceMem rotary_cos_buf(
      rotary_cos_host.get_element_space_size_in_bytes());
  ck_tile::DeviceMem rotary_sin_buf(
      rotary_sin_host.get_element_space_size_in_bytes());
  ck_tile::DeviceMem drop_seed_buf(drop_prefs ? sizeof(uint64_t) : 0);
  ck_tile::DeviceMem drop_offset_buf(drop_prefs ? sizeof(uint64_t) : 0);
  ck_tile::DeviceMem randval_buf(
      randval_host.get_element_space_size_in_bytes());
  ck_tile::DeviceMem alibi_slope_buf(
      alibi_slope_host.get_element_space_size_in_bytes());
  ck_tile::DeviceMem block_table_buf(
      block_table_host.get_element_space_size_in_bytes());
  ck_tile::DeviceMem cache_batch_idx_buf(
      cache_batch_idx_host.get_element_space_size_in_bytes());
  ...
  seqstart_k.ToDevice(seqlen_kpads[0] < 0
                          ? seqstart_k_host.data()
                          : seqstart_k_with_padding_host.data());
  seqlen_k_buf.ToDevice((mode == mode_enum::batch && use_kvcache) ||
                                0 <= seqlen_kpads[0]
                            ? seqlen_ks.data()
                            : nullptr);
  cache_seqlen_k_buf.ToDevice(need_append_kvcache ? cache_seqlen_ks.data()
                                                  : nullptr);
  ...
  std::cout << "[" << prec << "|" << mode << "|" << io_layout(i_perm, o_perm)
            << "] b:" << batch << ", h:" << nhead << "/" << nhead_k
            << ", s:" << seqlen_qs[0] << "/" << seqlen_ks[0]
            << (seqlen_kpads[0] < 0 ? ""
                                    : (std::string("(") +
                                       std::to_string(seqlen_kpads[0]) + ")"))
            << ", d:" << hdim_q << "/" << hdim_v << ", scale_s:" << scale_s
            << ", bias:" << bias << ", p_drop:" << p_drop << ", lse:" << lse
            << ", squant:" << squant << ", mask:" << mask << ", v:" << vlayout;
  ...
  const auto init_args = [&, k_paddings_ = seqlen_kpads](auto &args) {
    ...
    /// NOTE: we broadcast bias from [1, 1, seqlen_q, seqlen_k] to [batch,
    /// nhead, seqlen_q,
    ...
    const ck_tile::index_t stride_bias =
        (i_perm ? max_seqlen_k : 1 * max_seqlen_k);
    const ck_tile::index_t stride_randval = (max_seqlen_k);
    const ck_tile::index_t stride_o_acc = (hdim_v);
    const ck_tile::index_t stride_o = (o_perm ? hdim_v : nhead * hdim_v);
    // setup nhead_stride_* arguments
    const ck_tile::index_t nhead_stride_q =
        (i_perm ? shape_seqlen_q * hdim_q : hdim_q);
    ...
    const ck_tile::index_t nhead_stride_o_acc =
        (num_splits * shape_seqlen_q * hdim_v);
    const ck_tile::index_t nhead_stride_o =
        (o_perm ? shape_seqlen_q * hdim_v : hdim_v);
    ...
    const ck_tile::index_t batch_stride_bias =
        (0 * nhead * shape_seqlen_q * max_seqlen_k);
    ...
    const ck_tile::index_t batch_stride_lse_acc =
        (nhead * num_splits * shape_seqlen_q);
    const ck_tile::index_t batch_stride_o_acc =
        (nhead * num_splits * shape_seqlen_q * hdim_v);
    const ck_tile::index_t batch_stride_o = (nhead * shape_seqlen_q * hdim_v);
    const ck_tile::index_t batch_stride_block_table =
        (max_num_page_blocks / batch);
    ...
    if constexpr (std::is_same_v<fmha_fwd_appendkv_args,
      ...
      args.rotary_cos_ptr =
          (0 < rotary_dim ? rotary_cos_buf.GetDeviceBuffer() : nullptr);
      args.rotary_sin_ptr =
          (0 < rotary_dim ? rotary_sin_buf.GetDeviceBuffer() : nullptr);
      ...
      args.cache_batch_idx =
          (use_cache_batch_idx ? cache_batch_idx_buf.GetDeviceBuffer()
                               : nullptr);
    ...
    } else // fmha_fwd_args or fmha_fwd_splitkv_args
    {
      ...
      args.seqlen_k =
          shape_seqlen_k; // unused in group mode (or kvcache enabled)
      ...
      if constexpr (std::is_same_v<fmha_fwd_args,
                                   std::decay_t<decltype(args)>>) {
        ...
        if (drop_prefs) {
          args.drop_seed_offset =
              std::make_pair(drop_seed_buf.GetDeviceBuffer(),
                             drop_offset_buf.GetDeviceBuffer());
        } else {
          args.drop_seed_offset = std::make_pair(drop_seed, drop_offset);
        }
      } else if constexpr (std::is_same_v<fmha_fwd_splitkv_args,
                                          std::decay_t<decltype(args)>>) {
  ...
  const float fwd_ave_time = [&] {
    ...
    if (1 < num_splits || use_kvcache) {
      ...
      return aiter::mha_fwd_splitkv(fmha_splitkv_args, stream_config, data_type,
                                    mode == mode_enum::group, mask.type,
                                    bias.type, lse);
    }
#endif
    ...
    return aiter::mha_fwd(fmha_args, stream_config, data_type,
                          mode == mode_enum::group, mask.type, bias.type, lse,
                          fwd_v3);
  }();
  ...
  if (do_validation == 2) {
    ...
    naive_a.context_len_ptr =
        nullptr;                      // used when seqlen kv come from a pointer
    naive_a.page_table_ptr = nullptr; // [batch, num_blocks] seqlen_kv is in
                                      // different block(paged attn)
    naive_a.hdim = hdim_q;
    naive_a.hdim_v =
        hdim_v; // could be cross-attn, where V and Q/K hdim are different
    naive_a.batch_q = batch;
    naive_a.batch_kv = batch;
    naive_a.batch_ratio_kv = 1; // batch_q / batch_kv
    naive_a.seqlen_q = seqlen_qs[0];
    naive_a.seqlen_kv =
        seqlen_ks[0]; // if context_len_ptr is not nullptr, ignore this field
    naive_a.nhead_q = nhead;
    naive_a.nhead_kv = nhead_k;
    naive_a.nhead_ratio_kv =
        naive_a.nhead_q / naive_a.nhead_kv; // nhead_q / nhead_kv
    ...
    bool pass_ = ck_tile::check_err(
        o_host, o_naive_ref, std::string("OUT Error: Incorrect results!"),
        rtol_, atol_);
  ...
  for (ck_tile::index_t wb = 0; wb < batch; ++wb) {
    ...
    ck_tile::HostTensor<PDataType> p_host_ref(
        {nhead, real_seqlen_q, real_seqlen_k});
    ck_tile::HostTensor<SMPLComputeDataType> lse_host_ref(
        {nhead, real_seqlen_q});
    ...
    // reference
    ck_tile::reference_batched_gemm<QDataType, KDataType, SaccDataType,
                                    SMPLComputeDataType>(
        q_host_ref, k_host_ref, s_host_ref, ck_tile::identity{},
        ck_tile::identity{}, ck_tile::scales(scale_s));

    if (0.f < logits_soft_cap) {
      ck_tile::reference_unary_elementwise<SaccDataType, SaccDataType,
                                           SaccDataType>(
          s_host_ref, s_host_ref, [logits_soft_cap](SaccDataType logits) {
            return ck_tile::type_convert<SaccDataType>(
                logits_soft_cap * std::tanhf(ck_tile::type_convert<float>(
    ...
    if (bias.type == bias_enum::elementwise_bias) {
      // elementwise bias
      ck_tile::HostTensor<BiasDataType> bias_host_ref(
          {1, real_seqlen_q, real_seqlen_k});
      // clang-format off
            if(i_perm)
      ...
      // broadcast from [1, real_seqlen_q, real_seqlen_k] to [nhead,
      // real_seqlen_q, real_seqlen_k]
      ck_tile::reference_batched_elementwise<SMPLComputeDataType, BiasDataType,
                                             SMPLComputeDataType,
                                             SMPLComputeDataType>(
      ...
      auto alibi_host = [&]() {
        if (mask.type != mask_enum::no_mask) {
          return ck_tile::make_alibi_from_lr_mask<SaccDataType, true>(
              0, mask.left, mask.right, real_seqlen_q, real_seqlen_k,
              static_cast<ck_tile::GenericAttentionMaskEnum>(mask.type));
        } else {
          return ck_tile::Alibi<SaccDataType, true>{
              0, real_seqlen_q, real_seqlen_k,
              ck_tile::AlibiMode::FROM_BOTTOM_RIGHT};
        }
      }();
      ...
      // [nhead, real_seqlen_q, real_seqlen_k]
      ck_tile::reference_batched_elementwise<SMPLComputeDataType, SaccDataType,
                                             SMPLComputeDataType,
                                             SMPLComputeDataType>(
    ...
    if (p_drop > 0) {
      ...
      ck_tile::reference_batched_dropout(p_host_ref, randval_host_ref,
                                         p_undrop_in_uint8_t, rp_undrop);
    }

    ck_tile::reference_batched_gemm<PDataType, VDataType, OaccDataType,
                                    ODataType>(
        p_host_ref, v_host_ref, o_host_ref, ck_tile::identity{},
        ck_tile::identity{}, oacc_element_func);

    ck_tile::HostTensor<ODataType> o_host_result(
        {nhead, real_seqlen_q, hdim_v});
    // clang-format off
        // permute
    ...
    bool cur_pass = ck_tile::check_err(
        o_host_result, o_host_ref, std::string("OUT Error: Incorrect results!"),
        rtol, atol);
    pass &= cur_pass;
    if (!cur_pass) {
    ...
    if (lse) {
      ck_tile::HostTensor<SMPLComputeDataType> lse_host_result(
          {nhead, real_seqlen_q});
      lse_host_result.ForEach([&](auto &self, auto idx) {
        self(idx) = lse_host(b_idx, idx[0], idx[1] + query_offset);
      });

      cur_pass = ck_tile::check_err(lse_host_result, lse_host_ref,
                                    "LSE Error: Incorrect results!", rtol, atol,
