...
auto create_args(int argc, char *argv[]) {
  ...
  arg_parser.insert("v", "1", "weather do CPU validation or not")
      ...
      .insert("deterministic", "0",
              "if set to 1 will use multi-buffer reduction strategy for dq, "
              "atomic opeartion "
              "will not be used")
      .insert("bwd_v3", "0",
              "if set to 1, some cases will call the bwd v3 dqdkdv kernel")
      .insert("v3_atomic_fp32", "1",
              "if set to 0 will use atomic fp16/bf16(w/o convert_dq kernel) "
              "when bwd_v3 is set to 1")
      .insert("v3_bf16_cvt", "1",
              "float to bf16 convert type when bwd_v3 is set to 1, 0:RTNE; "
              "1:RTNA; 2:RTZ");
...
bool run(const ck_tile::ArgParser &arg_parser) {
  ...
  mask_info mask =
      mask_info::decode(arg_parser.get_str("mask"), seqlen_q, seqlen_k);
  ...
  auto get_lengths = [&](bool permute, ck_tile::index_t b /*batch*/,
  ...
  ck_tile::HostTensor<BiasGradDataType> dbias_host(
      use_dbias ? get_lengths(i_perm, shape_batch, nhead, shape_seqlen_q,
                              max_seqlen_k)
                : std::array<ck_tile::index_t, 4>{
                      1, 1, 1, 1} /* dummy shape for simplifying code */);
  ck_tile::HostTensor<AccDataType> dq_acc_host(std::array<ck_tile::index_t, 5>{
      nsplits, shape_batch, nhead, shape_seqlen_q, hdim_q});

  if (init_method == 0) {
    ck_tile::FillUniformDistributionIntegerValue<QDataType>{-2.f, 2.f,
                                                            seed}(q_host);
    ck_tile::FillUniformDistributionIntegerValue<KDataType>{-2.f, 2.f,
                                                            seed}(k_host);
    ck_tile::FillUniformDistributionIntegerValue<VDataType>{-2.f, 2.f,
                                                            seed}(v_host);
    ck_tile::FillUniformDistributionIntegerValue<BiasDataType>{-2.f, 2.f,
                                                               seed}(bias_host);
    ck_tile::FillUniformDistributionIntegerValue<OGradDataType>{-2.f, 2.f,
                                                                seed}(do_host);
  ...
  std::cout << "[" << prec << "|" << mode << "|" << io_layout(i_perm, o_perm)
            ...
            << ", s:" << seqlen_q << "/" << seqlen_k << ", d:" << hdim_q << "/"
            << hdim_v << ", scale:" << scale << ", bias:" << bias
            << ", dbias:" << use_dbias << ", p_drop:" << p_drop
            << ", s_randval:" << s_randval
            << ", deterministic:" << deterministic << ", mask:" << mask
            << std::flush;

  std::size_t workspace_size = dq_acc_host.get_element_space_size_in_bytes() *
                               sizeof(AccDataType) / (1024 * 1024);
  ...
  auto fmha_args = [&]() {
    ...
    const ck_tile::index_t stride_dbias =
        (i_perm ? max_seqlen_k : nhead * max_seqlen_k);
    ...
    const auto drop_seed_offset =
        [&]() -> decltype(fmha_bwd_args::drop_seed_offset) {
      if (drop_prefs) {
  ...
  float ave_time = aiter::mha_bwd(
      fmha_args, stream_config, data_type, mode == mode_enum::group, mask.type,
      bias.type, use_dbias, s_randval, deterministic, bwd_v3, v3_atomic_fp32,
      v3_bf16_cvt);
  if (ave_time < 0) {
  ...
  for (ck_tile::index_t wb = 0; wb < batch; ++wb) {
    ...
    ck_tile::HostTensor<QDataType> q_host_ref(
        {nhead, real_seqlen_q, hdim_q}); // q_g_m_k
    ck_tile::HostTensor<KDataType> k_host_ref(
        {nhead, real_seqlen_k, hdim_q}); // k_g_n_k
    ck_tile::HostTensor<VDataType> v_host_ref(
        {nhead, hdim_v, real_seqlen_k}); // v_g_o_n
    ck_tile::HostTensor<ODataType> o_host_ref(
        {nhead, real_seqlen_q, hdim_v}); // o_g_m_o
    ck_tile::HostTensor<LSEDataType> lse_host_ref(
        {nhead, real_seqlen_q}); // lse_g_m
    ...
    ck_tile::HostTensor<AccDataType> p_dropped_hp_host_ref(
        {nhead, real_seqlen_q,
         real_seqlen_k}); // p_dropped_hp_g_m_n high precision
    ...
    if (bias.type == bias_enum::elementwise_bias) {
      ...
      ck_tile::reference_batched_elementwise<AccDataType, BiasDataType,
      ...
      // [nhead, real_seqlen_q, real_seqlen_k]
      ck_tile::reference_batched_elementwise<AccDataType, AccDataType,
                                             AccDataType, AccDataType>(
    ...
    if (p_drop > 0) {
      p_hp_host_ref.ForEach([&](auto &self, auto idx) {
        p_dropped_hp_host_ref(idx) = self(idx);
      });
      randval_host_ref.ForEach([&](auto &self, auto idx) {
        self(idx) = randval_host(b, idx[0], idx[1] + query_offset, idx[2]);
      });
      ck_tile::reference_batched_dropout(p_dropped_hp_host_ref,
                                         randval_host_ref, p_undrop_in_uint8_t,
                                         rp_undrop);
    ...
    // O = P * V
    ck_tile::reference_batched_gemm<GemmDataType, VDataType, AccDataType,
                                    ODataType>(
  ...
  ck_tile::stream_config stream_config_v{
      nullptr, true, 0,
      0,       1,    arg_parser.get_str("timer") == std::string("gpu")};
  aiter::mha_bwd(fmha_args, stream_config_v, data_type,
                 mode == mode_enum::group, mask.type, bias.type, use_dbias,
                 s_randval, deterministic, bwd_v3, v3_atomic_fp32, v3_bf16_cvt);
  ...
  for (ck_tile::index_t wb = 0; wb < batch; ++wb) {
    ...
    // dP = dO@V     w/o dropout
    auto v_t_host_ref =
        v_host_refs[wb].transpose({0, 2, 1}); // v_g_o_n -> v_g_n_o
    ck_tile::reference_batched_gemm<OGradDataType, VDataType, AccDataType,
                                    AccDataType>(
        do_host_ref, v_t_host_ref,
        dp_hp_host_ref); // dp_g_m_n = do_g_m_o@v_g_n_o

    if (p_drop > 0) {
      ck_tile::reference_batched_dropout(dp_hp_host_ref, randval_host_refs[wb],
    ...
    ds_hp_host_ref.ForEach([&](auto &self, auto idx_gmn) {
      ...
      for (int o = 0; o < hdim_v; o++) {
        ...
        idx_gmo[2] = o;
        do_dot_o +=
            ck_tile::type_convert<AccDataType>(do_host_ref(idx_gmo)) *
    ...
    if (use_dbias) {
      ds_hp_host_ref.ForEach([&](auto &self, auto idx) {
        dbias_host_ref(idx) =
            ck_tile::type_convert<BiasGradDataType>(self(idx));
      });
    }
    ...
    auto do_t_host_ref =
        do_host_ref.transpose({0, 2, 1}); // do_g_m_o -> do_g_o_m
    ck_tile::reference_batched_gemm<GemmDataType, OGradDataType, AccDataType,
                                    VGradDataType>(
        p_t_lp_host_ref, do_t_host_ref,
        dv_host_ref); // dv_g_n_o = p_lp_g_n_m@do_g_o_m

    // dQ = scale * dS@K^T
    auto k_t_host_ref =
        k_host_refs[wb].transpose({0, 2, 1}); // k_g_n_k -> k_g_k_n
    ck_tile::reference_batched_gemm<GemmDataType, KDataType, AccDataType,
                                    QGradDataType>(
        ds_lp_host_ref, k_t_host_ref, dq_host_ref, ck_tile::identity{},
        ck_tile::identity{},
        ck_tile::scales(scale)); // dq_g_m_k = ds_g_m_n@k_g_k_n

    // dK = scale * dS^T@Q^T
    auto ds_t_lp_host_ref =
        ds_lp_host_ref.transpose({0, 2, 1}); // ds_g_m_n -> ds_g_n_m
    auto q_t_host_ref =
        q_host_refs[wb].transpose({0, 2, 1}); // q_g_m_k -> q_g_k_m
    ck_tile::reference_batched_gemm<GemmDataType, QDataType, AccDataType,
                                    KGradDataType>(
        ds_t_lp_host_ref, q_t_host_ref, dk_host_ref, ck_tile::identity{},
        ck_tile::identity{},
    ...
    bool dq_cur_pass = ck_tile::check_err(
        dq_host_result, dq_host_ref,
        std::string("Error: QGrad Incorrect results!"), rtol, atol);
    bool dk_cur_pass = ck_tile::check_err(
        dk_host_result, dk_host_ref,
        std::string("Error: KGrad Incorrect results!"), rtol, atol);
    bool dv_cur_pass = ck_tile::check_err(
        dv_host_result, dv_host_ref,
        std::string("Error: VGrad Incorrect results!"), rtol, atol);

    bool dbias_cur_pass = true;
    if (use_dbias) {
      dbias_cur_pass = ck_tile::check_err(
          dbias_host_result, dbias_host_ref,
          std::string("Error: BiasGrad Incorrect results!"), rtol, atol);
    }
