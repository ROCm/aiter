...
emit a CSV (best config + whether HIP wins). The wrapper loads this CSV instead
of the hand-tuned _MFMA_CFG / _DENSE_ALLOWLIST.
...
DEVICE = "cuda"
FP8 = 448.0
SHAPES = [(6144, 2304), (2048, 6144), (6144, 1536), (1536, 6144)]
MS = [1, 2, 4, 8, 16, 32, 64]
MFMA_M = {8, 16, 32, 64}
N_SUBS = (1, 2, 4)
K_SPLITS = (1, 2, 4, 8)
...
def quant(x):
    ...
    return (q.reshape(*x.shape[:-1], K).contiguous(),
...
def dq(q, s):
    ...
    return (qb * torch.exp2(s.reshape(*s.shape, 1).float() - 127)).reshape(*q.shape[:-1], K)


@triton.jit
def _tl(xp, xsp, wp, wsp, op, Mn, N, K, sxm, sxk, sxsm, sxsk, swn, swk, swsn, swsk,
        som, son, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pm = tl.program_id(0); pn = tl.program_id(1)
    om = pm * BM + tl.arange(0, BM); on = pn * BN + tl.arange(0, BN)
    ok = tl.arange(0, BK); osk = tl.arange(0, BK // 32)
    mm = om < Mn; nm = on < N
    ...
    for _ in range(0, tl.cdiv(K, BK)):
        x = tl.load(xpt, mask=mm[:, None], other=0.); w = tl.load(wpt, mask=nm[:, None], other=0.)
        xs = tl.load(xspt, mask=mm[:, None], other=0); ws = tl.load(wspt, mask=nm[:, None], other=0)
        acc += tl.dot_scaled(x, xs, "e4m3", w.T, ws, "e4m3")
        xpt += BK * sxk; wpt += BK * swk; xspt += (BK // 32) * sxsk; wspt += (BK // 32) * swsk
...
def triton_dense(xq, xs, wq, ws):
    ...
    _tl[g](xq, xs, wq, ws, out, Mn, N, K, xq.stride(0), xq.stride(1), xs.stride(0),
           xs.stride(1), wq.stride(0), wq.stride(1), ws.stride(0), ws.stride(1),
           out.stride(0), out.stride(1), BM=BM, BN=BN, BK=BK, num_warps=8)
    return out
...
def main():
    ...
    print(f"{'K':>5} {'N':>5} {'M':>4} {'kern':>5} {'best cfg':>10} {'HIP us':>7} "
    ...
    for K, N in SHAPES:
        ...
        for m in MS:
            ...
            if m in MFMA_M:
                ...
                for ns in N_SUBS:
                    for ks in K_SPLITS:
                        if kiters % ks:
                            continue
                        try:
                            o = smallm_mxfp8_mfma(u8(xq), xs, u8(wq), ws, torch.bfloat16, ns, ks)
                            if relerr(o, ref) > 5e-2:
                                continue
                            t = bench(lambda: smallm_mxfp8_mfma(u8(xq), xs, u8(wq), ws,
                                                                torch.bfloat16, ns, ks))
                ...
                print(f"{K:>5} {N:>5} {m:>4} {'mfma':>5} {f'ns{ns},ks{ks}':>10} {hip:>7.2f} "
                ...
                print(f"{K:>5} {N:>5} {m:>4} {'gemv':>5} {'-':>10} {hip:>7.2f} "
