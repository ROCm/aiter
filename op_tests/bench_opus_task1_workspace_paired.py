...
"""Adjacent ABBA for upstream internal and current Torch-owned A16 workspace.

The process imports the pinned upstream/main pybind module while loading the
current module only as a private C ABI library.  On gfx950, upstream's optional
workspace argument is explicitly ``None`` so the launch uses its per-stream
C++ registry. This allows every upstream/current measurement to share one
process, stream, tensor set and external GPU-load window without mixing the
incompatible Python extension module names.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="provenance path for the already-built upstream module",
    )
...
class CurrentCabi:
    ...
    def __init__(self, module_path: pathlib.Path) -> None:
        ...
        self.raw_stream = getattr(torch._C, "_cuda_getCurrentRawStream", None)
    ...
    def _fill(tensor: Tensor, descriptor: aiter_tensor_t) -> aiter_tensor_t:
        ...
        descriptor.device_id = tensor.get_device()
        return descriptor
...
def _paired_current_a16w16_cabi(
    ...
    """Mirror the production eager-direct/compile-guarded backend boundary."""
    if (
        _torch_is_compiling()
        or XQ.is_meta
        or getattr(XQ, "fake_mode", None) is not None
    ):
        return _paired_current_a16w16_cabi_guarded(
            XQ, WQ, Y, bias, workspace, kid, split_k
        )
    return _paired_current_a16w16_cabi_direct(
        XQ, WQ, Y, bias, workspace, kid, split_k
    )
...
class Runner:
    ...
    def run(self) -> None:
        ...
        _baseline_raw(x_prime, w_prime, y_prime, None, None, 200, 2)
            ...
            + json.dumps(
                {
                    ...
                    "python_binding_source": str(
                        pathlib.Path(aiter.__file__).resolve()
                    ),
                    "baseline_source_root": (
                        None
                        if self.args.baseline_source_root is None
                        else str(self.args.baseline_source_root.resolve())
                    ),
            ...
            for kid in WORKSPACE_KIDS:
                ...
                for dtype_name, dtype, rtol, atol in output_cases:
                    ...
                    def baseline_call() -> Tensor:
                        # upstream/main gfx950 ignores the optional external
                        # workspace slot and uses its per-stream C++ registry.
                        return _baseline_raw(xq, wq, old_y, None, None, kid, 2)
