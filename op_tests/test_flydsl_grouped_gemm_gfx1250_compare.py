...
def _fused_case_from_logical(
    *,
    ...
    """Build ``fused_moe`` inputs from explicit GGUU logical tensors.

    Same shuffle/scale prep as ``_prepare_grouped_moe_case``, but driven from
    tensors supplied by the caller instead of regenerated from a seed. Used by
    the ``compare`` scenario so the FlyDSL grouped path and the gluon path are
    fed byte-identical logical weights/scales/bias/routing.
    """
    K = model_dim
    inter = inter_dim
...
# ---------------------------------------------------------------------------
# Compare: FlyDSL grouped vs gluon, same logical case, same torch ref.
#
# Both paths are built from one shared GGUU logical case (the gluon test's
# ``_make_case``), evaluated against the single shared ``_torch_moe_ref``, then
# timed with the same ``run_perftest``. The gluon path is imported lazily here
# because the gluon e2e module imports helpers from THIS module at load time --
...
def _compare(args: argparse.Namespace) -> None:
    ...
    # Lazy import (see note above): gluon e2e module + its builders.
    from op_tests.triton_tests.moe.test_moe_gemm_a8w4_e2e import (
        _make_case,
        _ref_from_case,
        _prepare_from_case,
        _run_gluon_stages,
    ...
    # Shared reference (decodes the GGUU logical weights at fp32).
    ...
    print(
        "\n| path   | rel_l2 vs ref | tol    | latency (us) | speedup |",
        flush=True,
    )
    print("|--------|---------------|--------|--------------|---------|", flush=True)
    print(
...
def main() -> None:
    ...
    # gluon kernel-config overrides (compare scenario; read by the gluon path's
