...
The default formal kernel uses AP1 (preshuffled A). To run the same six cases
with AP0 (row-major A), including both input/scale pairs and four repeats each:

```bash
python -m op_tests.test_mxfp8fp4gemm_perf --apre 0
```

`--apre 1` explicitly selects AP1. The `indexer_wq_b` case selects the registered
128x128 K128/PF8 variant for either layout; the other five cases use the existing
256x256 variants. Both layouts use the split-K defaults in the table below.
For an existing installation with a cached HIP module from before AP0 128x128
was registered, select a fresh JIT directory on the first run:
...
native test runs four times. The benchmark order follows the FlyDSL native test:

| Formal layout | Benchmark order |
|---|---|
| AP0 (`--apre 0`) | Torch reference -> formal ASM AP0 |
| AP1 (`--apre 1`, default) | Torch reference -> ASM AP0 prebenchmark -> formal ASM AP1 |

Each stage uses 2 warmups and 100 timed iterations via the native
`aiter.test_common.run_perftest`, including its automatic input rotation.
Each stage warms up its own kernel. AP0 has no separate AP0 prebenchmark.
...
Both `constant/constant` and `uniform/auto` data/scale pairs use seed 0.
...
python -m op_tests.test_mxfp8fp4gemm_perf --apre 0 --cases indexer_wq_b
...
python -m op_tests.test_mxfp8fp4gemm \
  ...
  --pre-benchmark --no-reduce --repeat 4 \
...
new directory. `perf.csv` contains the A layout (`apre`), individual times and
...
this final table.
The JSON stage windows are `torch, formal_ap0` for AP0 and `torch, ap0, ap1`
for AP1. AP0 prebenchmark fields such as `ap0 us` and `ap0 result` are present
only for AP1 runs; formal results always use `asm us` and `asm result`.

A formal call should produce 100 raw GEMM events. If the profiler returns fewer,
the reproduction test retries the entire four-call group in a fresh process, up
