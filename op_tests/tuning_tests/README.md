...
|------|-------|-----|---------------|
| `test_csv_validation.py` | 0 | No | Tuned CSV integrity: duplicates (all families), invalid times, errRatio, git conflicts, explicit FMoE `gfx` |
| `test_fmoe_gfx_schema.py` | 1 | No | FMoE `gfx` schema, legacy one-shot warnings, cross-gfx merge/lookup, FlyDSL AOT gfx selection |
