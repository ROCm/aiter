...
class ArtifactsAndRuntimeBoundaries(unittest.TestCase):
    ...
    def test_local_backend_path_verification(self):
        ...
        def local_module(name):
            relative = name.removeprefix("aiter.ops.flydsl.").replace(".", "/") + ".py"
            return types.SimpleNamespace(__file__=str(root / relative))

        with patch.object(tuning.importlib, "import_module", side_effect=local_module):
            self.assertEqual(set(tuning._backend_paths(pa)), set(tuning.SOURCE_FILES))
        with patch.object(
            tuning.importlib,
            "import_module",
            return_value=types.SimpleNamespace(
                __file__="/another/tree/pa_decode_plan.py"
            ),
        ), self.assertRaisesRegex(RuntimeError, "source tree"):
