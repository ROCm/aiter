...
{
    public:
    Winner(const std::string& library_path, const std::string& code_dir, int m, int n, int k)
        : hardware_(tl::hip::GetCurrentDevice()),
          problem_(tl::ContractionProblemGemm::GEMM_Strides(true,
                                                            false,
    ...
    {
        auto library =
            std::dynamic_pointer_cast<tl::MasterSolutionLibrary<tl::ContractionProblemGemm>>(
        ...
        {
            if(entry.path().extension() == ".co" || entry.path().extension() == ".hsaco")
            {
    ...
    void Launch(std::uintptr_t a,
                ...
                std::uintptr_t synchronizer,
                std::uintptr_t stream)
    {
        void* output_pointer = reinterpret_cast<void*>(output);
        tl::ContractionInputs inputs(reinterpret_cast<const void*>(a),
