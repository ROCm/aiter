...
TARGET_D = ("gfx1201", 64)  # Radeon AI PRO R9700
...
def test_get_build_targets():
    ...
    try:
        ...
        # 1.4 gfx1201 resolves to its full-device CU count without a live GPU.
        ...
        # 1.8 GFX_CU_NUM_MAP covers the production targets used by these tests.
        _check(
            "GFX_CU_NUM_MAP contains gfx942, gfx950, and gfx1201",
            all(gfx in GFX_CU_NUM_MAP for gfx in ("gfx942", "gfx950", "gfx1201")),
        )

        # 1.9 Production dispatch must allow the gfx1201 CK module whose tuned
        # rows are generated by this target.
        try:
            os.environ["GPU_ARCHS"] = TARGET_D[0]
            from aiter.ops.gemm_op_a8w8 import _BLOCKSCALE_HIP_PREBUILT_ARCHES

            _check(
                "blockscale CK dispatch includes gfx1201",
                "gfx1201" in _BLOCKSCALE_HIP_PREBUILT_ARCHES,
            )
        except Exception as e:  # noqa: BLE001
            _check("blockscale CK dispatch includes gfx1201", False, str(e))

        # 1.10 Live GPU fallback — requires torch and a GPU; skipped otherwise
...
def test_blockscale_kernel_name_forwarding():
    ...
    saved = {
        ...
        "triton": a8w8_mod._gemm_a8w8_blockscale_triton,
    ...
    def fake_triton(*args, **kwargs):
        record["libtype"] = "triton"
        record["kwargs"] = dict(kwargs)
        return torch.empty(m, n, dtype=torch.bfloat16, device="cpu")
    ...
    try:
        ...
        a8w8_mod._gemm_a8w8_blockscale_triton = fake_triton
        ...
        # 6.4 Explicit Triton row → preserves the faster backend for a shape
        # where the best available CK candidate loses.
        csv_triton = _make_temp_csv(f"""
            gfx,cu_num,M,N,K,kernelId,libtype,splitK,us,kernelName,tflops,bw,errRatio
            {gfx},{cu_num},{m},{n},{k},-1,triton,0,10.0,,100.0,500.0,0.0
        """)
        csv_paths.append(csv_triton)
        a8w8_mod._CKGEMM_CONFIG_CACHE = {}
        a8w8_mod._CKGEMM_HAS_GFX = {}
        get_CKGEMM_config.cache_clear()
        with _override_blockscale_csv(csv_triton):
            record.clear()
            a8w8_mod.gemm_a8w8_blockscale(
                XQ, WQ, x_scale, w_scale, dtype=torch.bfloat16
            )
            _check(
                "triton CSV: gemm_a8w8_blockscale routed to Triton",
                record.get("libtype") == "triton",
                f"recorded libtype={record.get('libtype')}",
            )

        # 6.5 No tuned row: gfx1201 retains its pre-existing Triton fallback;
        # other CK-supported architectures use the CK default heuristic.
        ...
        with _override_blockscale_csv(csv_empty):
            ...
            expected = "triton" if gfx == "gfx1201" else "ck"
            _check(
                f"no tuned row: routed to {expected} fallback",
                record.get("libtype") == expected,
                f"recorded libtype={record.get('libtype')}",
            )
            if expected == "ck":
                _check(
        ...
        a8w8_mod._gemm_a8w8_blockscale_triton = saved["triton"]
