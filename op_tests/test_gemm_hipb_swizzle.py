...
def swizzle_test_fp8(m, n, k):
    ...
    out_sw, time_sw = aiter_hip_swizzle(
        inp, weight_swizzle, x_scale, w_scale, torch.bfloat16
    )
    ...
    err_b = checkAllclose(
        out_sw,
        out_torch,
        msg=f"swizzle time:{time_sw}, hipblaslt time:{time_torch} ",
        rtol=1e-2,
        atol=1e-2,
    )
...
def swizzle_test_bf16(m, n, k):
    ...
    weight_swizzle = shuffle_weight(weights, layout=(16, 16), use_int4=False)
    out_swizzle, time_swizzle = aiter_hip_swizzle(
        inp, weight_swizzle, None, None, torch.bfloat16
    )

    err_b = checkAllclose(
        out_swizzle,
        out_torch,
        msg=f"swizzle time:{time_swizzle}, hipblaslt time:{time_torch} ",
        rtol=1e-2,
        atol=1e-2,
    )
