...
def test_gfx950_ctypes_torch_compile_executes_on_device():
    ...
    for seed in (31, 47):
        ...
        assert compiled(XQ, WQ, Y, workspace) is Y
...
def test_ctypes_eager_bypasses_registered_torch_op(monkeypatch):
    gemm = importlib.import_module("aiter.ops.opus.gemm_op_a16w16")
    launch = gemm._launch_a16w16_backend
    body = inspect.unwrap(launch)
    calls = []

    monkeypatch.setattr(gemm, "_TORCH_IS_COMPILING", lambda: False)
    monkeypatch.setattr(
        launch,
        "_torch_compile_guarded",
        lambda *_args: calls.append("guarded"),
    )
    # The decorator closes over the original registered callable, so replace
    # the original body's only external action and mark it already primed.
    monkeypatch.setattr(gemm, "_opus_a16w16_cabi_primed", True)
    monkeypatch.setattr(
        gemm,
        "_invoke_opus_a16w16_cabi",
        lambda *_args: calls.append("direct"),
    )

    tensor = SimpleNamespace(is_meta=False)
    launch(tensor, object(), object(), None, object(), 200, 2)
    assert calls == ["direct"]
