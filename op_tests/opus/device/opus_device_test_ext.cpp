 ...
 * ... opus_device_test.run_vector_add(A, B, Result) opus_device_test.run_async_load(Src, Dst)
...
static void
run_mfma_torch(torch::Tensor A, torch::Tensor B, torch::Tensor C, const std::string& variant)
{
    ...
    TORCH_CHECK(C.dtype() == expected_out_dtype,
                "C must be ",
                (expected_out_dtype == torch::kFloat32 ? "float32" : in_dtype_name),
                " for variant ",
                variant);
    ...
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{32, 2}), "A must be 32x2 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{32, 2}), "B must be 32x2 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{32, 32}), "C must be 32x32 for variant ", variant);
        run_mfma_32x32x2_f32(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "16x16x4_f32")
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{16, 4}), "A must be 16x4 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{16, 4}), "B must be 16x4 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{16, 16}), "C must be 16x16 for variant ", variant);
        run_mfma_16x16x4_f32(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "32x32x8_f16")
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{32, 8}), "A must be 32x8 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{32, 8}), "B must be 32x8 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{32, 32}), "C must be 32x32 for variant ", variant);
    ...
    else if(variant == "32x32x8_bf16")
    {
        ...
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{32, 32}), "C must be 32x32 for variant ", variant);
        run_mfma_32x32x8_bf16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "16x16x16_f16")
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{16, 16}), "A must be 16x16 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{16, 16}), "B must be 16x16 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{16, 16}), "C must be 16x16 for variant ", variant);
        run_mfma_16x16x16_f16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "16x16x16_bf16")
    {
        ...
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{16, 16}), "C must be 16x16 for variant ", variant);
        run_mfma_16x16x16_bf16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "32x32x16_f16")
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{32, 16}), "A must be 32x16 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{32, 16}), "B must be 32x16 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{32, 32}), "C must be 32x32 for variant ", variant);
        run_mfma_32x32x16_f16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "32x32x16_bf16")
    {
        ...
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{32, 32}), "C must be 32x32 for variant ", variant);
        run_mfma_32x32x16_bf16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
    else if(variant == "16x16x32_f16")
    {
        TORCH_CHECK(
            (A.sizes() == torch::IntArrayRef{16, 32}), "A must be 16x32 for variant ", variant);
        TORCH_CHECK(
            (B.sizes() == torch::IntArrayRef{16, 32}), "B must be 16x32 for variant ", variant);
        TORCH_CHECK(
            (C.sizes() == torch::IntArrayRef{16, 16}), "C must be 16x16 for variant ", variant);
        run_mfma_16x16x32_f16(
            A.data_ptr(), B.data_ptr(), C.data_ptr(), stride_a, stride_b, stride_c);
    }
...
static void run_mxfp_torch(torch::Tensor A,
                           torch::Tensor B,
                           torch::Tensor C,
                           ...
                           int64_t scale_b)
{
    ...
    {
        bool dtype_ok =
            (A.dtype() == torch::kFloat8_e4m3fnuz || A.dtype() == torch::kFloat8_e4m3fn);
    ...
    {
        TORCH_CHECK(
            A.dtype() == torch::kUInt8, "A must be uint8 (packed fp4x2) for variant ", variant);
        TORCH_CHECK(
            B.dtype() == torch::kUInt8, "B must be uint8 (packed fp4x2) for variant ", variant);
...
static void run_vector_add_torch(torch::Tensor A, torch::Tensor B, torch::Tensor Result)
{
    TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor");
    TORCH_CHECK(B.is_cuda(), "B must be a CUDA tensor");
    ...
    TORCH_CHECK(A.dim() == 1 && B.dim() == 1 && Result.dim() == 1, "A, B, Result must be 1-D");
...
static void run_async_load_torch(torch::Tensor Src, torch::Tensor Dst)
{
    ...
    TORCH_CHECK(Src.dim() == 1 && Dst.dim() == 1, "Src, Dst must be 1-D");
    int n = static_cast<int>(Src.numel());
    TORCH_CHECK(Dst.numel() == n, "Src and Dst must have the same number of elements");
...
static void run_dtype_convert_torch(torch::Tensor In, torch::Tensor Out, const std::string& variant)
{
    ...
    TORCH_CHECK(In.is_contiguous() && Out.is_contiguous(), "In, Out must be contiguous");
    TORCH_CHECK(In.dim() == 1 && Out.dim() == 1, "In, Out must be 1-D");
    int n = static_cast<int>(In.numel());
    TORCH_CHECK(Out.numel() == n, "In and Out must have the same number of elements");
    ...
    {
        TORCH_CHECK(n % 4 == 0, "For fp32_fp8, n must be a multiple of 4 (packed x4 conversion)");
        run_dtype_convert_fp32_fp8(In.data_ptr(), Out.data_ptr(), n);
    }
    else if(variant == "fp32_fp4")
    {
        TORCH_CHECK(n % 8 == 0, "For fp32_fp4, n must be a multiple of 8 (packed x8 conversion)");
...
static void run_predicated_copy_torch(torch::Tensor Src, torch::Tensor Dst)
{
    TORCH_CHECK(Src.is_cuda(), "Src must be a CUDA tensor");
    TORCH_CHECK(Dst.is_cuda(), "Dst must be a CUDA tensor");
    TORCH_CHECK(Src.dtype() == torch::kFloat32, "Src must be float32");
...
static void run_free_func_add_torch(torch::Tensor A, torch::Tensor B, torch::Tensor Result)
{
    TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor");
    TORCH_CHECK(B.is_cuda(), "B must be a CUDA tensor");
    TORCH_CHECK(Result.is_cuda(), "Result must be a CUDA tensor");
...
static void run_predicated_async_load_torch(torch::Tensor Src, torch::Tensor Dst, int64_t n_padded)
{
    TORCH_CHECK(Src.is_cuda(), "Src must be a CUDA tensor");
    TORCH_CHECK(Dst.is_cuda(), "Dst must be a CUDA tensor");
    TORCH_CHECK(Src.dtype() == torch::kFloat32, "Src must be float32");
