    ...
    def NumericCheck(
        ...
        if mismatch_count > 0:
            ...
            for i in range(num_to_display):
                ...
                print(f"  Index [{idx}]: Golden={golden_val:.6e}, experi={experi_val:.6e}, Abs Diff={abs_diff:.2e}")
        else:
            print(f"Numeric Check [{name} Success]")

    exp_sums_golden, max_logits_golden, tmp_out_golden = split_workspace(workspace_golden)
    exp_sums_experi, max_logits_experi, tmp_out_experi = split_workspace(workspace_experi)
