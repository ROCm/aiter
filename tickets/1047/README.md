...
``BLOCK_N=16`` paged gather, QK/PV MFMA (heads padded to 16), split-K +
LSE merge. gfx950 uses K32 QK; gfx942 retains K16. Split outputs are BF16
and one merge wave computes LSE weights once. Oracle `err=0`. Decode
**beats #4882 Triton** (~20 µs vs ~113 µs at ``M=1``) but **does not**
beat live AMD (~11 µs). Prefill ~0.54–0.57 ms vs ~205–287 µs AMD /
~225–282 µs #4882. Not a win claim.
