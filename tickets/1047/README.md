...
## Phase 3d — family A FlyDSL K2 tiled MFMA QK (not closed)

GPU 6 / gfx950 / `FLYDSL_RUNTIME_ENABLE_CACHE=0`. Same ABI: ``BLOCK_N=16``
paged gather, ``v_mfma_f32_16x16x16_bf16`` QK (heads padded to 16), scalar
PV, split-K + LSE merge. Oracle `err=0`. Decode **beats #4882 Triton**
(~51 µs vs ~113 µs at ``M=1``) and is ~2× 3b; **does not** beat live AMD
(~11 µs). Prefill ~4.3 ms vs ~201–284 µs AMD / ~220–281 µs #4882. Not a
win claim.
