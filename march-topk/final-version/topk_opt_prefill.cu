...
// 旧版 main: 只有 Prefill, 无 Decode, rowStarts/rowEnds=nullptr
// 用于对比编译后的性能差异

int main()
{
    const int rows = 1;
    constexpr int cols = 60000;
    constexpr int k = 2048;

    std::cout << "[OPT5-OLD (Prefill only, no Decode instantiation)] rows=" << rows << " cols=" << cols << " k=" << k << "\n";
    ...
    aiter::standalone_stable_radix_topk_one_block_<float, int, 11, 1024, false>(
        nullptr, workspace_size, d_logits, static_cast<int*>(nullptr), rows, cols,
    ...
    auto run_kernel = [&]() {
        aiter::standalone_stable_radix_topk_one_block_<float, int, 11, 1024, false>(
            d_workspace, workspace_size, d_logits, static_cast<int*>(nullptr), rows, cols,
            static_cast<int*>(nullptr), static_cast<int*>(nullptr), k, nullptr, d_indices, false, stream, true);
    };
    ...
    {
        for(int i = 0; i < warmup_iters; ++i) run_kernel();
