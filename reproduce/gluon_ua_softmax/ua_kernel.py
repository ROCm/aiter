...
def strip_annotate(cls):
    # Python 3.14 (PEP 649): hide __annotate__ from triton's aggregate hash walker.
    _ = cls.__annotations__
    cls.__annotate__ = None
    return cls


e4m3_dtype = torch.float8_e4m3fn  # gfx950
...
# 1 -> scale the whole S tile up front (the path that aborts LLVM on triton 3.8)
# 0 -> scale the reduced max vector instead (equivalent, and compiles)
SOFTMAX_BROKEN = gl.constexpr(os.environ.get("UA_SOFTMAX_BROKEN", "0") == "1")
