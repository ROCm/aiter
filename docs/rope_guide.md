...
| **Vision models (ViT)** | `rope_fwd_2d` | Triton | 2D height/width position encoding |
...
| **Fused with QK norm + cache** | `fused_qk_norm_rope_cache_*` | CUDA/CK | Maximum fusion for inference |
...
output = rope_fwd_3d(input, cos_t, sin_t, cos_h, sin_h, cos_w, sin_w, ...)
```
