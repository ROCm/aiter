...
## 16.5 重跑 `unfused_tri` 对比（op_tests 口径，PR 描述用这张）

给 PR 用的那张表是修复前的数字，`G=16` 两行已经过时。重跑：bf16、`-l n32k4 --group-size 32`、
`h=16`，跑三遍取中位数（小 `S` 单跑的离散度有 10%+，见下）。`unfused_tri` = Triton rope +
`dynamic_mxfp8_quant_n32k4_mbn`，两个 kernel 对我们一个，是 n32k4 唯一对等的基线。

| shape (G, S) | fused us | tri unfused us | fused TB/s | 加速 |
|---|--:|--:|--:|--:|
| (2, 128) | 5.90 | 8.36 | 0.54 | 1.42× |
| (2, 512) | 6.76 | 9.06 | 1.89 | 1.34× |
| (2, 1024) | 8.04 | 9.99 | 3.18 | 1.24× |
| (2, 4096) | 16.74 | 18.27 | 6.11 | 1.09× |
| (2, 16384) | 54.02 | 56.18 | 7.57 | 1.04× |
| (4, 128) | 6.34 | 8.42 | 0.50 | 1.33× |
| (4, 512) | 6.68 | 9.51 | 1.91 | 1.42× |
| (4, 1024) | 8.03 | 11.42 | 3.18 | 1.42× |
| (4, 4096) | 16.71 | 19.87 | 6.12 | 1.19× |
| (4, 16384) | 53.97 | 59.65 | 7.58 | 1.11× |
| (8, 128) | 5.91 | 8.51 | 0.54 | 1.44× |
| (8, 512) | 7.51 | 9.31 | 1.70 | 1.24× |
| (8, 1024) | 8.88 | 11.17 | 2.88 | 1.26× |
| (8, 4096) | 17.18 | 22.81 | 5.95 | 1.33× |
| (8, 16384) | 49.19 | 63.81 | 8.31 | 1.30× |
| (16, 128) | 6.01 | 8.64 | 0.53 | 1.44× |
| (16, 512) | 6.77 | 10.45 | 1.89 | 1.54× |
| (16, 1024) | 8.37 | 13.39 | 3.05 | 1.60× |
| (16, 4096) | 17.43 | 30.88 | 5.87 | 1.77× |
| (16, 16384) | 54.87 | 92.16 | 7.45 | 1.68× |

`G=16` 的加速比从修复前的 1.45 / 1.47 / 1.47 / 1.52 / 1.25× 变成 1.44 / 1.54 / 1.60 / 1.77 /
**1.68×**（按 `S` 递增）；`G=2/4/8` 基本不动。原来 `G=16` 大 `S` 最差（1.25×，因为塌陷同时
拖着我们），现在反而最好。

**别把这张表和上一版混着读。** 小 `S` 的绝对 µs 整体比上一版高了 ~20%（`(2,128)` 4.95→5.90），
但 Triton 那一侧同步抬高（7.46→8.36），而 §16.4 的 A/B 里 `(2,128)` 修复前后是 6.02 / 6.20、
`(2,1024)` 是 8.01 / 8.09 —— 两个二进制在同一 session 里读数一致。所以这是 session 级的漂移，
不是回退：这些点只有 0.5–3 TB/s，是 launch/延迟受限，对时钟和机器占用敏感。跨 session 只有
**比值**可比，绝对值必须同 run。

**基线自身的数值噪声**：`unfused_tri` 的 `out` 检查在 `G=2` 的 `S=4096/16384` 会 warning
（5145/33.5M、11772/134M 个元素，max delta 0.375）。e8m0 是按 `atol=1` 过的，也就是少数 group
的指数差 1 ULP、fp8 值差 2×，来自 Triton 那边 amax 归约顺序不同。fused 在更严的 `atol=0.01`
下全过，跟本 PR 无关。
