...
Added `LINEAR_HEADS_FIRST_LAYOUT = 3` alongside the existing
...
(`-1 = auto`, `0 = vectorized`, `1 = linear`, `3 = linear_heads_first`) plus
...
(`kv_layout = 3` = `LINEAR_HEADS_FIRST_LAYOUT`) reads the strides positionally
...
2. into the cross-layer non-contiguous `[N, H, B, D]` view sliced from a 6D
   physical buffer (heads-first path, `kv_layout = 3`)
