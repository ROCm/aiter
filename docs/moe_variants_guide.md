...
| **Maximum compression** | MXFP4 or INT4 W4A16 | Triton (MXFP4) or CK | 4x weight reduction |
...
output = fused_moe(
    ...
    topk_weights, topk_ids,
    fc1_scale=fc1_block_scale,  # shape: [E, inter_dim/128, model_dim/128]
    fc2_scale=fc2_block_scale,  # shape: [E, model_dim/128, inter_dim/128]
...
output = fused_moe(
    ...
    topk_weights, topk_ids,
    fc1_scale=fc1_e8m0_scale,  # E8M0 format scales
    fc2_scale=fc2_e8m0_scale,
...
| **INT4 W4A16** | - | - | Yes | INT4 weights, BF16 activations |
| **INT4 W4A4** | - | - | Yes | INT4 weights and activations |
...
└── No
    ...
    │   └── INT4 W4A16 — Triton only, good accuracy
...
| **Triton** | Most flexible, broadest quantization support | Prototyping, INT4, MXFP4, new GPUs |
...
| `op_tests/triton_tests/moe/test_moe_gemm_a4w4.py` | Triton INT4 |
