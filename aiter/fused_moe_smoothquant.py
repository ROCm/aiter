...
def fused_moe_gelu_sqi8(
    ...
    quant_output=False,  # whether to quantize the output tensor between the second GEMM to ReduceSum into int8 internally. If False, the output will be in bfloat16.
):
    ...
    def reduce_i8(x, scale):
        ...
        out = torch.empty(
            (x.shape[0], x.shape[2]), dtype=torch.bfloat16, device=x.device
        )
        get_kernel(
            f"{AITER_CORE_DIR}/hsa/gfx950/fmoe_smoothquant/quant-i8:reduce_i8",
            compile_time_args,
        )([x.shape[0]], [256], x, scale, out, x.shape[0])
        return out
    ...
    def moe_gemm(input, pt_scale, weight, pc_scale, stage_index):
        if input.ndim == 3:
        ...
        if quant_output and stage_index == 2:
            assert odims % oquant_block_size == 0
            output = torch.empty(
                (num_tokens, topk, odims), dtype=torch.int8, device=device
            )
            o_scales = torch.empty(
                (num_tokens, topk, odims // oquant_block_size),
                dtype=torch.bfloat16,
                device=device,
            )
        else:
            output = torch.empty(
                (num_tokens, topk, odims), dtype=torch.bfloat16, device=device
            )
            o_scales = None
    ...
    a2_bf16, _ = moe_gemm(a1, a1_scale, w1, w1_scale, 1)
    ...
    stage2_out, stage2_out_scales = moe_gemm(a2, a2_scale, w2, w2_scale, 2)

    if quant_output:
        # fused 'dequant stage2_out into bf16' + 'reduce sum'
        moe_out = reduce_i8(stage2_out, stage2_out_scales)
    else:
