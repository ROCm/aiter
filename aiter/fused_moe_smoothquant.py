...
def fused_moe_gelu_sqi8(
    ...
    def quant_act(x, topk, M, model_dim, smooth_scale, sorted_ids, sorted_expert_ids, num_valid_ids, is_gemm1=True, topk_ids=None):
        ...
        if is_gemm1:
            get_kernel(f"{AITER_CORE_DIR}/hsa/gfx950/fmoe_smoothquant/quant-i8:quant1", compile_time_args)(
                [div_up(M, kwargs["ROW_PER_BLOCK1"])], [64], 
                x, smooth_scale, x_quant, x_quant_scale, 
                topk_ids, M)
            x_quant_scale.is_sorted = False
        else:
            get_kernel(f"{AITER_CORE_DIR}/hsa/gfx950/fmoe_smoothquant/quant-i8:quant2", compile_time_args)(
                [sorted_expert_ids.shape[0], kwargs["BLOCK_SIZE_M"] // kwargs["BLOCK_M2"]], [256], 
                x, smooth_scale, x_quant, x_quant_scale, 
    ...
    def moe_gemm(output, input, pt_scale, weight, pc_scale, stage_index):
        ...
        kwargs = {"is_input_over_4GB":is_input_over_4GB,
                ...
                "bpreshuffle":False,
                "TOPK":topk}
        get_kernel(f"{AITER_CORE_DIR}/hsa/gfx950/fmoe_smoothquant/moe_gemm_8wave_gelu", tuple(kwargs.items()))([num_oc_blocks * num_e_blocks],[8*64],
                        sorted_ids,
                        sorted_weights,
                        sorted_expert_ids,
                        num_valid_ids,
