...
@compile_ops(
    "module_moe_opus_bwd",
    fc_name="opus_moe_gather_sum_dscore_router_dx_cached_meta_token8_h2048_e64_bf16",
    develop=True,
)
def _opus_moe_gather_sum_dscore_router_dx_cached_meta_token8_h2048_e64_bf16_raw(
    src: Tensor,
    token_routes: Tensor,
    route_scores: Tensor,
    partials: Tensor,
...
def opus_moe_gather_sum_dscore_router_dx_cached_meta_token8_h2048_e64_bf16(
    src: Tensor,
    token_routes: Tensor,
    route_scores: Tensor,
    partials: Tensor,
    ...
    """Flat-dscore full tail caching route and expert ids once per token."""
    T = route_scores.shape[0]
    dx = torch.empty(T, 2048, device=src.device, dtype=torch.bfloat16)
    dlogits = torch.empty(T, 64, device=src.device, dtype=torch.bfloat16)
