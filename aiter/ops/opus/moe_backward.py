...
def _select_saved_x_dw1_kernel(
    ...
    tuple.  Explicit kids 14/15 remain available for family benchmarking.
    """
    ...
    if kernel_id not in (-1, 14, 15):
        raise ValueError(
            "saved_x_sorted currently supports auto or K4 kernel_id=14/15"
        )
    if kernel_id in (14, 15):
        return kernel_id

    if num_experts < 2:
        raise ValueError(
            "saved_x_sorted auto requires at least two experts; use "
            "kernel_id=14/15 only for explicit tuning"
        )
    ...
    if not (
        ...
        raise ValueError(
            ...
            "BM256 K4 geometry; use kernel_id=14/15 only for explicit tuning"
        )
...
def _reject_sorted_x_dw1_without_cache(kernel_id: int) -> None:
    ...
    if int(kernel_id) in (14, 15):
        raise ValueError("K4 kernel_id=14/15 requires saved_x_sorted")
