...
def dynamic_mxfp4_quant_2d(
    ...
    Args:
        x: Contiguous CUDA tensor with shape ``(M, N)`` and dtype ``bfloat16``
    ...
    """
    _LOGGER.info(f"DYNAMIC_MXFP4_QUANT_2D: x={tuple(x.shape)}")
    if x.dim() != 2:
    ...
    # A program owns exactly one semantic 32x32 scale tile. This fixed launch
    # shape follows the operator contract and is not architecture-specific
    # tuning. Leave execution configuration to Triton's defaults.
    grid = (M // block_size, N // block_size)
    _dynamic_mxfp4_quant_2d_kernel[grid](
        x,
        x_fp4,
        blockscale_e8m0,
        *x.stride(),
