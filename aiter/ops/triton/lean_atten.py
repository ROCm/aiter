...
def persistent_lean_attention(
    ...
    (
        ...
        causal, batch_size,  N_CTX_Q, N_CTX_K, H, H, HEAD_DIM_Q, BLOCK_M, BLOCK_N, total_programs
    )
    print(f"high_load_wgs={high_load_wgs}, max_tiles_per_wg={max_tiles_per_wg}, tiles_per_head={tiles_per_head}")
    print(f"total_programs={total_programs}, num_splits={num_splits}, even_split={even_split}")
...
def la_persistent(
    ...
    while iter < cta_end_tile_gid:
        ...
        if causal:
            ...
            tile_iter = tile_head_idx * tiles_per_head + (tile_batch_idx * (tiles_per_head // batch_size)) + total_blocks
            tile_iter_end = tile_iter + (per_head_tile_size)
            tile_idx = (tile_head_idx * batch_size + tile_batch_idx) * num_n_blocks + per_head_tile_idx
        else:
            tile_idx = tile_head_idx * batch_size       # Output tile idx, 1 output tile per head per batch 
        ...
        if causal:
            ...
            b_seq_size = 0
            if tile_batch_idx > 0 :
                b_seq_size = tl.load(batch_num_block_n + tile_batch_idx - 1)     #Previous batch size
        
        k_offs = (b_seq_size + local_iter) * BLOCK_N * stride_kn + tile_head_idx * stride_kh + offs_n[None, :] * stride_kn + offs_k[:, None] * stride_kk
        v_offs = (b_seq_size + local_iter) * BLOCK_N * stride_vn + tile_head_idx * stride_vh + offs_n[:, None] * stride_vn + offs_k[None, :] * stride_vk
        ...
        q_offs = q_idx * BLOCK_M * stride_qm + tile_head_idx * stride_qh + offs_m[:, None] * stride_qm + offs_k[None, :] * stride_qk
        q_ptrs = Q + q_offs
        ...
        if host_block: # and finishing_block:
            ...
            o_h_offs = q_idx * BLOCK_M * stride_om + tile_head_idx * stride_oh + offs_m[:, None] * stride_om + offs_k[None, :] * stride_on
            o_ptrs = Out + o_h_offs
