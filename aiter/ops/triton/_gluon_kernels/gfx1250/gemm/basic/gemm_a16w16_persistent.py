...
_gemm_a16w16_persistent_bandwidth_bound_large_repr = make_kernel_repr(
    "gemm_a16w16_persistent_bandwidth_bound_large_gfx1250_kernel_", _GLUON_REPR_KEYS
)
...
@gluon.jit(repr=_gemm_a16w16_persistent_bandwidth_bound_large_repr)
def gemm_a16w16_persistent_bandwidth_bound_large_kernel_(
    a_ptr,
    b_ptr,
    bias_ptr,
    c_ptr,
    ...
    for tile_id in range(start_pid, num_tiles, NUM_SMS):
        ...
        if split_k_start < K:
            ...
            # pre-load tile 0 into registers
            cur_a = gl.amd.cdna4.async_copy.load_shared_relaxed(
                a_buffer.index(compute_idx % NUM_BUFFERS), OPERAND_LAYOUT_A
            )
            if TRANSPOSE:
...
_KERNEL_MAP = {
    ...
    "bandwidth_bound_large": gemm_a16w16_persistent_bandwidth_bound_large_kernel_,
