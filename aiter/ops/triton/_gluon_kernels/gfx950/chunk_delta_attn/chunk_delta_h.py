...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64_gluon(
    ...
    # A chunk is always read as a full BT-row tile, so a sequence whose length is
    # not a multiple of BT would have its last tile run past the end — into the
    # next sequence, or past the tensor entirely for the last one. Instead of
    # predicating that tile, slide it back to finish on the sequence's last row:
    # `row0` below is the tile's first row, and it stops advancing once the tile
    # would overhang. Sliding keeps the prefetch in the main loop a scalar update,
    # where predicating would rebuild a per-lane offset tile every iteration. The
    # rows the slide re-covers were already folded into the state by the previous
    # chunk, so the epilogue masks them out of both the accumulation and the store.
    row0_max = gl.maximum(T - BT, 0)

    # A sequence shorter than BT has nothing to slide back into, so its rows are
    # clamped to the last valid one and masked off later. Only the prologue can
    # ever load such a sequence: reaching the loop means NT > 1, hence T > BT.
    row_lim_0 = gl.minimum(T, BT) - 1
    row_w_0 = gl.minimum(range_w_bt, row_lim_0)
    row_v_0 = gl.minimum(range_v_bt, row_lim_0)
    row_k_0 = gl.minimum(range_k_bt, row_lim_0)
    base_offs_w_0 = row_w_0[:, None] * HV * K + range_w_64[None, :]
    base_offs_v_0 = row_v_0[:, None] * HV * V + i_v * BV + range_v_bv[None, :]
    base_offs_k_0 = range_k_64[:, None] + row_k_0[None, :] * H * K
    ...
    # i_t = 0, so the tile starts at row 0 and only the clamp above applies
    offs_w1_0 = base_offs_w_0
    offs_w2_0 = offs_w1_0 + 64
    offs_v_0 = base_offs_v_0
    offs_k1_0 = base_offs_k_0
    ...
    for i_t in range(NT - 1):
        ...
        row0_n = gl.minimum(i_t_next * BT, row0_max)
        offs_w1_n = row0_n * HV * K + base_offs_w
        offs_w2_n = offs_w1_n + 64
        offs_v_n = row0_n * HV * V + base_offs_v
        offs_k1_n = row0_n * H * K + base_offs_k
    ...
    # Rows of the last tile that belong to this chunk. Sliding the tile back (or,
    # for a sequence shorter than BT, clamping it) means everything outside
    # [row_lo, row_hi) is a repeat of rows an earlier chunk already owns: they must
    # not be added to the state a second time, nor written back over v_new.
    row0_last = gl.minimum(i_t_last * BT, row0_max)
    row_lo = i_t_last * BT - row0_last
    row_hi = T - row0_last
    ...
    b_v = v_load.to(gl.float32) - b_v
    # The repeated rows have to be zeroed, not merely skipped on the store: they
    # also feed the k·v product below, where b_k holds real data for them and would
    # otherwise fold them into the state a second time.
    range_bv_row = gl.arange(0, BT, layout=gl.SliceLayout(1, mfma_layout_b))
    keep_v = (range_bv_row >= row_lo) & (range_bv_row < row_hi)
    b_v = gl.where(keep_v[:, None], b_v, 0.0)
    b_v = b_v.to(dtype)

    # vectorized v_new store via second LDS read in mfma_layout
    b_v_st = gl.convert_layout(b_v, blocked_st)
    offs_v_st_last = row0_last * HV * V + base_offs_v_st
    # The column term carries no condition of its own; it broadcasts the row
    # predicate to the store's shape.
    mask_v_st = (
        (range_vst_bt[:, None] >= row_lo)
        & (range_vst_bt[:, None] < row_hi)
        & (range_vst_bv[None, :] < BV)
    )
    gl.amd.cdna4.buffer_store(
        b_v_st,
        ptr=v_new,
        offsets=base_v_new + offs_v_st_last,
        mask=mask_v_st,
        cache=STORE_CACHE,
    )
