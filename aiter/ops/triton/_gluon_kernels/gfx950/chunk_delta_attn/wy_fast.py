...
def recompute_w_u_fwd_kda_kernel_gluon_small_h(
    ...
    Grid = (NT=T//BT, B*HV): one CTA per (chunk, head) pair. Under ``IS_VARLEN`` the
    first grid axis indexes the flat chunk list instead, so a CTA still covers exactly
    ...
    i_t = gl.program_id(0)  # chunk index (0..NT-1), or flat chunk id when IS_VARLEN
    i_bh = gl.program_id(1)
    i_b = i_bh // HV
    i_hv = i_bh % HV
    i_h = i_hv // (HV // H)

    # Row origin of this CTA's sequence and the chunk index within it.
    if IS_VARLEN:
        i_n = gl.load(chunk_indices + i_t * 2).to(gl.int32)
        i_t = gl.load(chunk_indices + i_t * 2 + 1).to(gl.int32)
    ...
    # Valid rows in this chunk: BT except in the tail chunk of a sequence.
    rows_valid = gl.minimum(BT, T_local - i_t * BT)

    base_k = (bos * H + i_h) * K
    base_v = (bos * HV + i_hv) * V
    base_u = (bos * HV + i_hv) * V
    base_w = (bos * HV + i_hv) * K
    base_gk = (bos * HV + i_hv) * K
    base_beta = bos * HV + i_hv
    base_A = (bos * HV + i_hv) * BT
    base_kg = (bos * HV + i_hv) * K
    ...
    # Rows past a sequence's end read the last valid row again instead of being
    # predicated off: `buffer_load_to_shared` skips masked lanes without honouring
    # `other`, so LDS would keep uninitialized garbage, and exp2() of garbage reaches
    # inf — which the zeroed A/beta operands below then turn into 0 * inf = nan in
    # *valid* rows. Re-reading a real row keeps everything finite; those rows still
    # contribute nothing, because A and beta are zero there.
    if IS_VARLEN:
        row_v = gl.minimum(range_v_bt, rows_valid - 1)
        row_k = gl.minimum(range_k_bt, rows_valid - 1)
        row_gk = gl.minimum(range_gk_bt, rows_valid - 1)
    else:
        row_v = range_v_bt
        row_k = range_k_bt
        row_gk = range_gk_bt

    base_offs_v = row_v[:, None] * (HV * V) + range_v_dim[None, :]
    base_offs_k = row_k[:, None] * (H * K) + range_k_dim[None, :]
    base_offs_gk = row_gk[:, None] * (HV * K) + range_gk_dim[None, :]
    ...
    # gn [K]: last row of gk for this chunk — gk[i_t*BT + rows_valid - 1, hv, :]
    # SliceLayout(0, mfma_layout_b) gives N-dim (K=128) distribution of B-operand.
    # `rows_valid` rather than BT: in a sequence's tail chunk the last populated row
    # comes before BT, and reading row BT-1 there would sample the next sequence.
    range_gn = gl.arange(0, K, layout=gl.SliceLayout(0, mfma_layout_b))
    offs_gn = i_t * BT * HV * K + (rows_valid - 1) * HV * K + range_gn
    ...
    # Ragged-tail masks, consumed only when IS_VARLEN. Zeroing A and beta is what
    # makes the re-read tail rows contribute nothing. The column term carries no
    # condition of its own; it broadcasts the row predicate to the offset shape.
    mask_a = (range_a0[:, None] < rows_valid) & (range_a1[None, :] < rows_valid)
    mask_b = range_b < rows_valid
    mask_st = (range_st_bt[:, None] < rows_valid) & (range_st_dim[None, :] < V)

    # Issue async loads: v, k, gk → LDS (row-clamped above, so never predicated)
    offs_v = i_t * BT * HV * V + base_offs_v
    offs_k = i_t * BT * H * K + base_offs_k
    offs_gk = i_t * BT * HV * K + base_offs_gk

    async_load(buf_v, v, base_v + offs_v, None, False, LOAD_CACHE)
    async_load(buf_k, k, base_k + offs_k, None, False, LOAD_CACHE)
    async_load(buf_gk, gk, base_gk + offs_gk, None, False, LOAD_CACHE)

    # Sync register loads: A, beta, gn (overlap with async above)
    offs_A = i_t * BT * HV * BT + base_offs_a
    a = reg_load(A, base_A + offs_A, mask_a, IS_VARLEN, LOAD_CACHE)
    b_b = reg_load(beta, base_beta + offs_b, mask_b, IS_VARLEN, LOAD_CACHE)
    # gn reads one fully-populated row, so it needs no predicate.
    ...
    reg_store(
        b_u_st,
        u,
        base_u + i_t * BT * HV * V + base_offs_st_v,
        mask_st,
        IS_VARLEN,
        STORE_CACHE,
    )
    ...
    reg_store(
        b_w_st,
        w,
        base_w + i_t * BT * HV * K + base_offs_st_k,
        mask_st,
        IS_VARLEN,
        STORE_CACHE,
    )
    ...
    reg_store(
        b_kg_st,
        kg,
        base_kg + i_t * BT * HV * K + base_offs_st_k,
        mask_st,
        IS_VARLEN,
        STORE_CACHE,
    )
