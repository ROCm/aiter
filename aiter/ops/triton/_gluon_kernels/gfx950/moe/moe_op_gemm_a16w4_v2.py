...
def _moe_gemm_a16w4(
    ...
    w_buffer = gl.allocate_shared_memory(
        W.dtype.element_ty, shape= [NUM_BUFFERS, BLOCK_N, PACKED_BLOCK_K_W], layout=SHARED_LAYOUT_W
    )
    ...
    if NUM_BUFFERS == 1:
      for k in range(num_k_iter):
        ...
        w_scales = ws_buffer.index(0).load(REG_LAYOUT_WS)
        if SWIZZLE_MX_SCALE == "CDNA4_SCALE":
      ...
      for k in range(num_k_iter - 1):
          ...
          w_scales = ws_buffer.index(read_idx % NUM_BUFFERS).load(REG_LAYOUT_W)
          read_idx += 1
      ...
      x = x_buffer.index(read_idx % NUM_BUFFERS).load(DOT_LAYOUT_X)
      w = w_buffer.index(read_idx % NUM_BUFFERS).load(REG_LAYOUT_W)
      w_scales = ws_buffer.index(read_idx % NUM_BUFFERS).load(REG_LAYOUT_WS)
      read_idx += 1
