...
def _moe_gemm_a16w4(
    ...
    #    size_per_thread=[1, 4],
    #    threads_per_warp=[256 // PACKED_BLOCK_K_W, PACKED_BLOCK_K_W // 4],
    #    warps_per_cta=[num_warps, 1],
    #    order=[1, 0],
    #)
    LOAD_LAYOUT_W: gl.constexpr = gl.BlockedLayout(
        size_per_thread=[4, 1],
        threads_per_warp=[PACKED_BLOCK_K_W // 4, 256 // PACKED_BLOCK_K_W],
        warps_per_cta=[num_warps, 1],
        order=[0, 1],
    )
    ...
    # W is loaded transposed [PACKED_BLOCK_K_W, N] (packed fp4 along axis 0 = K).
    # scaled_upcast(axis=0) unpacks axis 0, so the scale/output layout is this
    # layout with the axis-0 size_per_thread doubled (4 -> 8).
    REG_LAYOUT_W: gl.constexpr = gl.BlockedLayout(
        size_per_thread=[4, 1],
        threads_per_warp=[PACKED_BLOCK_K_W // 4, 256 // PACKED_BLOCK_K_W],
        warps_per_cta=[num_warps, 1],
        order=[0, 1]
    )
    REG_LAYOUT_WS: gl.constexpr = gl.BlockedLayout(
        size_per_thread=[8, 1],
        threads_per_warp=[PACKED_BLOCK_K_W // 4, 256 // PACKED_BLOCK_K_W],
        warps_per_cta=[num_warps, 1],
        order=[0, 1]
    )
    # N-major layout used only to load + unswizzle + broadcast the scale into a
    # logical [BLOCK_N, BLOCK_K] tile; it is then transposed to [BLOCK_K, BLOCK_N]
    # and converted to REG_LAYOUT_WS for the axis=0 scaled_upcast.
    REG_LAYOUT_WS_CONSUME: gl.constexpr = gl.BlockedLayout(
        size_per_thread=[1, 8],
        threads_per_warp=[1, 64],
        warps_per_cta=[num_warps, 1],
        order=[1, 0]
    ...
    MFMA_LAYOUT: gl.constexpr = gl.amd.AMDMFMALayout(
        version=4, instr_shape=[16, 16, 32], transposed=True, warps_per_cta=[1, num_warps]
    )
    ...
    x_buffer = gl.allocate_shared_memory(
        X.dtype.element_ty, shape= [NUM_BUFFERS, BLOCK_M, BLOCK_K], layout=SHARED_LAYOUT_X
    )
    w_buffer = gl.allocate_shared_memory(
        W.dtype.element_ty, shape= [NUM_BUFFERS, PACKED_BLOCK_K_W, PACKED_BLOCK_N_W], layout=SHARED_LAYOUT_W
    )
    ws_buffer = gl.allocate_shared_memory(
        WMxScale.dtype.element_ty, shape= [NUM_BUFFERS, SCALE_BLOCK_N, PACKED_MX_BLOCK], layout=SHARED_LAYOUT_W_SCALES
    )
    ...
    # Single-buffered path (NUM_BUFFERS == 1): load -> wait -> consume, no
    # cross-iteration overlap. Used for large tiles where double-buffering the
    # X/W/scale LDS would overflow shared memory. The pipelined path below assumes
    # NUM_BUFFERS >= 2 (its prologue primes exactly one tile).
    if NUM_BUFFERS == 1:
      for k in range(num_k_iter):
        gl.amd.cdna4.async_copy.buffer_load_to_shared(x_buffer.index(0), X_base, x_offsets)
        gl.amd.cdna4.async_copy.buffer_load_to_shared(
            w_buffer.index(0), W_base, w_offsets, cache_modifier=W_CACHE_MODIFIER
        )
        gl.amd.cdna4.async_copy.buffer_load_to_shared(
            ws_buffer.index(0), WMxScale_base, w_scale_offsets
        )
        gl.amd.cdna4.async_copy.commit_group()
        gl.amd.cdna4.async_copy.wait_group(0)

        x = x_buffer.index(0).load(DOT_LAYOUT_X)
        w = w_buffer.index(0).load(REG_LAYOUT_W)
        w_scales = ws_buffer.index(0).load(REG_LAYOUT_WS_CONSUME)
        if SWIZZLE_MX_SCALE == "CDNA4_SCALE":
            w_scales = unswizzle_mx_scale_cdna4(w_scales, BLOCK_N, MX_SCALE_BLOCK_K)
        # expand to [BLOCK_N, BLOCK_K], then transpose to [BLOCK_K, BLOCK_N] and
        # move to the K-major unpacked layout for the axis=0 upcast.
        w_scales = (
        ...
        w_scales = gl.convert_layout(w_scales, REG_LAYOUT_WS)
        # W is already [K, N]; upcast along axis 0 (K) -> [BLOCK_K, BLOCK_N], no transpose.
      ...
      #Load X and W
      gl.amd.cdna4.async_copy.buffer_load_to_shared(x_buffer.index(write_idx), X_base, x_offsets)
      gl.amd.cdna4.async_copy.buffer_load_to_shared(
            w_buffer.index(write_idx), W_base, w_offsets, cache_modifier=W_CACHE_MODIFIER
        )
      # W scales.
      gl.amd.cdna4.async_copy.buffer_load_to_shared(
          ws_buffer.index(write_idx), WMxScale_base, w_scale_offsets
      )
      gl.amd.cdna4.async_copy.commit_group()
      ...
      for k in range(num_k_iter - 1):
          #Load X and W
          gl.amd.cdna4.async_copy.buffer_load_to_shared(x_buffer.index(write_idx % NUM_BUFFERS), X_base, x_offsets)
          gl.amd.cdna4.async_copy.buffer_load_to_shared(
              w_buffer.index(write_idx % NUM_BUFFERS), W_base, w_offsets, cache_modifier=W_CACHE_MODIFIER
          )

          # W scales.
          gl.amd.cdna4.async_copy.buffer_load_to_shared(
                  ws_buffer.index(write_idx % NUM_BUFFERS), WMxScale_base, w_scale_offsets
          )
          gl.amd.cdna4.async_copy.commit_group()
          write_idx += 1
          ...
          # LDS -> registers
          x = x_buffer.index(read_idx % NUM_BUFFERS).load(DOT_LAYOUT_X)
          w = w_buffer.index(read_idx % NUM_BUFFERS).load(REG_LAYOUT_W)
          w_scales = ws_buffer.index(read_idx % NUM_BUFFERS).load(REG_LAYOUT_WS_CONSUME)
          read_idx += 1
      ...
      acc = gl.amd.cdna4.mfma(x, w_kn, acc)
        ...
        gl.static_assert(False, f"NUM_BUFFERS not 1 or 2")
