...
@gluon.jit
def _sparse_mla_fwd_nat_kernel(
    ...
    # far-prefetch tile-1 topk (dedup: single neutral layout, convert at use)
    p1_off_topk = TILE_K + offs_tile_topk
    tkraw = gl.amd.cdna4.buffer_load(
        ptr=TopK_ptr, offsets=topk_base.to(tl.int32) + p1_off_topk,
        mask=p1_off_topk < TOPK, other=-1)
    ...
    for t in range(NUM_TILES - 1):
        ...
        gl.amd.cdna4.async_copy.buffer_load_to_shared(
            dest=smem_klora.index(next_buf), ptr=KV_ptr, offsets=klora_offs_next.to(tl.int32), mask=valid_klora_next[None, :])
        krope_offs_next = safe_krope_next[None, :].to(tl.int64) * stride_kv_t + (D_V + offs_r_krope[:, None]).to(tl.int64)
        gl.amd.cdna4.async_copy.buffer_load_to_shared(
            dest=smem_krope.index(next_buf), ptr=KV_ptr, offsets=krope_offs_next.to(tl.int32), mask=valid_krope_next[None, :])
        gl.amd.cdna4.async_copy.commit_group()
        tkraw_n = gl.amd.cdna4.buffer_load(
            ptr=TopK_ptr, offsets=topk_base.to(tl.int32) + ((t + 2) * TILE_K + offs_tile_topk),
            mask=((t + 2) * TILE_K + offs_tile_topk) < TOPK, other=-1)
...
# Launcher
# =====================================================================
def sparse_mla_fwd_v4_gluon_nat(q, kv, topk_indices, attn_sink=None, kv_lora_rank=512, scale=None):
    ...
    _sparse_mla_fwd_nat_kernel[grid](
