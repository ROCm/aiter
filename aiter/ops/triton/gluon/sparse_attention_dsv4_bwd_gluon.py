...
Two kernels, both operating on the official V4 form (``K == V == kv``, one dense 512-wide
tensor, RoPE already applied in place caller-side, scale ``1/sqrt(512)``, ``attn_sink`` in the
softmax denominator only, ``topk == -1`` masked):
