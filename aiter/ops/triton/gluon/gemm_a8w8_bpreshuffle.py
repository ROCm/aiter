...
def _gemm_a8w8_ptpc(
    ...
    **scale_n = (N + GROUP_N - 1) // GROUP_N
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        offs_b0 = gl.arange(0, 16, layout=gl.SliceLayout(1,gl.SliceLayout(2,gl.SliceLayout(3, blocked_kn_preshuffled))))
        offs_b1 = gl.arange(0, 16, layout=gl.SliceLayout(0,gl.SliceLayout(2,gl.SliceLayout(3, blocked_kn_preshuffled))))
        offs_b2 = gl.arange(0, BLOCK_SIZE_K // 16, layout=gl.SliceLayout(0,gl.SliceLayout(1,gl.SliceLayout(3, blocked_kn_preshuffled))))
        offs_b3 = gl.arange(0, BLOCK_SIZE_N // 16, layout=gl.SliceLayout(0,gl.SliceLayout(1,gl.SliceLayout(2, blocked_kn_preshuffled))))

        # offs_bk_split = pid_k * SPLITK_BLOCK_SIZE + offs_b0
        ...
        # print("cur_b:",cur_b.to(gl.float32))
        cur_b_fma = gl.convert_layout(cur_b, layout=dot_b_layout)
        # print("cur_b_fma:",cur_b_fma.to(gl.float32))
...
def _gemm_a8w8_ptpc2(
    ...
    **scale_n = (N + GROUP_N - 1) // GROUP_N
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        smem_a_1 = gl.allocate_shared_memory(
            a_ptr.type.element_ty, [BLOCK_SIZE_M, BLOCK_SIZE_K], layout=shared_a
        )
    ...
    **scale_n = (N + GROUP_N - 1) // GROUP_N
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        for k in range(pid_k * num_k_iter, ((pid_k + 1) * num_k_iter)):
            ...
            # 第1步：开始内存访问调度组，优先处理A矩阵加载
            gl.amd.cdna3.sched_group_barrier(sync_group_size=1, sync_scope=1)
            
            # A矩阵异步加载到shared memory
            if EVEN_K:
                gl.amd.cdna4.async_copy.buffer_load_to_shared(
                    smem_a,
                    a_ptr,
            ...
            # 第2步：调度屏障，确保A矩阵加载请求发出后再处理B矩阵
            # 这避免了同时发射过多内存请求导致的带宽饱和
            gl.amd.cdna3.sched_group_barrier(sync_group_size=1, sync_scope=1)
            
            # ========== B矩阵分批加载，避免连续buffer_load ==========
            
            # 将B矩阵分成两半加载，中间插入调度屏障
            half_n = BLOCK_SIZE_N // 2
            
            # 第一半B矩阵的偏移计算
            offs_b_first = offs_b0[:, None, None, None] + \
                        offs_b1[None, :, None, None] * 16 + \
                        offs_b2[None, None, :, None] * 256 + \
                        (offs_b3[None, None, None, :half_n] * 16 + pid_n * BLOCK_SIZE_N) * stride_bn
            
            # 加载第一半B数据
            if EVEN_K:
                b_first_half = gl.amd.cdna4.buffer_load(
                    ptr=b_ptr,
                    offsets=offs_b_first,
                    cache=cache_modifier,
                )
            else:
                b_first_half = gl.amd.cdna4.buffer_load(
            ...
            # 第3步：切换到计算调度组，在内存访问间隙进行地址计算
            gl.amd.cdna3.sched_group_barrier(sync_group_size=1, sync_scope=2)
            
            # 在等待期间更新下一次迭代的地址（轻量级计算）
            offs_a += BLOCK_SIZE_K * stride_ak
            offs_b += BLOCK_SIZE_K * 16
            
            # 第4步：回到内存访问调度组，加载第二半B数据
            gl.amd.cdna3.sched_group_barrier(sync_group_size=1, sync_scope=1)
            
            # 第二半B矩阵的偏移计算
            offs_b_second = offs_b0[:, None, None, None] + \
                            offs_b1[None, :, None, None] * 16 + \
                            offs_b2[None, None, :, None] * 256 + \
                            (offs_b3[None, None, None, half_n:] * 16 + pid_n * BLOCK_SIZE_N + half_n * 16) * stride_bn
            
            # 加载第二半B数据
            if EVEN_K:
                b_second_half = gl.amd.cdna4.buffer_load(
                    ptr=b_ptr,
                    offsets=offs_b_second,
                    cache=cache_modifier,
                )
            else:
                b_second_half = gl.amd.cdna4.buffer_load(
            ...
            # 合并两半B数据
            # 注意：需要沿着正确的维度合并
            b = gl.cat([b_first_half, b_second_half], dim=3)  # 沿着N维度合并
            ...
            acc = gl.amd.cdna4.mfma(
                cur_a,
                gl.convert_layout(
                    b.permute(2, 0, 3, 1).reshape(BLOCK_SIZE_K, BLOCK_SIZE_N),
                    layout=dot_b_layout,
                ),
                acc
            )
            
            # 第7步：循环结束前的调度屏障，确保计算完成
            gl.amd.cdna3.sched_group_barrier(sync_group_size=1, sync_scope=2)
            # if EVEN_K:
            #     b = gl.amd.cdna4.buffer_load(
            #         ptr=b_ptr,
