...
- **`bh16bn64`** (`nhead &le; 16`, bf16 KV): BLOCK_H=16, BLOCK_N=64, 2-D grid `(batch_size, NUM_KV_SPLITS)`. The wrapper launches a graph-stable maximum split count (up to 112 for batch 1). A device-side policy kernel reads the runtime sequence metadata once and writes the active 16/48/64/96/112 split count shared by stage-1 and stage-2. This prevents CUDA Graph capture from freezing a caller's default `min_kv_seq_len=1` into a one-workgroup decode and benefits the existing low-head regime without adding a model-specific regime. `NHEAD < BLOCK_H` masks OOB heads on Q load and O store. On a 12-head, 100K unique scattered-page workload, the runtime policy measures 69-72 us versus 102 us at 256 fixed splits; an explicit 96-split override measures 66 us. The maximum batch-1 BF16 partial-output allocation is 1.31 MiB at 112 slots instead of 3.0 MiB at 256 splits.
