...
The wrapper dispatches by `(nhead, kv_c.dtype)` to one of four compile-time regimes (single `@gluon.jit` kernel, REGIME constexpr gates layouts and grid mapping):
...
- **`bh12bn64`** (`nhead == 12`, bf16 KV): uses the same masked 16&times;16 MFMA tile and BLOCK_N=64 arithmetic as `bh16bn64`. The wrapper launches a graph-stable maximum split count (up to 112 for batch 1), while both kernel stages select the active 16/48/64/96/112 bucket from the device-side sequence length. This prevents CUDA Graph capture from freezing a caller's default `min_kv_seq_len=1` into a one-workgroup decode. CDNA4 has no 12-row MFMA, so four rows remain masked. On a 100K unique scattered-page workload, the runtime policy measures 69-72 us versus 102 us at 256 fixed splits; an explicit 96-split override measures 66 us. The maximum batch-1 BF16 partial-output allocation is 1.31 MiB at 112 slots instead of 3.0 MiB at 256 splits.
...
All four regimes run the full decode and dsv4 prefill. `return_lse=True` also returns the merged fp32 lse `[batch, nhead]`, so `mla_gluon(...)` returns `(o, final_lse)` instead of `(o, None)`.

`num_kv_splits` is an optional benchmark/tuning override for the low-head
regimes. It must be in `[1, min_kv_seq_len]`; normal callers should leave it
unset and use the regime's automatic policy.
