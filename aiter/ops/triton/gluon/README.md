...
- **`bh12bn64`** (`nhead == 12`, `batch_size == 1`, bf16 KV): uses the same masked 16&times;16 MFMA tile and BLOCK_N=64 arithmetic as `bh16bn64`, with a context-bucketed split policy (16/48/64/96/112). CDNA4 has no 12-row MFMA, so four rows remain masked; the speedup comes from balancing stage-1 occupancy against stage-2 reduction work. On a 100K unique scattered-page workload, 96 splits reduces total decode from 102 us at 256 splits to 66 us. The BF16 partial-output allocation `[1, 12, splits, 512]` is 1.12 MiB at 96 splits instead of 3.0 MiB at 256 splits.
