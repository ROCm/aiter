...
def batched_gemm_a8wfp4(
    ...
    """
    Batched MXFP GEMM Y[b] = dequant(X[b]) @ dequant(W[b])^T with per-1x32
    e8m0 microscales on both operands.

    Variants (a_dtype): "fp8" (MXFP8 E4M3 A x MXFP4 B, a8w4) or "fp4"
    (MXFP4 A x MXFP4 B, a4w4). Both A and B scales are folded into a scaled
    MFMA.
    """
    _LOGGER.info(
        f"BATCHED_GEMM_A8WFP4: x={tuple(x.shape)} w={tuple(w.shape)} "
        f"x_scale={tuple(x_scales.shape)} w_scale={tuple(w_scales.shape)} "
        f"a_dtype={a_dtype}"
    )
    ...
    assert y.shape == (Batch, M, N), (
        f"y must be (B,M,N)={(Batch, M, N)}, got {tuple(y.shape)}"
    )
