...
def _resolve_config_and_output(
    x, M, N, K, dtype, y, config, backend, skip_reduce=False
):
    if config is None:
        if backend == "gluon":
...
def gemm_a8w8(
    ...
    y, config = _resolve_config_and_output(
        x, M, N, K, dtype, y, config, backend, skip_reduce
    )
...
def gemm_a8w8_preshuffle(
    ...
    y, config = _resolve_config_and_output(x, M, N, K, dtype, y, config, "gluon")
