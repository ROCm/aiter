...
def run_torch_reference(
    ...
    """
    Run reference implementation using PyTorch for batched GEMM BF16.

    Based on test_batched_gemm_bf16.py pattern.

    Args:
        x: Input tensor of shape (B, M, K)
        w: Weight tensor of shape (B, N, K)
        bias: Bias tensor with shape (B, 1, N)
        dtype: Output dtype
    ...
    for b in range(B):
        b_out = F.linear(
            x[b, :, :].to(torch.float32), w[b, :, :].to(torch.float32)
        )
        if bias is not None:
...
def input_helper(
    ...
    """
    Generate input tensors for batched GEMM BF16 kernel.
    ...
    Returns:
        Tuple of (x, w, bias_tensor, y, config)
    """
    # Generate BF16 tensors with range -20 to 20 (matching test file)
    x = torch.randint(-20, 20, (B, M, K), dtype=dtype, device="cuda")
    w = torch.randint(-20, 20, (B, N, K), dtype=dtype, device="cuda")

    # Generate bias tensor
    bias_tensor = torch.rand([B, 1, N], dtype=dtype, device="cuda") * 10 if bias else None
    ...
    return x, w, bias_tensor, y, config
...
def make_run_and_gt_fn_factory(
    ...
    def make_run_and_gt(config: Dict[str, Any]) -> Tuple[Callable[[], Any], Any]:
        x, w, bias_tensor, y, _ = input_helper(B, M, N, K, dtype, bias=True)

        def run() -> torch.Tensor:
            return batched_gemm_bf16(x, w, bias_tensor, dtype, YQ=y, config=config)

        ground_truth = run_torch_reference(x, w, bias_tensor, dtype)
...
def get_configs_compute_bound() -> List[Dict[str, Any]]:
    """
    Generate configuration space for tuning the batched gemm_bf16 kernel.

    Full comprehensive search space for batched operations (A16W16 - no quantization).
...
def tune_gemm_bf16_batched(
    ...
    """
    Tune the batched GEMM BF16 kernel for specific dimensions.
...
def tune_and_save_configs(
    ...
    for M in M_values:
        config = tune_gemm_bf16_batched(batch_size, M, N, K, dtype, search_space)
    ...
    # Generate file name based on whether it's default or specialized
    # Default config (N=1024, K=1024) gets saved without N,K suffix
    # Other configs get saved with N,K suffix
    if N == 1024 and K == 1024:
        json_file_name = f"{device_name}-BATCHED_GEMM-A16W16.json"
    else:
        json_file_name = f"{device_name}-BATCHED_GEMM-A16W16-N={N}-K={K}.json"
