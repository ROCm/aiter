...
flat mappings of scalar tuning values, so a deep copy would only add hot-path
overhead — the rule `get_conv_config()` already follows. A nested value breaks
it: `op_tests/tuning_tests/test_gemm_config_flat.py` rejects one, and a table
that needs one needs a deep copy in the loader in the same PR.
