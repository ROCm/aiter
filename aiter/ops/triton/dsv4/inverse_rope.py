...
"""Fused inverse RoPE (GPT-J / interleaved) for the DeepSeek-V4 attention output.

Indexes the cos/sin cache by per-token positions and applies the inverse rotation
in place on the rope slice of the attention output. Port of ATOM's
...
def inverse_rope_reference(
    x: torch.Tensor,
    cos: torch.Tensor,
    sin: torch.Tensor,
    positions: torch.Tensor,
) -> torch.Tensor:
    """Pure-torch inverse GPT-J RoPE. Returns a new tensor (does not mutate x).

    out[2i]   =  x[2i]*c + x[2i+1]*s
    out[2i+1] = -x[2i]*s + x[2i+1]*c
    where (c, s) for pair i is cos/sin[pos, i] (reuse_freqs_front_part).
    """
    S, H, rd = x.shape
    xf = x.float()
    c = cos.index_select(0, positions).reshape(S, rd // 2).float()  # [S, rd/2]
    s = sin.index_select(0, positions).reshape(S, rd // 2).float()
    c = c[:, None, :]  # [S, 1, rd/2]
    s = s[:, None, :]
    even = xf[..., 0::2]
    odd = xf[..., 1::2]
    ...
    out = torch.stack([new_even, new_odd], dim=-1).reshape(S, H, rd)
