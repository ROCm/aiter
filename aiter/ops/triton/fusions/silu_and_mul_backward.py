...
def silu_and_mul_backward(
    ...
    Tuning:
        The last-dimension half-width selects the smallest matching
        ``N_LEQ_*`` entry in the current architecture's ``DEFAULT.json``, or
        the ``any`` entry when no threshold matches. Each entry supplies
        ``BLOCK_M`` and ``BLOCK_N`` tile sizes plus Triton's ``num_warps`` and
        ``num_stages`` launch parameters.
    """
    ...
    _LOGGER.info(
        f"SILU_AND_MUL_BACKWARD: x={tuple(x.shape)} grad={tuple(grad_output.shape)}"
    )
