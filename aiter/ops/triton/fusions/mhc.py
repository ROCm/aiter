...
def mhc(
    ...
    _LOGGER.info(
        "MHC: x=%s phi=%s alpha_pre=%s alpha_post=%s alpha_res=%s hc_pre_eps=%s hc_post_mult_value=%s sinkhorn_iters=%s num_ksplit=%s BLOCK_M=%s BLOCK_N=%s BLOCK_K=%s BLOCK_C=%s N_TOTAL_POW2=%s RES_PID_C=%s",
        tuple(x.shape),
        tuple(phi.shape),
        alpha_pre,
        alpha_post,
...
def mhc_post(
    ...
    _LOGGER.info(
        "MHC_POST: layer_input=%s residual=%s post_mix=%s comb_mix=%s BLOCK_M=%s BLOCK_C=%s",
...
def mhc_post_pre(
    ...
    _LOGGER.info(
        "MHC_POST_PRE: layer_input=%s residual_in=%s phi=%s alphas=%s@%s hc_pre_eps=%s hc_post_mult_value=%s sinkhorn_iters=%s BLOCK_M=%s BLOCK_K=%s BLOCK_C_SPLIT=%s BLOCK_C=%s NUM_KSPLIT=%s NUM_C_BLOCKS=%s N_TOTAL_POW2=%s",
        tuple(layer_input.shape),
