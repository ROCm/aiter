...
def is_gfx950_retune(arch, head_size):
    """Retune only gfx950 decode path for small head_size

    Every other arch, head size and path keeps the previously
    shipped configuration as-is.
    """
    return (
        DEVICE_ARCH == "gfx950"
...
def select_2d_config(
    ...
    # pure decode config
        ...
        if is_gfx950_retune(arch, head_size) and sliding_window > 0:
            TILE_SIZE = 64
...
def select_3d_config(
    ...
    if IS_DEVICE_ARCH_GFX12:
        ...
        occ = waves_per_eu * 4 // attn_warps
        use_retune = is_gfx950_retune(arch, head_size)
        if not use_retune:
            # gfx950 retune: the occupancy multiplier over-segments the KV split
            # for this decode path.
        ...
        # the >= 8 floor would clamp the retuned split (4 and 2) straight back to 8
        MIN_SEGMENTS = 1 if use_retune else min(8, MAX_SEGMENTS)
        if head_size >= 512 and not arch.is_rdna:
        ...
        # The retune drives num_segments as low as 2, so use segment count directly
        # Keep 1-warp reduce for small segment counts
        if num_segments <= (2 if use_retune else MIN_SEGMENTS):
            reduce_num_warps = 1
