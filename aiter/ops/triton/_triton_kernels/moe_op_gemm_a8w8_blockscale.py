...
def _moe_gemm_a8w8_blockscale(
    ...
    - x_scale: Scale tensor for A with shape (M // group_m, K // group_k)
    ...
    # tl.static_assert(BLOCK_K % GROUP_K == 0, "BLOCK_K must be multiple of GROUP_K")
    # tl.static_assert(BLOCK_N % GROUP_N == 0, "BLOCK_N must be multiple of GROUP_N")
    ...
    if is_x_blockscale:
        if PER_ROW_X_SCALE:
            XBlockScale += start_m * stride_x_bs_m
    if is_w_blockscale:
    ...
    for k in range(num_k_iter):
        ...
        # Gets the start of the block k and divide by GROUP_K to get the grouped scale index
        offs_k_scale = (BLOCK_K * pid_k + k * (BLOCK_K * SPLIT_K)) // GROUP_K
        if is_x_blockscale:
            if PER_ROW_X_SCALE:
                # XScale: [M, K_blocks]
                XScalePtrs = (
                ...
                offs_x_scale_m = (start_m + offs_x_m) // GROUP_M
                XScalePtrs = (
                    XBlockScale
        ...
        acc += tl.dot(x, w, input_precision="ieee") * x_scale[:, None] * w_scale[None, :]
    ...
    if not EVEN_K:
        ...
        mask_k = offs_k < MASK_K_LIMIT
        offs_k_scale = (BLOCK_K * pid_k + num_k_iter * (BLOCK_K * SPLIT_K)) // GROUP_K

        x = tl.load(XPtrs, mask=mask_k[None, :], other=0.0)
        w = tl.load(WPtrs, mask=mask_k[:, None], other=0.0, cache_modifier=W_CACHE_MODIFIER)
        if is_x_blockscale:
            if PER_ROW_X_SCALE:
                # XScale: [M, K_blocks]
                XScalePtrs = (
