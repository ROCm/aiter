...
def _rope_norm_store_kv_kernel(
    ...
    key_cache_ptr,           # NHD or paged head-major K cache
    value_cache_ptr,         # NHD or paged head-major V cache
    eps,
    num_rows,
    ...
    stride_kc_b, stride_kc_t, stride_kc_h, stride_kc_d, stride_kc_x,
    ...
    KV_CACHE_HEAD_MAJOR: tl.constexpr,
    K_X: tl.constexpr,
):
        ...
        if WRITE_K_TO_CACHE:
            if KV_CACHE_HEAD_MAJOR:
                k_cache_offs = (
                    ...
                    + hq * stride_kc_h
                    + (d[None, :] // K_X) * stride_kc_d
                    + block_row[:, None] * stride_kc_t
                    + (d[None, :] % K_X) * stride_kc_x
                )
            else:
                k_cache_offs = (
                    phys_block[:, None] * stride_kc_b
    ...
    if seq_len > 0:
        ...
        if KV_CACHE_HEAD_MAJOR:
            k_offs = (
                ...
                + h * stride_kc_h
                + (d_qk[None, :] // K_X) * stride_kc_d
                + slot_offs[:, None] * stride_kc_t
                + (d_qk[None, :] % K_X) * stride_kc_x
            )
        else:
            k_offs = (
                phys_block * stride_kc_b
