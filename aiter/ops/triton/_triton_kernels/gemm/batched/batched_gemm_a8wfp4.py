...
@triton.heuristics(
    {
        # K is in true (unpacked) element units here, so all divisibility checks are
        # against element counts directly (no /2 packed convention).
...
def _batched_gemm_a8wfp4_kernel(
    ...
    A is MXFP8 (e4m3, A_PACK=1) or MXFP4 (e2m1, A_PACK=2); B is MXFP4 (e2m1).
    A_scales and B_scales are per-1x32 e8m0 microscales folded into a scaled MFMA.
    A has shape (M, K), B has shape (K, N) and C has shape (M, N). K is in true
    (unpacked) element units; A packs A_PACK elems per byte, B packs 2 per byte.
    """
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        # Byte columns per K block: A packs A_PACK elems/byte, B packs 2 elems/byte.
        ...
        # Create pointers for the first block of A and B scales (per-1x32 e8m0).
        ...
        for k in range(pid_k * num_k_iter, (pid_k + 1) * num_k_iter):
            ...
            # Load the next block of A and B. On the ragged-K tail, mask out-of-bounds
            # columns to 0 (in byte units, hence the per-operand pack divisor).
            if EVEN_K:
                a = tl.load(a_ptrs)
...
def get_splitk(K: int, BLOCK_SIZE_K: int, NUM_KSPLIT: int):
    ...
    # (unpacked) element units, so SPLITK_BLOCK_SIZE / BLOCK_SIZE_K are elements too.
    NUM_KSPLIT_STEP = 2
    BLOCK_SIZE_K_STEP = 2
...
def _get_config(
    ...
    # Config files use K=2*K in their naming (FP4 weights are packed 2 per byte),
