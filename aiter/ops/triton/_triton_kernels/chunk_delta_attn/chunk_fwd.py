...
from aiter.ops.triton._triton_kernels.chunk_delta_attn.intra_attn import (
    ...
    chunk_delta_attn_fwd_intra_opt,
)
...
def _gluon_gla_fwd_o(
    q, v, g, A, h, scale, chunk_size, cu_seqlens=None, chunk_indices=None
):
    ...
    if cu_seqlens is not None:
        # Varlen: one chunk per CTA over the flat chunk list, so a CTA can never
        # straddle a sequence boundary. This gives up the cross-chunk prefetch
        # pipeline (BLOCK_T > BT) but keeps every other Gluon optimization.
        chunk_offsets = prepare_chunk_offsets(cu_seqlens, BT)
        grid_g = (len(chunk_indices), B * HV, triton.cdiv(V, BV_gluon))
...
def _gluon_recompute_w_u(k, v, beta, A, gk, cu_seqlens=None, chunk_indices=None):
    ...
    if cu_seqlens is not None:
        # Varlen: the non-persistent kernel already covers one chunk per CTA, so it
        # only needs the flat chunk list to locate the sequence.
        grid_g = (len(chunk_indices), B * HV)
...
def _gluon_delta_h(
    k, w, u, gk, chunk_size, output_final_state, cu_seqlens=None, chunk_indices=None
):
    ...
    if not (8 <= H <= 64 and K == 128 and not output_final_state):
        return None
    # One CTA walks a whole sequence, so varlen only changes how many CTAs there
    # are and how many chunks `h` holds: the recurrence itself is per-sequence
    # either way.
    ...
    final_state = None
    BV_gluon = 32
    grid_g = (N * H, triton.cdiv(V, BV_gluon))
...
def chunk_delta_attn_fwd(
    ...
    _skip_wu = (
        use_gluon and _gluon_wu_smallh_kernel is not None and not disable_recompute
    )
    ...
    # ------------------------------------------------------------------
    h_result = None
    if use_gluon and _gluon_dh_kernel is not None and initial_state is None:
    ...
    # ------------------------------------------------------------------
    o = None
    if use_gluon and _gluon_o_kernel is not None:
        o = _gluon_gla_fwd_o(
            q,
            v_new,
            g_cumsum,
