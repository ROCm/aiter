...
def chunk_gated_delta_rule_fwd_opt_vk(
    ...
    if use_chunk_hip:
        ...
        # The FlyDSL hidden-state wrapper expects ``g`` in head-major
        # [B, H, T] layout (matches Triton VK / HIP). ``g_cumsum`` from the
        # prepare stage is already head-major, so pass it through
        # directly. The wrapper accepts
