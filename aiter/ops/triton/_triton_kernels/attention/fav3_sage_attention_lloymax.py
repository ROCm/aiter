...
@triton.jit
def _lm_lookup(
    idx, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, c14, c15
):
...
def _sage_fwd_no_mask_lloymax(
    ...
    m_i,
    q_lo,
    q_hi,  # (BLOCK_M, HALF_D) float32 — pre-dequantized Q halves
    ...
    for start_n in range(block_min, block_max, BLOCK_N):
        ...
        if PRE_LOAD_V:
            ...
            if PADDED_HEAD_V:
                v = tl.load(
                    v_ptrs, mask=offs_d_v[None, :] < ACTUAL_BLOCK_DMODEL_V, other=0.0
                )
            else:
                v = tl.load(v_ptrs)
        ...
        qk = tl.dot(
            q_lo.to(tl.bfloat16), k_lo.to(tl.bfloat16), out_dtype=ACC_TYPE, acc=qk
        )
        qk = tl.dot(
            q_hi.to(tl.bfloat16), k_hi.to(tl.bfloat16), out_dtype=ACC_TYPE, acc=qk
        )
...
def _sage_fwd_mask_lloymax(
    ...
    for start_n in range(block_min, block_max, BLOCK_N):
        ...
        k_norms = tl.load(
            kn_base_ptrs + start_n + offs_n, mask=kv_offs_n < seqlen_k, other=1.0
        ).to(tl.float32)
...
def sage_fwd_lloymax(
    ...
    q_norms = tl.load(qn_ptrs, mask=offs_m < seqlen_q, other=0.0).to(
        tl.float32
    )  # (BLOCK_M,)
    ...
    k_base = K_packed + off_z * stride_kz + off_h_k * stride_kh
    ...
    if not USE_BLOCK_SPARSE:
        ...
        lut_idx = (
            off_z * HQ * tl.cdiv(seqlen_q, BLOCK_M)
            + off_h_q * tl.cdiv(seqlen_q, BLOCK_M)
            + start_m
        )
