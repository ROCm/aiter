...
def _winograd_hw_f4x3_input_transform_kernel(
    ...
    """V = BᵀdB per (n, d) input plane. tile index runs over N*D*tile_H*tile_W.

    CBLOCKED=False reads NCDHW; CBLOCKED=True reads channel-blocked NCDHWc
    ``[N, C_blocks, D, H, W_in, Cb]`` for coalesced channel loads (stays
    coalesced when BLOCK_C <= Cb). Only the load addressing differs — the
    transform math and V store are identical."""
...
def _winograd_hw_f4x3_batched_gemm_kernel(
    ...
    """M[alpha, (n,od,tile), k] = Σ_t Σ_c U[t,alpha,k,c] · V[alpha, (n,id,tile), c]
    with id = od - pad_d + t masked to [0, D). 2D batched GEMM + 3-tap depth loop."""
...
def _winograd_hw_f4x3_output_transform_kernel(
    ...
    """Y = AᵀMA + bias/act per (n, od, tile). NCDHW output."""
    # M: [36, T_out, K_out]
    stride_m_k: tl.constexpr = 1
    stride_m_tile: tl.constexpr = K_out
    stride_m_alpha: tl.constexpr = T_out * K_out
