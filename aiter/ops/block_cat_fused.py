...
def _build_triton_fw_kernel():
    ...
    def _block_cat_fused_fw(
        ...
        tl.store(cat_out_ptr + xindex, out.to(cat_out_ptr.dtype.element_ty), mask=xmask)
...
def _build_triton_kernel():
    ...
    def _block_cat_fused_bw(
        ...
        tl.store(
            grad_l12_out_ptr + xindex,
            grad_l12.to(grad_l12_out_ptr.dtype.element_ty),
            mask=xmask,
        )
        tl.store(
            grad_l4_out_ptr + l4_idx,
            grad_l4_local.to(grad_l4_out_ptr.dtype.element_ty),
            mask=xmask & is_l2,
        )
