...
@compile_ops(
    "module_fmha_v3_fwd",
    fc_name="fmha_v3_fwd_mxfp4",
    gen_fake=_gen_fmha_v3_fwd_mxfp4_fake_tensors,
    mutates_args=[],
)
def fmha_v3_fwd_mxfp4(
    q: Tensor,                    # [b, sq, hq, hd/2 = 64], int8/uint8 (fp4-packed)
    k: Tensor,                    # [b, sk, hk, 64], int8/uint8
    v: Tensor,                    # [b, sk, hk, 128], fp8
    q_descale: Tensor,            # E8M0 per-block bytes, [b, sq, hq, hd/32 = 4]
...
@compile_ops(
    "module_fmha_v3_fwd",
    fc_name="fmha_v3_fwd_fp8_sparse",
    gen_fake=_gen_fmha_v3_fwd_fp8_sparse_fake_tensors,
    mutates_args=[],
)
def fmha_v3_fwd_fp8_sparse(
...
def flash_attn_fp8_sparse_pertensor_func(
    ...
    Sibling of flash_attn_i8fp8_sparse_pertensor_func that goes through the
    same aiter v3 ASM host loader (module_fmha_v3_fwd) but routes to the
    hand-written all-fp8 sparse kernel fwd_hd128_fp8_sparse.co. The only
    contract difference from the i8fp8 variant is that Q/K are fp8 (E4M3)
    rather than int8.

    Args:
        q: fp8 tensor [b, sq, hq, 128], bshd
        k: fp8 tensor [b, sk, hk, 128], bshd
        v: fp8 tensor [b, sk, hk, 128], bshd
        q_descale, k_descale, v_descale: fp32 scalars or [b, hk]
        kv_block_indices, lut_start, lut_count: int32 LUT produced by
            aiter.ops.triton.attention.utils.block_attn_mask_to_ragged_lut
...
def flash_attn_mxfp4_pertensor_func(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    q_descale: torch.Tensor,
    ...
    """Dense mxfp4 FMHA forward (hd=128, gfx950).

    Dense sibling of ``flash_attn_mxfp4_sparse_pertensor_func`` (drops the LUT):
    routes to the hand-written mxfp4-Q,K * fp8-V dense kernel
    ``fwd_hd128_mxfp4.co``.
    ...
    Constraints (assert-checked C++-side, see asm_mha_fwd_sparse.cu::
    fmha_v3_fwd_mxfp4): hd (logical) == 128, hq % hk == 0, (hq/hk) a power of 2,
    batch mode, non-causal, gfx950.
