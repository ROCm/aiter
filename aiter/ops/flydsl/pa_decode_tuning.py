...
SOURCE_FILES = (
    ...
    # The tile entry point delegates code generation to these operation helpers.
    "kernels/pa_decode/__init__.py",
    "kernels/pa_decode/context.py",
    "kernels/pa_decode/traits.py",
    "kernels/pa_decode/pipeline.py",
    "kernels/pa_decode/op_lds.py",
    "kernels/pa_decode/op_gemm.py",
    "kernels/pa_decode/op_softmax.py",
    "kernels/pa_decode/op_epilog.py",
    "kernels/pa_decode_reduce.py",
    # Shared addressing, arithmetic, and launch helpers also affect the result.
...
def _backend_paths(pa):
    ...
    paths = {}
    for name in SOURCE_FILES:
        if name == "pa_decode_tuning.py":
            actual = Path(__file__).resolve()
        else:
            # Import the package itself for __init__.py so its existing module
            # identity is checked instead of executing a second initializer.
            module_name = name.removesuffix(".py").replace("/", ".")
            module_name = module_name.removesuffix(".__init__")
            module = (
                pa
                if name == "pa_decode.py"
                else importlib.import_module("aiter.ops.flydsl." + module_name)
            )
