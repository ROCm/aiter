...
from .kernels.gemm_decode_wave import (
    compile_gemm_decode_wave_bf16,
    compile_gemm_decode_wave_fp8,
)
...
# ---------------------------------------------------------------------------
# FlyDSL decode GEMM (small exact-M) kernels
# ---------------------------------------------------------------------------
...
def validate_gemm_decode_tensors(
    ...
    tensors = {"A": A, "B": B, "C": C}
    expected_dtypes = {"A": operand_dtype, "B": operand_dtype, "C": torch.bfloat16}
    for name, tensor in tensors.items():
        ...
        if tensor.dtype != expected_dtypes[name]:
            raise ValueError(f"{name} must have dtype {expected_dtypes[name]}")
...
def compile_gemm_decode_fp8(
    ...
    *,
    ...
    """Compile one exact per-tensor FP8 ``(arch, M, N, K, config)`` identity."""
    if isinstance(config, WaveDecodeConfig):
        return compile_gemm_decode_wave_fp8(m, n, k, config, arch)
    raise TypeError(
        "per-tensor FP8 decode currently supports only the Wave policy, got "
...
def gemm_decode_fp8(
    ...
    """Launch one Wave configuration over per-tensor-quantized E4M3 operands."""
    runtime_arch = get_gfx_runtime()
    m, n, k = validate_gemm_decode_tensors(
        A,
        B,
        C,
        arch=runtime_arch,
        operand_dtype=A.dtype,
    )
    if A.dtype not in (torch.float8_e4m3fn, torch.float8_e4m3fnuz):
        raise ValueError(f"A and B must be E4M3 FP8, got {A.dtype}")
    for name, scale in (("x_scale", x_scale), ("w_scale", w_scale)):
        if not isinstance(scale, torch.Tensor) or scale.dtype != torch.float32:
            raise ValueError(f"{name} must be a float32 tensor")
        if scale.numel() != 1:
            raise ValueError(f"{name} must be a single per-tensor scale")
        if scale.device != A.device:
            raise ValueError(f"{name} must be on the same device as A")
    launch_stream = _normalize_launch_stream(A.device, stream)
    launcher = compile_gemm_decode_fp8(m, n, k, config, arch=runtime_arch)
    launcher(
        A,
        B,
        C,
        x_scale.reshape(1),
        w_scale.reshape(1),
        stream=fx.Stream(launch_stream),
    )
    return C
