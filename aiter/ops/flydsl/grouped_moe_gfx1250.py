...
def _grouped_a8w4_tdm_moe(
    ...
    *,
    ...
    if n_warp2 is None:
        n_warp2 = n_warp
    # The A-scale preshuffle layout is fixed by the WMMA M-repeat of the WAVE
    # tile (tile_m // m_warp // 16), so wmma_rep must come from the wave grid,
    # not tile_m, once m_warp > 1.
...
def grouped_gemm_gfx1250_a8w4(
    ...
    *,
    ...
    if expert_mask is None or not doweight_stage1:
        ...
        # AITER_TDM_TILE_M / _TILE_N / _TILE_K / _M_WARP / _N_WARP / _NUM_BUFFERS
        # (+ *_M2/_N2/_K2/_M_WARP2/_N_WARP2/_NUM_BUFFERS2) to sweep the felix TDM
        # batched GEMM tiles and wave grid (e.g. t256x256x256 w2x2 b3).
        def _tdm_env(name):
        ...
        if any(
            v is not None
            for v in (
                _ov_m, _ov_n, _ov_k, _ov_mw, _ov_nw, _ov_nb,
                _ov_m2, _ov_n2, _ov_k2, _ov_mw2, _ov_nw2, _ov_nb2,
            )
        ):
            ...
            # *_M2/_N2/_K2/_M_WARP2/_N_WARP2/_NUM_BUFFERS2 override is set. This
            # intentionally discards any CSV stage2 value so an env sweep stays
            # self-consistent.
