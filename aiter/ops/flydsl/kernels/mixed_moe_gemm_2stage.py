...
def compile_mixed_moe_gemm1(
    *,
    ...
    # symbol collision when multiple multi-B partition configs coexist in one process.
    ...
    if True:
        ...
        @flyc.kernel(known_block_size=[total_threads, 1, 1], name=_gpu_kn_s1)
        def moe_gemm1(
            arg_out: fx.Tensor,
            arg_x: fx.Tensor,
            arg_w_0: fx.Tensor,
            ...
            if const_expr(not is_f16_b):
                ...
                sw_rsrc_0 = buffer_ops.create_buffer_resource(
                    arg_scale_w_0, max_size=False, num_records_bytes=sw_nbytes_i32
                )
            ...
            def _moe_gemm1_body():
                ...
                if num_b_tensors == 1:
                    ...
                    _local_0 = expert_idx
                    _off_0 = _local_0 * arith.constant(2 * inter_dim, index=True)
                    if num_b_tensors >= 2:
                        _thresh_1 = arith.index(b_tensor_l_offsets[1])
                        _local_1 = expert_idx - _thresh_1
                        _off_1 = _local_1 * arith.constant(2 * inter_dim, index=True)
                    ...
                    if num_b_tensors >= 3:
                        ...
                        _off_2 = _local_2 * arith.constant(2 * inter_dim, index=True)
                    ...
                    if num_b_tensors >= 4:
                        ...
                        _off_3 = _local_3 * arith.constant(2 * inter_dim, index=True)
                ...
                def write_row_to_lds(
                    *,
                    ...
                    for ni in range_constexpr(num_acc_n):
                        ...
                        v = vector.extract(
                            acc_v[acc_idx], static_position=[ii], dynamic_position=[]
                        )
                        if const_expr(_apply_weight):
                            v = v * tw
                ...
                def _make_write_row_to_lds(acc_v):
                    def _write_row_to_lds_bound(
                        *,
                        mi: int,
                        ii: int,
                        row_in_tile,
                    ...
                    return _write_row_to_lds_bound
                ...
                if const_expr(gate_only):
                    ...
                    c_shuffle_epilog(
                        ...
                        write_row_to_lds=_make_write_row_to_lds(acc),
    ...
    # Unique host function name to prevent LLVM JIT symbol collision
    if num_b_tensors > 1:
...
def compile_mixed_moe_gemm2(
    *,
    ...
    # Unique GPU kernel name per b_tensor_l_offsets
    ...
    if True:
        ...
        @flyc.kernel(known_block_size=[total_threads, 1, 1], name=_gpu_kn_s2)
        def moe_gemm2(
            arg_out: fx.Tensor,
            arg_x: fx.Tensor,
            arg_w_0: fx.Tensor,
            ...
            def _moe_gemm2_then_body():
                ...
                # Compute partition-local B base from multi-B corrected expert_off_idx.
                # expert_off_idx = local_expert * model_dim (already adjusted for partition).
                # _local_b_base = local_expert * _expert_b_stride
                #               = (expert_off_idx / model_dim) * _expert_b_stride
                # NOTE: stored in _local_b_base (not _expert_b_base) because the outer
                # _expert_b_base is yielded by scf.for in legacy (non-persistent) mode
                # and must stay in the parent scope to satisfy SSA dominance.
                _c_b_stride = arith.constant(_expert_b_stride, index=True)
                _local_b_base = (expert_off_idx // n_idx) * _c_b_stride
