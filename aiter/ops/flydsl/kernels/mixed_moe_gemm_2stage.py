    ...
    *,
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            with ir.InsertionPoint(for_persist.body):
                ...
                def moe_gemm1_body():
                    ...
                    if const_expr(k_wave > 1):
                        ...
                        grp_x_bytes = wave_k_id * arith.constant(
                            single_x_bytes, index=True
                        )
                        x_view_ty = _mT.memref(
                            input_elems, x_lds_elem(), memory_space=_lds_space()
                        )
                        pong_off = (
                            arith.constant(lds_pong_offset, index=True) + grp_x_bytes
                        )
                        ping_off = (
                            arith.constant(lds_ping_offset, index=True) + grp_x_bytes
                        )
                    ...
                    def rearrange_a_scale(raw_i32):
                        ...
                        if const_expr(pack_M >= scale_mn_pack):
                            return raw_i32
                        b_k0 = arith.andi(
                            arith.shrui(raw_i32, scale_shift), scale_mask_lo
                        )
                        b_k1 = arith.andi(
                    ...
                    def rearrange_b_scale(raw_i32):
                        ...
                        if const_expr(pack_N >= scale_mn_pack):
                            return raw_i32
                        b_k0 = arith.andi(
                            arith.shrui(raw_i32, bscale_shift), scale_mask_lo
                        )
                        b_k1 = arith.andi(
                    ...
                    def prefetch_full_a_from_lds(lds_buffer, ku_limit=k_unroll):
                        ...
                        for k_idx in range_constexpr(ku_limit):
                            col_base = (
                                col_offset_base + (k_idx * 128) // a_elem_vec_pack
                            )
                            ...
                            if const_expr(enable_bias):
                                if const_expr(gate_up_interleave):
                                    ...
                                    for ni in range_constexpr(num_acc_n):
                                        ...
                                        bias_offset = (
                                            expert_off_idx + up_off + logical_col
                                        )
                                        bias_pf.append(
                    ...
                    def interleaved_half(
                        ...
                        abs_k_dma = k_base_idx + arith.constant(
                        ...
                        for _p in range_constexpr(pipe_n_phases):
                            if const_expr(pp_has_scale[_p]):
                                ...
                                new_gs_list = []
                                new_us_list = (
                                    [] if const_expr(not single_b_pipe) else None
                                )
                        ...
                        for i_gs in range_constexpr(len(new_gs_list)):
                            cur_gate_bs.append(
                                vector.from_elements(
                                    T.vec(1, T.i32), [new_gs_list[i_gs]]
                                )
                            )
                        if const_expr(not single_b_pipe):
                            cur_up_bs = []
                    ...
                    if const_expr(k_wave > 1 and not kwave_fused):
                        ...
                        for ai in range_constexpr(nm):
                            ...
                            for g in range_constexpr(k_wave):
                                peer = (
                                    arith.constant(g * num_n_waves, index=True)
                                    + wave_n_id
                                )
                    ...
                    if const_expr(enable_bias and not is_splitk):
                        ...
                        if const_expr(bias_pf is not None):
                            ...
                            for ni in range_constexpr(num_acc_n):
                                ...
                                bias_gate_vals.append(
                                    load_bias_scalar(bias_rsrc, bias_off)
                                )
                    ...
                    if const_expr(gate_up_interleave and not is_splitk):
                        ...
                        for mi in range_constexpr(m_repeat):
                            for ni in range_constexpr(gui_out_n):
                                ...
                                acc[out_idx] = act_vec4(
                                    acc_gate[g_idx], acc_gate[u_idx]
                                )
                    ...
                    def f32_to_e2m1(qx_f32):
                        ...
                        denorm_f32 = qx_abs.bitcast(T.f32) + c0x4A800000_i32.bitcast(
                            T.f32
                        )
                    ...
                    def store_pair(*, row_local, row, row_ctx, col_pair0, col_g0, frag):
                        ...
                        if const_expr(need_quant and not is_splitk):
                            ...
                            col_idx = col_g0 + arith.constant(
                                sk_n_offset[0], index=True
                            )
                    ...
                    if const_expr(kwave_fused):
                        ...
                        mock_gate_only or (gate_up_interleave and is_splitk)
                    ):
                        eff_e_vec = e_vec_sk
                        acc = acc_gate
                        c_shuffle_epilog(
            ...
            with ir.InsertionPoint(for_persist.body):
                ...
                def moe_gemm2_then_body():
                    ...
                    x_load_vec_elems = (
                        x_load_bytes
                        if a_elem_bytes == 1
                        else x_load_bytes // a_elem_bytes
                    )
                    ...
                    if const_expr(
                        b_split_enabled and is_prefill_shape and k_unroll >= 4
                    ):
                    ...
                    def apply_k_shift(scale_vec, k_shift_bits):
                        if const_expr(k_shift_bits > 0):
                            ...
                            val = arith.shrui(
                                val, arith.constant(k_shift_bits, type=T.i32)
                            )
                    ...
                    if const_expr(use_async_copy):
                        ...
                        c_a_elem_bytes_dma = arith.constant(
                            int(a_elem_bytes), index=True
                        )
                        c_wave_dma_bytes = arith.constant(
                            wave_size * dma_bytes, index=True
                        )
                    ...
                    def emit_tid_lds_prologue():
                        ...
                        with ir.InsertionPoint(if_tid.then_block):
                            ...
                            if const_expr(use_buf_atomic_pre):
                                ...
                                s_pre = arith.shrui(
                                    tid_val, arith.constant(24, type=T.i32)
                                )
                            ...
                            tid_vec1 = vector.from_elements(
                                T.vec(1, T.i32), [stored_val]
                            )
                            vector.store(tid_vec1, lds_tid, [tx])
                            if const_expr(doweight_stage2):
                                tw_vec1 = vector.from_elements(
                                    T.vec(1, T.f32), [tw_val_m]
                                )
                    ...
                    if const_expr(k_main2_py > 0):
                        for k_iv_py in range_constexpr(0, k_main2_py, tile_k * 2):
                            ...
                            a1_prefetch_ping = (
                                lds_load_packs_k64(
                                    row_a_lds, a1_col_base, lds_base_ping
                                )
                                if pack_K >= 2
                                else None
                            )
