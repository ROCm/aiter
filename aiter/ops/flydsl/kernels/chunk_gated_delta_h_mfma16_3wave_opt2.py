...
("mfma16_3wave_opt2"), a divergence base for a 3-wave occupancy variant.

NOTE: this file is currently an EXACT copy of ``chunk_gated_delta_h_mfma16_2wave_opt1.py``
(the 2-wave, LDS-bound, HIP-aligned store-overlap kernel), created as the
starting point for a distinct "3-wave opt2" line of optimization. Until it is
diverged, its body/behavior is identical to 2wave_opt1 (input k pre-transposed
``[B,Hg,K,T_flat]``, output public VK ``[..., V, K]`` direct). Only the compile /
entry / kernel symbols differ (``mfma16_3wave_opt2``). The occupancy/pipeline
notes below still describe the copied 2-wave implementation.

Exports ``compile_chunk_gated_delta_h_mfma16_3wave_opt2`` / emits
``chunk_gdn_fwd_h_flydsl_mfma16_3wave_opt2`` and backs the host
``_fork="mfma16_3wave_opt2"`` path. Sibling ``chunk_gated_delta_h_kv.py`` is the
...
def compile_chunk_gated_delta_h_mfma16_3wave_opt2(
    *,
    K: int,
    V: int,
    BT: int = 64,
    ...
    # ON vs OFF, all 41 mfma16_3wave_opt2 allclose pass):
    ...
    allocator = SmemAllocator(
        None,
        arch=GPU_ARCH,
        global_sym_name=f"gdn_h_mfma16_3wave_opt2_vkb128alias_bv{BV}u{int(USE_LDS_U)}vn{int(USE_LDS_VN_STORE)}g{int(USE_LDS_G)}ht{int(LDS_HT_INDEPENDENT)}wpf{int(W_PF_INTERLEAVE)}kpf{int(K_PF_INTERLEAVE)}wg1{int(W_PF_INTERLEAVE_GEMM1)}hov{int(HT_STORE_OVERLAP_GEMM2)}_smem_v{_K5_KERNEL_REVISION}",
    )
    ...
    @flyc.kernel(name="chunk_gdn_fwd_h_flydsl_mfma16_3wave_opt2")
    def gdn_h_kernel(
        k_tensor: fx.Pointer,
        v_tensor: fx.Pointer,
        w_tensor: fx.Pointer,
