...
# Preshuffled B (WeightLayout.PRESHUFFLED) is pinned on the host in
# aiter.ops.flydsl.warp_decode_moe: CDNA shuffle_weight / make_preshuffle_b_layout
# N-major (permute 0,1,3,4,2,5). Do not invent a second permutation here.
...
# in **i32** elements (16B kpack = 4 dwords). ``packed_k`` is the k_contiguous
# last dim (FP8 elems, BF16 elems, or MXFP4 bytes).
_KPACK_I32 = 4
_KPACK_NLANE = 16
_KPACK_KLANE = 4


def _nmajor_kpack_i32_layout(n_out: int, packed_k: int, elem_bytes: int):
    """Shape/stride of one expert's preshuffled B as i32 kpack tiles."""
    if elem_bytes not in (1, 2):
        raise ValueError(f"elem_bytes must be 1 or 2, got {elem_bytes}")
    k_tile = 64 if elem_bytes == 1 else 32
    if n_out % _KPACK_NLANE != 0:
        raise ValueError(f"N={n_out} must be divisible by {_KPACK_NLANE}")
    if packed_k % k_tile != 0:
        raise ValueError(f"packed_k={packed_k} must be divisible by {k_tile}")
    k0 = packed_k // k_tile
    stride_nlane = _KPACK_I32
    stride_klane = _KPACK_NLANE * stride_nlane
    stride_k0 = _KPACK_KLANE * stride_klane
    stride_n0 = k0 * stride_k0
    shape = (n_out // _KPACK_NLANE, k0, _KPACK_KLANE, _KPACK_NLANE, _KPACK_I32)
...
def _preshuffled_expert_b_i32_tensor(ptr, expert, n_out, packed_k, elem_bytes):
    ...
    view = fx.make_view(fx.inttoptr(pt, base), fx.make_layout(shape, stride))
    return fx.rocdl.make_buffer_tensor(view, num_records_bytes=bytes_per_expert)
...
def _fp8_gate_up_weight_bundle(
    preshuffled,
    wg_ptr,
    wu_ptr,
    e,
    ...
    *,
    ...
    """Gate/up B views. Python ``if`` (not kernel SSA) so names are not live-outs."""
    if preshuffled:
        ...
        return wg_b, wu_b, None, None, fx.Int32(0)
    ...
    return None, None, wg_rsrc, wu_rsrc, w_word_base


def _fp8_down_weight_bundle(
    preshuffled,
    wd_ptr,
    wd_rsrc,
    e,
    ...
    *,
    ...
    """Down B view. Python ``if`` so ``w_word_base`` is not an SSA live-out."""
    if preshuffled:
        wd_b = _preshuffled_expert_b_i32_tensor(wd_ptr, e, hidden, inter, 1)
        return wd_b, None, [fx.Int32(0) for _ in range(kh_per_warp)]
    ...
    return None, wd_rsrc_e, w_word_base


def _gate_up_weight_ptrs(preshuffled, wg_ptr, wu_ptr, e, n_out, packed_k, elem_bytes):
    """Split or kpack gate/up pointers; Python ``if`` (not kernel SSA)."""
    if preshuffled:
...
def _down_preshuffled_b(preshuffled, wd_ptr, e, n_out, packed_k, elem_bytes):
    if preshuffled:
        return _preshuffled_expert_b_i32_tensor(wd_ptr, e, n_out, packed_k, elem_bytes)
    return None
...
# -------------------------------------------------------------------------
# Preshuffled B probe (i64 expert fold + i32 kpack view + fx.copy)
# -------------------------------------------------------------------------
def build_preshuffled_b_load_module(
    n_out: int,
    packed_k: int,
    elem_bytes: int,
    expert: int,
    ...
    """Load one in-expert dword from preshuffled B (subtask 3 view/i64 fold)."""

    @flyc.kernel
    def _kernel(w_ptr: fx.Pointer, out_ptr: fx.Pointer):
        t = _preshuffled_expert_b_i32_tensor(w_ptr, expert, n_out, packed_k, elem_bytes)
        val = _kpack_load_i32(t, n_idx, k_packed, elem_bytes=elem_bytes)
        out_t = ptr_buf_tensor(out_ptr, fx.Int32)
        if gpu.thread_id("x") == 0:
            buf_copy_store(out_t, 0, val)

    @flyc.jit
    def _launch(w_ptr: fx.Pointer, out_ptr: fx.Pointer, stream: fx.Stream):
        _kernel(w_ptr, out_ptr).launch(
            grid=(1, 1, 1),
            block=(WARP_SIZE, 1, 1),
            stream=stream,
        )
...
def _reduce_klane4_f32(val):
    """Reduce the four klane partials while preserving each nlane output."""
    val = fx.Float32(val)
    val = val + val.shuffle_xor(fx.Int32(16), fx.Int32(WARP_SIZE))
    val = val + val.shuffle_xor(fx.Int32(32), fx.Int32(WARP_SIZE))
    return val


def _build_gate_up_fp8_preshuffled_native(
    hidden: int,
    inter: int,
    top_k: int,
    *,
    ...
    if hidden % 64 != 0 or inter % 16 != 0:
        raise ValueError("preshuffled FP8 gate_up needs HIDDEN % 64 and INTER % 16")
    ...
    if w_scale_mode == "block2d":
        ...
        if hidden % scale_bk != 0 or scale_bk % 16 != 0:
            raise ValueError(
                "native preshuffled FP8 gate_up needs scale_bk to divide HIDDEN "
    ...
    def _kernel(
        ...
        for k0 in range_constexpr(num_kpack):
            ...
            if const_expr(block2d):
                ...
                gs = _f32_load(wgs_t, row_blk * scale_cols + col_blk)
                us = _f32_load(wus_t, row_blk * scale_cols + col_blk)
                gate_l = gate_l + fx.Float32(gd) * fx.Float32(gs)
                up_l = up_l + fx.Float32(ud) * fx.Float32(us)
            else:
...
# -------------------------------------------------------------------------
def _build_gate_up_fp8_act_preshuffled_native(
    hidden: int,
    inter: int,
    top_k: int,
    *,
    ...
    if (
        hidden % scale_bk != 0
        or scale_bk % 16 != 0
        or hidden % scale_bxk != 0
        or scale_bxk % 16 != 0
    ):
        raise ValueError(
            "native preshuffled FP8-act gate_up needs weight and activation "
    ...
    def _kernel(
        ...
        for k0 in range_constexpr(num_kpack):
            ...
            for ipair in range_constexpr(8):
                word = ipair // 2
                hi = (ipair % 2) == 1
                x_i32 = bf16x2_to_i32(fp8x2_to_bf16x2(xw[word], one_f32, hi=hi))
                g_i32 = bf16x2_to_i32(fp8x2_to_bf16x2(gw[word], one_f32, hi=hi))
            ...
            us = _f32_load(wus_t, row_blk * scale_cols_g + col_blk)
...
def _build_down_fp8_preshuffled_native(
    ...
    *,
    ...
    """N-major kpack-native FP8 down: one wave computes 16 output rows of one expert.

    Grid is ``B * TOPK * (HIDDEN/16)`` so occupancy matches gate/up native and
    k-contiguous H2 at decode B. Experts that share an output row atomic-add
    FP32 partials into a caller-zeroed accumulator.
    """
    ...
    *,
    ...
    def _kernel(
        ...
        for k in range_constexpr(top_k):
            ...
            wd_b, wd_rsrc_e, w_word_base = _fp8_down_weight_bundle(
                preshuffled,
                wd_ptr,
                wd_rsrc,
                e,
...
# -------------------------------------------------------------------------
def _build_gate_up_fp4_preshuffled_native(
    hidden: int,
    inter: int,
    top_k: int,
    *,
    ...
    if hidden % 128 != 0 or inter % 16 != 0:
        raise ValueError("preshuffled MXFP4 gate_up needs HIDDEN % 128 and INTER % 16")
    if hidden % scale_bk != 0 or scale_bk % 32 != 0:
        raise ValueError(
            "native preshuffled MXFP4 gate_up needs scale_bk to divide HIDDEN "
    ...
    def _kernel(
        ...
        for k0 in range_constexpr(num_kpack):
            ...
            for ipair in range_constexpr(16):
                word = ipair // 4
                sel = ipair % 4
                g_i32 = bf16x2_to_i32(fp4x2_to_bf16x2(gw[word], gs, sel=sel))
...
# -------------------------------------------------------------------------
def _build_down_fp4_preshuffled_native(
    inter: int,
    hidden: int,
    top_k: int,
    *,
    ...
    """N-major kpack-native MXFP4 down: one wave computes 16 output rows of one expert.

    Same TOPK-parallel 16x4 grid as FP8 native down. ``packed_k = INTER/2``;
    each k0 tile is 128 INTER elems (4 klanes x 32 fp4). E8M0 is applied in
    ``cvt_scalef32_pk_bf16_fp4``. Experts atomic-add FP32 partials into a
    caller-zeroed accumulator.
    """
    if inter % 128 != 0 or hidden % 16 != 0:
        raise ValueError("preshuffled MXFP4 down needs INTER % 128 and HIDDEN % 16")
    if inter % scale_bk != 0 or scale_bk % 32 != 0:
        raise ValueError(
            "native preshuffled MXFP4 down needs scale_bk to divide INTER "
            "and be a multiple of 32"
        )
    num_kpack = inter // 128
    ...
    def _kernel(
        ...
        w_row = e * hidden + out_j
        row_blk = w_row // scale_bn
        wd_b = _preshuffled_expert_b_i32_tensor(wd_ptr, e, hidden, inter // 2, 1)
        inter_row = token_b * top_k + expert_k
        ...
        for k0 in range_constexpr(num_kpack):
            k_base = k0 * 128 + klane * fx.Int32(32)
            a_word0 = (inter_row * inter + k_base) // 2
            aw = load_i32_words(inter_rsrc, a_word0, 16)
            dw = _native_kpack_words(wd_b, n0, k0, klane, nlane)
            col_blk = k_base // scale_bk
            sidx = fx.Int32(row_blk * scale_cols + col_blk)
            ...
            for ipair in range_constexpr(16):
                word = ipair // 4
                sel = ipair % 4
                d_i32 = bf16x2_to_i32(fp4x2_to_bf16x2(dw[word], blk_scale, sel=sel))
                pairs.append((aw[ipair], d_i32))
            acc_l = acc_l + fx.Float32(
                drain_or_chain(pairs, dot2_acc=dot2_acc, serialize=serialize_dot2)
            )
...
def build_down_reduce_fp4_module(
    ...
    *,
    ...
    Preshuffled legal tiles (INTER % 128, HIDDEN % 16) use
    ``_build_down_fp4_preshuffled_native`` (16x4 pack loads, TOPK in the grid).
    Gather is only the illegal-tile fallback.
    """
    if preshuffled and inter % 128 == 0 and hidden % 16 == 0:
        return _build_down_fp4_preshuffled_native(
            inter,
            hidden,
            top_k,
            serialize_dot2=serialize_dot2,
    ...
    def _kernel(
        ...
        for k in range_constexpr(top_k):
            ...
            rw = _f32_load(rwt_t, ridx)
            w_row = [e * hidden + out_j0 + h for h in range(kh_per_warp)]
            wd_b = _down_preshuffled_b(preshuffled, wd_ptr, e, hidden, inter // 2, 1)
...
# -------------------------------------------------------------------------
def _build_gate_up_bf16_preshuffled_native(
    hidden: int,
    inter: int,
    top_k: int,
    *,
    ...
    *,
    ...
    def _kernel(
        ...
        for k in range_constexpr(top_k):
            ...
            rw = _f32_load(rwt_t, ridx)
            w_row = [e * hidden + out_j0 + h for h in range(kh_per_warp)]
            wd_b = _down_preshuffled_b(preshuffled, wd_ptr, e, hidden, inter, 2)
