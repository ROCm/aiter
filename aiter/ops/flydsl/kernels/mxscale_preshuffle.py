...
"""MXFP4/8 by preshuffled MXFP4/8 GEMM for gfx950 scaled MFMA."""
...
def _bq_view(arg_bq_addr, row_elems, KH4, k_tiles, k_halves, pair, b_bytes):
    """Build one preshuffled-B N-row view in i32 units."""
    col_base = rocdl.readfirstlane(T.i32, row_elems * KH4)
    byte_off = fx.Int64(col_base) * fx.Int64(4)
    i32_ptr_type = fx.PointerType.get(
        T.i32, address_space=fx.AddressSpace.Global, alignment=16
    )
    base_iter = fx.inttoptr(i32_ptr_type, arg_bq_addr + byte_off)
    ...
    return fx.rocdl.make_buffer_tensor(
        view, max_size=False, num_records_bytes=b_bytes - byte_off
    )
...
def launch_gemm(
    ...
    scheduled_gmem = (
        cooperative_loads + num_acc_n * k_halves * B_BLK_PER_MMA + m_pairs + n_pairs
    )
    ...
    def kernel_gemm(
        ...
        a_flat = fx.rocdl.make_buffer_tensor(
            fx.Tensor(
                fx.make_view(
                    ...
                    fx.make_layout(PRESHUFFLE_M_MAX * a_row_bytes, 1),
                )
            ),
            max_size=False,
        ...
        def dma_a_to_lds(k_tile, parity):
            ...
            for index in range_constexpr(cooperative_loads):
                ...
                global_byte = (block_m + row) * a_rstride + base_k_byte + col
        ...
        def read_a(parity):
            ...
            for m_index in range_constexpr(m_chunks):
                for k_half in range_constexpr(k_halves):
                    ...
                    low_block = k_half * (A_KH_I32 // 4) + lane_div_16 * (A_GK_I32 // 4)
                    if const_expr(swizzle_lds):
                        low_offset = row_base + (low_block ^ (row % k_blocks16)) * 4
                    else:
                        low_offset = (
                            row_base + k_half * A_KH_I32 + lane_div_16 * A_GK_I32
                        )
                    if const_expr(a_dtype == "fp4"):
                        ...
                        if const_expr(swizzle_lds):
                            high_offset = (
                                row_base
                                + ((low_block + A_HI_OFF // 4) ^ (row % k_blocks16)) * 4
                            )
                        else:
        ...
        b_bytes = fx.Int64(i32_n) * fx.Int64(b_row_bytes)
        b_views = [
            _bq_view(
                arg_b,
        ...
        a_scale_flat = fx.logical_divide(
            fx.rocdl.make_buffer_tensor(
                fx.Tensor(
                    fx.make_view(fx.inttoptr(i32_global, arg_scale_a), scale_layout)
                ),
                max_size=False,
        ...
        b_scale_flat = fx.logical_divide(
            fx.rocdl.make_buffer_tensor(
                fx.Tensor(
                    fx.make_view(fx.inttoptr(i32_global, arg_scale_b), scale_layout)
                ),
                max_size=False,
        ...
        def load_scales(chunk_k_tile):
            ...
            scales_a = [
                load_scale(
                    a_scale_flat[
                        None,
                        rocdl.readfirstlane(T.i32, a_scale_base[pair_index] + k_offset)
                        + scale_lane,
                    ]
                )
                for pair_index in range_constexpr(m_pairs)
            ...
            scales_b = [
                load_scale(
                    b_scale_flat[
                        None,
                        rocdl.readfirstlane(T.i32, b_scale_base[pair_index] + k_offset)
                        + scale_lane,
                    ]
                )
                for pair_index in range_constexpr(n_pairs)
        ...
        def compute(accumulators, a_fragments, b_fragments, scales_a, scales_b, shift):
            ...
            if const_expr(BN < 128):
                n_shift = ((block_n + wave * (BN // num_waves)) % 32) // 16 * 8
                scales_b = [
                    value.shrui(n_shift) & fx.Int32(_SCALE_N_HALF_MASK)
                    for value in scales_b
                ]
            c_fragments = [
        ...
        for iteration, state in range(
            ...
            chunk_k_tile = k_tile if tiles_per_chunk == 1 else k_tile // tiles_per_chunk
            scale_shift = (
                None if tiles_per_chunk == 1 else (k_tile % tiles_per_chunk) * 16
            )
        ...
        c_num_records = (
            fx.Int64(rows_workgroup) * fx.Int64(c_stride) * fx.Int64(element_bytes)
        )
        ...
        for m_index in range_constexpr(m_chunks):
            ...
            for n_index in range_constexpr(num_acc_n):
                ...
                accumulator = Vec(accumulators[m_index * num_acc_n + n_index]).to(
                    store_elem
                )
                for item in range_constexpr(4):
                    fragment = fx.make_rmem_tensor(1, store_elem)
                    fragment.store(Vec.from_elements([accumulator[item]], store_elem))
                    offset = (row_local + item) * c_rstride + column
