...
def compile_moe_reduction(
    *,
    ...
    module_name = f"moe_reduction_kernel_{'masked' if use_mask else 'plain'}" f"_{dtype_str}_topk{topk}_md{model_dim}"
    ...
    if True:
        ...
        def moe_reduction_kernel(
            ...
            if const_expr(use_mask):
                ...
                topk_ids_rsrc = _ptr_buffer_resource_off(topk_ids, fx.Int64(tk_slab_nbytes), tk_base_off_i64)
                ...
                expert_mask_rsrc = _ptr_buffer_resource_off(expert_mask, fx.Int64(em_nbytes), None)
                    ...
                    with _if_then(_if_full):
                        ...
                        for k in range_constexpr(topk):
                            ...
                            for si in range_constexpr(n_sub):
                                off_elems_i32 = fx.Int32(k_off_elems + fx.Index(si * copy_vec_width))
                        ...
                        for si in range_constexpr(n_sub):
                            ...
                            y_off_elems_i32 = fx.Int32(col_base + fx.Index(si * copy_vec_width))
