...
# WAVES_PER_EU OCCUPANCY-HINT variant (clone of jagged_dense_bmm_gen).
#
# Identical to jagged_dense_bmm_gen except for a waves_per_eu occupancy-hint
# knob (public kwarg waves_per_eu=0, 0 = no hint = gen behavior) threaded into
# the AMDGPU backend via FlyDSL's built-in compile-hints path.
#
...
def _build_launcher(N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, THREADS, BM_TILES, N_GROUPS, XCD_C, XCD_W, USE_MFMA_K32, WAVES_PER_EU=0):
    ...
    def _launch(
        ...
        launcher = jdbba_kernel(C, A, B, BIAS, SEQ_OFFSETS, tiled_mma, tiled_copy_g2s_A, tiled_copy_s2g_C)

        # --- waves_per_eu occupancy hint ---
        # WAVES_PER_EU=0 means no hint (compiler default == jagged_dense_bmm_gen
        # behavior). A nonzero value sets the `rocdl.waves_per_eu` attribute on the
        # just-emitted gpu.func; `convert-gpu-to-rocdl` lowers it to the LLVM
        # `"amdgpu-waves-per-eu"` function attribute, telling the AMDGPU backend the
        # target min waves per SIMD. The backend then caps VGPR allocation to hit
        # that occupancy (higher value -> fewer VGPRs allowed -> more resident
        # waves, possibly at the cost of register spills).
        #
        # NOTE: the built-in compile-hints path (--amdgpu-waves-per-eu CLI opt to
        # gpu-module-to-binary) is a silent no-op in this stack -- it does not
        # change VGPR allocation -- so we use the function-attribute form, which is
        # the production-proven idiom (cf. small_m_hgemm.py / splitk_hgemm.py).
        if fx.const_expr(WAVES_PER_EU > 0):
...
def jagged_dense_bmm(
    ...
    waves_per_eu is an occupancy hint (0 = no hint = default/gen behavior). A
    nonzero value tells the AMDGPU backend the target min waves per SIMD; the
    backend caps VGPR allocation to reach it (higher -> more resident waves,
    possibly at the cost of register spills). Baked into the compiled kernel /
    JIT cache key, so each value memoizes its own kernel."""
