    ...
    *,
    ...
    if const_expr(preshuffle_b):
        if N % 16 != 0 or tile_n % 16 != 0:
            raise ValueError(f"preshuffle_b requires N and tile_n divisible by 16, got N={N}, tile_n={tile_n}")
        if K % 16 != 0 or tile_k % 16 != 0:
            raise ValueError(f"preshuffle_b requires K and tile_k divisible by 16, got K={K}, tile_k={tile_k}")
    ...
    if split_k_chunk % tile_k != 0:
        raise ValueError(f"K/split_k (={split_k_chunk}) must be divisible by tile_k={tile_k}")
    if split_k_chunk % group_k != 0:
        raise ValueError(f"K/split_k (={split_k_chunk}) must be divisible by group_k={group_k}")
    ...
    if num_k_tiles < num_buffers:
        raise ValueError(f"{num_buffers}-stage buffering requires num_k_tiles >= {num_buffers}, got {num_k_tiles} (K/split_k={split_k_chunk}, tile_k={tile_k})")
    ...
    stage_layout = SmemAllocator(None, arch=gpu_arch, global_sym_name="bmm_fp8_oob_layout")
    ...
    use_deep_pipeline = tile_m == 128 and tile_n == 256 and tile_k == 128 and num_buffers == 4
    ...
    def kernel_bmm_w8a8_bpreshuffle_gfx1250(
        ...
        if const_expr(split_k > 1):
            ...
            c_global_base_i64 = llvm_dialect.PtrToIntOp(T.i64, fly.extract_aligned_pointer_as_index(c_global_ptr_type, arg_c.__extract_to_ir_values__()[0])).result
        ...
        if const_expr(layout == "mbn"):
            ...
            scale_a_batch_off_i32 = bz_i32 * m_idx_i32 * arith.constant(k_k_blocks, type=T.i32)
        scale_b_batch_off_i32 = bz_i32 * arith.constant(n_n_blocks * k_k_blocks, type=T.i32)
        scale_b_k_stride_i32 = arith.constant(k_k_blocks, type=T.i32)
        split_k_scale_off_i32 = arith.index_cast(T.i32, ks_split_idx) * arith.constant(split_k_chunk // group_k, type=T.i32)

        warp_n_block_base_i32 = by_i32 * arith.constant(tile_n // group_n, type=T.i32) + (wave_n_index_i32 * arith.constant(warp_tile_n, type=T.i32)) // arith.constant(group_n, type=T.i32)
        ...
        def make_desc_b(memref, k_base):
            if const_expr(preshuffle_b):
                return tdm_ops.make_tensor_descriptor_2d(
                    global_ptr=arg_b,
                    lds_memref=memref,
                    global_offset=(b_batch_off // arith.index(16) + blk_n // arith.index(16), k_base * arith.index(16)),
        ...
        def _load_a_scales(kt_idx_i32):
            k_block_base_i32 = split_k_scale_off_i32 + kt_idx_i32 * arith.constant(k_blocks_per_tile, type=T.i32)
            a_scales = []
            for wm in range_constexpr(wmma_m_rep):
                row_i32 = blk_m_i32 + warp_m_i32 + arith.constant(wm * WMMA_M, type=T.i32) + lane16_i32
                row_off = row_i32 * scale_a_row_stride_i32 + scale_a_batch_off_i32 + k_block_base_i32
                kb_vals = []
                for kb in range_constexpr(k_blocks_per_tile):
                    off = row_off + arith.constant(kb, type=T.i32)
                    i8_val = buffer_ops.buffer_load(scale_a_rsrc, off, vec_width=1, dtype=T.i8)
        ...
        def _load_b_scales(kt_idx_i32):
            ...
            for nb_local in range_constexpr(n_nblocks):
                n_block_i32 = warp_n_block_base_i32 + arith.constant(nb_local, type=T.i32)
                row_off = scale_b_batch_off_i32 + n_block_i32 * scale_b_k_stride_i32 + k_block_base_i32
                kb_vals = []
                for kb in range_constexpr(k_blocks_per_tile):
                    off = row_off + arith.constant(kb, type=T.i32)
                    i8_val = buffer_ops.buffer_load(scale_b_rsrc, off, vec_width=1, dtype=T.i8)
                    kb_vals.append(_pack_e8m0_byte(arith.extui(T.i32, i8_val)))
        ...
        def _l2_prefetch(k_base):
            ...
            if const_expr(preshuffle_b):
                tdm_ops.l2_prefetch_tile(
                    arg_b,
                    (b_batch_off // arith.index(16) + blk_n // arith.index(16), pf_k * arith.index(16)),
                    (tile_n // 16, tile_k * 16),
                    (K * 16, 1),
                    elem_bytes=1,
                    thread_id=tx,
        ...
        def _a_streaming_pipeline(accs, a_buf, a_bases, b_buf, b_bases, a_scales, b_scales, mid_compute_callback=None, emit_filler=None):
            current_accs = list(accs)

            for ks in range_constexpr(k_wmma_steps):
                kb = _kb_for(ks)
                b_raw = [_issue_frag_loads_b(b_buf, b_bases[wn], ks) for wn in range_constexpr(wmma_n_rep)]
                ...
                for wm in range_constexpr(wmma_m_rep):
                    ...
                    for wn in range_constexpr(wmma_n_rep):
                        _emit_wmma(current_accs, wm, wn, a_frag, b_frags[wn], a_scales[wm][kb], b_scales[wn][kb])
        ...
        def compute_tile(accs_in, lds_a_idx, lds_b_idx, a_scales, b_scales, mid_compute_callback=None, emit_filler=None):
        ...
        stages_a = [SmemPtr(arena_base_ptr, stage_a_data_off[i], T.f16, shape=(lds_a_data_f16,)) for i in range_constexpr(num_buffers)]
        stages_b = [SmemPtr(arena_base_ptr, stage_b_data_off[i], T.f16, shape=(lds_b_data_f16,)) for i in range_constexpr(num_buffers)]
        ...
        stages_a_lds_addr = [_dg0_lane(make_desc_a(stages_a_mem[i], split_k_base), 1) for i in range_constexpr(num_buffers)]
        stages_b_lds_addr = [_dg0_lane(make_desc_b(stages_b_mem[i], split_k_base), 1) for i in range_constexpr(num_buffers)]
        ...
        if const_expr(wave_specialized_tdm):
            ...
            active_stage_lds_addr = [_select_wave_tdm_value(stages_a_lds_addr[i], stages_b_lds_addr[i]) for i in range_constexpr(num_buffers)]
            ...
            active_pred_const = arith.select(tdm_wave_id < fx.Int32(2), fx.Int32(1), fx.Int32(0))
        ...
        def _issue_ws(load_stage, addr_box, k_prefetch=None):
            ...
            dg0 = _pack_dg0(active_pred_const, active_stage_lds_addr[load_stage], addr_box[0][0], active_addr_hi)
            ...
            if const_expr(wave_specialized_tdm):
                ...
                for loop_iter, state in range(0, loop_iters, 1, init=init_args):
                    ...
                    for buf_idx in range_constexpr(num_buffers):
                        ...
                        kt_idx_i32 = arith.index_cast(T.i32, loop_iter * arith.index(num_buffers) + arith.index(buf_idx))
                        ...
                        def _mid_ws(
                            _ls=load_stage,
                            _ab=addr_box,
                            _k_off=(split_k_base + loop_iter * arith.index(num_buffers * tile_k) + arith.index(buf_idx * tile_k)),
                        ):
                            _issue_ws(_ls, _ab, k_prefetch=_k_off)

                        rocdl.sched_barrier(0)
                        accs_in = compute_tile(accs_in, stages_a_idx[buf_idx], stages_b_idx[buf_idx], a_scales, b_scales, mid_compute_callback=_mid_ws)
                        cur_lo = addr_box[0][0]
                        hot_loop_scheduler()
                ...
                for loop_iter, state in range(0, loop_iters, 1, init=init_args):
                    ...
                    for buf_idx in range_constexpr(num_buffers):
                        ...
                        rocdl.sched_barrier(0)
                        accs_in = compute_tile(accs_in, stages_a_idx[buf_idx], stages_b_idx[buf_idx], a_scales, b_scales, mid_compute_callback=_mid_ab)
        ...
        def epilogue_prepare_addrs():
            ...
            for wm in range_constexpr(wmma_m_rep):
                for wn in range_constexpr(wmma_n_rep):
                    ...
                    col_base = c_batch_off + blk_n + warp_n_base + arith.index(wn * WMMA_N) + lane_kgrp * arith.index(8)
        ...
        def epilogue_stores_guarded(final_accs, addrs):
            ...
            for wm in range_constexpr(wmma_m_rep):
                for wn in range_constexpr(wmma_n_rep):
                    idx = wm * wmma_n_rep + wn
                    row_in_batch = blk_m + warp_m_base + arith.index(wm * WMMA_M) + lane16
                    n_slots = 1 if _half_out else 2
                    addr_arg = addrs[addr_idx] if _half_out else addrs[addr_idx : addr_idx + 2]

                    def _emit_store(_acc=final_accs[idx], _addr=addr_arg):
                        if const_expr(_half_out):
                            store_acc_vec8_to_buffer(_acc, c_rsrc, _addr, out_elem=_out_elem, offset_is_bytes=True)
                        else:
        ...
        def _atomic_fadd_global(val, byte_off):
            addr_i64 = llvm_dialect.AddOp(c_global_base_i64, arith.index_cast(T.i64, byte_off), llvm_dialect.IntegerOverflowFlags(0)).result
        ...
        for _load_stage, _compute_stage, _outstanding in tail_plan:
            ...
            if const_expr(_outstanding == -1):
                ...
                if const_expr(tdm_store_enabled):
                    rocdl.sched_barrier(0)
                    accs = compute_tile(accs, stages_a_idx[_compute_stage], stages_b_idx[_compute_stage], _tail_a_scales, _tail_b_scales)
                else:
                    ...
                    rocdl.sched_barrier(0)
                    accs = compute_tile(accs, stages_a_idx[_compute_stage], stages_b_idx[_compute_stage], _tail_a_scales, _tail_b_scales, emit_filler=_emit_epi_addrs)
            else:
                ...
                rocdl.sched_barrier(0)
                accs = compute_tile(accs, stages_a_idx[_compute_stage], stages_b_idx[_compute_stage], _tail_a_scales, _tail_b_scales, mid_compute_callback=_tail_mid_cb)
                hot_loop_scheduler()
        ...
        if const_expr(split_k > 1):
            ...
            d_lane_base = warp_lds_off + lane16 * arith.index(_lds_d_stride_elems) + lane_kgrp * arith.index(4 * elem_bytes_d)
            ...
            d_desc = tdm_ops.make_tensor_descriptor_2d(
                ...
                global_offset=(blk_m + warp_m_off_sgpr, c_batch_off + blk_n + warp_n_off_sgpr),
    ...
    def launch_bmm_w8a8_bpreshuffle_gfx1250(
        ...
        launcher = kernel_bmm_w8a8_bpreshuffle_gfx1250(arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale, i32_m)
        for op in ctx.gpu_module_body.operations:
            if hasattr(op, "attributes") and op.OPERATION_NAME == "gpu.func":
                if effective_waves_per_eu is not None:
                    _wpe = int(effective_waves_per_eu)
                    if _wpe >= 1:
                        op.attributes["rocdl.waves_per_eu"] = ir.IntegerAttr.get(ir.IntegerType.get_signless(32), _wpe)
                if use_cluster:
                    op.attributes["rocdl.cluster_dims"] = ir.StringAttr.get(f"{cluster_m},{cluster_n},1")
