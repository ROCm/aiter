...
"""TP clone of MegaMoE v2 fused dispatch+GEMM1.

Copies ``mega_moe_stage1`` (ticket, launch_ready, planner, dispatch CUs,
``build_fused_gemm1``, work pool). ``tp_broadcast_payload=True`` sends each
published row to all ranks instead of the expert-owner rank. Compute may be
wrong; this is a speed alignment experiment against EP stage1.
"""
...
from .tp_dispatch_broadcast import emit_tp_broadcast_fixed_slot_payload
...
# fmt: off
@functools.cache
def compile_tp_mega_moe_stage1(
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    swiglu_limit: float = 0.0, tp_broadcast_payload: bool = True,
):
    ...
    kernel_name = (
        f"tp_megamoe_stage1_{dispatch_path}_t{sort_block_m}x{tile_n}x{tile_k}"
        ...
        f"_bcast{int(tp_broadcast_payload)}"
        f"{swiglu_suffix}"
    )
    ...
    def kernel(
        ...
        if compact_producer:
            if const_expr(direct_fixed_slot):
                if const_expr(tp_broadcast_payload):
                    emit_tp_broadcast_fixed_slot_payload(
...
def run_tp_mega_moe_stage1(out, x, w, scale_x, scale_w, sorted_token_ids, expert_ids, num_valid_ids, out_scale,
    ...
    launch = compile_tp_mega_moe_stage1(
        ...
        payload_tile_ready=payload_tile_ready,
        swiglu_limit=swiglu_limit, tp_broadcast_payload=tp_broadcast_payload,
    )
    _run_compiled(
...
def run_tp_ep_clone_stage1(mega, x, wts, scales, topk_ids, stream=None):
    """Run the real EP fused stage1 (same kernel MegaMoEV2 uses)."""
    return mega._run_fused_stage1(x, wts, scales, topk_ids, stream=stream)


def run_tp_ep_broadcast_stage1(mega, x, wts, scales, topk_ids, stream=None):
    """Same MegaMoE stage1 fusion, but each payload row is written to all ranks."""
    import flydsl.expr as fx

    if stream is None:
        stream = fx.Stream(torch.cuda.current_stream())
    cur_tok = int(x.shape[0])
    if cur_tok > mega.mtpr:
        raise ValueError(f"run_tokens={cur_tok} > max_tok_per_rank={mega.mtpr}")
    config = mega._select_config(cur_tok).stage1
    op = mega._s1_op
    run_tp_mega_moe_stage1(
        mega._s1_out, mega._s1_rx, mega._s1_w1, mega._s1_scale_i32, mega._s1_w1_scale,
        op.tile_row_base, op.sorted_expert_ids, op.num_valid, mega._s1_osd, fx.Int32(mega._s1_nvm),
        fx.Int64(mega._s1_disp.data_ptr()), fx.Int32(cur_tok), fx.Int64(x.data_ptr()),
        fx.Int64(topk_ids.data_ptr()), fx.Int64(wts.data_ptr()), fx.Int64(scales.data_ptr()),
        fx.Int64(mega._s1_epoch_parity.data_ptr()), fx.Int64(mega._s1_epoch_expected.data_ptr()),
        stream, model_dim=mega.model_dim, inter_dim=mega.inter_dim, rank=mega.rank,
        experts_per_rank=mega.epr, fuse_npes=mega.world_size, fuse_topk=mega.topk,
        fuse_cap=mega._s1_cap, fuse_mtpr=mega.mtpr, fuse_scale_dim=mega._s1_scale_dim,
        fixed_slot_dispatch=mega._s1_fixed_slot, num_cu=mega._s1_num_cu,
        ...
        swiglu_limit=mega.swiglu_limit, tp_broadcast_payload=True,
    )
    return config.sort_block_m
# fmt: on
