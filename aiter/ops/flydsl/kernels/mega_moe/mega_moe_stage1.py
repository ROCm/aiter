...
from .mega_moe_config import (
    ...
    Stage1Config,
    mega_moe_bundle_source_fingerprint,
)
...
_ENTRY_DISPATCH_SLOTS = 256
STAGE1_EPOCH_SLOT_COUNT = len(_GRID_MULT_VALUES) * _ENTRY_DISPATCH_SLOTS


def stage1_epoch_slot(grid_mult: int, dispatch_blocks: int) -> int:
    """Return a generation-counter slot with a stable launch population."""
    if grid_mult not in _GRID_MULT_VALUES:
...
def compile_mega_moe_stage1(
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    assert grid_mult in _GRID_MULT_VALUES, "grid_mult out of range"
    dispatch_blocks = int(num_dispatch_cu)
    grid_epoch_slot = stage1_epoch_slot(grid_mult, dispatch_blocks)
    ...
    row_map_entries = (
        sort_block_m if indexed_payload or deduplicate_payload else 1
    )
    ...
    if (indexed_payload or deduplicate_payload) and (
        fz_source_rows + 1
    ) * model_dim >= _BUFFER_RESOURCE_ABI_BYTES:
        raise ValueError(
            "indexed Stage1 payload exceeds the signed 32-bit buffer-resource ABI"
        )
    if (indexed_payload or deduplicate_payload) and (
        fz_source_rows + 1
    ) * int(fuse_scale_dim) >= _BUFFER_RESOURCE_ABI_BYTES:
        raise ValueError(
            "indexed Stage1 scales exceed the signed 32-bit buffer-resource ABI"
        )
    if indexed_payload and int(fuse_scale_dim) % 16:
        raise ValueError(
            "indexed Stage1 E8M0 rows must be a multiple of 16 bytes"
        )
    ...
    def kernel(
                ...
                if const_expr(deduplicate_payload):
                    if payload_active:
                        emit_unique_payload(
                            ...
                            addr_in_tok=addr_in_tok, addr_in_idx=addr_in_idx,
                    ...
                    if payload_active:
                        emit_dispatch_payload(
                            ...
                            chunks_per_destination=chunks_per_destination,
                            payload_tile_ready=payload_tile_ready,
                            deduplicate_payload=True, enable_payload_copy=False,
                        )
                elif payload_active:
                    emit_dispatch_payload(
                        num_waves=NUM_WAVES, fz_epr=fz_epr, fz_k=fz_k,
        ...
        if consumer_active:
            ...
            gemm_x = x
            row_index_rsrc = None
            active_row_map_lds = None
            source_capacity_rows = None
            if const_expr(indexed_payload):
                row_index_rsrc = _make_buffer_from_addr(
                    _disp_ptr(DispatchSlot.SRCMAP), fx.Int32
                )
                active_row_map_lds = row_map_lds
                source_capacity_rows = fz_source_rows + 1
            ...
            expert_of_flat, _do_scheduled_tile = build_fused_gemm1(
                ...
                row_map_lds=active_row_map_lds,
    ...
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    for config in variants:
        spec = compile_mega_moe_stage1(
            ...
            tile_n=config.tile_n, tile_k=config.tile_k, num_waves=config.num_waves,
            grid_mult=config.grid_mult, pipe_weights=config.pipe_weights,
    ...
    # ``kernels`` is a list of closure objects, so FlyDSL's stable closure
    # fingerprint cannot see the constexpr profile captured by each kernel.
    # Make it explicit: otherwise AOT jobs for different ranks share one cache
    # entry and every rank may load whichever rank-specific artifact won the
    # compile race, deadlocking the communication protocol.
    bundle_profile_tag = (
        model_dim,
        inter_dim,
        rank,
