...
class MegaMoEV2:
    ...
    def __init__(self, *, rank: int, world_size: int, model_dim: int, inter_dim: int, experts: int, topk: int,
        ...
        use_kernel_bundle: bool | None = None, preload_bundles: bool = True):
    # fmt: on
        if quant != "a8w4":
        ...
        if use_kernel_bundle is None:
            use_kernel_bundle = bool(int(os.environ.get("AITER_MEGA_MOE_KERNEL_BUNDLE", "0")))
        self.use_kernel_bundle = bool(use_kernel_bundle)
        ...
        self._bundle_plan = build_mega_moe_bundle_plan(
            self.mtpr, experts_per_rank=self.epr, model_dim=self.model_dim,
            inter_dim=self.inter_dim,
        )
        if self._bundle_plan.fixed_slot_dispatch != self._s1_fixed_slot:
            raise AssertionError("MegaMoE bundle dispatch mode does not match the instance")
        p2p_quants = {entry.config.p2p_quant for entry in self._bundle_plan.entries}
        if len(p2p_quants) != 1:
            raise ValueError("one MegaMoEV2 bundle cannot mix Stage2 wire formats")
        self._stage2_p2p_quant = p2p_quants.pop()
        # The historical expand-and-copy payload deduplication protocol is
        # superseded by direct source-key indexing for large compact profiles.
        self._s1_payload_dedup = False
        # fmt: off
        self.comb_cfg = FlyDSLDispatchCombineConfig(rank=self.rank, world_size=self.world_size,
            ...
            stage2_p2p_quant=self._stage2_p2p_quant,
        ...
        if self.use_kernel_bundle and preload_bundles:
            self.preload_kernel_bundle()
    ...
    def _build_fused_stage1(self, w1, w1_scale):
        ...
        self._s1_epoch_slot_count = STAGE1_EPOCH_SLOT_COUNT
    ...
    def _allocate_dispatch_workspace(self, op, metadata_blocks):
        ...
        workspace = {
            ...
            # Stage1 shards each launch-generation counter to avoid serializing
            # every over-issued CTA on one cache line.
            "entry_count": torch.zeros(
                self._s1_epoch_slot_count * 16,
                dtype=torch.int64,
                device=self.dev,
            ),
            "epoch_gate": torch.zeros(
                self._s1_epoch_slot_count,
                dtype=torch.int32,
                device=self.dev,
            ),
            ...
            # Fixed-slot producers use one completion counter per destination
            # group and the owner clears the complete world-size range.  Keep
            # the allocation consistent with that device-side address contract;
            # compact dispatch continues to use element zero only.
        ...
        if self._s1_payload_dedup:
            source_keys = self.world_size * self.mtpr
            workspace["unique_token"] = op._sym(
                (source_keys + 1, self.model_dim), torch.uint8
            )
            workspace["unique_scale"] = op._sym(
                (source_keys + 1, self._s1_scale_dim), torch.uint8
            )
            workspace["unique_token"][source_keys].zero_()
            workspace["unique_scale"][source_keys].zero_()
            source_ready_words = (
                2 * self.world_size * 32 + 2 * self.world_size
            )
            workspace["source_ready"] = torch.zeros(
                source_ready_words, dtype=torch.int32, device=self.dev
            )
            workspace["expand_head"] = torch.zeros(
                1, dtype=torch.int32, device=self.dev
            )
            workspace["expanded_tile_ready"] = torch.zeros(
        ...
        if self._s1_payload_dedup:
            workspace["p2p_unique_token"] = op._p2p_table(workspace["unique_token"])
            workspace["p2p_unique_scale"] = op._p2p_table(workspace["unique_scale"])
        self._s1_dispatch_workspace = workspace

    def _build_v2_disp_table(self):
        op = self._s1_op
        workspace = self._s1_dispatch_workspace
        if workspace["group_done"].numel() < self.world_size:
            raise AssertionError("group_done must cover every fixed-slot destination")
        ...
        if self._s1_payload_dedup:
            table[DispatchSlot.UNIQUE_TOKEN] = workspace["unique_token"].data_ptr()
            table[DispatchSlot.P2P_UNIQUE_TOKEN] = workspace[
                "p2p_unique_token"
            ].data_ptr()
            table[DispatchSlot.UNIQUE_SCALE] = workspace["unique_scale"].data_ptr()
            table[DispatchSlot.P2P_UNIQUE_SCALE] = workspace[
                "p2p_unique_scale"
            ].data_ptr()
            table[DispatchSlot.SOURCE_READY] = workspace["source_ready"].data_ptr()
            table[DispatchSlot.EXPAND_HEAD] = workspace["expand_head"].data_ptr()
            table[DispatchSlot.EXPANDED_TILE_READY] = workspace[
                "expanded_tile_ready"
            ].data_ptr()
    ...
    def _select_bundle_entry(self, tokens: int) -> MegaMoEBundleEntry:
        entry = self._bundle_plan.entry_for_tokens(tokens)
        self._active_config = entry.config
        self._active_pair_id = entry.pair_id
        return entry

    def _run_fused_stage1(self, x, wts, scales, topk_ids, stream=None, config: Stage1Config | None = None,
        bundle_entry: MegaMoEBundleEntry | None = None):
        if stream is None:
        ...
        if self.use_kernel_bundle and bundle_entry is None:
            bundle_entry = self._select_bundle_entry(cur_tok)
        if config is None:
            config = (
                bundle_entry.config.stage1
                if bundle_entry is not None
                else self._select_config(cur_tok).stage1
            )
        op = self._s1_op
        common_args = (
        ...
        common_kwargs = {
            "model_dim": self.model_dim, "inter_dim": self.inter_dim, "rank": self.rank,
            "experts_per_rank": self.epr, "fuse_npes": self.world_size, "fuse_topk": self.topk,
            "fuse_cap": self._s1_cap, "fuse_mtpr": self.mtpr,
            "fuse_scale_dim": self._s1_scale_dim, "fixed_slot_dispatch": self._s1_fixed_slot,
            "num_cu": self._s1_num_cu, "swiglu_limit": self.swiglu_limit,
        }
        if self.use_kernel_bundle:
            if bundle_entry is None or bundle_entry.config.stage1 != config:
                raise ValueError("Stage1 bundle entry does not match the selected config")
            self._s1_mega_bundle(
                *common_args, bundle_entry.stage1_variant_id, stream,
                variants=self._bundle_plan.stage1_variants, **common_kwargs,
            )
        else:
            self._s1_mega(
                *common_args, stream, sort_block_m=config.sort_block_m, tile_n=config.tile_n,
                tile_k=config.tile_k, num_waves=config.num_waves, grid_mult=config.grid_mult,
                pipe_weights=config.pipe_weights, mfma_amajor=config.mfma_amajor,
                swizzle_a=config.swizzle_a, async_a_copy=config.async_a_copy,
    ...
    def quantize(self, x_bf16):
        if x_bf16.shape[0] == 0:
            # Preserve collective participation without launching a zero-grid
            # quantization kernel on an empty rank.
            return (
                torch.empty(
                    (0, self.model_dim),
    ...
    def _run_joint(self, x, scales, wts, topk_ids, run_tokens, stream, slice_output):
        if self.use_kernel_bundle:
            bundle_entry = self._select_bundle_entry(run_tokens)
            config = bundle_entry.config
        else:
            bundle_entry = None
        ...
        return self._run_stage2(run_tokens, stream, slice_output, config, bundle_entry)

    def _run_stage2(self, run_tokens, stream, slice_output, config: MegaMoEConfig,
        bundle_entry: MegaMoEBundleEntry | None = None):
        ret = self._run_fused_stage2(run_tokens, config, stream, bundle_entry)
    ...
    def _build_fused_stage2(self):
        ...
        self._g2_bundle_run = run_mega_moe_stage2_bundle
        self._g2_invariants_by_quant = {}
        for p2p_quant in (self._stage2_p2p_quant,):
            p2p_row_nbytes = (
    ...
    def preload_kernel_bundle(self):
        """Load the two profile modules without launching a MegaMoE kernel."""
        from .mega_moe_stage1 import compile_mega_moe_stage1_bundle
        from .mega_moe_stage2 import compile_mega_moe_stage2_bundle

        stream = fx.Stream(torch.cuda.current_stream())
        stage1 = compile_mega_moe_stage1_bundle(
            model_dim=self.model_dim, inter_dim=self.inter_dim, rank=self.rank,
            experts_per_rank=self.epr, fuse_npes=self.world_size, fuse_topk=self.topk,
            ...
            variants=self._bundle_plan.stage1_variants, swiglu_limit=self.swiglu_limit,
        )
        stage1._cf = flyc.compile(
            stage1,
            self._s1_out, self._s1_rx, self._s1_w1, self._s1_scale_i32, self._s1_w1_scale,
            self._s1_op.tile_row_base, self._s1_op.sorted_expert_ids, self._s1_op.num_valid,
            self._s1_osd, fx.Int32(self._s1_nvm), fx.Int64(self._s1_disp.data_ptr()),
            fx.Int32(1), fx.Int64(self._s1_rx.data_ptr()), fx.Int64(0), fx.Int64(0),
            fx.Int64(0), fx.Int64(self._s1_epoch_parity.data_ptr()),
            fx.Int64(self._s1_epoch_expected.data_ptr()), fx.Int32(-1), stream,
        )

        comb_sizes = tuple(
            (quant, values["comb_inp_nbytes"])
            for quant, values in self._g2_invariants_by_quant.items()
        )
        stage2 = compile_mega_moe_stage2_bundle(
            model_dim=self.model_dim, inter_dim=self.inter_dim, experts=self.epr,
            topk=self.topk, rank=self.rank, npes=self.world_size, max_tok=self.mtpr,
            recv_cap=self.max_recv, comb_inp_nbytes_by_quant=comb_sizes,
            HIDDEN_MAX=self.model_dim, INTER_MAX=self.inter_dim, cu_num=self._s1_num_cu,
            variants=self._bundle_plan.stage2_variants,
        )
        stage2._cf = flyc.compile(
            stage2,
            *([fx.Int64(0)] * 11), fx.Int32(1), fx.Int32(1), fx.Int32(self.inter_dim),
            fx.Int32(self.model_dim), fx.Int32(0), fx.Int32(0), fx.Int32(-1), stream,
        )
    ...
    def _run_fused_stage2(self, run_tokens, config: MegaMoEConfig, stream=None,
        ...
        common_args = (
            ...
            self._g2v2_inter, self._g2v2_hidden,
        )
        if self.use_kernel_bundle:
            if bundle_entry is None or bundle_entry.config != config:
                raise ValueError("Stage2 bundle entry does not match the selected config")
            if bundle_entry.pair_id != getattr(self, "_active_pair_id", None):
                raise ValueError("Stage1 and Stage2 bundle pair_id mismatch")
            comb_sizes = tuple(
                (quant, values["comb_inp_nbytes"])
                for quant, values in self._g2_invariants_by_quant.items()
            )
            self._g2_bundle_run(
                *common_args, bundle_entry.stage2_variant_id, s_fx,
                comb_inp_nbytes_by_quant=comb_sizes,
                variants=self._bundle_plan.stage2_variants,
                **{key: invariants[key] for key in (
                    "model_dim", "inter_dim", "experts", "topk", "rank", "npes",
                    "max_tok", "recv_cap", "HIDDEN_MAX", "INTER_MAX", "cu_num",
                )},
            )
        else:
            self._g2_run(
                *common_args, s_fx, BM=stage2.block_m, SBM=config.stage1.sort_block_m,
                BN=stage2.block_n, BK=stage2.block_k, use_nt=stage2.use_nt,
                g2_bhoist=stage2.b_hoist, g2_ascale_pf=stage2.ascale_prefetch,
                g2_spart=stage2.spatial_partition, persist=stage2.persist,
                persist_cu=stage2.persist_cu, persist_strided=stage2.persist_strided,
                skew_cu=stage2.skew_cu, g2_bf16_lds=stage2.bf16_lds, **invariants,
            )
