...
def _build_launcher(N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, THREADS, N_GROUPS):
    ...
    def jdbba_kernel(
        ...
        # Each block strides through the linear tile ids [0, total_tiles). Linear
        # t is group-major over CUM, so consecutive ids a block claims share a
        # group (Dense[b]/Bias[b] stay L2-resident). No loop-carried state.
        for wi in range(fx.Int32(blk_id), fx.Int32(total_tiles), fx.Int32(n_blocks_grid)):
