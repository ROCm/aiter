...
def _build_launcher(N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, THREADS, BM_TILES, N_GROUPS, XCD_C, XCD_W):
        ...
        if start_m < M_b:
            ...
            def run_pipeline_stage(read_stage, next_k, read_next=True):
                ...
                for block_k_iter in fx.range_constexpr(BLOCK_K // 32):
                    ...
                    # 16x16x32 atom: one atom spans tile_K_perm=32, so the
                    # fragment K dimension is FLAT -> index by block_k_iter (not
                    # the hierarchical (None, block_k_iter) the 2-atom K=16 path
                    # needed). Trip count BLOCK_K // 32 is unchanged.
                    fx.gemm(
                        tiled_mma,
                        mma_frag_C,
    ...
    def _launch(
        ...
        # MFMA 16x16x32 bf16 (CDNA4 v_mfma_f32_16x16x32_bf16): twice the K per
        # instruction vs 16x16x16. One 32-wide atom covers tile_K_perm=32 (vs 2
        # atoms of K=16), so the K-permute layout becomes (8,4),(1,8) and the
        # fragment K dim is FLAT. Mirrors preshuffle_gemm_v2's use_mfma_k32 path.
