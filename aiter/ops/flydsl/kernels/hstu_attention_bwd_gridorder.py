...
@functools.lru_cache(maxsize=16384)
def build_hstu_attention_bwd_dvdk_gridorder(
    num_heads: int,
    head_dim: int,
    hidden_dim: int,
    batch: int,
    ...
    *,
    ...
    waves_per_eu: int = 0,
    has_perm: bool = False,
    grid_order: str = "group",
):
    # grid_order selects the block_id -> (hz, tile) decode (CK-style axis swap):
    #   "group" : baseline -- NUM_GRID_GROUPS interleaved hz streams, tile-major within.
    #   "tile"  : CK jagged  -- tile fastest, all tiles of one (batch,head) consecutive.
    #   "batch" : CK uniform -- hz(batch) fastest, same tile across sequences co-resident.
    assert grid_order in ("group", "tile", "batch"), grid_order
    ...
    # Total grid blocks depends on the decode ordering (Python scope; the @flyc.jit
    # launch body can't carry a Python `if`-assigned int out to .launch()).
    if grid_order == "group":
        _grid_blocks = num_kv_tiles * hz_per_group * NUM_GRID_GROUPS
    else:
        _grid_blocks = num_kv_tiles * HZ_TOTAL
    ...
    def hstu_attention_bwd_dvdk_go(
        q: fx.Tensor,
        k: fx.Tensor,
        v: fx.Tensor,
        do: fx.Tensor,
        ...
        if const_expr(grid_order == "tile"):
            # CK jagged: tile fastest -> all tiles of one (batch,head) consecutive.
            kv_tile_idx = block_id % fx.Int32(num_kv_tiles)
            hz_idx = block_id // fx.Int32(num_kv_tiles)
        elif const_expr(grid_order == "batch"):
            # CK uniform: hz(batch) fastest -> same tile across sequences co-resident.
            hz_idx = block_id % fx.Int32(HZ_TOTAL)
            kv_tile_idx = block_id // fx.Int32(HZ_TOTAL)
        else:
    ...
    @flyc.jit
    def launch_hstu_attention_bwd_dvdk_go(
        q: fx.Tensor,
        k: fx.Tensor,
        v: fx.Tensor,
        do: fx.Tensor,
    ...
    launch_hstu_attention_bwd_dvdk_go.compile_hints = _hstu_compile_hints
    return launch_hstu_attention_bwd_dvdk_go
