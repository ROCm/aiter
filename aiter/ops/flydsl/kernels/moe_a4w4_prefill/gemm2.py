...
def compile_moe_gemm2(
    *,
    ...
    OUT_SC_COLS = H // 32
    # Row strides of the two output views. ``fp8_pitch`` packs the e8m0 scales
    # into the same row as the e4m3 data at byte offset H (aiter's fp8 partial
    # layout: ``fp8out_row_bytes``, scales at ``model_dim``, pitch 64-aligned),
    # so aiter's own ``moe_reduction`` consumes them and no separate fp8 reduce
    # kernel is needed. ``OUT_scale`` is then the same buffer viewed from +H.
    # Without it the two views are tightly packed and separate.
    assert fp8_pitch is None or FP8, "fp8_pitch only applies to the fp8 route-out"
    OUT_ROW_STRIDE = OUT_ROW_BYTES if fp8_pitch is None else int(fp8_pitch)
    OUT_SC_STRIDE = OUT_SC_COLS if fp8_pitch is None else int(fp8_pitch)
    ...
    def kernel_gemm2(
        ...
        if block_valid:
            ...
            out_rsrc = buffer_ops.create_buffer_resource(
                OUT, max_size=False, num_records_bytes=n_tokens * (topk * OUT_ROW_STRIDE)
            )
            osc_rsrc = buffer_ops.create_buffer_resource(
                OUT_scale,
                max_size=False,
                num_records_bytes=n_tokens * (topk * OUT_SC_STRIDE),
            )
            ...
            out_off = [
                [
                    _orow(_pid[h * N_ST + k]) * fx.Int32(OUT_ROW_STRIDE)
                    for k in range(N_ST)
                ]
                for h in range(2)
            ]
            ...
            # 0..31)
            sc_off = (
                [_orow(_pid[2 * N_ST + h]) * fx.Int32(OUT_SC_STRIDE) for h in range(2)]
                if FP8
                else None
            )
