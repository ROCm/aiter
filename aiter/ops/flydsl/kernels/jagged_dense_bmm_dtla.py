...
def _build_launcher(N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, THREADS, BM_TILES, N_GROUPS, XCD_C, XCD_W):
    ...
    def jdbba_kernel(
        ...
        if start_m < M_b:
            ...
            # DTLA requires a PLAIN (non-swizzled) LDS layout: buffer_load_lds writes
            # the wave's loaded data contiguously to LDS and does NOT honor a per-lane
            # XOR swizzle (verified: with the SwizzleType(3,3,3) composed layout cos
            # drops to ~0.13; plain layout gives cos=1.0). The original swizzle only
            # mattered for the ds_write reg->LDS path we removed. Trade-off: the s2r
            # read of sA now uses a plain layout (possible LDS read bank conflicts).
