...
#
# ASYNC + BLOCK_M-SWEEP VARIANT (experiment): clone of the DTLA variant. The A
# operand is staged global->LDS with a DirectToLDS async load (frees the ~32 VGPR
# the sync staging used) and BLOCK_M is overridable via the public entry so we can
# test whether the freed VGPR lets a larger BLOCK_M (higher arithmetic intensity)
# fit at >1 wave/SIMD. The MFMA atom is also overridable for the last knob.
#
# DTLA VARIANT: the A operand is staged global->LDS with a DirectToLDS async load
# (rocdl buffer_load_dwordx4 ... lds, via the BufferCopyLDS copy atom) instead of
# the global->VGPR->LDS round-trip. This removes copy_frag_A (~32 VGPR/iter) and
# the ds_write_b128 reg->LDS store for A. The async load completes on vmcnt, so a
# `s_waitcnt vmcnt(0)` is issued before each `s_barrier` that precedes the s2r
# read of sA (a plain s_barrier does NOT drain in-flight buffer_load_lds writes).
# B stays register-resident (BufferCopy128b) and is unchanged.
#
...
def _build_launcher(N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, THREADS, BM_TILES, N_GROUPS, XCD_C, XCD_W):
    ...
    def jdbba_kernel(
        ...
        if start_m < M_b:
            ...
            # DirectToLDS atom: global (buffer_desc) -> LDS (shared), single
            # buffer_load_dwordx4 ... lds, no VGPR round-trip. Carries `soffset`
            # state for the per-k-tile advance, like BufferCopy128b.
            buffer_copy_lds_128b = fx.make_copy_atom(fx.rocdl.BufferCopyLDS128b(), fx.BFloat16)
            ...
            # NOTE: the DirectToLDS DMA (BufferCopyLDS128b) does not honor a
            # Swizzle(3,3,3) composed layout consistently with the s2r read at
            # BLOCK_K=64 (write/read addresses disagree -> cos~0.13). Use a plain
            # ordered LDS layout for the async path; correctness verified cos=1.0
            # across all D=256/512 shapes. (A bank-conflict swizzle would have to
            # be applied manually to the DMA destination address, as splitk_hgemm
            # does with swizzle_xor16 -- not done here.)
            ...
            def run_pipeline_stage(read_stage, next_k, read_next=True):
                ...
                if fx.const_expr(read_next):
                    ...
                    # DirectToLDS: async global -> LDS straight into the swizzled
                    # write-stage of sA (no copy_frag_A / ds_write round-trip).
                    fx.copy(
                        buffer_copy_lds_128b,
                        thr_gA_k[None, None, None, 0],
                        thr_sA[None, None, None, write_stage],
                ...
                # Drain in-flight DirectToLDS A writes (and the B vmem loads)
                # before the barrier so the next stage's s2r read of sA is safe;
                # buffer_load_lds completes on vmcnt, which s_barrier does not wait.
                fx.rocdl.s_waitcnt(0)
                fx.gpu.barrier()

            # Prologue: async-load K-tile 0 straight into sA stage 0 (DirectToLDS).
            fx.copy(buffer_copy_lds_128b, thr_gA_k[None, None, None, 0], thr_sA[None, None, None, 0])
...
def jagged_dense_bmm(
    ...
    block_m overrides BLOCK_M for the experiment (default module BLOCK_M=128)."""
    N = B.shape[0] // n_groups
    K = B.shape[1]
    bM = BLOCK_M if block_m is None else block_m
    ...
    bm = (max_seq_len + bM - 1) // bM
    launch = _build_launcher(N, K, bM, BLOCK_N, block_k, STAGES_A, THREADS, bm, n_groups, xcd_c, xcd_w)
