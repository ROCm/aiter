...
kStages = 2
kBS_stride_k0_dw = 64
# Bump whenever the externally visible preshuffled scale layout changes.  It
# is included in producer/consumer kernel symbols so FlyDSL's persistent disk
# cache cannot reuse an artifact compiled against an older layout contract.
MXFP4_SCALE_LAYOUT_TAG = "sc2"
...
def kas_c_k1_for(k):
    # Host preshuffle pads the number of 1x32 scale groups to a multiple
    # of eight.  Round up here as well: for K=384 there are 12 logical
    # groups but 16 stored groups (two 256-K scale chunks).  Truncating to
    # one chunk makes ``(row, chunk=1)`` alias ``(row+1, chunk=0)``.
    return ((k // 32) + 7) // 8


def kbs_c_k1_for(k):
    # Match shuffle_scale_a16w4/e8m0_shuffle's eight-group K padding.
    return ((k // 32) + 7) // 8
