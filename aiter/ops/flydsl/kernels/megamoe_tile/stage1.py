...
def compile_megamoe_tile_ep16_stage1(
    ...
    *,
    ...
    if HIDDEN > 8192:
        raise ValueError("Stage-1 hidden must be <= 8192 for the 256-thread quantizer")
    ...
    if (BM, BN) != (32, 256):
        raise ValueError("Stage-1 requires BM/BN=32/256")
    ...
    def kernel(
                ...
                for batch_item in range_constexpr(cco_chunks_per_flush):
                    ...
                    for item in range_constexpr(records_per_qp):
                        ...
                        token_active = (
                            ...
                            if const_expr(MAX_TOKENS % records_per_chunk == 0)
                            else token < ntokens
                        )
                    ...
                    if const_expr(enable_cco):
                        ...
                        payload_bytes = (
                            fx.Int32(qp_bytes)
                            if const_expr(MAX_TOKENS % records_per_chunk == 0)
                            else payload_tokens * fx.Int32(record_bytes)
                        )
        ...
        def _dispatch_route(record, dest, source_index, topk_slot):
            ...
            if row_in_tile == fx.Int32(0):
                ...
                if tx == fx.Int32(0):
                    comm_ops.spin_until_ge_i64_system(map_ready, generation)
        ...
        if is_finisher:
            ...
            if const_expr(expert_major_output):
                ...
                if tx < fx.Int32(LOCAL_EXPERTS):
                    ...
                    # across unrolled loops.  LOCAL_EXPERTS loads of one cache
                    # line, reduced pairwise into two scalars.
                    for e in range_constexpr(LOCAL_EXPERTS):
                        c = buffer_ops.buffer_load(
                            counts_res, fx.Int32(e), vec_width=1, dtype=T.i32
                        )
                        n = (c + fx.Int32(BM - 1)) // fx.Int32(BM)
                        my_base = my_base + (fx.Int32(e) < tx).select(
                            n, fx.Int32(0)
                        )
                        all_tiles = all_tiles + n
                    if tx == fx.Int32(0):
                        # Sum_e ceil(count[e]/BM) must equal tile_alloc: every
                        # physical tile was allocated when one expert bucket
                        # opened, so the permutation is a bijection.
                        if all_tiles != tiles:
                            comm_ops.atomic_add_system(error_addr, fx.Int32(1))
                    my_count = buffer_ops.buffer_load(
                        counts_res, tx, vec_width=1, dtype=T.i32
                    )
                    my_tiles = (my_count + fx.Int32(BM - 1)) // fx.Int32(BM)
                    tile_map = buffer_ops.create_buffer_resource_from_addr(
                        local_addr("expert_tile_map")
                    )
                    for j in range(fx.Int32(0), my_tiles, fx.Int32(1)):
                        map_index = tx * fx.Int32(max_tiles_per_expert) + j
                        comm_ops.spin_until_ge_i64_system(
                            local_addr("expert_tile_map_ready")
                            + fx.Int64(map_index) * fx.Int64(8),
                            generation,
                ...
                for row in range(
                    tx, tiles * fx.Int32(BM), fx.Int32(THREADS)
                ):
        ...
        def _run_gemm1_job(job):
            _gemm1_body(
                ...
                BM=BM,
                expert_major=expert_major_output,
                BN=BN,
                BK=BK,
                inline_quant=False,
                K=HIDDEN,
        ...
        if const_expr(not diagnostic_comm_only):
            ...
            if is_compute:
                ...
                if const_expr(tile_pipeline):
                    ...
                    if tx == fx.Int32(0):
                        comm_ops.spin_until_ge_i64_system(local_addr("h1_queue_eos"), generation)
                    gpu.barrier()
                    comm_ops.fence_system_acquire()
                    tiles = buffer_ops.buffer_load(
                        buffer_ops.create_buffer_resource_from_addr(
