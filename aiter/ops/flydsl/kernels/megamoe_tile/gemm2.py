...
from .gemm_common import _lds_swizzle_mask as lds_swizzle_mask
...
def gemm2_compute_v2(
    ...
    *,
    ...
    # Standalone GEMM2 callers flatten by the runtime number of N blocks.
    # Fused Stage2 may instead pack m_block in the upper BM-aligned bits and
    # n_block in the low bits. BM is a power of two and Stage2 guarantees that
    # its number of N blocks does not exceed BM, so this carrier decodes with a
    # shift/mask and avoids the runtime divisor.
    if const_expr(packed_tile_coord):
        if not (BM > 0 and BM & (BM - 1) == 0):
            raise ValueError("packed GEMM2 tile coordinates require power-of-two BM")
        coord_shift = (BM - 1).bit_length()
        m_block_idx = fx.Int32(bx_i32) >> fx.Int32(coord_shift)
        n_block_idx = fx.Int32(bx_i32) & fx.Int32(BM - 1)
    else:
