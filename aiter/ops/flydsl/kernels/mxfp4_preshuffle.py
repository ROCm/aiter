...
# Vendored from FlyDSL kernels/gemm/mxfp4_preshuffle.py at 16a56b643df5. Two
# edits, both mechanical, so re-syncing stays a copy plus this header:
#   1. `kernels.common.mma.mfma_preshuffle_pipeline` -> the relative
#      `.mfma_preshuffle_pipeline`, matching how the other vendored kernels here
#      resolve it (see preshuffle_gemm.py);
#   2. that import moved into its own block, to satisfy this repo's ruff.
#
# The file carries more than the current caller uses: fp8 B, blockscale, split-K
# and launch_splitk_reduce are all reachable from launch_gemm but unexercised by
# flydsl_batched_gemm_mxfp4 today. Kept rather than pruned so this stays a
# straight copy of upstream -- pruning would fork it.
