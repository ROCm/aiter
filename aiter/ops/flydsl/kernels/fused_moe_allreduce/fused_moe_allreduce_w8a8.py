...
KBLK = HIDDEN // 128
UG_WAVE_CH = KBLK // NWAVE
UG_EXPERT_BYTES = 2 * INTER * HIDDEN
UG_TILE_BYTES = 16 * HIDDEN
UG_SC_EXPERT = (2 * INTER // 128) * KBLK
DN_EXPERT_BYTES = HIDDEN * INTER
DN_BLK_BYTES = 24 * INTER
DN_SC_EXPERT = KBLK * 2
ROUTER_BLK_BYTES = 8 * HIDDEN * 2
ROUTER_WAVE_KC = HIDDEN // 64 // NWAVE
RT_CH = (KBLK + NWAVE - 2) // (NWAVE - 1)
...
def fused_moe_allreduce_consts(samples: int, proto: int) -> dict:
    ...
    c["L_MID8"] = take(SLOTS * INTER)
    c["L_MSC"] = take(SLOTS * 2 * 4)
...
def compile_fused_moe_allreduce(
    ...
    L_TIDX, L_TPRB, L_MID8, L_MSC = c["L_TIDX"], c["L_TPRB"], c["L_MID8"], c["L_MSC"]
    L_SIG = c["L_SIG"]
    ...
    def half_wave_max_pos(x, lane):
        v = f32(x).bitcast(fx.Int32)
        for sh in (8, 4, 2, 1):
            v = fx.max(v, dpp(v, ROW_ROR[sh]))
        r0, r1 = readlane_i(v, 0), readlane_i(v, 16)
        r2, r3 = readlane_i(v, 32), readlane_i(v, 48)
        lo, hi = fx.max(r0, r1), fx.max(r2, r3)
        return (lane < i32(32)).select(lo, hi).bitcast(fx.Float32)
    ...
    def fp8x4(v, inv):
        ...
        w = rocdl.cvt_scalef32_pk_fp8_f32(
            T.vec(2, T.i16), _u(zero), _u(v[0] * inv), _u(v[1] * inv), one, False
        )
        w = rocdl.cvt_scalef32_pk_fp8_f32(
            T.vec(2, T.i16), w, _u(v[2] * inv), _u(v[3] * inv), one, True
        )
        return i32(fx.Vector(w).bitcast(fx.Int32)[0])

    def quant_scale(amax):
        sc = amax.maximumf(f32(AMAX_EPS)) * f32(INV_FP8_MAX)
        return sc, f32(1.0) / sc

    def cat8(a, b):
        return fx.Vector(a).shuffle(fx.Vector(b), list(range(8)))
    ...
    def act_quant(L, wave, lane, nvs):
        ...
        for j in range_constexpr(3):
            ...
            am = half_wave_max_pos(am, lane)
            sc, inv = quant_scale(am)
            lds_st(L, L_ACT + e0 + i32(j * 256), fp8x4(nv, inv))
            _store_if_half(
                L, lane, L_ASC + (wave * i32(6) + i32(2 * j) + lane // i32(32)) * i32(4), sc
            )
    ...
    @traced
    def _store_if_half(L, lane, off, v):
        if (lane % i32(32)) == i32(0):
            lds_st(L, off, v)
    ...
    def tile_issue(a, e, t, wave, lane, nbytes=None):
        base = i64(a["ug_w"]) + i64(e) * i64(UG_EXPERT_BYTES) + i64(t) * i64(UG_TILE_BYTES)
        rs = rsrc(base, nbytes)
        ws = []
        for ch in range_constexpr(UG_WAVE_CH):
            for h in range_constexpr(2):
                off = (wave * i32(UG_WAVE_CH) + i32(ch)) * i32(2048) + i32(h * 1024) + lane * i32(16)
                ws.append(ld_v4i(rs, off, AUX_NT))
        rb = (lane < i32(32)).select(i32(0), i32(2)) + t // i32(16)
        soff = ((e * i32(UG_SC_EXPERT)) + rb * i32(KBLK) + wave * i32(UG_WAVE_CH)) * i32(4)
        srs = rsrc(a["ug_scales"])
        s4 = fx.Vector(bld(srs, soff, T.vec(4, T.f32)))
        s2 = fx.Vector(bld(srs, soff + i32(16), T.vec(2, T.f32)))
        scl = [f32(s4[k]) for k in range(4)] + [f32(s2[k]) for k in range(2)]
        return ws, scl

    def tile_mfma(L, wave, lane, ws, scl, red):
        acc = fx.Vector.filled(4, 0.0, fx.Float32)
        for ch in range_constexpr(UG_WAVE_CH):
            kb = wave * i32(UG_WAVE_CH) + i32(ch)
            b8 = lds_ld(L, L_ACT + kb * i32(128) + (lane >> i32(4)) * i32(32), T.vec(8, T.i32), 16)
            p = mfma_fp8(cat8(ws[2 * ch], ws[2 * ch + 1]), b8)
            f = scl[ch] * lds_f(L, L_ASC + kb * i32(4))
            acc = acc + p * fx.Vector.filled(4, f, fx.Float32)
        _store_tile_red(L, lane, red + (wave * i32(16) + (lane >> i32(4)) * i32(4)) * i32(4), acc)

    def routed_issue(a, e, t, wave, lane):
        base = i64(a["ug_w"]) + i64(e) * i64(UG_EXPERT_BYTES) + i64(t) * i64(UG_TILE_BYTES)
        rs = rsrc(base, UG_TILE_BYTES)
        cb = (wave - i32(1)) * i32(RT_CH)
        ws = []
        for ch in range_constexpr(RT_CH):
            for h in range_constexpr(2):
                off = (cb + i32(ch)) * i32(2048) + i32(h * 1024) + lane * i32(16)
                ws.append(ld_v4i(rs, off, AUX_NT))
        rb = (lane < i32(32)).select(i32(0), i32(2)) + t // i32(16)
        soff = ((e * i32(UG_SC_EXPERT)) + rb * i32(KBLK) + cb) * i32(4)
        srs = rsrc(a["ug_scales"])
        sa = fx.Vector(bld(srs, soff, T.vec(4, T.f32)))
        sb = fx.Vector(bld(srs, soff + i32(16), T.vec(4, T.f32)))
        scl = [f32(sa[k]) for k in range(4)] + [f32(sb[k]) for k in range(3)]
        return ws, scl

    def routed_mfma(L, wave, lane, ws, scl, red):
        cb = (wave - i32(1)) * i32(RT_CH)
        acc = fx.Vector.filled(4, 0.0, fx.Float32)
        for ch in range_constexpr(RT_CH):
            kbr = cb + i32(ch)
            kb = fx.min(kbr, i32(KBLK - 1))
            b8 = lds_ld(L, L_ACT + kb * i32(128) + (lane >> i32(4)) * i32(32), T.vec(8, T.i32), 16)
            p = mfma_fp8(cat8(ws[2 * ch], ws[2 * ch + 1]), b8)
            f = (kbr < i32(KBLK)).select(scl[ch] * lds_f(L, L_ASC + kb * i32(4)), f32(0.0))
            acc = acc + p * fx.Vector.filled(4, f, fx.Float32)
        _store_tile_red(L, lane, red + (wave * i32(16) + (lane >> i32(4)) * i32(4)) * i32(4), acc)
    ...
    @traced
    def tile_finish(L, a, tid, red, midt, s, slot, t, tag, active, w_first=0):
        if tid < i32(8):
            g = f32(0.0)
            u = f32(0.0)
            for w in range_constexpr(w_first, NWAVE):
    ...
    def routed_tiles(L, a, wave, lane, grp, tile, tl, tid, bid):
        if wave != i32(0):
            ...
            for m in range_constexpr(S):
                ...
                ws, scl = cur
                routed_mfma(L, wave, lane, ws, scl, L_REDT + (1 + m) * NWAVE * 64)
            mark(tl, tid, bid, 14, 64)

    def routed_wait_issue(L, a, wave, lane, grp, tile, m):
        r = (grp ^ i32(1)) + i32(G * m)
        e = _spin_rank(L, r)
        t = routed_issue(a, e + i32(1), tile, wave, lane)
    ...
    def down_issue(L, a, wave, lane, lb):
        ...
        for k in range_constexpr(IPW):
            ...
            rs = rsrc(
                i64(a["down_w"])
                + i64(e) * i64(DN_EXPERT_BYTES)
                + i64(lb * i32(S) + q) * i64(DN_BLK_BYTES)
            )
            ws = [ld_v4i(rs, down_lane_off(part, h, lane), AUX_NT) for h in range(2)]
            items.append((valid, part, j, q, ws))
        return items

    def down_lane_off(part, h, lane):
        kg = lane >> i32(4)
        cc = (lane >> i32(3)) & i32(1)
        m8 = lane & i32(7)
        main_off = part * i32(2048) + i32(h * 1024) + lane * i32(16)
        tail_off = i32(4096 + h * 512) + cc * i32(1024) + (kg * i32(8) + m8) * i32(16)
        return (part == i32(2)).select(tail_off, main_off)

    def down_scale_issue(L, a, tid, lb):
        t = fx.min(tid, i32(ROWS - 1))
        rowblk = (lb * i32(ROWS) + t) // i32(128)
        srs = rsrc(a["down_scales"])
        dsc = []
        for j in range_constexpr(SLOTS):
            e = slot_expert(L, i32(j))
            v = fx.Vector(bld(srs, (e * i32(DN_SC_EXPERT) + rowblk * i32(2)) * i32(4), T.vec(2, T.f32)))
            dsc.append((f32(v[0]), f32(v[1])))
        return dsc
    ...
    def mid_slot(L, a, lane, s, j, tag):
        ...
        am = half_wave_max_pos(am, lane)
        sc, inv = quant_scale(am)
        lds_st(L, L_MID8 + j * i32(INTER) + lane * i32(4), fp8x4(xv, inv))
        if (lane % i32(32)) == i32(0):
            lds_st(L, L_MSC + (j * i32(2) + lane // i32(32)) * i32(4), sc)
    ...
    def down_mfma(L, lane, items):
        kg = lane >> i32(4)
        cc = (lane >> i32(3)) & i32(1)
        for valid, part, j, q, ws in items:
            tail = part == i32(2)
            bchunk = tail.select(cc, part)
            b8 = lds_ld(L, L_MID8 + j * i32(INTER) + bchunk * i32(128) + kg * i32(32), T.vec(8, T.i32), 16)
            p = mfma_fp8(cat8(ws[0], ws[1]), b8)
            writer = tail.select(
        ...
        if tid < i32(ROWS):
            ...
            for j in range_constexpr(SLOTS):
                ...
                for cc in range_constexpr(2):
                    fac = (dsc[j][cc] * wj) * lds_f(L, L_MSC + i32((j * 2 + cc) * 4))
                    pv = lds_f(L, L_P + (((i32(j * S) + q) * i32(2) + i32(cc)) * i32(24) + rr) * i32(4))
                    acc = acc + pv * fac
    ...
    def routed_publish(L, a, tid, wave, lane, s, lb, grp, tile, tag):
        for m in range(S):
            ...
            tile_finish(
                ...
                r + i32(1), tile, tag, fx.Boolean(True), 1,
            )
    ...
    def fused_moe_allreduce_kernel(
        ...
        shared_on = grp == i32(1)
        ws0, sc0 = tile_issue(
            a, i32(0), tile, wave, lane, shared_on.select(i32(UG_TILE_BYTES), i32(0))
        )
        rocdl.sched_barrier(0)
        ...
        tile_mfma(L, wave, lane, ws0, sc0, L_REDT)
        gpu.barrier()
        tile_finish(L, a, tid, L_REDT, L_MIDT, s, i32(0), tile, tag, shared_on)
        mark(tl, tid, bid, 3)
        ...
        dsc = down_scale_issue(L, a, tid, lb)
        mark(tl, tid, bid, 5)
