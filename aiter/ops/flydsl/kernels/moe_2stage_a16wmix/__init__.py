...
def _default_tile_n(N, *, w_dtype="mxfp4"):
    ...
    Minimal documented fallback for shapes/tokens with no ``a16wmix_tuned.csv`` row.
    """
    if w_dtype == "int4":
        if N % 128 == 0:
            return 128
...
# =============================================================================
# OURS-tuned per-(shape, token) tile config, data-driven from
# ``a16wmix_tuned.csv`` (generated by scripts/gen_a16wmix_tuned_csv.py from the
# validated-optimal heuristics -- a data-vs-code split, NOT a re-tune).
#
# The CSV is the authoritative source for the tuned shapes x the token sweep.
# For an unlisted (shape, token) the resolver returns None and the launcher
# uses the slim documented fallback (_default_tiles_fallback), which preserves
# the original heuristic behavior for arbitrary shapes.
# =============================================================================

_OURS_TUNED_CSV = os.path.join(os.path.dirname(__file__), "a16wmix_tuned.csv")


@functools.cache
def _load_ours_tuned_csv(csv_path):
    """Parse a16wmix_tuned.csv into
...
def _resolve_ours_tuned(
    *, w_dtype, model_dim, inter_dim, experts, topk, tokens, stage, csv_path=None
):
    """Return the ours-tuned tile-config dict for one (shape, token, stage), or None
    when no exact CSV row exists. Only the tuned shapes x the discrete token sweep are
    populated; unlisted (shape, token) fall through to _default_tiles_fallback."""
    path = csv_path or _OURS_TUNED_CSV
...
def _default_tiles_fallback(*, D_HIDDEN, D_INTER, tokens, w_dtype, tile_m, stage):
    """Slim documented tile-config fallback for (shape, token) with no CSV row.

    This is the original default-path heuristic, consolidated into one place (it is no
    longer scattered inline in the launchers). The CSV rows are generated FROM this
    exact logic, so for a tuned cell the CSV and this fallback agree byte-for-byte.

    Returns the same dict shape as the CSV resolver. Assumes the caller left all tile
    args at their defaults (explicit caller overrides are applied afterwards, upstream).
    """
    BM = int(tile_m)
    _m = int(tokens)
    if stage == 1:
...
def resolve_a16wmix_gemm1_config(
    *, w_dtype, model_dim, inter_dim, experts, topk, tokens, tile_m, csv_path=None
):
    ...
    if w_dtype == "int4" and inter_dim % 64 == 0:
        cfg["tile_n"] = (
            64 if int(tile_m) == 64 else _default_tile_n(inter_dim, w_dtype=w_dtype)
        )
...
def flydsl_a16w4_gemm1(
    *,
    ...
    _m = int(n_tokens)
    TILE_N = tile_n
    # Ours-tuned per-token tile config (data-driven from a16wmix_tuned.csv, else the
    # slim documented fallback). Fills only the tile args the caller left at default;
