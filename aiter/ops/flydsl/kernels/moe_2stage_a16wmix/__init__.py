...
# OURS-tuned per-(shape, token) tile config, data-driven from a16wmix_tuned.csv
# (generated by scripts/gen_a16wmix_tuned_csv.py from the validated heuristics -- a
# data-vs-code split, not a re-tune). Unlisted (shape, token) falls through to the
# _default_tiles_fallback heuristic.
...
# aiter tuned-CSV config loader for bf16-A MoE: decode each
# flydsl_moe{1,2}_abf16_w{fp4,int4,bf16} kernelName into a tile-config dict (tile
# GEOMETRY only; aiter's gemm bodies differ).
#
