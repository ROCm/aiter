...
def load_kvec(rsrc, base_elem, stream_b=False):
    ...
    return tuple(
        Vec(buffer_ops.buffer_load(rsrc, base_elem + fx.Int32(i * 4), vec_width=4, dtype=T.bf16, cache_modifier=cm))
...
def gemm_decode_bf16_kernel(
    ...
    lt1 = _arith_ops.CmpIOp(_arith_ops.CmpIPredicate.slt, _to_ir(r1), _to_ir(M_runtime)).result
    lt2 = _arith_ops.CmpIOp(_arith_ops.CmpIPredicate.slt, _to_ir(r2), _to_ir(M_runtime)).result
    lt3 = _arith_ops.CmpIOp(_arith_ops.CmpIPredicate.slt, _to_ir(r3), _to_ir(M_runtime)).result
    ...
    acc00 = _const_f32(0.0); acc01 = _const_f32(0.0)
    acc10 = _const_f32(0.0); acc11 = _const_f32(0.0)
    acc20 = _const_f32(0.0); acc21 = _const_f32(0.0)
    acc30 = _const_f32(0.0); acc31 = _const_f32(0.0)
    ...
    for i in range_constexpr(num_iter):
        ...
        bv1 = load_kvec(rsrc_b, (n_base + fx.Int32(1))  * fx.Int32(K) + k_elem, stream_b=True)

        for v in range_constexpr(nv):
            p0a = pack_bf16x2(av0[v][0], av0[v][1]); p0b = pack_bf16x2(av0[v][2], av0[v][3])
            p1a = pack_bf16x2(av1[v][0], av1[v][1]); p1b = pack_bf16x2(av1[v][2], av1[v][3])
            p2a = pack_bf16x2(av2[v][0], av2[v][1]); p2b = pack_bf16x2(av2[v][2], av2[v][3])
            p3a = pack_bf16x2(av3[v][0], av3[v][1]); p3b = pack_bf16x2(av3[v][2], av3[v][3])
            q0a = pack_bf16x2(bv0[v][0], bv0[v][1]); q0b = pack_bf16x2(bv0[v][2], bv0[v][3])
            q1a = pack_bf16x2(bv1[v][0], bv1[v][1]); q1b = pack_bf16x2(bv1[v][2], bv1[v][3])
            acc00 = dot2_f32_bf16(acc00, p0a, q0a); acc01 = dot2_f32_bf16(acc01, p0a, q1a)
            acc10 = dot2_f32_bf16(acc10, p1a, q0a); acc11 = dot2_f32_bf16(acc11, p1a, q1a)
            acc20 = dot2_f32_bf16(acc20, p2a, q0a); acc21 = dot2_f32_bf16(acc21, p2a, q1a)
            acc30 = dot2_f32_bf16(acc30, p3a, q0a); acc31 = dot2_f32_bf16(acc31, p3a, q1a)
            acc00 = dot2_f32_bf16(acc00, p0b, q0b); acc01 = dot2_f32_bf16(acc01, p0b, q1b)
            acc10 = dot2_f32_bf16(acc10, p1b, q0b); acc11 = dot2_f32_bf16(acc11, p1b, q1b)
            acc20 = dot2_f32_bf16(acc20, p2b, q0b); acc21 = dot2_f32_bf16(acc21, p2b, q1b)
            acc30 = dot2_f32_bf16(acc30, p3b, q0b); acc31 = dot2_f32_bf16(acc31, p3b, q1b)
...
class GemmDecodeRunner:
    ...
    def warmup(self, B: torch.Tensor, M: int = 1):
        ...
        for _ in range(self.warmup_iters):
            gemm_decode_bf16(A_dummy, B, C_dummy, M, N, K, stream=fx.Stream(self._stream))
    ...
    def capture_graph(self, B: torch.Tensor, M: int = 1):
        ...
        for _ in range(3):
            gemm_decode_bf16(self._graph_A, B, self._graph_C, M, N, K,
                             stream=fx.Stream(self._stream))
        torch.cuda.synchronize()
        # Capture
...
if __name__ == "__main__":
    import statistics, subprocess, re

    def get_sclk():
        r = subprocess.run(["rocm-smi","--showclkfrq"], capture_output=True, text=True)
        in_sclk = False
        for line in r.stdout.split('\n'):
            if 'GPU[0]' in line and 'sclk' in line.lower(): in_sclk = True
            if in_sclk and 'GPU[1]' in line: break
            if in_sclk:
    ...
    def evict(): _evict.fill_(1.0); _ = _evict.sum().item(); torch.cuda.synchronize()
    ...
    for _ in range(30):
        ...
        A_prod.normal_()
        st.record(); runner.run(A_prod, B_prod, M=2); en.record()
        torch.cuda.synchronize()
        t = st.elapsed_time(en) * 1000
        if t > 5: times_graph.append(t)
    ...
    for _ in range(30):
        ...
        st.record(); gemm_decode_bf16(A_prod, B_prod, C2, 2, N_prod, K_prod, stream=fx.Stream(s)); en.record()
        torch.cuda.synchronize()
        t = st.elapsed_time(en) * 1000
        if t > 5: times_direct.append(t)

    bw = lambda t: (2*K_prod*2 + N_prod*K_prod*2)/(t*1e-6)/1e9
    print(f"\n  {'Method':25s}  {'med µs':>8}  {'GB/s':>8}  {'% peak':>7}")
    print(f"  {'─'*55}")
    if times_graph:
        tg = statistics.median(times_graph)
        print(f"  {'CUDA graph replay':25s}  {tg:>8.1f}  {bw(tg):>8.0f}  {bw(tg)/80:>7.1f}%")
    if times_direct:
