...
# WARP-LAYOUT-TUNABLE CLONE of jagged_dense_bmm_gen.py: the MMA warp layout
# ((M_warps, N_warps, K_warps)) and tile_n (BLOCK_N) are now factory args so we
# can sweep the warp distribution along N. THREADS derives from total warps.
#
...
# Default MMA warp layout (matches the production gen kernel: 4 warps all on N).
M_WARPS = 1
N_WARPS = 4
K_WARPS = 1


def _warp_layout(m_warps, n_warps, k_warps):
    """Build the make_tiled_mma warp layout ((Mw, Nw, Kw), (strides)). Degenerate
    (==1) dims get stride 0; the remaining dims get column-major linear strides in
    M, N, K order, so total_warps = Mw*Nw*Kw warps are distributed bijectively
    across the (M, N, K) MMA output partition. Matches (1,4,1),(0,1,0) when only
    N varies. THREADS = Mw*Nw*Kw * 64."""
    sm = sn = sk = 0
    run = 1
    if m_warps > 1:
        sm = run
    ...
    if k_warps > 1:
        sk = run
        run *= k_warps
    return fx.make_layout((m_warps, n_warps, k_warps), (sm, sn, sk))
...
def _build_launcher(
    N, K, BLOCK_M, BLOCK_N, BLOCK_K, STAGES_A, M_WARPS, N_WARPS, K_WARPS,
    BM_TILES, N_GROUPS, XCD_C, XCD_W, USE_MFMA_K32,
):
    """Build (and memoize) a launch wrapper specialized to this (N, K, tiling,
    warp-layout). N (output) and K (reduction) are baked in as closure constants.
    The MMA warp layout (M_WARPS, N_WARPS, K_WARPS) is now a tunable arg; THREADS
    derives as M_WARPS*N_WARPS*K_WARPS*64. BM_TILES (= ceil(max_seq_len/BLOCK_M))
    and N_GROUPS are also baked in so the chiplet (XCD) block-ID remap dimensions
    are compile-time; this makes each distinct (shape, warp-layout, XCD_C, XCD_W,
    USE_MFMA_K32) memoize its own compiled kernel."""
    THREADS = M_WARPS * N_WARPS * K_WARPS * WARP_SIZE
    ...
    def _launch(
        ...
        warp_layout = _warp_layout(M_WARPS, N_WARPS, K_WARPS)
        if fx.const_expr(USE_MFMA_K32):
...
def jagged_dense_bmm(
    ...
    m_warps / n_warps / k_warps set the MMA warp layout (THREADS = product * 64);
    block_n overrides BLOCK_N (tile_n). Sweeping n_warps (and block_n alongside)
    distributes more warps across the N output dimension."""
    N = B.shape[0] // n_groups
    K = B.shape[1]
    block_n = BLOCK_N if block_n is None else block_n
    ...
    launch = _build_launcher(
        N, K, BLOCK_M, block_n, block_k, STAGES_A, m_warps, n_warps, k_warps,
        bm, n_groups, xcd_c, xcd_w, use_mfma_k32,
    )
