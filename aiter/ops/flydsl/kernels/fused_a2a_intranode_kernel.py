...
_JIT_SCHEMA_VERSION = "v1-single-tensor-push"


def make_fused_a2a_kernel(*, rank, npes, peer_nbytes, block_num, warp_num_per_block):
    if peer_nbytes % 16 != 0:
        raise ValueError(f"peer_nbytes must be 16-byte aligned, got {peer_nbytes}")

    chunks_per_peer = peer_nbytes // 16
    total_chunks = npes * chunks_per_peer
    ...
    def fused_a2a_push(
        ...
        for chunk_idx in range(global_warp_id, total_chunks, global_warp_num):
            dest_pe = chunk_idx // chunks_per_peer
            dest_chunk = chunk_idx % chunks_per_peer
            peer_base = fx.memref_load(p2p_bases, dest_pe)
            dst_addr = peer_base + fx.Int64(rank * peer_nbytes + dest_chunk * 16)
            rsrc_dst = create_buffer_resource_from_addr(dst_addr)
            i32_offset = chunk_idx * 4
            value = buffer_load(rsrc_input, i32_offset, vec_width=4, dtype=T.i32)
            buffer_store(value, rsrc_dst, 0)
...
def make_fused_a2a_jit(*, rank, npes, peer_nbytes, block_num, warp_num_per_block):
