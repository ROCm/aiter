...
def _build_sparse_mla_prefill_kernel(*, num_heads: int, has_attn_sink: bool):
    ...
    def kernel(
        ...
        if const_expr(has_sink):
            ...
            token_scale_active = fx.rocdl.exp2(f32, arith.subf(m_final, merged_max))
            numerator_scale = arith.select(has_tokens, token_scale_active, c_zero)
        ...
        for elem in range_constexpr(_VALUES_PER_LANE):
            ...
            out_values.append(arith.select(has_denominator, quotient, c_zero))

        def _store_head():
            out_bf16_values = [arith.trunc_f(T.bf16, value) for value in out_values]
...
def _build_sparse_mla_prefill_mfma64_kernel(
    *, has_attn_sink: bool, use_lds_dma: bool, split_kv: bool = False
    ...
    assert (
        not split
        or split_stats_elements + split_output_elements <= kv_group_elements // 2
    )
    ...
    @fx.struct
    class SharedStorage:
        kv: fx.Array[fx.BFloat16, worker_groups * kv_group_elements, 16]
        probabilities: fx.Array[
            fx.BFloat16, worker_groups * probability_group_elements, 16
        ]
        maxima: fx.Array[fx.Float32, worker_groups * stats_group_elements, 16]
        sums: fx.Array[fx.Float32, worker_groups * stats_group_elements, 16]
    ...
    def kernel(
        ...
        sink_t = GTensor(attn_sink, dtype=T.f32, shape=(-1,))
        lds = fx.SharedAllocator().allocate(SharedStorage).peek()
        kv_lds = lds.kv.ptr + worker_group * fx.Int32(kv_group_elements)
        kv_lds_base_i32 = fx.Int32(fx.ptrtoint(kv_lds))
        kv_lds_ptr_type = kv_lds.type
        probability_lds = lds.probabilities.ptr + worker_group * fx.Int32(
            probability_group_elements
        )
        maxima_lds = lds.maxima.ptr + worker_group * fx.Int32(stats_group_elements)
        sums_lds = lds.sums.ptr + worker_group * fx.Int32(stats_group_elements)
        ...
        if const_expr(lds_dma):
            ...
            kv_dma_atom = fx.make_copy_atom(fx.rocdl.BufferCopyLDS128b(), 128)
        ...
        for k_step in range_constexpr(k_steps):
            q_packs.append(
                load_bf16x8(
                    q_row_i32,
                    fx.Int32(k_step * (mfma_k // 2)) + lane_group * fx.Int32(4),
                )
            )
        ...
        for ragged_pos, state in range(
            ...
            ragged_base = fx.Int32(arith.index_cast(i32, _to_raw(ragged_pos)))
            ...
            for load_step in range_constexpr(kv_load_steps):
                tile_token = wave * fx.Int32(kv_load_steps) + fx.Int32(load_step)
                token_pos = tile_base + tile_token
                token_in_range = token_pos < row_end
                safe_token_pos = token_in_range.select(token_pos, row_start)
                staging_slots.append(fx.Int32(indices_t[safe_token_pos]))
            ...
            for load_step in range_constexpr(kv_load_steps):
                ...
                slot_valid = token_in_range & (slot >= fx.Int32(0)) & (slot < num_kv)
                if const_expr(lds_dma):
                    ...
                    source_byte = safe_slot * fx.Int32(
                        _HEAD_DIM * 2
                    ) + chunk * fx.Int32(16)
                    ...
                    safe_slot = slot_valid.select(slot, fx.Int32(0))
                    kv_row_i32 = make_row(kv, fx.Int64(fx.Uint32(safe_slot)), T.i32)
                    ...
                    fx.ptr_store(
                        staged,
                        kv_lds + kv_lds_row_offset(tile_token) + chunk * fx.Int32(8),
                    )
            ...
            for element in range_constexpr(4):
                ...
                token_valid = token_in_range & (slot >= fx.Int32(0)) & (slot < num_kv)
                raw_score = arith.MulFOp(
                    _to_raw(fx.Float32(scores[element])),
                    _to_raw(scale_log2),
                    fastmath=fm_fast,
            ...
            tile_max = reduce_max_token_groups(
                fx.Float32(
                    fx.ptr_load(maxima_lds + lane_group * fx.Int32(heads) + lane_mod)
                )
            )
            new_max = running_max.maximumf(tile_max)
            correction = fx.Float32(fx.rocdl.exp2(f32, _to_raw(running_max - new_max)))
            ...
            fx.ptr_store(
                ...
                + xor_swizzle_bf16x8(
                    lane_mod,
                    wave * fx.Int32(mfma_tile) + lane_group * fx.Int32(4),
                ),
            )
            ...
            tile_sum = reduce_sum_token_groups(
                fx.Float32(
                    fx.ptr_load(sums_lds + lane_group * fx.Int32(heads) + lane_mod)
                )
            )
            new_sum = running_sum * correction + tile_sum
            correction4 = fx.Vector.from_elements([correction] * 4, dtype=fx.Float32)
            output_acc = [
                fx.Vector(accumulator) * correction4 for accumulator in output_acc
            ]
            ...
            for token_half in range_constexpr(2):
                token_base = fx.Int32(token_half * 32) + lane_group * fx.Int32(8)
                ...
                for output_tile in range_constexpr(output_tiles):
                    ...
                    value_pack = value_lo.shuffle(value_hi, list(range(8)))
                    output_acc[output_tile] = mfma(
                        value_pack, probability_pack, output_acc[output_tile]
                    )
        ...
        if const_expr(split):
            ...
            stats_offset = (wave * fx.Int32(heads) + lane_mod) * fx.Int32(2)
            if worker_group == fx.Int32(1):
                if lane_group == fx.Int32(0):
                    fx.ptr_store(running_max, reduction_lds + stats_offset)
                    fx.ptr_store(
                        running_sum,
                        reduction_lds + stats_offset + fx.Int32(1),
                    )
                for output_tile in range_constexpr(output_tiles):
                    split_output_offset = fx.Int32(split_stats_elements) + (
                        (wave * fx.Int32(output_tiles) + fx.Int32(output_tile))
                        * fx.Int32(_WAVE_SIZE)
                        + lane
                    ) * fx.Int32(4)
                    fx.ptr_store(
            ...
            other_max = fx.Float32(fx.ptr_load(reduction_lds + stats_offset))
            other_sum = fx.Float32(
                fx.ptr_load(reduction_lds + stats_offset + fx.Int32(1))
            )
            ...
            split_scale4 = fx.Vector.from_elements([split_scale] * 4, dtype=fx.Float32)
            other_scale4 = fx.Vector.from_elements([other_scale] * 4, dtype=fx.Float32)
        ...
        output_row = fx.Int64(fx.Uint32(query)) * fx.Int64(heads) + fx.Int64(
            fx.Uint32(lane_mod)
        )
