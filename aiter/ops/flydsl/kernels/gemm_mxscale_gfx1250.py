    ...
    *,
    ...
    def kernel_mxscale_gemm(
        ...
        def _emit_tile(
            ...
            def _k_valid_inner(k_base, div, mul):
                """Per-tile valid extent along K (inner descriptor dim), in this
                tensor's inner units, for TDM-load OOB zero-fill.  Returns an i32
                ``max(0, k_valid_eff - k_base) / div * mul`` when K-pad is active,
                else None (descriptor keeps its full-tile extent, no OOB).  k_base
                and k_valid_eff are both multiples of SCALE_BLOCK so the division
                is exact."""
            ...
            def make_desc_a(memref, k_base):
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    valid_inner=_k_valid_inner(k_base, PACK_FACTOR_A, 1),
                )
            ...
            def make_desc_b(memref, k_base, n_offset=0):
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    atomic_barrier_enable=atomic_barrier_enable,
                    valid_inner=_k_valid_inner(k_base, PACK_FACTOR_B, 16),
                )
            ...
            def make_desc_as(memref, k_base):
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    workgroup_mask=a_mcast_mask,
                    atomic_barrier_enable=atomic_barrier_enable,
                    valid_inner=_k_valid_inner(k_base, SCALE_BLOCK, wmma_m_rep),
                )
            ...
            def make_desc_bs(memref, k_base, n_offset=0):
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    workgroup_mask=b_mcast_mask,
                    atomic_barrier_enable=atomic_barrier_enable,
                    valid_inner=_k_valid_inner(k_base, SCALE_BLOCK, b_scale_load_rep),
                )
