            ...
            def _emit_stage1_quant_blocks(entries):
                ...
                for m_off, mx_block, chunks in entries:
                    ...
                    if const_expr(_q_is_fp8):
                        # Pack each chunk's f32 cols into fp8 e4m3 bytes (1
                        # byte/col). Same f32*recip approach as the fp4 path so
                        # the conversion stays in f32 without a bf16 round-trip.
                        # gguu (dual-B) feeds 8-wide chunks -> v2i32 (8 bytes);
                        # gugu (interleaved) de-interleaves to 4-wide chunks ->
                        # i32 (4 bytes).
