...
def _emit_pk8_lane_amax(dwords4, bf16x8, c):
...
def _emit_quant_block_loop(c: SimpleNamespace) -> None:
    ...
    for it in range_constexpr(c.block_iters):
        ...
        if const_expr(prequantized):
            ...
            block_amax = _emit_pk8_lane_amax(dwords4, bf16x8, c)
...
def _emit_quant_result_stores(c, dst_payload, mx_block, payload_val, e8m0_scale):
    ...
    Payload stores are unpredicated. Scale stores are a single lead-lane
    ``scf.if`` wrapping all destinations, so a 6-dest kernel emits one IfOp
    per MX block instead of six.
    """
...
def _emit_row_major_scale_vec4(c, mx_block_lo, e8m0_lo, e8m0_hi):
    ...
    quad = vector.from_elements(
        T.vec(4, i32), [_raw(lo), _raw(lo_peer), _raw(hi), _raw(hi_peer)]
    )
        ...
        if token0 < fx.Uint32(token_num):
            ...
            rows = [
                fx.Uint32(
                    buffer_ops.buffer_load(
                        rows_rsrc,
                        route0 + arith.constant(k, type=i32),
                        vec_width=1,
                        is_scalar=True,
                    )
                )
