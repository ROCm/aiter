            ...
            if const_expr(descs is not None):
                ...
                for _ in range_constexpr(TDM_LOADS_PER_STAGE):
                    if const_expr(_tdm_di < len(descs)):
                        _atom, _gt, _lds = descs[_tdm_di]
                        sched_barrier(0)
                        fx.copy(_atom, _gt, _lds)
                        sched_barrier(0)
                        _tdm_di += 1
...
class TDM:
    ...
    def build_descs(dg1, addr_i64, stride_adv_i64, lds_base, su_p_size, n_su,
        ...
        oob_dim1_list: optional list of n_su i32 OOB row counts.  When
        provided and dg1 is a single v8i32, field [2] of dg1 is patched
        per-SU with ``(oob_dim1 & 0xFFFF) << 16`` so that each SU gets
        its own OOB clamped descriptor.
        """
        if oob_dim1_list is not None and not isinstance(dg1, list):
            # Build per-SU dg1 vectors with OOB dim1 patched into field [2].
            _dg1_list = []
            for su in range(n_su):
                oob_field = (oob_dim1_list[su] & 0xFFFF) << 16
                patched = llvm_dialect.insertelement(
                    dg1, oob_field, arith.constant(2, type=T.i32)
                )
                _dg1_list.append(patched)
        else:
    ...
    @staticmethod
    def build_copy_descs(
        ptr_tensor, offset, stride_seq, stride_32, wave_id, lds_base_i32,
        *, dim0_valid, row_bytes, su_p_size, oob_dim1_list=None,
    ):
        """Build per-SU (atom, gt, lds) tuples for deferred fx.copy dispatch.

        Same address/layout logic as load_tile, but returns descriptors
        instead of issuing copies immediately.  The interleaving engine
        calls ``fx.copy(atom, gt, lds)`` one SU at a time.

        Returns: list of CNT_SU tuples ``(atom, gt_view, lds_view)``.
        """
        _DIM1_ROWS = 8
        row_elems = row_bytes // 2

        pad_amount_elems = row_elems - dim0_valid
        use_hw_padding = pad_amount_elems > 0 and (dim0_valid & (dim0_valid - 1)) == 0
        pad_kw = (
            {"pad_interval": dim0_valid, "pad_amount": pad_amount_elems}
            if use_hw_padding
            else {}
        )

        addr = compute_global_addr(ptr_tensor, offset, wave_id, 8 * stride_seq)
        lds_warp_base = lds_base_i32 + wave_id * (8 * row_bytes)

        p_bf16_global = fx.PointerType.get(
            fx.BFloat16.ir_type, address_space=fx.AddressSpace.Global, alignment=2
        )
        p_bf16_shared = fx.PointerType.get(
            fx.BFloat16.ir_type, address_space=fx.AddressSpace.Shared, alignment=2
        )

        stride_seq_elems = stride_seq >> 1
        stride_adv = fx.Int64(stride_32)

        descs = []
        cur_addr = addr
        for su in range(CNT_SU):
            g_ptr = fx.inttoptr(p_bf16_global, cur_addr)
            gt = fx.Tensor(fx.make_view(
                g_ptr,
                fx.make_layout((_DIM1_ROWS, dim0_valid), (dim0_valid, 1)),
            ))

            oob_dim1 = oob_dim1_list[su] if oob_dim1_list is not None else None

            atom = fx.rocdl.make_tdm_atom(
                gt, [oob_dim1, None], strides=[stride_seq_elems, None],
                num_warps=1, **pad_kw,
            )

            lds_addr = lds_warp_base + su * su_p_size
            lds_ptr = fx.inttoptr(p_bf16_shared, fx.Int32(lds_addr))
            lds_view = fx.Tensor(fx.make_view(
                lds_ptr,
                fx.make_layout((_DIM1_ROWS, dim0_valid), (row_elems, 1)),
            ))

            descs.append((atom, gt, lds_view))

            if su < CNT_SU - 1:
                cur_addr = cur_addr + stride_adv
    ...
    def load_tile(
        ptr_tensor, offset, stride_seq, stride_32, wave_id, lds_base_i32,
        *, dim0_valid, row_bytes, su_p_size,
        kv_type, oob_dim1_list=None,
    ):
        """Load one tile_n=128 of K or V into LDS via TDM.

        4 TDM loads (SU 0-3), per-warp: 4 warps x 8 rows = 32 rows per SU.
        Uses make_tdm_atom + fx.copy instead of raw TDM descriptors.

        dim0_valid: global read width in bf16 elements.
        row_bytes: LDS row stride in bytes (includes any padding).
        oob_dim1_list: optional list of CNT_SU per-warp OOB row counts (i32).
        """
        _DIM1_ROWS = 8

        # LDS row stride in bf16 elements
        row_elems = row_bytes // 2

        # TDM HW padding: only used when dim0_valid is a power of two
        # (HW requirement). For K (dim0_valid=192, not pow2), the LDS
        # row gap is handled purely by the view stride difference.
        pad_amount_elems = row_elems - dim0_valid
        use_hw_padding = pad_amount_elems > 0 and (dim0_valid & (dim0_valid - 1)) == 0
        pad_kw = (
            {"pad_interval": dim0_valid, "pad_amount": pad_amount_elems}
        ...
        # Global base address (i64 bytes), already per-warp
        addr = compute_global_addr(ptr_tensor, offset, wave_id, 8 * stride_seq)

        # Per-warp LDS base (i32 bytes)
        lds_warp_base = lds_base_i32 + wave_id * (8 * row_bytes)
        ...
        # Stride in bf16 elements for make_tdm_atom
        stride_seq_elems = stride_seq >> 1

        stride_adv = fx.Int64(stride_32)

        # Issue CNT_SU TDM loads
        cur_addr = addr
        for su in range(CNT_SU):
            g_ptr = fx.inttoptr(p_bf16_global, cur_addr)
            gt = fx.Tensor(fx.make_view(
            ...
            lds_addr = lds_warp_base + su * su_p_size
            lds_ptr = fx.inttoptr(p_bf16_shared, fx.Int32(lds_addr))
            lds_su = fx.Tensor(fx.make_view(
                lds_ptr,
                fx.make_layout((_DIM1_ROWS, dim0_valid), (row_elems, 1)),
            ))

            fx.copy(atom, gt, lds_su)
            rocdl.s_barrier_signal(-1)
            rocdl.s_barrier_wait(-1)
    ...
    @staticmethod
    def load_k_only(ptr_K, k_offset, stride_k_seq, stride_k_32, wave_id, lds_base_i32, oob_dim1_list=None):
        TDM.load_tile(
            ptr_K, k_offset, stride_k_seq, stride_k_32, wave_id, lds_base_i32,
            dim0_valid=QK_HDIM, row_bytes=K_ROW_BYTES,
            su_p_size=LDS_K_SU_P_SIZE, kv_type=KV_K,
            oob_dim1_list=oob_dim1_list,
        )

    @staticmethod
    def load_v_only(ptr_V, v_offset, stride_v_seq, stride_v_32, wave_id, lds_base_i32, oob_dim1_list=None):
        TDM.load_tile(
            ptr_V, v_offset, stride_v_seq, stride_v_32, wave_id, lds_base_i32,
            dim0_valid=128, row_bytes=V_ROW_BYTES,
            su_p_size=LDS_V_SU_P_SIZE, kv_type=KV_V,
            oob_dim1_list=oob_dim1_list,
        )
