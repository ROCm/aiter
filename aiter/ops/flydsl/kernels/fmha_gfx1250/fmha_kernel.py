...
def compile_fmha_fwd(*, is_causal: bool = False, return_lse: bool = False):
    ...
    def fmha_fwd_kernel(
        ...
        def apply_causal_mask(su_sp_tiles, n_start_fx):
            lane_lo = lane_id & 15
        ...
        def apply_kv_oob_mask(su_sp_tiles, kv_remain_raw):
            lane_hi = lane_id >> 4
        ...
        # ================================================================
        # OOB protection: pre-compute per-SU OOB row counts for TDM loads
        # ================================================================
        m_start_raw = m_start
        ...
        if wg_valid_compute:
            ...
            if const_expr(IS_CAUSAL):
                ...
                apply_causal_mask(all_su_sp_tiles, pro_causal_n)

            # -- 2d'': KV OOB mask on prologue tile --
            apply_kv_oob_mask(all_su_sp_tiles, actual_kv_len)
            ...
            TDM.load_k_only(
                ...
                wave_id,
                k_b_base_i32,
                oob_dim1_list=k_tile1_oob_dim1,
            )
            rocdl.sched_barrier(0)
            ...
            def fmha_pipeline(
                ...
                causal_n_start=None,
                # list[CNT_SU] i32 OOB row counts for GEMM1 V TDM
                endtile_v_oob_dim1=None,
                # raw i32: valid K cols (None = all 128)
                kv_oob_cols=None,
                # list[CNT_SU] i32 OOB row counts for K prefetch
                loop_k_oob_dim1=None,
                # list[CNT_SU] i32 OOB row counts for V load
                loop_v_oob_dim1=None,
            ):
                ...
                if has_tdm_k_g1:
                    k_descs = TDM.build_copy_descs(
                        ptr_K, tdm_k_offset, stride_k_seq, stride_k_32,
                        wave_id, tdm_k_target,
                        dim0_valid=QK_HDIM, row_bytes=K_ROW_BYTES,
                        su_p_size=LDS_K_SU_P_SIZE,
                        oob_dim1_list=loop_k_oob_dim1,
                    )
                    tdm_state["k_descs"] = k_descs
                    tdm_state["k_desc_idx"] = 0

                if has_tdm_v_g1:
                    v_descs = TDM.build_copy_descs(
                        ptr_V, tdm_v_offset, stride_v_seq, stride_v_32,
                        wave_id, tdm_v_target,
                        dim0_valid=128, row_bytes=V_ROW_BYTES,
                        su_p_size=LDS_V_SU_P_SIZE,
                        oob_dim1_list=endtile_v_oob_dim1,
                    )
                ...
                # o_rescale_by_stage[s] = list of closures for GEMM1 stage s
                o_rescale_by_stage = []
                # s = tile index = stage index
                for s in fx.range_constexpr(N_PV_WMMA_N):
                    stage_closures = []
                    for dm in fx.range_constexpr(NUM_MSB):  # 4 closures per stage
                ...
                if const_expr(causal_n_start is not None):
                    apply_causal_mask(su_sp_tiles_list, causal_n_start)
                if const_expr(kv_oob_cols is not None):
                    apply_kv_oob_mask(su_sp_tiles_list, kv_oob_cols)
            ...
            def tile_iteration(tile_idx, iter_args, causal_n_start=None):
                ...
                def mk_zero_v4i32():
                    return fx.constant_vector(0, T.vec(4, T.i32))

                def mk_zero_v8i32():
                    return fx.constant_vector(0, T.vec(8, T.i32))
                ...
                (
                    ...
                    loop_k_oob_dim1=loop_k_oob_dim1,
                    loop_v_oob_dim1=loop_v_oob_dim1,
                )
            ...
            for tile_idx, iter_args, loop1_results in scf.for_(
                ...
                yield tile_iteration(tile_idx, iter_args)
            ...
            for tile_idx, iter_args, loop_results in scf.for_(
                ...
                yield tile_iteration(tile_idx, iter_args, causal_n_start=causal_n)
            ...
            def _ep_finish(
                ...
                dw = db32 + dw_wv
                # D store lane layout: (16,2):(1,16) → same as K/Q
                ...
                #   elem_off = by*stride_o_head + o_tok*stride_o_seq
                ...
                # Global view: (WV_SUBQD, 128) bf16 with runtime row stride
                _D_DIM0 = 128
                _D_DIM1 = WV_SUBQD
                p_bf16_g = fx.PointerType.get(
                    fx.BFloat16.ir_type,
                    address_space=fx.AddressSpace.Global,
                    alignment=2,
                )
                g_ptr_o = fx.inttoptr(p_bf16_g, oadr64)
                gt_o = fx.Tensor(fx.make_view(
                    g_ptr_o,
                    fx.make_layout((_D_DIM1, _D_DIM0), (_D_DIM0, 1)),
                ))
                atom_o = fx.rocdl.make_tdm_atom(
                    gt_o,
                    [o_oob_dim1, None],
                ...
                # LDS view: (WV_SUBQD, 128) bf16, row stride = TDM_D_TILE_DIM0/2
                olds2 = extract_lds_base_i32(lds_alloc_v_a.get_base()) + wsgpr * LDS_D_WV_SIZE
                p_bf16_s = fx.PointerType.get(
                    fx.BFloat16.ir_type,
                    address_space=fx.AddressSpace.Shared,
                    alignment=2,
                )
                lds_ptr_o = fx.inttoptr(p_bf16_s, fx.Int32(olds2))
                _lds_row_elems = TDM_D_TILE_DIM0 // 2
                lv_o = fx.Tensor(fx.make_view(
                    lds_ptr_o,
                    fx.make_layout((_D_DIM1, _D_DIM0), (_lds_row_elems, 1)),
                ))

                # Store LDS -> Global
                fx.copy(atom_o, lv_o, gt_o)
            ...
            if is_multi:  # N>=2: endtile core_loop then ep_finish
                ...
                _, _, et_o, _, et_psp_lo, et_psp_hi, et_ped = fmha_pipeline(
                    ty,
                    False,
                    q_frags,
                    ep_kv_tiles,
                    ...
                    endtile_v_oob_dim1=et_v_oob_dim1,
