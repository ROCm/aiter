...
"""Small-M atomic Stage2 compression and TP collectives."""
...
def compile_stage2_tp_reduce_scatter(config: Config):
    ...
    @flyc.kernel(
        name=f"comm_fused_moe_atomic_m{config.m}_rs_g{config.reduce_scatter_grid}",
...
def compile_stage2_tp_all_gather(config: Config):
    ...
    @flyc.kernel(
        name=f"comm_fused_moe_atomic_m{config.m}_ag_g{config.all_gather_grid}",
        known_block_size=[BLOCK, 1, 1],
    )
    def kernel(
        ...
        source = (rank + source_slot + fx.Int32(1)) % fx.Int32(TP)
