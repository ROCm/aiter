...
"""Multi-launch GEMM2/TP pipeline overlapped across output-width windows."""
...
from .gemm2_tp_producer import Gemm2TPComposition
...
class Gemm2TPWindowPipelineConfig:
    shape: Gemm2TPShape
    m: int
    tile_m: int
    tile_n: int
...
def _compile_compute(config: Gemm2TPWindowPipelineConfig, window: int, compose=None):
    shape = config.shape
...
@functools.cache
def compile_stage2_compute(config: Gemm2TPWindowPipelineConfig, window: int):
    """Compile one compact Stage2 window."""
    return _compile_compute(config, window)


def _emit_local(config: Gemm2TPWindowPipelineConfig, route, partial, shared, worker):
    shape = config.shape
    m = config.m
...
def _compose_cycle(
    config: Gemm2TPWindowPipelineConfig,
    window_index: int,
    *,
        ...
        *,
        ...
        def launch(
            ...
            with ir.InsertionPoint(ctx.gpu_module_body):
                allocator.finalize()
            compute_workers = arith.index_cast(
                T.index, size_expert_ids
            ) * arith.constant(tiles_per_window, index=True)
...
def compile_stage2_drain(
        ...
        if const_expr(has_local):
            ...
            local_if = scf.IfOp(
                arith.cmpi(
                    CmpIPredicate.uge,
                    worker,
                    fx.Int32(reduce_scatter_grid if has_reduce_scatter else 0),
                )
            )
            with ir.InsertionPoint(local_if.then_block):
    ...
    def launch(
        ...
        service_grid = (
            ...
            else all_gather_grid if has_all_gather else 0
        )
        kernel(
            route,
            partial,
