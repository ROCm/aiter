...
"""Uncached HIP IPC helpers for the FlyDSL quick-allreduce inbox."""
...
# Granularity every IPC-shareable allocation is rounded up to; see ``alloc``.
_PAGE_BYTES = 4096
...
class UncachedIpcHeap:
    ...
    def device_of(cls, ptr: int) -> int | None:
        ...
        err = hip.hipPointerGetAttributes(ctypes.byref(attr), ctypes.c_void_p(int(ptr)))
        if int(err) != 0:
            return None
    ...
    def alloc(
        ...
        *expected_device*, when given, is the device index the allocation has
        ...
        # hipIpcGetMemHandle on uncached allocations under 2 MiB requires a
        # whole number of 4 KiB pages. The kernel's own layout still uses the
        # unrounded size, so this only ever grows the mapping.
        if size < 1:
            raise ValueError(f"allocation size must be positive, got {size}")
        size = -(-int(size) // _PAGE_BYTES) * _PAGE_BYTES
        buf = ctypes.c_void_p()
