...
_FLYDSL_ENV_LOCK = RLock()
_RUN_ONLY_CACHE_MISS_PREFIX = "FLYDSL_RUNTIME_RUN_ONLY=1 but no usable AOT cache"
...
def _flydsl_environment(
    ...
    *,
    ...
    # os.environ is process-global.  Serialize the entire compile/load scope so
    # two contexts targeting different GPUs cannot observe each other's target
    # or RUN_ONLY state.
    with _FLYDSL_ENV_LOCK:
        ...
        *,
        ...
        try:
            ...
            if not isinstance(request, CompileRequest) or not isinstance(
                ...
                raise
            is_strict_miss = load and str(error).startswith(_RUN_ONLY_CACHE_MISS_PREFIX)
