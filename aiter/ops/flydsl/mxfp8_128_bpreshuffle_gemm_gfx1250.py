...
from .kernels.splitk_atomic_epilogue_gfx1250 import FLAG_STRIDE_I32
...
NAME_SUFFIX_RE = (
    BASE_NAME_SUFFIX_RE + r"(?P<splitk_mode>_nofsk|_fsk|_atm)?"
    r"(?P<a_preshuffle>_apre)?"
    r"(?:_ps(?P<persistent_n_tiles>\d+))?$"
)
# No suffix means the clustered epilogue, as before atomic came back; atomic is
# opt-in via _atm so mainline rows keep resolving exactly as they did.
_SPLITK_MODE_SUFFIX = {
    None: "fsk",
    "_nofsk": "none",
    "_fsk": "fsk",
