...
class Stage2CompileDecision:
    ...
    gemm_out_dtype: str
    reduction_input_dtype: str
    reduction_out_dtype: str
...
def resolve_stage1_compile_decision(
    ...
    if not isinstance(enable_bias, bool):
        raise TypeError(f"enable_bias must be a bool, got {enable_bias!r}")
...
def resolve_stage2_persist_m(
    *,
    ...
    if a_dtype == "fp8":
        # FP8 disables the persistent scheduler, but still uses persist_m as a
        # grid-y divisor so launches remain within HIP's 65535 limit.
        return max(1, (m_blocks + 65535 - 1) // 65535)
    return persist_m
...
def resolve_stage2_compile_decision(
    ...
    *,
    ...
    if not isinstance(return_per_slot, bool):
        raise TypeError(f"return_per_slot must be a bool, got {return_per_slot!r}")
    ...
    if reduction_out_dtype is None:
        raise ValueError(
            f"unsupported Stage2 output dtype: {builder_kwargs['out_dtype']!r}"
        )
    if not isinstance(fp8_intermediate, bool):
        raise TypeError(f"fp8_intermediate must be a bool, got {fp8_intermediate!r}")
    if fp8_intermediate and expected_accumulate:
        raise ValueError("fp8_intermediate requires a reduction target")
    gemm_out_dtype = "fp8" if fp8_intermediate else out_dtype
    reduction_input_dtype = "fp8" if fp8_intermediate else reduction_out_dtype
    if dtype_str is not None and dtype_str != reduction_input_dtype:
        raise ValueError(
            "reduction input dtype must match Stage2 GEMM output dtype: "
            f"expected {reduction_input_dtype!r}, got {dtype_str!r}"
        )

    if not isinstance(use_mask, bool):
        raise TypeError(f"use_mask must be a bool, got {use_mask!r}")
    ...
    if isinstance(num_experts, bool) or not isinstance(num_experts, int):
        raise TypeError(f"num_experts must be an integer, got {num_experts!r}")
    ...
    return Stage2CompileDecision(
        ...
        reduction_input_dtype=reduction_input_dtype,
        reduction_out_dtype=reduction_out_dtype,
