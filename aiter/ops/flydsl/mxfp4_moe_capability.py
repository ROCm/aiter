...
"""Dispatch capability checks for the A4W4 low-M FMoE fast paths.

A fast path may only run an invocation it fully supports; anything else has to
resolve legacy metadata instead. Keeping the checks in one place keeps that
decision identical wherever it is made.
"""
