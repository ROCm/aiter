...
def shuffle_weight_a16w4(
    ...
    if klane_inner:
        ...
        if gate_up:
            ...
            N0 = N_full // NLane
            K0 = K_pk // (KLane * KPack)
            x_ = src.view(experts_cnt, N0, NLane, K0, KLane, KPack)
            x_ = x_.permute(0, 1, 3, 4, 2, 5).contiguous()
            x_ = x_.view(experts_cnt, N0, K0, KLane, NLane, L_sub, L_sub)
            x_ = x_.permute(0, 1, 2, 5, 4, 3, 6).contiguous()
...
def shuffle_scale_a16w4(
    ...
    if klane_inner:
        ...
        if gate_up:
            ...
            N1 = n_ // N_Lane // N_Pack
            shfl_scale = src.view(experts_cnt, N1, N_Pack, N_Lane, K1, K_Pack, K_Lane)
            shfl_scale = shfl_scale.permute(0, 1, 4, 3, 6, 5, 2).contiguous()
        return shfl_scale.view(*src.shape).contiguous()
    return shuffle_scale(
