...
def get_mla_metadata_info_v1(
    ...
    # When nhead<16, mla_decode_fwd pads Q heads up to 16 via repeat_interleave
    # before launching the kernel. Size the metadata as if nhead==16 by routing
    # every buffer/tile-count computation through effective_num_head, so the
    # padded case matches a native nhead==16 launch exactly. For native nhead
    # (16/32/48/64/128) effective_num_head == num_head_qo, leaving behavior
    # identical to main.
    ...
    max_splits = get_mla_decode_fwd_max_splits(
        effective_num_head, max_seqlen_qo, q_dtype, kv_dtype
    )

    effective_seqlen_qo = 1 if is_sparse else max_seqlen_qo
    packed_qo_len = effective_seqlen_qo * effective_num_head
    ...
    if (
        ...
        and effective_num_head <= 64
        and (packed_qo_len < 128 or effective_num_head == 48)
    ):
        if effective_num_head * 2 > 64:
        ...
        or (
            get_gfx() == "gfx942"
            and effective_num_head in (16, 32, 64)
            and effective_num_head * effective_seqlen_qo == 128
            and kv_dtype == dtypes.fp8
            and q_dtype == dtypes.fp8
            and is_experimental_enabled()
        )
        or (
            ...
            and q_dtype == dtypes.fp8
            and (
                (effective_num_head == 32 and effective_seqlen_qo == 4)
                or (effective_num_head == 64)
                or (effective_num_head == 128)
            )
        )
        or (
        ...
        get_gfx() == "gfx950"
        and (packed_qo_len >= 128 or effective_num_head > 64)
        and kv_dtype == dtypes.bf16
        and q_dtype == dtypes.bf16
        and effective_num_head != 48
    ):
        if effective_num_head * 2 > 128:
            max_qo_tiles_per_batch = effective_seqlen_qo
        else:
            max_qo_tiles_per_batch = int(math.ceil(packed_qo_len / 128))
