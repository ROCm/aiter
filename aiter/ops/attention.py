...
def get_mla_metadata_info_v1(
    ...
    # before launching the kernel; size the metadata as if nhead==16 so the
    # tile-count math matches what the kernel actually sees.
    max_qo_tiles_per_batch = int(
        math.ceil(effective_seqlen_qo * effective_num_head / 16)
    )
    if (
        effective_num_head == 16
        or (
        ...
        max_qo_tiles_per_batch = int(
            math.ceil(effective_seqlen_qo * effective_num_head / 128)
        )
    elif (
        get_gfx() == "gfx950"
