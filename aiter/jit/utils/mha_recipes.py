def _ck_fmha_targets() -> str:
    from build_targets import ck_fmha_targets

    return ck_fmha_targets()
...
def get_mha_varlen_prebuild_variants_by_names(
    ...
    for md_name in md_names:
        ...
        blob_gen_cmd = [
            f"{ck_dir}/example/ck_tile/01_fmha/generate.py -d fwd --receipt {receipt} --filter {filter_pattern} --targets {_ck_fmha_targets()} --output_dir {{}}",
            f'{ck_dir}/example/ck_tile/01_fmha/generate.py -d fwd_splitkv --receipt {receipt} --filter " @ " --targets {_ck_fmha_targets()} --output_dir {{}}',
        ]
        variants.append(
