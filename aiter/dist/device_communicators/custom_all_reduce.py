...
class CustomAllreduce:
    ...
    def _probe_symm_mem(self, rank: int, world_size: int) -> bool:
        ...
        try:
            ...
            probe_buf = symm_mem.empty(64, dtype=torch.uint8, device=self.device)
            probe_handle = symm_mem.rendezvous(probe_buf, self.group)

            peer_ptrs = probe_handle.buffer_ptrs
            all_peers_mapped = len(peer_ptrs) == world_size and all(
                ptr != 0 for ptr in peer_ptrs
            )

            del probe_handle, probe_buf
    ...
    def _init_symm_mem(self, rank: int, world_size: int, max_size: int):
        ...
        All allocation and cross-rank handle exchange is delegated to
        torch.distributed._symmetric_memory.  The rendezvous() call
        exchanges fabric handles (or POSIX fds) internally — aiter only
        sees the resulting VA pointers, passed to the gfx1250 VMM-style
        C++ ops that accept raw pointer lists.
        """
        assert self._is_gfx1250, (
            "symm_mem transport is only supported on gfx1250; "
            "old-arch C++ ops require IPC handles, not VA pointer lists."
        )
        try:
            import torch.distributed._symmetric_memory as symm_mem
        except ImportError as e:
            raise ImportError(
                "AITER_CUSTOM_AR_USE_SYMM_MEM=1 requires "
                "torch.distributed._symmetric_memory (torch >= 2.6)"
            ) from e
        try:
            import mori.allocator  # noqa: F401 — auto-registers "MORI" backend
        except ImportError as e:
            raise ImportError(
                "AITER_CUSTOM_AR_USE_SYMM_MEM=1 requires the mori package "
                "(pip install mori)"
            ) from e
        ...
        all_input_ptrs = list(self._symm_input_hdl.buffer_ptrs)
