...
class CustomAllreduce:
    ...
    def fused_qknorm_ar_rope_cache_quant(
        self,
        qkv_in: torch.Tensor,
        q_w: torch.Tensor,
        k_w: torch.Tensor,
        ...
        reg = 0 if registered else self._pool["input"].data_ptr
        reg_bytes = 0 if registered else self._pool["input"].max_size
        ops.fused_qknorm_allreduce_rope_cache_quant(
            self._ptr,
            qkv_in,
            q_w,
            k_w,
    ...
    def custom_fused_qknorm_ar_rope_cache_quant(
        self,
        qkv_in: torch.Tensor,
        q_w: torch.Tensor,
        k_w: torch.Tensor,
        ...
        if self._IS_CAPTURING:
            if torch.cuda.is_current_stream_capturing():
                return self.fused_qknorm_ar_rope_cache_quant(
                    qkv_in,
                    q_w,
                    k_w,
                    cos_sin_cache,
