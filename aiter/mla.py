...
def mla_decode_fwd_ds32(
    q_nope_buffer,  # [B, H, D_NOPE]          fp8
    q_rope_buffer,  # [B, H, D_ROPE]          bf16
    kv_nope_buffer,  # [total_tokens, D_NOPE]  fp8
    kv_rope_buffer,  # [total_tokens, D_ROPE]  bf16
    o,  # [B, H, D_NOPE] bf16 (allocated if None)
    qo_indptr,
    kv_indptr,
    kv_indices,
    kv_last_page_lens,
    ...
    q_scale=None,  # [B, H, D_SCALE]         E8M0 uint8 or float factors
    ...
    B, H, d_nope = q_nope_buffer.shape
    d_rope = q_rope_buffer.shape[-1]
    nhead, v_head_dim = H, d_nope
    total_s = B * max_seqlen_q
    if sm_scale is None:
        sm_scale = 1.0 / ((d_nope + d_rope) ** 0.5)
    if o is None:
        o = torch.empty((B, H, d_nope), dtype=dtypes.bf16, device=device)
