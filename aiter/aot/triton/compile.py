...
def compile_kernel(
    ...
    if old_compiler:
        ...
        signature = {
            kernel.arg_names[i]: s.split(":")[0] for i, s in enumerate(signature)
        }
        ...
        doc_string += [
            ...
            f"waves_per_eu={waves_per_eu}",
            f"kpack={kpack}",
            f"matrix_instr_nonkdim={matrix_instr_nonkdim}",
        ]
        # compile ast into cubin
        for h in hints.values():
            assert h in [1, 16], f"Only 1 and 16 are valid hints, got {h}"
    ...
    params = {
        ...
        "arg_pointers": ", ".join(
            [f"&{arg}" for arg in arg_names_not_1] + ["&global_scratch"]
        ),
