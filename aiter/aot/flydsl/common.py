...
class OpKind(enum.Enum):
    ...
    CHUNK_GDN_H_OPT = "chunk_gdn_h_opt"
...
def _collect_aot_jobs_for(kind: OpKind) -> list[dict[str, Any]]:
    ...
    elif kind is OpKind.CHUNK_GDN_H_OPT:
        from .chunk_gdn_h import DEFAULT_CSVS_OPT as DEFAULT_CSVS
        from .chunk_gdn_h import parse_csv_opt as parse_csv
    else:
...
def _compile_one_config_for(kind: OpKind) -> Callable[..., dict[str, Any]]:
    if kind is OpKind.MOE:
        ...
        from .chunk_gdn_h import compile_one_config_opt as compile_one_config
    elif kind is OpKind.GROUPED_MOE:
