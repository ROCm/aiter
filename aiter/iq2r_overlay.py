...
def _glm5_text_config(config: dict[str, Any]) -> dict[str, Any]:
...
def _is_glm5_next(config: dict[str, Any]) -> bool:
    architectures = config.get("architectures") or []
    return config.get("model_type") == "glm5_next" or any(
...
def create_glm5_iq2r_overlay(
    ...
    *,
    ...
    index_path = model_dir / "model.safetensors.index.json"
    config = _read_json(config_path)
    if not _is_glm5_next(config):
        raise ValueError("create_glm5_iq2r_overlay requires a GLM-5 Next model")
    text_config = _glm5_text_config(config)
    source_index = _read_json(index_path)
    source_weight_map = source_index.get("weight_map")
    ...
    layer_count = int(text_config.get("num_hidden_layers", -1))
    first_moe_layer = int(text_config.get("first_k_dense_replace", -1))
    expert_count = int(text_config.get("n_routed_experts", -1))
    hidden_size = int(text_config.get("hidden_size", -1))
    intermediate_size = int(text_config.get("moe_intermediate_size", -1))
    if not (0 <= first_moe_layer < layer_count):
        raise ValueError("GLM-5 config has an invalid routed-MoE layer range")
    if expert_count <= 0 or hidden_size <= 0 or intermediate_size <= 0:
        raise ValueError("GLM-5 config has invalid routed-expert dimensions")
    ...
    for layer_index in range(first_moe_layer, layer_count):
        ...
        names = iq2r_glm5_overlay_keys(layer_index)
        tensors = {
