...
parser.add_argument('--warmup', type=int, default=50, help='warmup iterations before the profiled run (repeat>0 only)')
parser.add_argument('--cache_size', type=str, default="2048*1024*1024", help='')
...
def kernel_device_time_ms():
    """Per-call device time (ms) using CUDA graphs for replay + profiler for kernel-level timing."""
    from torch.profiler import profile, ProfilerActivity

    # Warmup run to ensure lazy allocations / JIT are done before capture.
    func()
    torch.cuda.synchronize()

    # Capture on a non-default stream (required by CUDA graphs).
    stream = torch.cuda.Stream()

    # Bake multiple iterations into a single graph to reduce replay overhead.
    iters_per_replay = 20
    ...
    num_replays = (repeat + iters_per_replay - 1) // iters_per_replay
    ...
    # Replay under profiler — filters by kernel name, so cache flush is excluded.
    def dev_time(evt):
