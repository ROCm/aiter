// test_topk_v1.cu — Standalone benchmark & correctness test for V1 multi-block radix topk
//
// V1 特点:
//   - 每 pass 一次 kernel launch (host for 循环, 3 launches for BPP=11)
//   - __threadfence() + atomicInc + last-block-election
//   - 单个 histogram buffer per batch (last block 在 pass 间清零)
//
// Compile:
//   hipcc -O3 -std=c++17 test_topk_v1.cu -o test_topk_v1 --offload-arch=gfx942
//
// Run:
//   ./test_topk_v1 [LEN] [K] [BATCH]
//   defaults: LEN=60000  K=2048  BATCH=1

#define TOPK_STANDALONE_TEST
#include "topk_bothPD_mulblocks_v1.cu"
...
int main(int argc, char** argv)
{
...

    {
        ...
        unsigned gd = aiter::calc_grid_dim_v1<float, int, 11, 1024, false, aiter::Phase::Prefill>(
            BATCH, LEN, sm_cnt);
        gd = std::min(gd, 8u);
        gd = std::max(gd, 2u);
