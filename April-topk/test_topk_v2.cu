// test_topk_v2.cu — Standalone benchmark & correctness test for V2 multi-block radix topk
//
// V2 优化 (基于 V1):
//   - Per-pass 独立 histogram buffer，消除 Barrier 2
//   - 总 barrier 从 5 降至 3 (BPP=11)
//   - 保留 __threadfence() 在 histogram flush 后
//
// Compile:
//   hipcc -O3 -std=c++17 test_topk_v2.cu -o test_topk_v2 --offload-arch=gfx942
//
// Run:
//   ./test_topk_v2 [LEN] [K] [BATCH]
//   defaults: LEN=60000  K=2048  BATCH=1

#define TOPK_STANDALONE_TEST
#include "topk_bothPD_mulblocks_v2.cu"
...
int main(int argc, char** argv)
{
...

    {
        ...
        unsigned gd = aiter::calc_grid_dim_v2<float, int, 11, 1024, false, aiter::Phase::Prefill>(
            BATCH, LEN, sm_cnt);
        gd = std::min(gd, 8u);
        gd = std::max(gd, 2u);
