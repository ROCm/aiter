// test_topk_v4.cu — V4 Radix TopK: BPP=10, grid_dim=15
//
// V4 优化 (基于 V3 kernel):
//   - BPP=10: 4 passes × 1024 buckets (10+10+10+2 = 32 bits)
//   - grid_dim=15: 最优 CU 利用率
//   - 60K/2048/1batch: ~25.8 us (V3 BPP=11 gd=8: ~28.7 us, -10%)
//
// Compile:
//   hipcc -O3 -std=c++17 test_topk_v4.cu -o test_topk_v4 --offload-arch=gfx942
//
// Run:
//   ./test_topk_v4 [LEN] [K] [BATCH]
//   defaults: LEN=60000  K=2048  BATCH=1

#define TOPK_STANDALONE_TEST
#include "topk_bothPD_mulblocks_v4.cu"
...
int main(int argc, char** argv)
{
    ...
    printf("=== V4 Radix TopK (BPP=10, gd=15) ===\n");
    ...
    std::vector<int> h_rowStarts(BATCH, 0), h_rowEnds(BATCH, LEN);
...

    {
        ...
        unsigned gd = aiter::calc_grid_dim_v3<float, int, 10, 1024, false, aiter::Phase::Prefill>(
            BATCH, LEN, sm_cnt);
        gd = std::min(gd, 15u);
        gd = std::max(gd, 2u);
