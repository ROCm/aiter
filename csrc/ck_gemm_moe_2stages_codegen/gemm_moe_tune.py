...
class Mxfp4FlydslTuner(FmoeTuner):
    ...
    _G1_BN = (64, 128, 256)
    # The swizzle groups m-blocks against one n-block so the group shares a B
    # tile, so the useful values scale with how many m-blocks an expert spans
    # -- a shape with many rows per expert wants a much wider group than the
    # powers of two this used to stop at. 7168x2048/E48/topk8 peaks at 6, and
    # sharply: 5 and 7 are both measurably worse, so the odd values have to be
    # in the sweep too.
    _G1_XCD_SWIZZLE = (0, 1, 2, 3, 4, 5, 6, 7, 8, 12)
    _G1_K_WAVE = (1, 2, 4)
    _G1_NUM_WAVES = (4, 2)
    # Spatial-partitioner encodings (GroupNum*100 + M01) swept for gemm2.
    # None keeps the dispatcher default; 402 pins that same value explicitly so
    # a tuned row records it in the name instead of inheriting an env default.
    _G2_SPART = (None, 402, 801, 1601, 2401, 3201)

    # The full cross-product is large (thousands of (g1, g2) pairs per shape).
    # These env hooks narrow one axis at a time so a targeted sweep can land in
    # minutes and the full sweep stays reachable by just not setting them.
    # Comma-separated ints; "none" is accepted in MXFP4_TUNE_G2_SPART to mean
    # "dispatcher default". Example:
    #   MXFP4_TUNE_G1_BM=64,128,160 MXFP4_TUNE_G1_BN=256 MXFP4_TUNE_G1_KW=1
    @staticmethod
    ...
    def _port_e2e(data, kn1, kn2, topk, ne, h, dtype):
        ...
        return _mxfp4_a4w4_stage2_fw(
            ...
            block_m=BM,
            sorted_weights=sw,
            # The 'reduce' epilog's fp8 route-out (AITER_FLYDSL_STAGE2_FP8) is
            # gated on topk_weights being present; without it the wrapper
            # silently drops to the bf16 route-out and the tuner would rank
            # every reduce candidate against the slower path production does
            # not use.
