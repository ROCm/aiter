...
def stage1_a8w4_effective_cosine_compare(ref, res, msg="", printLog=True):
    # A8W4 stage1 has logical inter_dim=512 but stage2 consumes only the
    # effective 384 columns. The remaining 128 columns are model padding and are
    # intentionally not produced by the Opus stage1 kernel.
    effective_cols = 384
    return cosine_diff_compare(
        ref[..., :effective_cols],
        res[..., :effective_cols],
        msg=msg,
        printLog=printLog,
...
class FmoeTuner(TunerCommon):
    ...
    def generate_data_opus_stage1_a8w4(
        ...
        return {
            ...
            "sorted_expert_ids": sorted_expert_ids,
            "sorted_weights": sorted_weights,
            "stage1_sorted_weights": sorted_weights_for_stage1,
            "num_valid_ids": num_valid_ids,
            "moe_buf": moe_buf,
            "w1_qt_shffle_ck": w1_qt_shffle_ck,
            "w1_scale_aiter": w1_scale_aiter,
    ...
    def run_torch_moe_stage1_a8w4_sparse_ref(
        ...
        if fuse_fp8:
            ...
            return a2_fp8_bytes.view(dtypes.fp8).view(token_num, route_topk, inter_dim)
        return ref1
        ...
        for s1_name, kparams in stage1_kernels.items():
            ...
            tasks_opus.append(
                (
                    ...
                    0.01,
                    0.01,
                    stage1_a8w4_effective_cosine_compare,
                )
            )
