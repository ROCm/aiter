...
struct UniversalGemmHostArgs_bias
{
    CK_TILE_HOST UniversalGemmHostArgs_bias(const std::array<const void*, NumATensor>& as_ptr_,
...
template <index_t NumATensor = 1, index_t NumBTensor = 1, index_t NumDTensor = 0>
struct UniversalGemmKernelArgs_bias
{
...
struct UniversalGemmKernel_bias
{
    ...
    using KernelArgs =
        UniversalGemmKernelArgs_bias<AsLayout::size(), BsLayout::size(), DsLayout::size()>;
    ...
    {
        using Kernel      = UniversalGemmKernel_bias<TilePartitioner, GemmPipeline, EpiloguePipeline>;
    ...
    MakeKernelArgs(const UniversalGemmHostArgs_bias<NumATensor, NumBTensor, NumDTensor>& hostArgs)
    {
                        ...
                        const SplitKBatchOffset& splitk_batch_offset)
    {
        ...
        return make_tuple(as_tensor_view, bs_tensor_view, ds_tensor_view, e_tensor_view,bias_tensor_view);
    }
    ...
    CK_TILE_DEVICE static auto MakeGemmPadViews(const TensorView& views)
    {
        ...
        const auto& e_pad_view = [&]() {
         ...
         const auto& bias_pad_view = [&]() {
          ...
          return pad_tensor_view(bias_tensor_view, make_tuple(number<TilePartitioner::NPerBlock>{}), sequence<false>{});
        }();
        if constexpr(GemmPipeline::Preshuffle)
        {
            // For flatmm, we need to use the flat B tensor view
            return make_tuple(as_pad_view, b_flat_pad_view, ds_pad_view, e_pad_view,bias_pad_view);
        }
        else
        {
            return make_tuple(as_pad_view, bs_pad_view, ds_pad_view, e_pad_view,bias_pad_view);
        }
    }
    ...
    MakeGemmTileWindows(const PadView& views, const index_t i_m, const index_t i_n)
    {
        ...
        auto bias_block_window = make_tile_window(
            bias_pad_view,
           make_tuple(number<TilePartitioner::NPerBlock>{}), {i_n});
        return make_tuple(as_block_window, bs_block_window, ds_block_window, e_block_window,bias_block_window);
    }
    /**
                                       ...
                                       const index_t block_idx_n)
    {
        ...
        const auto& gemm_tensor_views_tuple =
            MakeGemmTensorViews<EpiloguePipeline::MemoryOperation>(
                as_ptr, bs_ptr, ds_ptr,bias_ptr, e_ptr, kargs, splitk_batch_offset);
        ...
        const auto& c_block_tile =
            GemmPipeline{}(as_block_window[I0], bs_block_window[I0],bias_block_window, num_loop, smem_ptr_0);
                                           ...
                                           const index_t block_idx_n)
    {
        ...
        const auto& c_block_tile = GemmPipeline{}(
            as_block_window[I0], bs_block_window[I0],bias_block_window, num_loop, smem_ptr_0, smem_ptr_1);
        ...
        if constexpr(GemmPipeline::DoubleSmemBuffer == true)
        {
                           ...
                           is_any_of<EDataType, fp16_t, bf16_t>::value))
            {
                RunGemm2LDS(as_ptr,
                            bs_ptr,
                            kargs.ds_ptr,
                            static_cast<const EDataType*>(kargs.bias_ptr),
                            e_ptr,
                            smem_ptr_0,
                            smem_ptr_1,
                            kargs,
