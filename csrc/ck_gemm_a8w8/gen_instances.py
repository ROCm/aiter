...
fp8 blockscale-gemm instance gen 

"""


class gemm_fp8_blockscale_codegen:
    def __init__(self, working_path, istune=False):
        self.working_path = working_path
        self.impl_path = os.path.join(working_path, "impl")
        self.instances_path = os.path.join(working_path, "instances")
...
#endif // USE_ROCM
"""
        with open(os.path.join(self.working_path, "gemm_fp8_blockscale_lookup.h"), "w") as f:
            f.write(LOOKUP_head)
            for mnk, k in kernels_dict.items():
                # print((", ").join(map(lambda x: str(x), list(mnk))), ":", k.name)
                if not self.istune and (isinstance(mnk, tuple) and mnk[0] > 0):
...
"""
        ...
        with open(os.path.join(self.working_path, "gemm_fp8_blockscale_manifest.h"), "w") as f:
            f.write(MAINFEST_head)
            for mnk, k in kernels_dict.items():
                f.write(MAINFEST_template.format(kernel_name=k.name))
            f.write(MAINFEST_end)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="generate fp8 blockscale gemm instances"
    )
    ...
    if not args.blockscale: 
        codegen = gemm_a8w8_fwd_codegen(args.working_path, args.tune)
    else:
        codegen = gemm_fp8_blockscale_codegen(args.working_path, args.tune)
