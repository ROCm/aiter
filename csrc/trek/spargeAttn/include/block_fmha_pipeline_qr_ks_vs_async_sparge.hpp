...
struct BlockFmhaPipelineQRKSVSAsyncSparge
{
    ...
    operator()(const QDramBlockWindowTmp& q_dram_block_window_tmp, // M0*K0 tile
               ...
               const int* lut_dram,   // 1*1 tile
               int valid_block_num_value, // 1 tile
               ...
               DropoutType& dropout) const
    {
        ...
        auto m     = MLBlockTileType{}; // MIterPerWarp
        auto l     = MLBlockTileType{};
        // constexpr index_t WarpGemmM = Problem::BlockFmhaShape::Gemm0WarpTile::at(number<0>{});
        // constexpr index_t WarpGemmN = Problem::BlockFmhaShape::Gemm0WarpTile::at(number<1>{});
        ...
        // int num_total_loop_tmp = integer_divide_ceil(seqlen_k_end - seqlen_k_start, kN0);
        int num_total_loop = valid_block_num_value;

        // if (threadIdx.x%64==0 && blockIdx.x==0 && blockIdx.y==0) {
        //     printf("blockIdx.z : %d, seqlen_k_start: %d, seqlen_k_end: %d, num_total_loop: %d\n", int(blockIdx.z), seqlen_k_start, seqlen_k_end, num_total_loop);
        // }
        // if (threadIdx.x == 0)
        // {
        //     printf("blockIdx.x : %d, blockIdx.y : %d, CU: %d, clk: %lld\n", blockIdx.x, blockIdx.y, __smid(), __clock());
        // }
        ...
        // auto lut_dram_block_window =
        //     make_tile_window(lut_dram_block_window_tmp.get_bottom_tensor_view(),
        //                      lut_dram_block_window_tmp.get_window_lengths(),
        //                      lut_dram_block_window_tmp.get_window_origin(),
        //                      Policy::template MakeLutDramTileDistribution<Problem>());
        
        // // attention_sink must be true for inference, so jump to next block
        // // move_tile_window(lut_dram_block_window, {1,0});
        // lut_dram_block_window.init_raw();
        // auto lut = decltype(load_tile(lut_dram_block_window)){};
        // load_tile(lut, lut_dram_block_window);
        // __builtin_amdgcn_sched_barrier(0);
        
        int* lut = reinterpret_cast<int*>(smem_ptr) + GetSmemSize() / sizeof(int);
        amd_direct_load_global_to_lds<int, 1>(lut_dram, threadIdx.x, lut, threadIdx.x, threadIdx.x/32==0, 32);
        // buffer_load_fence(1);
        // __syncthreads();
        ...
        // lds_load_fence();
        // buffer_load_fence(0);
        // if (blockIdx.x == 23 && blockIdx.z == 1 && blockIdx.y == 0 && threadIdx.x < 6)
        // {
        //     printf("%d %d %d %d %d\n", int(blockIdx.z), int(threadIdx.x), lut[threadIdx.x%32], num_total_loop_tmp, valid_block_num_value);
        // }
        ...
        // buffer_load_fence(k_dram_window.get_num_of_access()+1, q.get_thread_buffer());
        buffer_load_fence(k_dram_window.get_num_of_access()+1);
        ...
        do
        {
            ...
            // float local_max_diff = threadIdx.x;
            // if (i_total_loops > 0) {
                float local_max_diff = float(m_local.get_thread_buffer()[0] - m_old.get_thread_buffer()[0]);
                // if (i_total_loops == 1 &&threadIdx.x < 64 && blockIdx.x==0 && blockIdx.y==0 && blockIdx.z==0) {
                //     printf("first: %d %d %f %f %f\n", i_total_loops, threadIdx.x, local_max_diff, float(m_old.get_thread_buffer()[threadIdx.x]), float(m_local.get_thread_buffer()[0]));
                // }
                
                // constexpr index_t nstage = integer_log2_floor(WarpGemmM);
                ...
                //     float m_remote = warp_shuffle_down(local_max_diff, lid_delta);

                //     // reduce
                //     local_max_diff = max(local_max_diff, m_remote);
                // });
            // }
            ...
            //     printf("second: %d %d %d %f %f\n", i_total_loops, int(threadIdx.x), lut[0], float(m_old.get_thread_buffer()[threadIdx.x]), float(m_local.get_thread_buffer()[0]));
            // }

            // i_total_loops++;
            // if (i_total_loops < num_total_loop) {
            //     // move K tile windows
            //     move_tile_window(k_dram_block_window, {kN0 * lut[i_total_loops], 0});
            ...
            // if (threadIdx.x % 64 == 0)
            //     printf("%d %d %d %d %d sparse\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.x, i_total_loops);
            ...
            // if (i_total_loops == 1) {
            //     printf("bz %d, by %d, bx %d, tx %d, local_max_diff=%f\n", int(blockIdx.z), int(blockIdx.y), int(blockIdx.x), int(threadIdx.x), local_max_diff);
            // }
    ...
    //             });
                ...
                // if(i_total_loops < num_total_loop)
                // {
                //     move_tile_window(v_dram_window, {0, kN0*(lut[i_total_loops]-1)});
                //     // move K tile windows
                //     move_tile_window(k_dram_block_window, {kN0 * lut[i_total_loops], 0});
                //     k_dram_window.set_window_origin(k_dram_block_window.get_window_origin());
                ...
                //     move_tile_window(k_dram_window, {0, kK0});
                //     // move_tile_window(lut_dram_block_window, {1,0});
                //     // load_tile_raw(lut, lut_dram_block_window);
                // }
                // tail
                {
                    block_sync_lds();
            ...
            // else {
            //     printf("%d %d %d %d %d\n o_acc %f, l: %f\n", 
            //         int(blockIdx.z), int(blockIdx.y), int(blockIdx.x), int(threadIdx.x), i_total_loops, float(o_acc.get_thread_buffer()[9]), float(l.get_thread_buffer()[9]));
            // }
            ...
            //     // printf("%d %d %d %d %d %f\n o_acc %f, l: %f\n", 
            //     //     int(blockIdx.z), int(blockIdx.y), int(blockIdx.x), int(threadIdx.x), i_total_loops, local_max_diff, float(o_acc.get_thread_buffer()[9]), float(l.get_thread_buffer()[9]));
            // }
            ...
            //     printf("i : %d ; o_acc %f\n", i_total_loops, float(o_acc.get_thread_buffer()[0]));
            // }
        ...
        // if (threadIdx.x == 0 && blockIdx.x ==0 && blockIdx.z == 0) {
        //         printf("l0 %f; o_acc out %f\n", float(l.get_thread_buffer()[0]), float(o_acc.get_thread_buffer()[0]));
        //     }
        // if (blockIdx.x == 23 && blockIdx.z == 1 && blockIdx.y == 0 && threadIdx.x == 0)
        // {
        //     sweep_tile_span(o_spans[number<0>{}], [&](auto idx0) {
        //         sweep_tile_span(o_spans[number<1>{}], [&](auto idx1) {
        //             constexpr auto i_j_idx = make_tuple(idx0, idx1);
        //             printf("%d %d %f\n", int(blockIdx.z), int(threadIdx.x), float(o_acc(i_j_idx)));
        //         });
        //     });
        // }
