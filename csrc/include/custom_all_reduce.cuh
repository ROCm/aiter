...
#pragma unroll
    for (int i = 0; i < ngpus; ++i)
    {
      ...
      ptrs[i] = (const P*)_dp->ptrs[i];
      tmps[i] = (P*)(((uint32_t*)get_tmp_buf<P>(sg.signals[i])) + signal_num);
    }
    start_sync<ngpus>(sg, self_sg, rank);
...
#pragma unroll
      ...
      if (lane_id == 0) {
        token_signal[warp_id][rank * m_gpu * signal_num_per_row + idx / tnum_gpu] = 111;
      }
    }
    // end_sync<ngpus, true>(sg, self_sg, rank);
    ...
    for (int bid = blockIdx.x; bid < m; bid += gridDim.x)
    {
      ...
      if (threadIdx.x == 0) {
        for (int i = 0; i < signal_num_per_row; i++) {
          while(token_signal[rank][bid * signal_num_per_row + i] != 111) {
            // asm volatile("s_nop 0;" ::: "memory");
            ++nop; // meaningless operation
          }
          token_signal[rank][bid * signal_num_per_row + i] == 222;
        }
      }
      __syncthreads();
