...
#define AITER_LIKELY(x) __builtin_expect(!!(x), 1)
#define AITER_UNLIKELY(x) __builtin_expect(!!(x), 0)
...
class AiterAsmKernel<detail::NoStaticDescriptor>
{
                                       ...
                                       const uint8_t* desc) noexcept
    {
        ...
        if(AITER_LIKELY(kernel_func != nullptr))
        {
        ...
        {
            (void)hipGetLastError(); // clean the last error
    ...
    {
        if(AITER_LIKELY(this->state.load(std::memory_order_acquire) == State::Loaded))
        {
            return this;
        }
...
struct AiterAsmKernelConfigMap
{
    ...
    {
        if(AITER_UNLIKELY(entry == nullptr))
        {
            return {};
        }
        auto entries = reinterpret_cast<const Entry*>(&this[1]);
