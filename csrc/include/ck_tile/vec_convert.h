...
CK_TILE_DEVICE fp4x2_v amd_assembly_cvt_scalef32_pk_fp4_f32(fp32_t a, fp32_t b,
                                                            float scale) {
#if defined(__gfx950__)
  int8x4_t c;
  // permute high bits and low bits to match the order of the original vector
  asm volatile("v_cvt_scalef32_pk_fp4_f32 %0, %1, %2"
               : "=v"(c)
               : "v"(b), "v"(a), "v"(scale));
  return bit_cast<fp4x2_v>(c[0]);
#endif
}
//     CK_TILE_DEVICE fp4x2_v amd_assembly_cvt_scalef32_pk_fp4_f16(fp16_t a,
//     fp16_t b, float scale)
//     {
// #if defined(__gfx950__)
//         int8x4_t c;
//         // permute high bits and low bits to match the order of the original
//         vector asm volatile("v_cvt_scalef32_pk_fp4_f16 %0, %1, %2" : "=v"(c)
//         : "v"(b), "v"(a), "v"(scale)); return bit_cast<fp4x2_v>(c[0]);
// #endif
//     }
//     CK_TILE_DEVICE fp4x2_v amd_assembly_cvt_scalef32_pk_fp4_bf16(bf16_t a,
//     bf16_t b, float scale)
//     {
// #if defined(__gfx950__)
//         int8x4_t c;
//         // permute high bits and low bits to match the order of the original
//         vector asm volatile("v_cvt_scalef32_pk_fp4_bf16 %0, %1, %2" : "=v"(c)
//         : "v"(b), "v"(a), "v"(scale)); return bit_cast<fp4x2_v>(c[0]);
// #endif
//     }
...
// fp32x2 -> fp8x2
CK_TILE_HOST_DEVICE constexpr fp8x2_v
fp32x2_t_to_fp8x2_t(fp32x2_v x, float inverted_scale) {
  ...
  fp32x2_v tmp =
      amd_assembly_pk_mul_f32(x, fp32x2_v{inverted_scale, inverted_scale});
...
// fp32x2 -> int8x2
CK_TILE_HOST_DEVICE constexpr int8x2_v
fp32x2_t_to_int8x2_t(fp32x2_v x, float inverted_scale) {
  fp32x2_v tmp =
      amd_assembly_pk_mul_f32(x, fp32x2_v{inverted_scale, inverted_scale});
...
// fp32x2 -> int8x2
CK_TILE_HOST_DEVICE constexpr int8x2_v
fp32x2x2_t_to_fp4x2x2_t(fp32x2_v x, float inverted_scale) {
  fp32x2_v tmp =
      amd_assembly_pk_mul_f32(x, fp32x2_v{inverted_scale, inverted_scale});
...
// fp32x2 -> fp4x2
CK_TILE_HOST_DEVICE constexpr fp4x2_v
fp32x2_t_to_fp4x2_t(fp32x2_v x, float inverted_scale) {
  return amd_assembly_cvt_scalef32_pk_fp4_f32(x[0], x[1], inverted_scale);
}
// // fp16x2 -> fp4x2
// CK_TILE_HOST_DEVICE constexpr fp4x2x2_t fp16x2x2_t_to_fp4x2x2_t(fp16x2x2_t x,
// float inverted_scale)
// {
//     return {amd_assembly_cvt_scalef32_pk_fp4_f16(x[0][0], x[0][1],
//     inverted_scale),
//             amd_assembly_cvt_scalef32_pk_fp4_f16(x[1][0], x[1][1],
//             inverted_scale)};
// }
// // bf16x2 -> fp4x2
// CK_TILE_HOST_DEVICE constexpr fp4x2x2_t bf16x2x2_t_to_fp4x2x2_t(bf16x2x2_t x,
// float inverted_scale)
// {
//     return {amd_assembly_cvt_scalef32_pk_fp4_bf16(x[0][0], x[0][1],
//     inverted_scale),
//             amd_assembly_cvt_scalef32_pk_fp4_bf16(x[1][0], x[1][1],
//             inverted_scale)};
// }
#define CK_TILE_TYPE_CONVERT(dtype_, stype_, vec_size_)                        \
  template <>                                                                  \
  CK_TILE_HOST_DEVICE constexpr vec_t<dtype_##_t, vec_size_>                   \
  vec_convert<dtype_##_t, stype_##_t, vec_size_>(                              \
      vec_t<stype_##_t, vec_size_> x, float inverted_scale) {                  \
    constexpr int iter_num = vec_size_ / 2;                                    \
    vec_t<dtype_##_t, vec_size_> out;                                          \
    for (size_t i = 0; i < iter_num; i++) {                                    \
      auto tmp = stype_##x2##_t_to_##dtype_##x2##_t(                           \
          vec_t<stype_##_t, 2>{x[i * 2], x[i * 2 + 1]}, inverted_scale);       \
      out[i * 2] = tmp[0];                                                     \
      out[i * 2 + 1] = tmp[1];                                                 \
    }                                                                          \
    return out;                                                                \
