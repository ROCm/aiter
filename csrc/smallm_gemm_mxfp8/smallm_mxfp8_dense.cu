    ...
    int M, int N, int K)
{
    ...
    // The value still feeds a 32-bit SRD num_records field, so the host guards
    // each tensor < 4GB (the AMD raw-buffer addressing limit).
    ...
    int64_t block_n)
{
    ...
    // Each tensor is addressed via one 32-bit raw-buffer SRD (num_records and
    // voffset are hardware-32-bit); reject >4GB tensors instead of silently
    // truncating. (>4GB would need per-row/per-tile base-pointer advance.)
    TORCH_CHECK((size_t)N * K <= 0xFFFFFFFFull && (size_t)M * K <= 0xFFFFFFFFull,
                "smallm_mxfp8_gemv: a tensor exceeds the 4GB raw-buffer limit");
