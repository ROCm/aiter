...
#define DISP_AD(M_TI, ADV)                                                                                       \
    do {                                                                                                          \
        if (has_weight) launch_one<M_TI, 8, ADV, true>(aqp, asp, bqp, bsp, op, sip, eip, npp, twp,                \
                                                       E, N, K, num_valid_tokens, M_act, block_m,                 \
                                                       num_m_tiles, stream);                                      \
        else            launch_one<M_TI, 8, ADV, false>(aqp, asp, bqp, bsp, op, sip, eip, npp, twp,               \
                                                       E, N, K, num_valid_tokens, M_act, block_m,                 \
                                                       num_m_tiles, stream);                                      \
    } while (0)
    ...
    c10::optional<at::Tensor> mul_weight_by)
{
    ...
    // 32-bit raw-buffer SRD (num_records/voffset are hardware-32-bit); reject
    // >4GB instead of silently truncating. (A per-expert base-pointer advance
    // would lift the E*N*K term -- tracked as a follow-up.)
    TORCH_CHECK((size_t)E * N * K <= 0xFFFFFFFFull &&
                    (size_t)M_act * K <= 0xFFFFFFFFull,
                "smallm_mxfp8_moe_grouped_gemm: a tensor exceeds the 4GB "
                "raw-buffer limit");
    ...
    int M_TI = 4;  // M_TILE_INNER=4: best register/throughput balance
    if (block_m % 4 != 0) M_TI = 1;
    if (M_TI == 4)      DISP_KERNEL(4);
    else if (M_TI == 2) DISP_KERNEL(2);
    else                DISP_KERNEL(1);
    return out;
}
