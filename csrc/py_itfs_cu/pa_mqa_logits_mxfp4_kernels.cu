...
//
// MXFP4 paged MQA logits (OPUS) -- ONE launcher for BOTH arches over ONE per-tile schedule.
// build_tiles + build_sched are arch-agnostic (int-only device builders); fwd_sched dispatches
// on the RUNTIME arch to the MFMA (gfx950) or WMMA (gfx1250) kernel family, and within it by
// (q_per_block, block_k). The input ABI and the caller's two contract conditions are in
// pa_mqa_logits_mxfp4_opus.h. gfx950 is driven at q_per_block == 1 (one row per tile).
...
// THE COMPILED CONFIGS.
//   gfx950 (MFMA 32x32x64, wave64): two KV-tile widths -- 256 -> 4-wave, 64 -> 1-wave. Same
//           compute core / ABI / scale layout; they differ only in KV split granularity.
//   gfx1250 (WMMA 32x16x128, wave32): two q_per_block, both KV tile 64 = 1 page.
// fwd_sched's dispatch below is the only statement of which (arch, q_per_block, block_k) exist.
...
// ══ gfx950 (MFMA) shape check + launch ══════════════════════════════════════════════════════
...
static void pa_mqa_logits_mxfp4_check_shapes_mfma(aiter_tensor_t& q,
...
template<class Traits>
static void pa_mqa_logits_mxfp4_launch_sched_mfma(aiter_tensor_t& q,
                          aiter_tensor_t& q_scale,
                          aiter_tensor_t& kv_cache,
                          aiter_tensor_t& kv_scale,
                          aiter_tensor_t& block_tables,
                          ...
                          int max_seq_len)
{
    pa_mqa_logits_mxfp4_check_shapes_mfma<Traits>(q, q_scale, kv_cache, kv_scale, block_tables,
...
// ══ gfx1250 (WMMA) shape check + launch, and the shared per-tile builders ════════════════════
...
template <class Traits>
static void pa_mqa_logits_mxfp4_check_shapes_wmma(aiter_tensor_t& q,
                                                     aiter_tensor_t& q_scale,
                                                     aiter_tensor_t& kv_cache,
                                                     aiter_tensor_t& kv_scale,
                                                     aiter_tensor_t& block_tables,
...
// writes every slot, so a previous forward's schedule cannot leak into this one.
template <class Traits>
static void pa_mqa_logits_mxfp4_launch_sched_wmma(aiter_tensor_t& q,
                                                     aiter_tensor_t& q_scale,
                                                     aiter_tensor_t& kv_cache,
                                                     aiter_tensor_t& kv_scale,
                                                     aiter_tensor_t& block_tables,
                                                     ...
                                                     int max_seq_len)
{
    pa_mqa_logits_mxfp4_check_shapes_wmma<Traits>(
...
// The ONLY place an instantiation (a TYPE) is chosen. aiter JITs per target GPU, so the compiled
// device body always matches the runtime arch on the arm that launches; the other family's
// kernel is an empty stub it never reaches. An unmatched pair RAISES and must not fall to a
// default -- a mismatch comes back correct and slow, never wrong.
...
{
    aiter_detail::g_aiter_can_throw = true;
    // Read the arch of THIS tensor's device (not the process-wide current one).
    HipDeviceGuard guard(q.device_id);
    const std::string arch = get_gpu_arch();

    if(arch == "gfx950")
    {
        // gfx950 runs ONE query row per tile; its instances are the KV-tile widths, and its
        // kernel reads each row's window from the record -- so local_starts / local_ends /
        // num_rows are not launch inputs on this arm.
        ...
        if(block_k == mqa_logits_fp4_traits_4wave::KV_TILE_SIZE)
            pa_mqa_logits_mxfp4_launch_sched_mfma<mqa_logits_fp4_traits_4wave>(
                q, q_scale, kv_cache, kv_scale, block_tables, weights, cta_info, out,
                num_ctas, weight_scale, block_k, kv_block_size, max_seq_len);
        else if(block_k == mqa_logits_fp4_traits_1wave::KV_TILE_SIZE)
            pa_mqa_logits_mxfp4_launch_sched_mfma<mqa_logits_fp4_traits_1wave>(
                q, q_scale, kv_cache, kv_scale, block_tables, weights, cta_info, out,
                num_ctas, weight_scale, block_k, kv_block_size, max_seq_len);
        else
    ...
    else if(arch == "gfx1250")
    {
        if(q_per_block == 4 && block_k == 64)
            pa_mqa_logits_mxfp4_launch_sched_wmma<mqa_logits_traits_qlen4_kv64>(
            ...
            pa_mqa_logits_mxfp4_launch_sched_wmma<mqa_logits_traits_qlen1_kv64>(
                q, q_scale, kv_cache, kv_scale, block_tables, weights, local_starts, local_ends,
                cta_info, out, num_rows, num_ctas, weight_scale, kv_block_size, max_seq_len);
        else
