...
// Thin wrapper over the device kernel template in `pa_mqa_logits_mxfp4_opus.h`
// (single-header, IMPL-guarded). Q/KV must already be MXFP4-quantized and
...
// window builder (`pa_mqa_logits_mxfp4_prefill_windows`) the prefill path consumes.
//
// Structurally a twin of pa_mqa_logits_mxfp8_kernels.cu; only the element type,
// the traits set and the q/kv shape checks differ.
...
// they differ only in KV split granularity + CTA width): 256 -> 4-wave, 64 -> 1-wave.
// Unlike fp8 the narrow variant needs no extra trait: fp4 reads Q straight from global,
// so there is no async-Q LDS layout to keep byte-identical across the two.
...
static void pa_mqa_logits_mxfp4_launch_prefill(aiter_tensor_t& q,
                          ...
                          int max_seq_len)
{
    ...
    // fp4 packs 2 elements per byte, so the last q axis is D/2 BYTES, not D elements.
    const int D_BYTES = static_cast<int>(q.size(2));
    AITER_CHECK(H == Traits::N_HEADS, "compiled for H=", (int)Traits::N_HEADS, ", got H=", H);
    AITER_CHECK(D_BYTES == Traits::Q_ROW_BYTES,
                "q last dim is D/2 packed bytes; compiled for ", (int)Traits::Q_ROW_BYTES,
    ...
    opus_logits::pa_mqa_logits_mxfp4_kernel<Traits, opus_logits::mqa_logits_sched::Prefill><<<grid, block, 0, stream>>>(kargs);
...
static void pa_mqa_logits_mxfp4_launch_decode(aiter_tensor_t& q,
                          ...
                          int max_seq_len)
{
    AITER_CHECK(q.dim() == 3, "q must be 3-D [total_q, H, D/2], got ndim=", q.dim());
    ...
    AITER_CHECK(kv_block_size == Traits::PAGE_SIZE,
                "compiled for kv_block_size=", (int)Traits::PAGE_SIZE, ", got ", kv_block_size);
    AITER_CHECK(q.dtype() == AITER_DTYPE_fp4x2 || q.dtype() == AITER_DTYPE_u8, "q must be fp4x2/u8");
    AITER_CHECK(kv_cache.dtype() == AITER_DTYPE_fp4x2 || kv_cache.dtype() == AITER_DTYPE_u8,
                "kv_cache must be fp4x2/u8");
    AITER_CHECK(weights.dtype() == AITER_DTYPE_bf16, "weights must be bf16");
    AITER_CHECK(block_tables.dtype() == AITER_DTYPE_i32, "block_tables must be int32");
    AITER_CHECK(out.dtype() == AITER_DTYPE_fp32, "out must be fp32");
    ...
    dim3 block(Traits::BLOCK_SIZE);
    opus_logits::pa_mqa_logits_mxfp4_kernel<Traits, opus_logits::mqa_logits_sched::Decode><<<grid, block, 0, stream>>>(kargs);
    HIP_CALL_LAUNCH(hipGetLastError());
}
                          ...
                          int max_seq_len)
{
    if(block_k == mqa_logits_fp4_traits_4wave::KV_TILE_SIZE) {
        ...
        pa_mqa_logits_mxfp4_launch_prefill<mqa_logits_fp4_traits_1wave>(
            q, q_scale, kv_cache, kv_scale, block_tables, weights,
            row_to_batch, local_starts, local_ends, out,
            num_rows, weight_scale, block_k, kv_block_size, max_seq_len);
    } else {
...
__global__ void mqa_logits_fp4_prefill_windows_kernel(const int* __restrict__ cu,
                                      ...
                                      int total_q)
{
    ...
    mqa_logits_fp4_prefill_windows_kernel<<<grid, WINDOW_BUILD_BLOCK, 0, stream>>>(
