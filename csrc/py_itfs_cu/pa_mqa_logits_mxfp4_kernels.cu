...
//
// `cta_info` is caller-allocated and reused in place, so a captured graph replays from one
// address while the builder refreshes its contents. The builder writes every slot, so a buffer
// carrying a previous forward's schedule cannot leak into this one.
template<class Traits>
...
// Fill `cta_info`. No host<->device sync and nothing read back, so it is capture-safe; a caller
// that cannot capture it calls it from its metadata builder instead.
