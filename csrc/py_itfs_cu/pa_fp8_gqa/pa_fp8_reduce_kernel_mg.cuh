...
//
// DEDICATED reduce kernel for the ql=3 (mtp>=3) multi-group decode path,
// ported from Page_Attetion_GQA_fp8_mtp_geak.  Wrapped in the nested namespace
// `pa_fp8_gqa::mg` so it is FULLY SEPARATE from the ql<=2 reduce
// (pa_fp8_reduce_kernel.cuh); the two never share symbols, so changes to one
// path do not affect the other.  Only the static split-k path is used by the
// mg entry (per_seq_nf defaults to nullptr).
//
...
void pa_fp8_reduce_kernel_v3(
    ...
    const int fixed_num_partitions,
    const int num_kblocks_per_fat_part,
    // Dynamic scheduling: when non-null, per_seq_nf[seq_idx] gives the
    // per-sequence partition count (overrides fixed_num_partitions).
...
)
{
    ...
    int num_partitions;
    if (per_seq_nf != nullptr) {
        // Dynamic scheduling: use per-sequence partition count directly.
        // Must match the main kernel's per_seq_nf[seq_idx] partition stride.
        const int seq_nf = per_seq_nf[seq_idx];
        const int eff_page_dyn = PAGQA_DIVUP(total_kblocks, seq_nf);
        const int active_dyn   = (eff_page_dyn > 0)
            ? PAGQA_DIVUP(total_kblocks, eff_page_dyn)
            : total_kblocks;
        num_partitions = min(active_dyn, seq_nf);
    } else {
        // FlyDSL-aligned page size, MUST match the main kernel's kbi slicing.
        //   v2 sentinel (fixed_num_partitions>0 && num_kblocks_per_fat_part<=0):
        //     derive AT RUNTIME from actual ctx + launched partition count,
        ...
        const int active_parts  = (eff_page > 0)
            ? PAGQA_DIVUP(total_kblocks, eff_page)
            : total_kblocks;
        num_partitions = (fixed_num_partitions > 0)
                             ? min(active_parts, fixed_num_partitions)
                             : active_parts;
    }
    const int ns_mtp_idx     = seq_idx * MTP + mtp;
