...
pa_fwd(torch::Tensor &Q, //   [num_seqs, num_heads, head_size]
       ...
       int max_num_blocks, int max_qlen = 1,
  ...
  if (qo_indptr && max_qlen > 1) {
    if (K.dtype() == at::ScalarType::BFloat16) {
      ...
      TORCH_CHECK(Q.dtype() == at::ScalarType::BFloat16 && K_QScale &&
                      K.dtype() == torch_fp8,
                  __func__,
    ...
    TORCH_CHECK(Q.is_contiguous() || Q.dtype() == at::ScalarType::BFloat16,
                __func__, ":a16w16_f16 only support Q.is_contiguous() for now");
    TORCH_CHECK(num_kv_heads == 1 || Q.dtype() == at::ScalarType::BFloat16,
                __func__, ":a16w16_f16 only support num_kv_heads==1, for now");
    if (Q.dtype() == at::ScalarType::Half) {
