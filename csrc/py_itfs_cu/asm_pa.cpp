...
torch::Tensor
pa_fwd(torch::Tensor &Q, //   [num_seqs, num_heads, head_size]
       torch::Tensor
       ...
       int max_num_blocks, std::optional<torch::Tensor> &K_QScale,
  ...
  TORCH_CHECK(block_size == 16, __func__,
              " for now only support block_size == 16");
  ...
  if (K_QScale) {
    if (Q.dtype() == at::ScalarType::Half) {
      if (K.dtype() == at::ScalarType::Byte ||
          K.dtype() == at::ScalarType::Char) {
    ...
    TORCH_CHECK(Q.is_contiguous(), __func__,
                ":a16w16 only support Q.is_contiguous() for now");
    TORCH_CHECK(num_kv_heads == 1, __func__,
                ":a16w16 only support num_kv_heads==1, for now");
    if (Q.dtype() == at::ScalarType::Half) {
      static AiterAsmKernel impl_a16w16_f16("pa_kernel_func",
                                            "pa_a16w16_f16.co");
      impl_ptr = &impl_a16w16_f16;
    } else if (Q.dtype() == at::ScalarType::BFloat16) {
      static AiterAsmKernel impl_a16w16_b16(
          "_ZN5aiter22pa_bf16_noquant_a16w16E",
          "/pa/pa_bf16_noquant_a16w16.co");
      impl_ptr = &impl_a16w16_b16;
    }
  }
  TORCH_CHECK(impl_ptr != nullptr, __func__,
              ": unsupport current Q_type:", Q.scalar_type());

  impl_ptr->launch_kernel({&args, &arg_size,
                           num_kv_heads, // gdx
                           batch,        // gdy
                           1,            // gdz
                           256,          // bdx: 4 wv64
