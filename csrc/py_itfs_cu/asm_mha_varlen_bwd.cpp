...
fmha_bwd_args get_asm_fmha_varlen_bwd_args(
    const mask_info &mask,
    // sizes
    const int b, const int max_seqlen_q, const int max_seqlen_k, const int h,
    const int h_k, const int hdim_q, const int hdim_v,
    // device pointers
    const at::Tensor q, const at::Tensor k, const at::Tensor v,
    const at::Tensor seqlens_q, const at::Tensor seqlens_k,
    std::optional<const at::Tensor> &alibi_slopes_, const at::Tensor out,
    const at::Tensor softmax_lse, const at::Tensor dout, at::Tensor dq_acc,
    at::Tensor d, at::Tensor dq, at::Tensor dk, at::Tensor dv,
    float softmax_scale, float p_dropout,
...
std::vector<at::Tensor> fmha_v3_varlen_bwd(
  ...
  if (is_causal) {
    ...
    mask =
        mask_info::decode(mask_identify, max_seqlen_q, max_seqlen_k); // casual
    ...
    mask =
        mask_info::decode(mask_identify, max_seqlen_q, max_seqlen_k); // local
  }
  ...
  auto opts = q.options();
  auto softmax_d = torch::empty({batch_size, num_heads, max_seqlen_q},
                                opts.dtype(at::kFloat));
  at::Tensor dq_accum;

  if (!deterministic) {
    dq_accum = torch::zeros({1, num_heads, total_q, head_size_v},
                            opts.dtype(at::kFloat));
  } else {
    ...
    dq_accum = torch::zeros({nsplits, num_heads, total_q, head_size_v},
                            opts.dtype(at::kFloat));
  }
  ...
  bias_enum bias_type =
      alibi_slopes_.has_value() ? bias_enum::alibi : bias_enum::no_bias;
  ...
  if (max_seqlen_q > 0) {
    ...
    auto args = get_asm_fmha_varlen_bwd_args(
        mask, batch_size, max_seqlen_q, max_seqlen_k, num_heads, num_heads_k,
        head_size_q, head_size_v, q, k, v, cu_seqlens_q, cu_seqlens_k,
        alibi_slopes_, out, softmax_lse, dout, dq_accum, softmax_d, dq,
        dk_expanded, dv_expanded, softmax_scale, p_dropout, drop_seed_offset);

    float t = aiter::mha_bwd(args, stream_config, q_dtype_str,
                             true, // is_group_mode
                             mask.type, bias_type,
                             false, // has_dbias
