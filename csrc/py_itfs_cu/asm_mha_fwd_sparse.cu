...
// Persistent/sorted/affine fp8 sparse sibling (prevfa surgical port). Identical
// data contract to fmha_v3_fwd_fp8_sparse plus a host-built `work_table`
// int32[total_tiles] tensor (entry k = packed q | (h<<16) | (b<<24), LPT-sorted).
// Routes to the persistent/sorted/affine .co's per (sorted, affine); only the
// affine-sorted .co is shipped in this branch today.
std::vector<at::Tensor>
fmha_v3_fwd_fp8_sparse_persistent(at::Tensor& q,
                                  const at::Tensor& k,
                                  const at::Tensor& v,
                                  const at::Tensor& q_descale,
                                  const at::Tensor& k_descale,
                                  ...
                                  bool affine)
{
    ...
    TORCH_CHECK((q.dtype() == at::ScalarType::Float8_e4m3fnuz ||
                    ...
                    (k.dtype() == at::ScalarType::Float8_e4m3fnuz ||
                     k.dtype() == at::ScalarType::Float8_e4m3fn),
                "fmha_v3_fwd_fp8_sparse_persistent: Q and K must be fp8.");
    TORCH_CHECK(v.dtype() == at::ScalarType::Float8_e4m3fnuz ||
                    v.dtype() == at::ScalarType::Float8_e4m3fn,
                "fmha_v3_fwd_fp8_sparse_persistent: V must be fp8.");
    TORCH_CHECK(q_descale.dtype() == torch::kFloat32 &&
                    k_descale.dtype() == torch::kFloat32 &&
                    v_descale.dtype() == torch::kFloat32,
                "fmha_v3_fwd_fp8_sparse_persistent: descale tensors must be fp32.");
    TORCH_CHECK(kv_block_indices.dtype() == torch::kInt32 &&
                    lut_start.dtype() == torch::kInt32 &&
                    lut_count.dtype() == torch::kInt32 &&
                    work_table.dtype() == torch::kInt32,
                "fmha_v3_fwd_fp8_sparse_persistent: LUT + work_table must be int32.");
    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v);
    CHECK_DEVICE(q_descale); CHECK_DEVICE(k_descale); CHECK_DEVICE(v_descale);
    CHECK_DEVICE(kv_block_indices); CHECK_DEVICE(lut_start); CHECK_DEVICE(lut_count);
    ...
    TORCH_CHECK(k.stride(-1) == 1, "K must have contiguous last dimension");
    TORCH_CHECK(v.stride(-1) == 1, "V must have contiguous last dimension");
    TORCH_CHECK(work_table.is_contiguous(),
                "fmha_v3_fwd_fp8_sparse_persistent: work_table must be contiguous.");
    ...
    TORCH_CHECK(head_size_q == ASM_SPARSE_HEAD_DIM && head_size_v == ASM_SPARSE_HEAD_DIM,
                "fmha_v3_fwd_fp8_sparse_persistent: only hd=", ASM_SPARSE_HEAD_DIM,
                " is supported (got Qd=", head_size_q, " Vd=", head_size_v, ").");
    TORCH_CHECK(num_heads % num_heads_k == 0,
                "fmha_v3_fwd_fp8_sparse_persistent: HQ must be divisible by HK.");
    const int gqa_ratio = num_heads / num_heads_k;
    TORCH_CHECK((gqa_ratio & (gqa_ratio - 1)) == 0,
                "fmha_v3_fwd_fp8_sparse_persistent: GQA ratio must be a power of 2.");
    ...
    const int64_t expected_lut_meta =
        static_cast<int64_t>(batch_size) * num_heads * num_q_blocks;
    TORCH_CHECK(lut_start.numel() == expected_lut_meta,
                "fmha_v3_fwd_fp8_sparse_persistent: lut_start.numel() = ",
                lut_start.numel(), ", expected ", expected_lut_meta, ".");
    TORCH_CHECK(lut_count.numel() == expected_lut_meta,
                "fmha_v3_fwd_fp8_sparse_persistent: lut_count.numel() = ",
                lut_count.numel(), ", expected ", expected_lut_meta, ".");
    TORCH_CHECK(work_table.numel() == expected_lut_meta,
                "fmha_v3_fwd_fp8_sparse_persistent: work_table.numel() = ",
                work_table.numel(), ", expected ", expected_lut_meta,
                " (= batch*HQ*num_q_blocks = total tiles).");

    if (lut_freeze.has_value())
    {
        TORCH_CHECK(lut_freeze->dtype() == torch::kInt32,
                    "fmha_v3_fwd_fp8_sparse_persistent: lut_freeze must be int32.");
        CHECK_DEVICE((*lut_freeze));
        TORCH_CHECK(lut_freeze->numel() == expected_lut_meta,
                    "fmha_v3_fwd_fp8_sparse_persistent: lut_freeze.numel() = ",
    ...
    if (out_.has_value())
    {
        out = out_.value();
        TORCH_CHECK(out.dtype() == torch::kBFloat16,
                    "fmha_v3_fwd_fp8_sparse_persistent: out must be bf16.");
        CHECK_DEVICE(out);
        TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        CHECK_SHAPE(out, batch_size, seqlen_q, num_heads, head_size_v);
    }
    ...
    a.total_tiles    = static_cast<uint32_t>(expected_lut_meta);
...
// Sorted-dispatch mxfp4 sparse sibling. Byte-for-byte the same mxfp4 data
// contract as fmha_v3_fwd_mxfp4_sparse (fp4-packed int8/uint8 Q/K, fp8 V, E8M0
// byte Q/K scales, fp32 V descale, int32 LUT, bf16 out) plus a host-built
// `work_table` int32[total_tiles] tensor; routes to the sorted .co via
// aiter::fmha_fwd_v3_mxfp4_sparse_sorted (flat grid (total_tiles,1,1)).
std::vector<at::Tensor>
fmha_v3_fwd_mxfp4_sparse_sorted(at::Tensor& q,
                               ...
                               std::optional<at::Tensor> out_)
{
    // ---- dtype + device checks (mirror fmha_v3_fwd_mxfp4_sparse) -----------
    auto is_byte = [](at::ScalarType d) {
        return d == at::ScalarType::Char || d == at::ScalarType::Byte;
    };
    TORCH_CHECK(is_byte(q.dtype().toScalarType()) && is_byte(k.dtype().toScalarType()),
    ...
    TORCH_CHECK(v.dtype() == at::ScalarType::Float8_e4m3fnuz ||
                    v.dtype() == at::ScalarType::Float8_e4m3fn,
                "fmha_v3_fwd_mxfp4_sparse_sorted: V must be fp8 (mxfp4 Q/K * fp8 V * bf16 out).");
    TORCH_CHECK(is_byte(q_descale.dtype().toScalarType()) &&
                    is_byte(k_descale.dtype().toScalarType()),
                "fmha_v3_fwd_mxfp4_sparse_sorted: Q/K per-block E8M0 scales must be "
                "int8/uint8 byte tensors.");
    TORCH_CHECK(v_descale.dtype() == torch::kFloat32,
                "fmha_v3_fwd_mxfp4_sparse_sorted: V descale must be fp32 (per output channel).");
    TORCH_CHECK(kv_block_indices.dtype() == torch::kInt32 &&
                    lut_start.dtype() == torch::kInt32 &&
                    lut_count.dtype() == torch::kInt32 &&
                    work_table.dtype() == torch::kInt32,
                "fmha_v3_fwd_mxfp4_sparse_sorted: LUT + work_table must be int32.");
    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v);
    CHECK_DEVICE(q_descale); CHECK_DEVICE(k_descale); CHECK_DEVICE(v_descale);
    CHECK_DEVICE(kv_block_indices); CHECK_DEVICE(lut_start); CHECK_DEVICE(lut_count);
    CHECK_DEVICE(work_table);
    ...
    TORCH_CHECK(v.stride(-1) == 1, "V must have contiguous last dimension");
    TORCH_CHECK(work_table.is_contiguous(),
                "fmha_v3_fwd_mxfp4_sparse_sorted: work_table must be contiguous.");
    ...
    TORCH_CHECK(head_size_q_logical == ASM_SPARSE_HEAD_DIM &&
                    head_size_v == ASM_SPARSE_HEAD_DIM,
                "fmha_v3_fwd_mxfp4_sparse_sorted: only hd=", ASM_SPARSE_HEAD_DIM,
                " is supported (got logical Qd=", head_size_q_logical,
                " Vd=", head_size_v, ").");
    TORCH_CHECK(num_heads % num_heads_k == 0,
                "fmha_v3_fwd_mxfp4_sparse_sorted: HQ must be divisible by HK (got HQ=",
                num_heads, " HK=", num_heads_k, ").");
    const int gqa_ratio = num_heads / num_heads_k;
    TORCH_CHECK((gqa_ratio & (gqa_ratio - 1)) == 0,
                "fmha_v3_fwd_mxfp4_sparse_sorted: GQA ratio (HQ/HK) must be a power of 2 "
                "(got ", gqa_ratio, ").");
    ...
    // ---- LUT + work_table shape checks ------------------------------------
    const int num_q_blocks =
        (seqlen_q + ASM_SPARSE_BLOCK_M - 1) / ASM_SPARSE_BLOCK_M;
    const int64_t expected_lut_meta =
        static_cast<int64_t>(batch_size) * num_heads * num_q_blocks;
    TORCH_CHECK(lut_start.numel() == expected_lut_meta,
                "fmha_v3_fwd_mxfp4_sparse_sorted: lut_start.numel() = ", lut_start.numel(),
                ", expected ", expected_lut_meta);
    TORCH_CHECK(lut_count.numel() == expected_lut_meta,
                "fmha_v3_fwd_mxfp4_sparse_sorted: lut_count.numel() = ", lut_count.numel(),
                ", expected ", expected_lut_meta);
    TORCH_CHECK(work_table.numel() == expected_lut_meta,
                "fmha_v3_fwd_mxfp4_sparse_sorted: work_table.numel() = ",
                work_table.numel(), ", expected ", expected_lut_meta,
                " (= batch*HQ*num_q_blocks = total tiles).");
    ...
    // Sorted-dispatch extension:
    a.work_table_ptr = work_table.data_ptr();
    a.total_tiles    = static_cast<uint32_t>(expected_lut_meta);

    ck_tile::stream_config stream_config{stream};
    float t = aiter::fmha_fwd_v3_mxfp4_sparse_sorted(a, stream_config);
    TORCH_CHECK(t >= 0, "fmha_v3_fwd_mxfp4_sparse_sorted: dispatcher returned an error "
                        "code (unsupported config or .co not found).");
    return {out};
}

// =====================================================================
// DENSE (non-sparse) mxfp4 entry. Identical contract to
// fmha_v3_fwd_mxfp4_sparse minus the LUT triple; routes to the dense
// kernel fwd_hd128_mxfp4.co via aiter::fmha_fwd_v3_mxfp4.
// =====================================================================
std::vector<at::Tensor>
                  ...
                  std::optional<at::Tensor> out_)
{
    ...
    const int gqa_ratio = num_heads / num_heads_k;
    TORCH_CHECK((gqa_ratio & (gqa_ratio - 1)) == 0,
                "fmha_v3_fwd_mxfp4: GQA ratio (HQ/HK) must be a power of 2 (got ",
                gqa_ratio, ").");
    ...
    {
        out = out_.value();
        TORCH_CHECK(out.dtype() == torch::kBFloat16, "fmha_v3_fwd_mxfp4: out must be bf16.");
        CHECK_DEVICE(out);
        TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        CHECK_SHAPE(out, batch_size, seqlen_q, num_heads, head_size_v);
    }
                 ...
                 std::optional<at::Tensor> out_)
{
    ...
    const int head_size_q_packed = sz[3];
    const int head_size_q_logical = head_size_q_packed * 2;
    // V is a col-major fp4 view whose last (logical) dim is the full head_dim (128);
    // the fp4 packing is carried by its byte strides, not by a halved last dim.
    const int head_size_v = v.sizes()[3];
    const int seqlen_k = k.size(1);
    const int num_heads_k = k.size(2);
    ...
    TORCH_CHECK(head_size_q_logical == ASM_SPARSE_HEAD_DIM &&
                    head_size_v == ASM_SPARSE_HEAD_DIM,
                "fmha_v3_fwd_f4f4: only hd=", ASM_SPARSE_HEAD_DIM,
                " is supported (got logical Qd=", head_size_q_logical,
                " Vd=", head_size_v, ").");
    TORCH_CHECK(num_heads % num_heads_k == 0,
                "fmha_v3_fwd_f4f4: HQ must be divisible by HK (got HQ=",
                num_heads, " HK=", num_heads_k, ").");
    const int gqa_ratio = num_heads / num_heads_k;
    TORCH_CHECK((gqa_ratio & (gqa_ratio - 1)) == 0,
    ...
    {
        out = out_.value();
        TORCH_CHECK(out.dtype() == torch::kBFloat16, "fmha_v3_fwd_f4f4: out must be bf16.");
        CHECK_DEVICE(out);
        TORCH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
        CHECK_SHAPE(out, batch_size, seqlen_q, num_heads, head_size_v);
    }
    ...
    ck_tile::stream_config stream_config{stream};
    float t = aiter::fmha_fwd_v3_f4f4(a, stream_config);
    TORCH_CHECK(t >= 0, "fmha_v3_fwd_f4f4: dispatcher returned an error code "
                        "(unsupported config or .co not found).");
    return {out};
}

// Dedicated persistent one-wave f4f4 entry. Keep the cooperative f4f4 API
// above unchanged; this sibling shares its tensor ABI and validation but
// always routes to the solo code object/launch contract.
std::vector<at::Tensor>
fmha_v3_fwd_f4f4_solo(at::Tensor& q,
                      const at::Tensor& k,
                      const at::Tensor& v,
                      const at::Tensor& q_descale,
                      const at::Tensor& k_descale,
                      ...
                      std::optional<at::Tensor> out_)
{
    ...
    TORCH_CHECK(is_byte(q.dtype().toScalarType()) && is_byte(k.dtype().toScalarType()),
                "fmha_v3_fwd_f4f4_solo: Q and K must be int8/uint8 (fp4-packed bytes).");
    TORCH_CHECK(is_byte(v.dtype().toScalarType()),
                "fmha_v3_fwd_f4f4_solo: V must be int8/uint8 (mxfp4 E2M1-packed bytes).");
    TORCH_CHECK(is_byte(q_descale.dtype().toScalarType()) &&
                    is_byte(k_descale.dtype().toScalarType()),
                "fmha_v3_fwd_f4f4_solo: Q/K per-block E8M0 scales must be "
                "int8/uint8 byte tensors.");
    TORCH_CHECK(v_descale.dtype() == at::ScalarType::Byte,
                "fmha_v3_fwd_f4f4_solo: V descale must be a uint8 E8M0 block-scale image.");
    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v);
    CHECK_DEVICE(q_descale); CHECK_DEVICE(k_descale); CHECK_DEVICE(v_descale);
    ...
    TORCH_CHECK(head_size_q_logical == ASM_SPARSE_HEAD_DIM &&
                    head_size_v == ASM_SPARSE_HEAD_DIM,
                "fmha_v3_fwd_f4f4_solo: only hd=", ASM_SPARSE_HEAD_DIM,
                " is supported (got logical Qd=", head_size_q_logical,
                " Vd=", head_size_v, ").");
    TORCH_CHECK(num_heads % num_heads_k == 0,
                "fmha_v3_fwd_f4f4_solo: HQ must be divisible by HK (got HQ=",
                num_heads, " HK=", num_heads_k, ").");
    const int gqa_ratio = num_heads / num_heads_k;
    TORCH_CHECK((gqa_ratio & (gqa_ratio - 1)) == 0,
                "fmha_v3_fwd_f4f4_solo: GQA ratio (HQ/HK) must be a power of 2 (got ",
                gqa_ratio, ").");
    ...
    if(out_.has_value())
    {
        out = out_.value();
        TORCH_CHECK(out.dtype() == torch::kBFloat16,
                    "fmha_v3_fwd_f4f4_solo: out must be bf16.");
        CHECK_DEVICE(out);
    ...
    TORCH_CHECK(t >= 0, "fmha_v3_fwd_f4f4_solo: dispatcher returned an error code "
                        "(unsupported config or .co not found).");
    return {out};
}
