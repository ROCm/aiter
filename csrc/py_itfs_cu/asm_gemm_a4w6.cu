...
#include "asm_f4f6gemm_configs.hpp"
#include <cstddef>
#include <cstdint>
...
constexpr size_t kPackedATileBytes  = 16384;
constexpr size_t kPackedBTileBytes  = 24576;
...
static_assert(sizeof(KernelArgs) == 0x180, "a4w6 KernelArgs must be 0x180 bytes");
static_assert(offsetof(KernelArgs, ptr_D) == 0x000, "a4w6 ptr_D ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_A) == 0x020, "a4w6 ptr_A ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_B) == 0x030, "a4w6 ptr_B ABI mismatch");
static_assert(offsetof(KernelArgs, alpha) == 0x040, "a4w6 alpha ABI mismatch");
static_assert(offsetof(KernelArgs, M) == 0x0e0, "a4w6 M ABI mismatch");
static_assert(offsetof(KernelArgs, N) == 0x0f0, "a4w6 N ABI mismatch");
static_assert(offsetof(KernelArgs, K) == 0x100, "a4w6 K ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_ScaleA) == 0x110, "a4w6 ptr_ScaleA ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_ScaleB) == 0x120, "a4w6 ptr_ScaleB ABI mismatch");

CFG* get_cfg(AiterDtype inp_dtype, AiterDtype out_dtype)
{
    if((inp_dtype == AITER_DTYPE_u8) && out_dtype == AITER_DTYPE_bf16)
    {
        return &cfg_f4f6gemm_bf16_per1x32Fp4Fp6;
    }
    else
    {
        AITER_CHECK(false,
...
// A4W6 (MXFP4 E2M1 activations, MXFP6 E2M3 weights, per-1x32 block scales)
// D = A * B^T * alpha
AITER_CTYPES_ERROR_DEF

AITER_CTYPES_DEFINE_ENTRYPOINT_VOID(
    gemm_a4w6_asm,
    (aiter_tensor_t * A,      // A: packed mxfp4 blob (compact C0 layout)
    ...
    (A, B, A_scale, B_scale, out, K, kernelName, alpha, stream))
{
    ...
    AITER_CHECK(!config_map->empty(), __func__, " no kernel support a4w6 for this gpu arch");
    ...
    if(it != config_map->end())
    {
        ...
        SUBN                = cfg.tile_N;
        bdx                 = cfg.block_size;
        AITER_CHECK(SUBM > 0 && SUBN > 0, __func__, " invalid a4w6 tile dimensions");
        AITER_CHECK(bdx > 0, __func__, " invalid a4w6 workgroup size");
        AITER_CHECK(cfg.splitK == 0, __func__, " a4w6 split-K is not supported");
        AITER_CHECK(cfg.pack_layout == kPackLayout,
                    __func__,
                    " unsupported a4w6 packed layout " + cfg.pack_layout);
