...
fmha_fwd_args get_asm_fmha_fwd_args(
    bool has_lse, bool has_dropout_randval, const mask_info &mask,
    // sizes
    const int b, const int seqlen_q, const int seqlen_k, const int h,
    const int h_k, const int d, const int d_v,
    // device pointers
    const at::Tensor q, const at::Tensor k, const at::Tensor v,
    std::optional<const at::Tensor> &bias_,
    std::optional<const at::Tensor> &alibi_slopes_, at::Tensor out,
    at::Tensor softmax_lse, at::Tensor dropout_randval, float softmax_scale,
    float p_dropout, std::pair<uint64_t *, uint64_t *> drop_seed_offset) {
  ...
  ck_tile::index_t stride_randval =
      has_dropout_randval ? dropout_randval.stride(2) : 0;
  ...
  ck_tile::index_t nhead_stride_randval =
      has_dropout_randval ? dropout_randval.stride(1) : 0;
  ...
  ck_tile::index_t batch_stride_randval =
      has_dropout_randval ? dropout_randval.stride(0) : 0;
  ...
  if (bias_.has_value()) {
    ...
    TORCH_CHECK(bias.stride(-1) == 1,
                "bias tensor must have contiguous last dimension");
    TORCH_CHECK(bias.sizes() == torch::IntArrayRef({seqlen_q, seqlen_k}),
                "bias shape should be [sq, sk]");
  ...
  return fmha_fwd_args{q.data_ptr(),
                       ...
                       v.data_ptr(),
                       bias_ptr,
                       has_dropout_randval ? dropout_randval.data_ptr()
                                           : nullptr,
...
fmha_v3_fwd(at::Tensor &q,       // [b, sq, hq, d]
  ...
  TORCH_CHECK(head_size_q <= 256,
              "CK only supports head dimension at most 256");
  TORCH_CHECK(head_size_v <= 256,
              "CK only supports head dimension at most 256");
  TORCH_CHECK(head_size_q % 8 == 0, "query, key, value, and out_ must have a "
                                    "head_size_q that is a multiple of 8");
  TORCH_CHECK(head_size_v % 8 == 0, "query, key, value, and out_ must have a "
                                    "head_size_q that is a multiple of 8");
  TORCH_CHECK(
  ...
  if (is_causal) {
    ...
    std::string mask_identify =
        "b:" + std::to_string(window_size_left) + "," + "0";
    mask = mask_info::decode(mask_identify, seqlen_q, seqlen_k); // casual
  } else if (window_size_left == -1 && window_size_right == -1) {
    mask = mask_info::decode("0", seqlen_q, seqlen_k); // no mask
  } else {
    // Local is the more general case where window_size_right >= 0 or
    // window_size_left >= 0.
  ...
  const int seqlenq_ngroups_swapped =
      ...
      window_size_right < 0 && p_dropout == 0.f && head_size_q % 8 == 0 &&
      !alibi_slopes_.has_value() && !bias_.has_value();
  const int ngroups = num_heads / num_heads_k;
  if (seqlenq_ngroups_swapped) {
    q = q.reshape({batch_size, num_heads_k, ngroups, head_size_q})
            .transpose(1, 2);
    seqlen_q = ngroups;
  ...
  if (out_.has_value()) {
    ...
    if (seqlenq_ngroups_swapped) {
      out = out.reshape({batch_size, num_heads_k, ngroups, head_size_v})
                .transpose(1, 2);
    }
  } else {
  ...
  if (return_dropout_randval) {
    ...
    p = torch::empty({batch_size, num_heads, seqlen_q, seqlen_k},
  ...
  if (p_dropout > 0.0) {
    ...
    hipLaunchKernelGGL(aiter::ParsePhiloxCudaState, dim3(1), dim3(64), 0, 0,
                       philox_args, rng_state_ptr);
  }
  ...
  if (seqlen_k > 0) {
    ...
    auto args = get_asm_fmha_fwd_args(
        has_lse, return_dropout_randval, mask, batch_size, seqlen_q, seqlen_k,
        num_heads, num_heads_k, head_size_q, head_size_v, q, k, v, bias_,
        alibi_slopes_, out, softmax_lse, p, softmax_scale, p_dropout,
        drop_seed_offset);

    float t = aiter::mha_fwd(args, stream_config, q_dtype_str,
                             false, // is_group_mode
                             mask.type, bias_type, has_lse, true);
  ...
  if (seqlenq_ngroups_swapped) {
    out = out.transpose(1, 2).reshape(
        {batch_size, 1, num_heads_k * seqlen_q, head_size_q});
