...
#include "asm_f6f4gemm_configs.hpp"
#include <cstddef>
#include <cstdint>
...
constexpr size_t kPackedATileBytes  = 24576;
constexpr size_t kPackedBTileBytes  = 16384;
constexpr size_t kScaleTileBytes    = 1024;
constexpr size_t kMaxBufferBytes    = size_t{1} << 31;
constexpr size_t kTensorAlignment   = 16;
constexpr char kPackLayout[]        = "mxfp6_mxfp4_c0_256_padk2";
constexpr char kDefaultKernelName[] = "_ZN5aiter39f6f4gemm_bf16_per1x32Fp6Fp4_m32_s0_a4_tE";

// KernelArgs layout is identical to the a4w4 asm gemm ABI; the mixed A6W4
// kernel was assembled against the same kernarg struct (0x180 bytes). Fields
// it does not consume (ptr_C, beta, A/B strides, k-split) are left
...
static_assert(sizeof(KernelArgs) == 0x180, "a6w4 KernelArgs must be 0x180 bytes");
static_assert(offsetof(KernelArgs, ptr_D) == 0x000, "a6w4 ptr_D ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_A) == 0x020, "a6w4 ptr_A ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_B) == 0x030, "a6w4 ptr_B ABI mismatch");
static_assert(offsetof(KernelArgs, alpha) == 0x040, "a6w4 alpha ABI mismatch");
static_assert(offsetof(KernelArgs, M) == 0x0e0, "a6w4 M ABI mismatch");
static_assert(offsetof(KernelArgs, N) == 0x0f0, "a6w4 N ABI mismatch");
static_assert(offsetof(KernelArgs, K) == 0x100, "a6w4 K ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_ScaleA) == 0x110, "a6w4 ptr_ScaleA ABI mismatch");
static_assert(offsetof(KernelArgs, ptr_ScaleB) == 0x120, "a6w4 ptr_ScaleB ABI mismatch");

CFG* get_cfg(AiterDtype inp_dtype, AiterDtype out_dtype)
{
    if((inp_dtype == AITER_DTYPE_u8) && out_dtype == AITER_DTYPE_bf16)
    {
        return &cfg_f6f4gemm_bf16_per1x32Fp6Fp4;
    }
    else
    {
        AITER_CHECK(false,
...
// A6W4 (MXFP6 E2M3 activations, MXFP4 E2M1 weights, per-1x32 block scales)
// D = A * B^T * alpha
...
AITER_CTYPES_DEFINE_ENTRYPOINT_VOID(
    ...
    (aiter_tensor_t * A,      // A: packed mxfp6 blob (pack_big layout)
     aiter_tensor_t* B,       // B: packed mxfp4 blob (compact C0 layout)
    ...
    (A, B, A_scale, B_scale, out, K, kernelName, alpha, bias, stream))
{
    ...
    AITER_CHECK(A->is_gpu() && B->is_gpu() && A_scale->is_gpu() && B_scale->is_gpu() &&
                    out->is_gpu(),
                __func__,
                " all tensors must be GPU tensors");
    ...
    const int64_t M64 = out->size(0);
    const int64_t N64 = out->size(1);
    AITER_CHECK(M64 <= 2147483647LL && N64 <= 2147483647LL,
                __func__,
                " output dimensions exceed the int32 kernel ABI");
    int Mdim = static_cast<int>(M64);
    int Ndim = static_cast<int>(N64);
    AITER_CHECK(K > 0 && K <= 2147483647LL, __func__, " K is outside the int32 kernel ABI");
    int Kdim = static_cast<int>(K);
    AITER_CHECK(
    ...
    const auto is_aligned = [](const aiter_tensor_t* tensor) {
        return reinterpret_cast<uintptr_t>(tensor->ptr) % kTensorAlignment == 0;
    };
    AITER_CHECK(is_aligned(A) && is_aligned(B) && is_aligned(A_scale) && is_aligned(B_scale) &&
                    is_aligned(out),
                __func__,
                " all tensor addresses must be aligned to 16 bytes");

    const size_t nk_pad     = static_cast<size_t>(Kdim / kKTileSize + kKPaddingTiles);
    const size_t expected_A =
        static_cast<size_t>(Mdim / kTileSize) * nk_pad * kPackedATileBytes;
    const size_t expected_B =
        static_cast<size_t>(Ndim / kTileSize) * nk_pad * kPackedBTileBytes;
    ...
    const auto fits_srd = [](const aiter_tensor_t* tensor) {
        return tensor->numel() <= kMaxBufferBytes / tensor->element_size();
    };
    AITER_CHECK(fits_srd(A) && fits_srd(B) && fits_srd(A_scale) && fits_srd(B_scale) &&
                    fits_srd(out),
                __func__,
    ...
    AITER_CHECK(!config_map->empty(), __func__, " no kernel support a6w4 for this gpu arch");
    ...
    if(it != config_map->end())
    {
        ...
        SUBN                = cfg.tile_N;
        bdx                 = cfg.block_size;
        AITER_CHECK(SUBM > 0 && SUBN > 0, __func__, " invalid a6w4 tile dimensions");
        AITER_CHECK(bdx > 0, __func__, " invalid a6w4 workgroup size");
        AITER_CHECK(cfg.splitK == 0, __func__, " a6w4 split-K is not supported");
        AITER_CHECK(cfg.pack_layout == kPackLayout,
                    __func__,
                    " unsupported a6w4 packed layout " + cfg.pack_layout);
