...
fmha_bwd_args get_asm_fmha_bwd_args(
...
fmha_v3_bwd(const at::Tensor &dout,        // [b, sq, hq, d_v]
  ...
  if (!deterministic) {
    dq_accum = torch::zeros({1, batch_size, num_heads, seqlen_q, head_size_v},
    ...
    dq_accum =
        torch::zeros({nsplits, batch_size, num_heads, seqlen_q, head_size_v},
                     opts.dtype(at::kFloat));
  }
  ...
  if (num_heads_k != num_heads) { // MQA / GQA
    dk_expanded =
        torch::empty({batch_size, seqlen_k, num_heads, head_size_q}, opts);
    dv_expanded =
        torch::empty({batch_size, seqlen_k, num_heads, head_size_v}, opts);
  } else {
    dk_expanded = dk;
    dv_expanded = dv;
  }
  ...
  if (seqlen_q > 0) {
    ...
    auto args = get_asm_fmha_bwd_args(
        mask, batch_size, seqlen_q, seqlen_k, num_heads, num_heads_k,
        head_size_q, head_size_v, q, k, v, alibi_slopes_, out, softmax_lse,
        dout, dq_accum, softmax_d, dq, dk_expanded, dv_expanded, softmax_scale,
        p_dropout, drop_seed_offset);
  ...
  if (num_heads_k != num_heads) {
    at::sum_out(
        ...
        at::reshape(dk_expanded, {batch_size, seqlen_k, num_heads_k,
                                  num_heads / num_heads_k, head_size_q}),
        {3});
    at::sum_out(
        dv,
        at::reshape(dv_expanded, {batch_size, seqlen_k, num_heads_k,
                                  num_heads / num_heads_k, head_size_v}),
        {3});
  }
