...
//
// Provides:
//   * opus_dispatch_a16w16_gfx942<T>      -- tuned (M,N,K) lookup -> heuristic
//   * opus_a16w16_tune_dispatch_gfx942<T> -- id-based tune dispatch
//
// Mirrors gfx950/opus_gemm_arch_gfx950.cuh. Included by opus_gemm.cu when
// the build targets gfx942 (controlled by opus_gemm_arch_config.h).
#pragma once
...
opus_dispatch_a16w16_gfx942(int M, int N, int K, int batch);

template <>
inline OpusA16W16NoscaleKernel
opus_dispatch_a16w16_gfx942<bf16_t>(int M, int N, int K, int batch)
{
    ...
    return opus_a16w16_heuristic_dispatch_gfx942<bf16_t>(M, N, K, batch);
}

template <>
inline OpusA16W16NoscaleKernel
opus_dispatch_a16w16_gfx942<fp32_t>(int M, int N, int K, int batch)
{
    using namespace opus_gfx942_detail;
    static constexpr OpusA16W16RuntimeEntry kLookup[] = {
    ...
    return opus_a16w16_heuristic_dispatch_gfx942<fp32_t>(M, N, K, batch);
}
