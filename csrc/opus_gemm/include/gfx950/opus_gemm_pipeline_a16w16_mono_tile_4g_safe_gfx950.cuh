...
void gemm_a16w16_mono_tile_4g_safe_kernel_gfx950(opus_gemm_mono_tile_kargs_gfx950 kargs) {
    ...
    unsigned int c_size_bytes = (bm_eff > 0 && bn_eff > 0)
        ...
        : 0u;
    // 64-bit index arithmetic (see persistent/_4g_safe sibling for rationale).
    ...
    auto g_c = make_gmem(reinterpret_cast<D_C*>(kargs.ptr_c) + (size_t)batch_id * kargs.stride_c_batch + (size_t)row * kargs.stride_c, c_size_bytes);
