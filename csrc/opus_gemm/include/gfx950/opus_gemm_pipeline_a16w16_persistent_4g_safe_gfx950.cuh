...
//
// Buffer-resource (BR) sized per-WG tile rectangle so the 32-bit num_records never wraps (safe for >4 GiB tensors).
// Legacy persistent's C size used the full row-band remaining, which wraps when M*N is huge; here we tighten to the
// per-tile rectangle:
...
// where bm_eff/bn_eff clamp to the current tile_m's actual rectangle.
// C base biases by +row*stride_c + col (same as legacy).
//
...
gemm_a16w16_persistent_4g_safe_kernel(opus_gemm_persistent_kargs_gfx950 kargs) {
    ...
    // side effect.
    // 4g_safe: clamp to the per-tile rectangle (was full (n-col)*stride_b row-band which wraps for big shapes). tile_n
    // is fixed for this WG so bn_eff is computed once at entry, clamped to T::B_N.
    ...
    unsigned int b_size_bytes = (bn_eff > 0)
        ...
        : 0u;
    // 64-bit index arithmetic: col*stride_b, batch_id*stride_b_batch can
    // exceed INT_MAX on >4 GiB shapes. Promote to size_t to avoid signed
    // overflow before the pointer add.
    auto g_b = make_gmem(
    ...
    for (int tile_m = tile_m_lo; tile_m < tile_m_hi; ++tile_m) {
        ...
        // 4g_safe: clamp to per-tile rectangle. C base biases by +col, so size spans (bm_eff-1)*stride_c + bn_eff (<=
        // UINT32_MAX for sane tiles); bn_eff set at entry (tile_n fixed across the outer loop).
