...
//
// Each splitk launcher needs a stable `opus_splitk_ws_handle*` for both the
// main and reduce kernels; captured HIP graphs bake in that pointer. The old
// `static thread_local` slot broke under vLLM/sglang TBO (two threads/streams
// concurrent, each graph needs its own baked pointer; TLS also tripped the
// in-capture grow guard on the second thread).
//
// Now owned by stream: a process-global mutex-protected map keyed by
// hipStream_t. Eager: lazy-create on first lookup. Capture: caller must
// pre-register via opus_gemm_workspace_init() (else the lookup throws, cleaner
// than the prior SIGABRT); the framework calls it once per TBO stream eagerly.
namespace {
