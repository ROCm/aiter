...
namespace aiter
{
    namespace torch_itfs
    {
                       ...
                       std::optional<at::Generator> gen_)
        {
            ...
            bias_enum bias_type = bias_.has_value() ? bias_enum::elementwise_bias : alibi_slopes_.has_value() ? bias_type = bias_enum::alibi
                                                                                                              : bias_enum::no_bias;
                ...
                else
                {
                    ...
                    auto args =
                        get_ck_fmha_varlen_fwd_args(
                            ...
                            p_dropout,
                            drop_seed_offset);
                    std::cout << "head_size_q: " << head_size_q << std::endl
                              << "head_size_v: " << head_size_v << std::endl
                              << "has_logits_soft_cap: " << (args.logits_soft_cap > 0.f) << std::endl
                              << "mask_type: " << static_cast<int>(mask.type) << std::endl
                              << "bias_type: " << static_cast<int>(bias_type) << std::endl
                              << "has_lse: " << has_lse << std::endl
                              << "has_dropout: " << (args.p_drop > 0.f) << std::endl
                              << "use_ext_asm: " << false << std::endl;
