...
hipError_t SamplingFromLogits(T* logits,
                              IdType* output,
                              IdType* indices,
                              uint32_t batch_size,
                              uint32_t d,
                              ...
                              hipStream_t stream = 0)
{
    ...
    DISPATCH_ALIGNED_VEC_SIZE(
        vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
            auto kernel = SamplingFromLogitsKernel<BLOCK_THREADS,
                                                   SCAN_ALGO,
                                                   REDUCE_ALGO,
                                                   VEC_SIZE,
                                                   DETERMINISTIC,
...
template <typename T, typename IdType>
hipError_t SamplingFromProb(T* probs,
                            IdType* output,
                            IdType* indices,
                            uint32_t batch_size,
                            uint32_t d,
                            ...
                            hipStream_t stream = 0)
{
    ...
    DISPATCH_ALIGNED_VEC_SIZE(
        vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
            auto kernel = SamplingFromProbKernel<BLOCK_THREADS,
                                                 SCAN_ALGO,
                                                 REDUCE_ALGO,
                                                 VEC_SIZE,
                                                 DETERMINISTIC,
                                ...
                                hipStream_t stream = 0)
{
    ...
    dim3 nthrs(BLOCK_THREADS);
    void* args[] = {
        &probs, &output, &indices, &top_k_arr, &top_k_val, &d, &philox_seed, &philox_offset};
...
template <uint32_t BLOCK_THREADS,
          ...
          uint32_t VEC_SIZE,
          typename DType>
__global__ void TopPRenormProbKernel(
...
hipError_t TopPRenormProb(DType* probs,
                          DType* renormed_prob,
                          float* top_p_arr,
                          uint32_t batch_size,
                          float top_p_val,
