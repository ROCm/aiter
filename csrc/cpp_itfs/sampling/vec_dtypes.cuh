...
#pragma once
 ...
 Hacky workaround for the error below:
   ...
   /home/git_repos/glen-amd/flashinfer/include/flashinfer/attention/../vec_dtypes_hip.cuh:200:38: error: use of undeclared identifier '__float2bfloat162_rn'; did you mean '__float22bfloat162_rn'?
     200 |       const __hip_bfloat162 bias_reg = __float2bfloat162_rn(*reinterpret_cast<const float*>(&BIAS));
         |                                      ^~~~~~~~~~~~~~~~~~~~
 ...
 */
 ...
 inline __attribute__((always_inline)) __device__ __hip_bfloat162 make_bfloat162(const __hip_bfloat16 x, const __hip_bfloat16 y) {
   __hip_bfloat162 t;
   t.x = x;
   t.y = y;
   return t;
   ...
   if constexpr (std::is_same_v<fp8_dtype, __hip_fp8_e5m2_fnuz> && std::is_same_v<fp16_dtype, half>) {
     ...
     if constexpr (std::is_same_v<fp16_dtype, half>) {
       ...
       *(half2*)&(output->y) = __hmul2(*reinterpret_cast<const half2*>(&Out2), bias_reg);
       ...
       const __hip_bfloat162 bias_reg = __float2bfloat162_rn(*reinterpret_cast<const float*>(&BIAS));
 ...
 struct vec_cast<__hip_bfloat16, __hip_fp8_e4m3_fnuz> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(__hip_bfloat16* dst, const __hip_fp8_e4m3_fnuz* src) {
     if constexpr (vec_size == 1) {
 ...
 struct vec_cast<__hip_bfloat16, __hip_fp8_e5m2_fnuz> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(__hip_bfloat16* dst, const __hip_fp8_e5m2_fnuz* src) {
     if constexpr (vec_size == 1) {
       dst[0] = __hip_bfloat16(src[0]);
     } else if constexpr (vec_size == 2) {
       dst[0] = __hip_bfloat16(src[0]);
 ...
 struct vec_cast<__hip_fp8_e4m3_fnuz, half> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(__hip_fp8_e4m3_fnuz* dst, const half* src) {
 ...
 struct vec_cast<__hip_fp8_e5m2_fnuz, half> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(__hip_fp8_e5m2_fnuz* dst, const half* src) {
 #ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
     if constexpr (vec_size == 1) {
 ...
 struct vec_cast<half, __hip_fp8_e4m3_fnuz> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(half* dst, const __hip_fp8_e4m3_fnuz* src) {
 #ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
     if constexpr (vec_size == 1) {
       dst[0] = half(src[0]);
     } else {
 ...
 struct vec_cast<half, __hip_fp8_e5m2_fnuz> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(half* dst, const __hip_fp8_e5m2_fnuz* src) {
 #ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
     if constexpr (vec_size == 1) {
       dst[0] = half(src[0]);
     } else {
 ...
 struct vec_cast<float, __hip_bfloat16> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(float* dst, const __hip_bfloat16* src) {
     if constexpr (vec_size == 1) {
       dst[0] = (float)src[0];
     } else {
 #pragma unroll
 ...
 struct vec_cast<__hip_bfloat16, float> {
   template <size_t vec_size>
   inline __attribute__((always_inline)) __device__ static void cast(__hip_bfloat16* dst, const float* src) {
     /*if constexpr (vec_size == 1) {
       dst[0] = __hip_bfloat16(src[0]);
     } else {
 #pragma unroll
 ...
 struct vec_t {
   ...
   inline __attribute__((always_inline)) __device__ static void memcpy(float_t* dst, const float_t* src);
 ...
 template <typename src_float_t, typename tgt_float_t, size_t vec_size>
 inline __attribute__((always_inline)) __device__ void cast_load_impl(vec_t<tgt_float_t, vec_size>& dst,
 ...
 template <typename src_float_t, typename tgt_float_t, size_t vec_size>
 inline __attribute__((always_inline)) __device__ void cast_store_impl(tgt_float_t* dst_ptr,
                                        const vec_t<src_float_t, vec_size>& src) {
   if constexpr (std::is_same_v<src_float_t, tgt_float_t>) {
     src.store(dst_ptr);
   } else {
 ...
 struct vec_t<__hip_fp8_e4m3_fnuz, 1> {
   ...
   inline __attribute__((always_inline)) __device__ __hip_fp8_e4m3_fnuz& operator[](size_t i) { return ((__hip_fp8_e4m3_fnuz*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ const __hip_fp8_e4m3_fnuz& operator[](size_t i) const {
     return ((const __hip_fp8_e4m3_fnuz*)(&data))[i];
   }
   inline __attribute__((always_inline)) __device__ __hip_fp8_e4m3_fnuz* ptr() { return reinterpret_cast<__hip_fp8_e4m3_fnuz*>(&data); }
   ...
   inline __attribute__((always_inline)) __device__ static void memcpy(__hip_fp8_e4m3_fnuz* dst, const __hip_fp8_e4m3_fnuz* src);
 };
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 1>::fill(__hip_fp8_e4m3_fnuz val) { data = val; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 1>::load(const __hip_fp8_e4m3_fnuz* ptr) { data = *ptr; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 1>::store(__hip_fp8_e4m3_fnuz* ptr) const { *ptr = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 1>::memcpy(__hip_fp8_e4m3_fnuz* dst,
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 2>::fill(__hip_fp8_e4m3_fnuz val) {
   data.__x = (__hip_fp8x2_storage_t(val.__x) << 8) | __hip_fp8x2_storage_t(val.__x);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 2>::load(const __hip_fp8_e4m3_fnuz* ptr) {
   data = *((__hip_fp8x2_e4m3_fnuz*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 2>::store(__hip_fp8_e4m3_fnuz* ptr) const {
   *((__hip_fp8x2_e4m3_fnuz*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 2>::memcpy(__hip_fp8_e4m3_fnuz* dst,
                                                        const __hip_fp8_e4m3_fnuz* src) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 4>::fill(__hip_fp8_e4m3_fnuz val) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 4>::load(const __hip_fp8_e4m3_fnuz* ptr) {
   data = *((__hip_fp8x4_e4m3_fnuz*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 4>::store(__hip_fp8_e4m3_fnuz* ptr) const {
   *((__hip_fp8x4_e4m3_fnuz*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 4>::memcpy(__hip_fp8_e4m3_fnuz* dst,
                                                        const __hip_fp8_e4m3_fnuz* src) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 8>::fill(__hip_fp8_e4m3_fnuz val) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 8>::load(const __hip_fp8_e4m3_fnuz* ptr) {
   data = *((uint2*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 8>::store(__hip_fp8_e4m3_fnuz* ptr) const {
   *((uint2*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e4m3_fnuz, 8>::memcpy(__hip_fp8_e4m3_fnuz* dst,
                                                        const __hip_fp8_e4m3_fnuz* src) {
   *((uint2*)dst) = *((uint2*)src);
 }
 ...
 struct vec_t<__hip_fp8_e5m2_fnuz, 1> {
   ...
   inline __attribute__((always_inline)) __device__ __hip_fp8_e5m2_fnuz& operator[](size_t i) { return ((__hip_fp8_e5m2_fnuz*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ const __hip_fp8_e5m2_fnuz& operator[](size_t i) const {
     return ((const __hip_fp8_e5m2_fnuz*)(&data))[i];
   }
   inline __attribute__((always_inline)) __device__ __hip_fp8_e5m2_fnuz* ptr() { return reinterpret_cast<__hip_fp8_e5m2_fnuz*>(&data); }
   ...
   inline __attribute__((always_inline)) __device__ static void memcpy(__hip_fp8_e5m2_fnuz* dst, const __hip_fp8_e5m2_fnuz* src);
 };
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 1>::fill(__hip_fp8_e5m2_fnuz val) { data = val; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 1>::load(const __hip_fp8_e5m2_fnuz* ptr) { data = *ptr; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 1>::store(__hip_fp8_e5m2_fnuz* ptr) const { *ptr = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 1>::memcpy(__hip_fp8_e5m2_fnuz* dst,
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 2>::fill(__hip_fp8_e5m2_fnuz val) {
   data.__x = (__hip_fp8x2_storage_t(val.__x) << 8) | __hip_fp8x2_storage_t(val.__x);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 2>::load(const __hip_fp8_e5m2_fnuz* ptr) {
   data = *((__hip_fp8x2_e5m2_fnuz*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 2>::store(__hip_fp8_e5m2_fnuz* ptr) const {
   *((__hip_fp8x2_e5m2_fnuz*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 2>::memcpy(__hip_fp8_e5m2_fnuz* dst,
                                                        const __hip_fp8_e5m2_fnuz* src) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 4>::fill(__hip_fp8_e5m2_fnuz val) {
   data.__x = (__hip_fp8x4_storage_t(val.__x) << 24) | (__hip_fp8x4_storage_t(val.__x) << 16) |
              (__hip_fp8x4_storage_t(val.__x) << 8) | __hip_fp8x4_storage_t(val.__x);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 4>::load(const __hip_fp8_e5m2_fnuz* ptr) {
   data = *((__hip_fp8x4_e5m2_fnuz*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 4>::store(__hip_fp8_e5m2_fnuz* ptr) const {
   *((__hip_fp8x4_e5m2_fnuz*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 4>::memcpy(__hip_fp8_e5m2_fnuz* dst,
                                                        const __hip_fp8_e5m2_fnuz* src) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 8>::fill(__hip_fp8_e5m2_fnuz val) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 8>::load(const __hip_fp8_e5m2_fnuz* ptr) {
   data = *((uint2*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 8>::store(__hip_fp8_e5m2_fnuz* ptr) const {
   *((uint2*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_fp8_e5m2_fnuz, 8>::memcpy(__hip_fp8_e5m2_fnuz* dst,
                                                        const __hip_fp8_e5m2_fnuz* src) {
   *((uint2*)dst) = *((uint2*)src);
 }
 ...
 struct vec_t<half, 1> {
   ...
   inline __attribute__((always_inline)) __device__ half& operator[](size_t i) { return ((half*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ const half& operator[](size_t i) const { return ((const half*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ half* ptr() { return reinterpret_cast<half*>(&data); }
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<half, 1>::load(const half* ptr) { data = *ptr; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 1>::store(half* ptr) const { *ptr = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 1>::memcpy(half* dst, const half* src) { *dst = *src; }
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<half, 2>::fill(half val) { data = make_half2(val, val); }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 2>::load(const half* ptr) { data = *((half2*)ptr); }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 2>::store(half* ptr) const { *((half2*)ptr) = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 2>::memcpy(half* dst, const half* src) {
   *((half2*)dst) = *((half2*)src);
 }
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<half, 4>::load(const half* ptr) { data = *((uint2*)ptr); }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 4>::store(half* ptr) const { *((uint2*)ptr) = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<half, 4>::memcpy(half* dst, const half* src) {
   *((uint2*)dst) = *((uint2*)src);
 }
 ...
 struct vec_t<__hip_bfloat16, 1> {
   __hip_bfloat16 data;
   inline __attribute__((always_inline)) __device__ __hip_bfloat16& operator[](size_t i) { return ((__hip_bfloat16*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ const __hip_bfloat16& operator[](size_t i) const {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 1>::fill(__hip_bfloat16 val) { data = val; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 1>::load(const __hip_bfloat16* ptr) { data = *ptr; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 1>::store(__hip_bfloat16* ptr) const { *ptr = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 1>::memcpy(__hip_bfloat16* dst, const __hip_bfloat16* src) {
   *dst = *src;
 }
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 2>::fill(__hip_bfloat16 val) {
   data = make_bfloat162(val, val);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 2>::load(const __hip_bfloat16* ptr) {
   data = *((__hip_bfloat162*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 2>::store(__hip_bfloat16* ptr) const {
   *((__hip_bfloat162*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 2>::memcpy(__hip_bfloat16* dst, const __hip_bfloat16* src) {
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 4>::fill(__hip_bfloat16 val) {
   *(__hip_bfloat162*)(&data.x) = make_bfloat162(val, val);
   *(__hip_bfloat162*)(&data.y) = make_bfloat162(val, val);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 4>::load(const __hip_bfloat16* ptr) {
   data = *((uint2*)ptr);
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 4>::store(__hip_bfloat16* ptr) const {
   *((uint2*)ptr) = data;
 }
 
 inline __attribute__((always_inline)) __device__ void vec_t<__hip_bfloat16, 4>::memcpy(__hip_bfloat16* dst, const __hip_bfloat16* src) {
   *((uint2*)dst) = *((uint2*)src);
 }
 ...
 struct vec_t<float, 1> {
   ...
   inline __attribute__((always_inline)) __device__ float& operator[](size_t i) { return ((float*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ const float& operator[](size_t i) const { return ((const float*)(&data))[i]; }
   inline __attribute__((always_inline)) __device__ float* ptr() { return reinterpret_cast<float*>(&data); }
   ...
   inline __attribute__((always_inline)) __device__ static void memcpy(float* dst, const float* src);
 };
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 1>::fill(float val) { data = val; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 1>::load(const float* ptr) { data = *ptr; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 1>::store(float* ptr) const { *ptr = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 1>::memcpy(float* dst, const float* src) { *dst = *src; }
 ...
 inline __attribute__((always_inline)) __device__ void vec_t<float, 2>::fill(float val) { data = make_float2(val, val); }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 2>::load(const float* ptr) { data = *((float2*)ptr); }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 2>::store(float* ptr) const { *((float2*)ptr) = data; }
 
 inline __attribute__((always_inline)) __device__ void vec_t<float, 2>::memcpy(float* dst, const float* src) {
