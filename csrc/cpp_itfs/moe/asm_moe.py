...
def select_tile(input_dtype:str, gate_fusion:str, gate_dtype:str, inter_dim:int, max_num_m_blocks:int, num_cu:int, a16:bool=False, fc_scale_blkn:int=None, fc_scale_blkk:int=None):
    if a16:
        if gate_fusion == "g1u1":
            if gate_dtype == "uint8_t":
                if inter_dim % 320 == 0:
                    ...
                    raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 320")
            elif gate_dtype == "__hip_fp8_e4m3_fnuz":
                selected_tile = get_heuristic_tile(inter_dim, max_num_m_blocks, [512, 320], num_cu)
                if selected_tile == 0:
                    raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 320 or 512")
                return selected_tile
    else:
        if gate_fusion == "g1u0":
            ...
            for tile in tiles:
                if inter_dim % tile == 0:
                    ...
                    break
            if selected_tile == 0:
                raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 128, 192, 256, 320, 384, 448 or 512")
            return selected_tile
        elif gate_fusion == "g1u1":
            if gate_dtype in ["uint32_t", "int32_t"]:
                selected_tile = get_heuristic_tile(inter_dim, max_num_m_blocks, [512, 256, 128], num_cu)
                if selected_tile == 0:
                    raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 128, 256 or 512")
                return selected_tile
            elif input_dtype == "uint8_t":
                selected_tile = get_heuristic_tile(inter_dim, max_num_m_blocks, [512, 448, 384, 320, 256, 192, 128], num_cu)
                if selected_tile == 0:
                    raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 128, 192, 256, 320, 384, 448 or 512")
                return selected_tile
                ...
                if inter_dim % 256 == 0 and fc_scale_blkn == 128 and fc_scale_blkk == 128:
                    return 256
                else:
                    raise ValueError(f"Unsupported inter_dim {inter_dim}, which should be divisible by 256 and fc_scale_blkn and fc_scale_blkk should be 128")
    return 512


def select_hsaco(input_dtype:str, gate_fusion:str, gate_dtype:str, activation:str, selected_tile:int, is_smooth_scale=False, a16=False, enable_vskip=False):
    if a16:
        if gate_fusion == "g1u1":
            if gate_dtype == "uint8_t":
                return f"fmoe_int8_g1u1_smf_subGU_{selected_tile}", f"fmoe_int8_g1u1_smf_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "true"
            elif gate_dtype == "__hip_fp8_e4m3_fnuz":
                return f"fmoe_fp8_g1u1_smf_subGU_{selected_tile}", f"fmoe_fp8_g1u1_smf_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "true"
        ...
        if gate_fusion == "g1u0": 
            if input_dtype == "_Float16":
                ...
                if activation == "gelu":
                    return f"fmoe_int8_g1u0_subGU_{selected_tile}_gelu", f"moe/gelu/fmoe_int8_g1u0_subGU_{selected_tile}_gelu.co", "uint8_t", "uint16_t", "false"
                elif activation == "silu":
                    return f"fmoe_int8_g1u0_subGU_{selected_tile}", f"fmoe/silu/fmoe_int8_g1u0_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
                
        elif gate_fusion == "g1u1":
            if gate_dtype in ["uint32_t", "int32_t"]:
                return f"fmoe_int4fp8_g1u1_subGU_{selected_tile}", f"fmoe_int4fp8_g1u1_subGU_{selected_tile}_gelu.co", "uint8_t", "uint16_t", "false"
            elif input_dtype == "uint8_t":
                if is_smooth_scale:
                    return f"fmoe_int8_g1u1_multix_subGU_{selected_tile}", f"fmoe_int8_g1u1_multix_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
                elif activation == "gelu":
                    return f"fmoe_int8_g1u1_subGU_{selected_tile}_gelu", f"fmoe/gelu/fmoe_int8_g1u1_subGU_{selected_tile}_gelu.co", "uint8_t", "uint16_t", "false"
                elif activation == "silu":
                    return f"fmoe_int8_g1u1_subGU_{selected_tile}", f"fmoe/silu/fmoe_int8_g1u1_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
            elif input_dtype == "__hip_fp8_e4m3_fnuz":
                if is_smooth_scale:
                    return f"fmoe_fp8_g1u1_multix_subGU_{selected_tile}", f"fmoe_fp8_g1u1_multix_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
                elif activation == "gelu":
                    return f"fmoe_fp8_g1u1_subGU_{selected_tile}_gelu", f"fmoe/gelu/fmoe_fp8_g1u1_subGU_{selected_tile}_gelu.co", "uint8_t", "uint16_t", "false"
                elif activation == "silu":
                    return f"fmoe_fp8_g1u1_subGU_{selected_tile}", f"fmoe/silu/fmoe_fp8_g1u1_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
            elif input_dtype == "__hip_bfloat16":
                if enable_vskip:
                    return f"fmoe_fp8_blockscale_g1u1_subGU_{selected_tile}", f"fmoe_fp8_blockscale_g1u1_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
                else:
                    return f"fmoe_fp8_blockscale_g1u1_novs_subGU_{selected_tile}", f"fmoe_fp8_blockscale_g1u1_novs_subGU_{selected_tile}.co", "uint8_t", "uint16_t", "false"
...
def asm_moe(hidden_states, # [num_tokens, dim] M,K
    ...
    if fc1_smooth_scale is None:
        ...
        if w1.dtype in [torch.float8_e4m3_fnuz, torch.int8] and inter_dim * 2 == w1.shape[1]:
            gate_fusion = "g1u1"
        elif w1.dtype == torch.int8 and inter_dim == w1.shape[1]:
            gate_fusion = "g1u0"
        else:
            raise ValueError(f"Unsupported w1.dtype {w1.dtype}, which should be {torch.float8_e4m3_fnuz} or {torch.int8} and w1.shape[1] should be {inter_dim} or {inter_dim*2}")
    else:
