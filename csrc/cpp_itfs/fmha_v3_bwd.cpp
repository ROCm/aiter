                                                                       ...
                                                                       CFG* post_cfgs)
{
    ...
    for(const auto& el : *pre_cfgs)
    {
        ...
        if(cfg.dtype == data_type && cfg.hdim_q == hdim_q && cfg.mode == mode)
        {
...
float mha_bwd(mha_bwd_args a)
{
    if (a.use_asm_v3 == 1) {
        ...
        if (a.use_asm_v3 == 0) {
            ...
            if(asm_ret == -1)
            {
                return fmha_bwd(traits, ck_args, a.stream);
            }
        } else {
            return fmha_bwd(traits, ck_args, a.stream);
        }
...
float fmha_v3_bwd(mha_bwd_args a)
{
    ...
    auto post_cfgs      = &cfg_fmha_bwd_dq_convert;
