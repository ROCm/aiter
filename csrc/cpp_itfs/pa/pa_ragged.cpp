                            ...
                            const hipStream_t stream)
{
    ...
    const int npar_loops = DIVIDE_ROUND_UP(max_num_partitions, get_warp_size_func());
    ...
    // Argument order must match the signature generated by pa_ragged.cpp.jinja
    // (and the Python driver in pa_ragged.py), which is:
    //   out, workspace, query, key, value,
    //   kv_indptr, kv_page_indices, kv_last_page_lens,
    //   alibi, q_scale, k_scale, v_scale, fp8_out_scale,
    //   scale, logits_soft_cap,
    //   num_seqs, num_kv_heads, num_heads, max_num_partitions,
    //   q_stride, kv_block_stride, kv_head_stride, kv_seq_stride, stream
    run_lib(func_name,
            folder.value(),
            out_ptr,
            ...
            alibi_slopes_ptr,
            // q_scale: this C++ entry point does not expose it yet; the Python
            // driver passes a null pointer when q_scale is None.
