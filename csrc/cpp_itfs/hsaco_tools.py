...
def hip_check_error(err, *args):
    if err != 0:
        raise Exception("HIP error:" + get_amdhip().hipGetErrorString(err).decode("utf-8") + repr(args))

@functools.cache
def get_lib(lib_fpath):
    hip = get_amdhip()
    p_lib = ctypes.c_void_p()
    hip_check_error(hip.hipLibraryLoadFromFile(ctypes.byref(p_lib), lib_fpath.encode('utf-8'), None, None,0, None,None, 0), lib_fpath)
    return p_lib
