...
3. Test the performance, modify the test instance in `op_tests/gemm/test_gemm_a8w8.py` and run it, please wait a few minutes as it will build gemm_a8w8_bpreshuffle_cktile kernels in `aiter/configs/a8w8_bpreshuffle_tuned_gemm.csv` via jit：  
`FLATMM_HIP_CLANG_PATH=/data/llvm-project/build/bin/ python3 op_tests/gemm/test_gemm_a8w8.py`
