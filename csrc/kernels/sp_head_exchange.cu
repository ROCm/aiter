...
template <int W>
__global__ void __launch_bounds__(512, 1) sp_head_exchange_kernel(RankData* dp,
                                                                  RankSignals signals,
                                                                  Signal* self,
                                                                  uint4* output,
                                                                  int rank,
                                                                  ...
                                                                  int width_packs)
{
    ...
    for(int i = blockIdx.x * threads_per_peer + lane; i < local_packs;
        i += gridDim.x * threads_per_peer)
    {
...
void sp_head_exchange(int64_t handle,
                      const aiter_tensor_t& input,
                      const aiter_tensor_t& output,
                      int64_t registered_buffer,
                      ...
                      int64_t blocks)
{
    ...
    sp_head_exchange_kernel<4>
        <<<blocks, 512, 0, stream>>>(pointers,
                                     comm->sg_,
                                     comm->self_sg_,
