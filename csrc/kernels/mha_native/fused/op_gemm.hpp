...
//
//   GEMM0:  S_acc = Q . K^T   (scores)                 -> feeds softmax
//   GEMM1:  O_acc += P . V    (attention-weighted sum) -> feeds epilogue
//   Both run on v_mfma_f32_32x32x8_bf16 (CDNA3 32x32x8 bf16). A-operand from LDS
//   (staged by op_lds.hpp); B-operand from registers.
//
// MFMA SHAPE (32x32x8 bf16, 1k variant)
//   One instr computes C[32x32] += A[32x8] . B[8x32] over a 64-lane wave. Each lane
//   supplies a v4h (4 bf16) slice of A and B, and holds 16 fp32 accumulators (v16f)
//   of C. Two MFMA "passes" of 4 bf16 cover the 8-deep K; ksteps walk the full
//   contraction.
//
// "TransposedC" REGISTER LAYOUT (the single most important fact)
//   C is laid out so each lane owns ONE M-row across all 16 registers, and the
//   register index walks the free (N for GEMM0, hdim for GEMM1) dimension:
//       m_row = (lane%32) + 32*warp                         [one row per lane]
//       free(r) = (r/8)*16 + (lane/32)*8 + (r%8),  r=0..15  [the other dim]
//   The MFMA maps each lane to a column of B, so choosing B = Q (GEMM0) / P (GEMM1)
//   is what makes "each lane owns one M-row" true — WHY this kernel uses the CK
//   convention A=LDS, B=register. softmax and the epilogue depend on this.
//
// k_sub PAIRING
//   k_sub = lane/32 splits the wave into two halves; lanes in the same half hold
//   the SAME 32 free-dim columns (halves are complementary). softmax later merges
//   them with one ds_bpermute (no butterfly).
//
// SWIZZLE
//   GEMM0 applies SwizzleA (swap bits 2,3 of the K column index, see swz()) reading
//   K from LDS, reordering the MFMA output into CK's groups-of-8 column pattern.
//   GEMM1 does NOT swizzle V.
//
// bf16 CASTS: all fp32->bf16 here TRUNCATE (drop low 16 bits), not RNE — matches CK.
...
// Applied to the K(A) column index in GEMM0 so the MFMA deposits S_acc in CK's
// groups-of-8 N-column pattern (see n_col in op_softmax.hpp). Pure index remap.
...
// (0..63), d = headdim (0..63). Odd strides (136, 544) are padding to keep
// consecutive ds_reads on distinct LDS banks. Returns bf16-element offset (*2 for
// bytes). Verified in Phase 0/1 K1/K2:
...
// matters within a 32-row sub-tile), d = headdim. 72 = kPaddedRowStride padding
// spreads ds_reads across banks. bf16-element offset. Verified in Phase 1 K5/K6:
...
// = 8 MFMA. Two calls (k_subtile_idx 0 then 1) accumulate the full 64-deep headdim
// contraction into s_acc_n0 / s_acc_n1.
//
// OPERANDS: K = A (LDS), Q = B (registers). B=Q per TransposedC so each lane owns
// one Q M-row of S_acc (see file header). SwizzleA: each lane reads K at column
// seqk = swz(lane & 31). The two N-tiles are the 32-wide halves of the 64-column
// score tile (seqk 0..31 and 32..63). Each kstep covers 16 hdim as two 4-bf16 MFMA
// passes (MFMA is 8-deep in K).
//
// Parameters:
...
// Pack P (fp32 softmax probs, accumulator layout) to bf16 for GEMM1's B-operand.
// P is B (not A) for the same TransposedC reason as Q in GEMM0: B=P makes each lane
// own one M-row of O_acc. Truncation via v_perm_b32 selector 0x07060302 (bytes 3,2
// of src0 + 7,6 of src1 = HIGH 16 bits of each fp32, no RNE), packing two fp32 per
// dword. Produces 4 v4h per N-tile half.
//
// (Standalone packer kept for reference; live GEMM1 below packs P inline so the
// compiler interleaves packing with MFMA co-execution.)
...
// swizzled), packs P to bf16 inline (B-operand), executes 8 MFMA (2 ksteps x 2
// passes x 2 hdim-tiles). Inline pack overlaps the v_perm VALU with MFMA.
//
// o_acc_d0 / o_acc_d1 are the two 32-wide hdim halves (0..31 and 32..63) of O, in
// the same TransposedC layout as GEMM0's S_acc.
...
// gemm0() is the pre-sub-tile GEMM0 (full 64-deep headdim in one call, old
// k_lds_offset layout, no SwizzleA). gemm1_bpermute() is the pre-MFMA GEMM1: a
// scalar FMA loop broadcasting P across lanes with ds_bpermute instead of the P.V
// MFMA. Both superseded by gemm0_subtile / gemm1_subtile above. `inline` so they
// emit no code while uncalled.
