...
__global__ void indexer_k_norm_rope_quant_and_cache_kernel(
    ...
    const bool is_neox)
{
    static_assert(HEAD_DIM == 128, "Indexer fused k cache currently supports head_dim=128");
    static_assert(ROPE_DIM == 64, "Indexer fused k cache currently supports rope_dim=64");
    ...
    float val = centered * inv_std * static_cast<float>(norm_weight[dim]) +
                static_cast<float>(norm_bias[dim]);
    // Match the unfused path: LayerNorm materializes a BF16/FP16 tensor before RoPE.
    val = static_cast<float>(static_cast<scalar_t>(val));
    normed[dim] = val;
    __syncthreads();
    ...
    if(dim < ROPE_DIM)
    {
        ...
        if(is_neox)
        {
            ...
            val = dim < HALF ? (val * cos_v - pair_val * sin_v)
                             : (val * cos_v + pair_val * sin_v);
        }
        else
        {
            ...
            const float cos_v = static_cast<float>(cos_ptr[cos_idx]);
            const float sin_v = static_cast<float>(sin_ptr[cos_idx]);
            val = (dim % 2 == 0) ? (val * cos_v - pair_val * sin_v)
                                 : (val * cos_v + pair_val * sin_v);
        }
        // The standalone RoPE kernel writes back to the BF16/FP16 k tensor before
        // indexer_k_quant_and_cache reads it.
        normed[dim] = static_cast<float>(static_cast<scalar_t>(val));
    }
    __syncthreads();

    auto max_func = [](float a, float b) { return fmaxf(a, b); };
    float amax = fabsf(normed[dim]);
    amax = block_reduce<float, decltype(max_func), HEAD_DIM, true>(amax, max_func);

    float scale =
        fmaxf(amax, 1e-4f) / static_cast<float>(opus::finfo<cache_t>::max());
    if(use_ue8m0)
    {
        scale = exp2f(ceilf(log2f(scale)));
    }
    ...
    kv_cache[dst_offset] = opus::cast<cache_t>(normed[dim] * inv_scale);
}
...
#define CALL_INDEXER_K_NORM_ROPE_QUANT_AND_CACHE(KV_T, CACHE_T, KV_DTYPE)                         \
    aiter::indexer_k_norm_rope_quant_and_cache_kernel<KV_T, CACHE_T, KV_DTYPE, 128, 64>           \
        <<<grid, block, 0, stream>>>(reinterpret_cast<KV_T*>(k.data_ptr()),                       \
                                     reinterpret_cast<CACHE_T*>(kv_cache.data_ptr()),             \
                                     reinterpret_cast<int64_t*>(slot_mapping.data_ptr()),          \
    ...
    bool is_neox)
{
    ...
    AITER_CHECK(k.device_id == slot_mapping.device_id,
                "k and slot_mapping must be on the same device");
    AITER_CHECK(k.device_id == norm_weight.device_id,
                "k and norm_weight must be on the same device");
    AITER_CHECK(k.device_id == norm_bias.device_id, "k and norm_bias must be on the same device");
