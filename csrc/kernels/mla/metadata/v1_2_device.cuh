                                  ...
                                  aiter_tensor_t& reduce_partial_map)
{
    ...
    const char* xcd_env       = std::getenv("AITER_MLA_XCD_LANE");
    params.xcd_lane_works     = (xcd_env != nullptr) ? (std::atoi(xcd_env) != 0)
                                                     : (natively_supported && xcd_multi_tile &&
                                                    (topk < 0) && (params.num_xcd > 1));
