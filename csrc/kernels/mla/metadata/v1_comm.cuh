...
struct MlaMetadataV1KernelParameter
{
    ...
    int32_t num_splits;
    bool is_causal;
    // round-robin context-parallel: when true, skip the per-qo-tile local-causal KV trim (batch_tail) so each work
    // spans the full local kv; the kernel masks on GLOBAL positions instead.
