...
{
    static_assert((NumWarps > 0) &&
                  ((NumWarps & (NumWarps - 1)) == 0)); // NumWarps should be power of 2
...
struct FlashMlaPrefillPolicy
{
    ...
    {
        constexpr int32_t BlockTileK =
            (Traits::kKVLoadOnce == false) ? Traits::kBlockK0 : KPerBlock;
        ...
        using TileGemmShape =
            ck_tile::TileGemmShape<BlockTile, BlockWarps, typename Traits::QKWarpTile>;

        using GemmProblem = ck_tile::BlockGemmProblem<InOutType,
                                                      InOutType,
                                                      AccType,
                                                      Traits::kNumThreadsGemm0,
                                                      TileGemmShape>;

        constexpr auto warp_gemm = []() {
            if constexpr(std::is_same_v<InOutType, ck_tile::fp16_t> &&
                         std::is_same_v<AccType, float>)
            {
            ...
            else if constexpr(std::is_same_v<InOutType, ck_tile::bf16_t> &&
                              std::is_same_v<AccType, float>)
            {
                if constexpr(Traits::kWarpGemm0M == 32)
                    return ck_tile::
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetKVBlockGemm()
    {
        ...
        using TileGemmShape =
            ck_tile::TileGemmShape<BlockTile, BlockWarps, typename Traits::KVWarpTile>;

        using GemmProblem = ck_tile::
            BlockGemmProblem<InOutType, InOutType, AccType, Traits::kNumThreads, TileGemmShape>;

        auto warp_gemm = []() {
            if constexpr(std::is_same_v<InOutType, ck_tile::fp8_t> &&
                         std::is_same_v<AccType, float>)
            {
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeKLdsBlockDescriptor()
    {
        ...
        constexpr auto k_lds_block_desc_0 = ck_tile::make_naive_tensor_descriptor(
            ...
            ck_tile::make_tuple(ck_tile::number<(kNPerBlock + 1) * kKPack>{},
                                ck_tile::number<kKPack>{},
                                ck_tile::number<1>{}),
            ck_tile::number<kKPack>{},
            ck_tile::number<1>{});
        ...
        constexpr auto k_lds_block_desc = ck_tile::transform_tensor_descriptor(
            ...
            ck_tile::make_tuple(
                ...
                ck_tile::make_merge_transform(
                    make_tuple(ck_tile::number<kKPerBlock / kKPack>{}, ck_tile::number<kKPack>{}))),
    ...
    MakeKLdsStoreBlockDescriptor(ck_tile::number<IBuf> = ck_tile::number<0>{})
    {
        ...
        if constexpr(KPerBlock == Traits::kSizeRope)
        {
            ...
            constexpr auto k_lds_block_desc_issues_warps_lanes =
                ck_tile::transform_tensor_descriptor(
                    k_lds_block_desc_0,
                    ck_tile::make_tuple(
                        ck_tile::make_pass_through_transform(ck_tile::number<NumIssues>{}),
                        ck_tile::make_pass_through_transform(ck_tile::number<NumWarps>{}),
                        ck_tile::make_merge_transform(
                            ck_tile::make_tuple(ck_tile::number<LaneGroups>{},
        ...
        else
        {
            constexpr auto k_lds_block_desc_0 = make_naive_tensor_descriptor_with_offset(
                ck_tile::make_tuple(ck_tile::number<NumIssues>{},  // n0
                                    ...
                                    ck_tile::number<KVector>{}),   // k1
                ck_tile::make_tuple(
                    ck_tile::number<NumWarps * kRepeatK*(warpSize * KVector + kPad)>{},
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeKLdsLoadBlockDescriptor()
    {
        ...
        {
            constexpr auto k_lds_block_desc_0 = ck_tile::make_naive_tensor_descriptor(
                ck_tile::make_tuple(ck_tile::number<Traits::kNumPrefetchK>{}, // num_buffers
        ...
        else
        {
            ...
            constexpr auto k_lds_block_desc = ck_tile::transform_tensor_descriptor(
                k_lds_block_desc_0,
                ck_tile::make_tuple(ck_tile::make_merge_transform(ck_tile::make_tuple(
                                        ck_tile::number<Traits::kNumPrefetchK>{},
                                        ck_tile::number<NumIssues>{},
                                        ck_tile::number<LaneGroups>{},
                                        ck_tile::number<NumWarps>{})),
                                    ck_tile::make_merge_transform(
                                        ck_tile::make_tuple(ck_tile::number<kRepeatK>{},
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeVLdsBlockDescriptor()
    {
        ...
        constexpr int32_t BufferSize =
            Traits::kKVLoadOnce ? GetSmemSizeSingleV() : GetSmemSizeSingleKV();
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeVLdsStoreBlockDescriptor()
    {
        ...
        constexpr auto k_lds_block_desc = transform_tensor_descriptor(
            ...
            ck_tile::make_tuple(
                ...
                make_merge_transform(ck_tile::make_tuple(ck_tile::number<kKPerBlock / kKPack>{},
                                                         ck_tile::number<kKPack>{}))),
            ck_tile::make_tuple(ck_tile::sequence<1>{}, ck_tile::sequence<0, 2>{}),
            ck_tile::make_tuple(ck_tile::sequence<0>{}, ck_tile::sequence<1>{}));
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeVLdsLoadBlockDescriptor()
    {
        ...
        constexpr auto k_lds_block_desc = ck_tile::transform_tensor_descriptor(
            v_lds_block_desc_0,
            ck_tile::make_tuple(
                ck_tile::make_merge_transform(ck_tile::make_tuple(ck_tile::number<NumIssues>{},
                                                                  ck_tile::number<LaneGroups>{},
                                                                  ck_tile::number<NumWarps>{})),
                ck_tile::make_merge_transform(ck_tile::make_tuple(
                    ck_tile::number<kKPerBlock / kKPack>{}, ck_tile::number<kKPack>{}))),
    ...
    {
        return Traits::kKVLoadOnce ? Traits::kNumPrefetchK * GetSmemSizeK() +
                                         Traits::kNumPrefetchV * GetSmemSizeSingleV()
                                   : Traits::kNumPrefetchKV * GetSmemSizeSingleKV();
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeQRegTileDistribution()
    {
        ...
        constexpr int32_t BlockTileK =
            (Traits::kKVLoadOnce == false) ? Traits::kBlockK0 : KPerBlock;
        return BlockGemm::template MakeABlockTileDistribution<Traits::kBlockM, BlockTileK>();
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetVTileDistributionStride()
    {
        ...
        constexpr int32_t VKVectors =
            VDstrEncode::hs_lengthss_[ck_tile::number<0>{}][ck_tile::number<3>{}];
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeVTileDistribution()
    {
        ...
        return ck_tile::make_static_tile_distribution(
            ck_tile::tile_distribution_encoding<
                ck_tile::sequence<>,
                ck_tile::tuple<ck_tile::sequence<kNRepeart, kNNumWarps, kLaneGroups, kNVector>,
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeVTTileDistribution()
    {
        ...
        return ck_tile::make_static_tile_distribution(
            ck_tile::tile_distribution_encoding<
                ck_tile::sequence<>,
                ck_tile::tuple<ck_tile::sequence<kKRepeart, kKNumWarps, kKLanes, kKVector>,
...
struct FlashMlaCombineKernelPolicy
{
    ...
    CK_TILE_DEVICE static constexpr auto MakeOutputTileDistribution()
    {
        constexpr int32_t kVectorN =
            GetVectorSizeForTile<Traits::kNumWarpsCombine, 1, Traits::kSizeDV, DataType>();
    ...
    public:
    CK_TILE_DEVICE static auto MakeOaccuTileWindow(void* p_output_accum,
                                                   const int32_t hsidx,
    ...
    CK_TILE_DEVICE static auto MakeOutputTileWindow(scalar_t* p_output)
    {
        const auto naive_view =
            ck_tile::make_naive_tensor_view<ck_tile::address_space_enum::global>(
                p_output,
