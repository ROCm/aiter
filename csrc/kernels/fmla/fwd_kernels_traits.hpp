...
struct FlashMlaPrefillKernelTrait
{
    ...
    static constexpr int32_t kBlockK1 =
        (kNumWarps == 8) ? kBlockN0 : 16; // TODO: make it tunable once slice tile issue is fixed.
    ...
    using QKWarpTile = std::
        conditional_t<kKVLoadOnce, ck_tile::sequence<16, 16, 32>, ck_tile::sequence<16, 16, 16>>;
    ...
    static constexpr int32_t Gemm0MaxRowWarps =
        kBlockN0 / kWarpGemm0N; // #warp in a row based on block size
    static constexpr int32_t Gemm0ResRowWarps =
        kNumWarps / Gemm0ColWarps; // residual #warp can be used to handle a row
