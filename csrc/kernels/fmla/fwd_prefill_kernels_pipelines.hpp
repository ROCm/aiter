...
// This function get the range of seqlen for the specified `split_idx`. `granularity` is the
// granularity of group of workload which cannot be further subdivded. The workload is divided as
// evenly as possible. When the workload cannot be evenly divided by num_splits, the high-ranking
// splits will get 1 additional `granularity` of tasks. E.g. when `num_seqlen` is `28`,
// `granularity` is `2` and `num_splits` is `3`, the 3 splits will be assigned the following tasks:
// split.0: [0, 10)  // 10 workloads
// split.1: [10, 20) // 10 workloads
// split.2: [20, 28) //  8 workloads
// split.3: [28, 36) // Note that this may not be what you're expecting. upper_bound may be helpful
// in this case.
CK_TILE_DEVICE static auto GetSeqlenRange(const int32_t num_seqlen,
...
CK_TILE_DEVICE static auto ShuffleSacc(const SaccType& s_acc, uint8_t* p_smem)
{
    ...
    if constexpr(RowWarps > 1)
    {
        ...
        auto p_shuffle_lds = ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
            reinterpret_cast<typename SaccType::DataType*>(p_smem),
            Policy::MakePShuffleLdsBlockDescriptor());
        auto p_st_lds_window =
            ck_tile::make_tile_window(p_shuffle_lds,
...
CK_TILE_DEVICE static auto GetValidatedMax(acc_t raw_m)
{
    ...
    {
        return (raw_m == -ck_tile::numeric<acc_t>::infinity()) ? ck_tile::type_convert<acc_t>(0.f)
                                                               : raw_m;
    }
    else
    {
...
template <typename Traits,
          ...
          typename Mask>
CK_TILE_DEVICE static inline void
kn_fmla_fwd_splitkv_prefill_tile_epilogue(LseDramBlockWindow& lse_dram_window_,
...
template <typename Traits,
          ...
          typename acc_t,
          typename out_t,
          bool kIsRopeSeparate,
          typename QNopeDramBlockWindow,
          typename QRopeDramBlockWindow,
          typename KNopeDramBlockWindow,
          typename KRopeDramBlockWindow,
          typename VDramBlockWindow,
          typename LseDramBlockWindow,
          typename OutDramBlockWindow,
          typename Mask>
CK_TILE_DEVICE static void
kn_fmla_fwd_splitkv_prefill_tile(const QNopeDramBlockWindow& q_nope_dram_window_,
                                 const QRopeDramBlockWindow& q_rope_dram_window_,
                                 const KNopeDramBlockWindow& k_nope_dram_window_,
                                 const KRopeDramBlockWindow& k_rope_dram_window_,
                                 ...
                                 uint8_t* p_smem)
{
    ...
    // 1. Allocate LDS
    //
    auto k_lds_st_windows = ck_tile::generate_tuple(
        [&](auto i_buf) {
            auto desc = Policy::template MakeKLdsStoreBlockDescriptor<Traits::kBlockK0, 1>(i_buf);
            auto view = ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
                reinterpret_cast<scalar_t*>(p_smem), desc);
            return ck_tile::make_tile_window(view, desc.get_lengths(), {0, 0, 0});
        },
        ck_tile::number<Traits::kNumPrefetchK>{});
    auto k_lds_ld_window = [&]() {
        auto desc = Policy::template MakeKLdsLoadBlockDescriptor<Traits::kBlockK0, 1>();
        auto view = ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
            reinterpret_cast<scalar_t*>(p_smem), desc);
        return ck_tile::make_tile_window(view, desc.get_lengths(), {0, 0});
    }();
    ...
    auto v_lds_window =
        ck_tile::make_tile_window(v_lds, Policy::MakeVLdsBlockDescriptor().get_lengths(), {0, 0});
    ...
    constexpr int32_t k0_nope_loops =
        kIsRopeSeparate ? Traits::kSizeNope / Traits::kBlockK0 : k0_loops;
    constexpr int32_t k0_rope_loops = Traits::kSizeRope / Traits::kBlockK0;
    ...
    using MLBlockTileType =
        decltype(block_reduce_2d(s_acc_shuffled, ck_tile::numeric<acc_t>::min(), reduce_max_func));
    ...
    const auto q_origin_m = q_nope_dram_window_.get_window_origin().at(ck_tile::number<0>{});
    auto [origin_start, origin_end] = mask.GetTileRangeAlongX(
        q_origin_m, ck_tile::number<Traits::kBlockM>{}, ck_tile::number<Traits::kBlockN0>{});
    auto [seqlen_k_start, seqlen_k_end] = GetSeqlenRange(seqlen_k,
                                                         Traits::kBlockN0,
                                                         num_splits,
                                                         split_id,
                                                         origin_start,
    ...
    auto q_dram_window_nope =
        ck_tile::make_tile_window(q_nope_dram_window_.get_bottom_tensor_view(),
    ...
    auto q_dram_window_rope =
        ck_tile::make_tile_window(q_rope_dram_window_.get_bottom_tensor_view(),
                                  q_rope_dram_window_.get_window_lengths(),
                                  q_rope_dram_window_.get_window_origin(),
                                  Policy::MakeQRegTileDistribution());
    using QTile = decltype(ck_tile::load_tile(q_dram_window_nope));
    QTile q_regs[2];

    // 5. Prepare KV
    //
    auto k_dram_window_nope_origin =
        ck_tile::make_tile_window(k_nope_dram_window_.get_bottom_tensor_view(),
                                  k_nope_dram_window_.get_window_lengths(),
                                  {seqlen_k_start, 0});
    ...
    auto k_dram_window_nope =
        ck_tile::make_tile_scatter_gather(k_dram_window_nope_origin.get_bottom_tensor_view(),
    ...
    auto k_dram_window_rope_origin =
        ck_tile::make_tile_window(k_rope_dram_window_.get_bottom_tensor_view(),
                                  k_rope_dram_window_.get_window_lengths(),
                                  {seqlen_k_start, 0});
    ...
    auto v_dram_window_origin =
        ck_tile::make_tile_window(v_dram_window_raw.get_bottom_tensor_view(),
    ...
    using VDramWindow =
        decltype(ck_tile::make_tile_scatter_gather(v_dram_window_origin.get_bottom_tensor_view(),
    ...
    ck_tile::static_for<0, n1_loops, 1>{}([&](auto n1_id) {
        ...
        v_dram_windows[n1_id] =
            ck_tile::make_tile_scatter_gather(v_dram_window_origin.get_bottom_tensor_view(),
                                              v_dram_window_origin.get_window_lengths(),
    ...
    for(int32_t loop_idx = 0; loop_idx < num_total_loop; loop_idx += 1)
    {
        ...
        if(Policy::HandleGemm0())
        {
            ...
            auto q_dram_window_nope =
                ck_tile::make_tile_window(q_nope_dram_window_.get_bottom_tensor_view(),
                                          q_nope_dram_window_.get_window_lengths(),
                                          q_nope_dram_window_.get_window_origin() + qk_origin_nope,
                                          Policy::MakeQRegTileDistribution());
            ...
            auto k_block_tile = ck_tile::load_tile(k_dram_window_nope);

            ck_tile::move_tile_window(k_dram_window_nope, qk_direction);
            ck_tile::store_tile(k_lds_window, k_block_tile);
            k_block_tile = ck_tile::load_tile(k_dram_window_nope);

            // TODO: we need to do Traits::kNumPrefetchK times prefetch here.
            ck_tile::async_load_tile_raw(k_lds_st_windows.at(ck_tile::number<0>{}),
                                         k_dram_window_nope,
                                         ck_tile::number<-1>{},
                                         k_oob_ck,
                                         k_pre_np);
            ck_tile::move_tile_window(k_dram_window_nope, qk_direction);
            ck_tile::async_load_tile_raw(k_lds_st_windows.at(ck_tile::number<1>{}),
                                         k_dram_window_nope,
                                         ck_tile::number<-1>{},
                                         k_oob_ck,
                                         k_pre_np);
            ...
            if constexpr(k0_nope_loops > 2)
            {
                ck_tile::static_for<0, k0_nope_loops - 2, 1>{}([&](auto k0_id) {
                    constexpr auto k_lds_id = k0_id % Traits::kNumPrefetchK;
                    ck_tile::block_sync_lds(); // TODO: do we need this only on even loop?
                    auto k_lds_sliced = ck_tile::get_slice_tile(
                        k_lds_ld_window,
                        ck_tile::sequence<k_lds_id * Traits::kBlockN0, 0>{},
                        ck_tile::sequence<(k_lds_id + 1) * Traits::kBlockN0, Traits::kBlockK0>{});
                    gemm_0(s_acc, q_regs[k0_id % 2], k_lds_sliced);
                    ck_tile::block_sync_lds(); // TODO: do we really need this?

                    // pre-load q
                    ck_tile::move_tile_window(q_dram_window_nope, qk_direction);
                    q_regs[k0_id % 2] = ck_tile::load_tile(q_dram_window_nope);

                    // pre-load k
                    ck_tile::move_tile_window(k_dram_window_nope, qk_direction);
                    ck_tile::async_load_tile_raw(k_lds_st_windows.at(k_lds_id),
                                                 k_dram_window_nope,
                                                 ck_tile::number<-1>{},
                                                 k_oob_ck,
                                                 k_pre_np);
            ...
            constexpr int32_t offset_neg_0 = (k0_nope_loops - 0) % Traits::kNumPrefetchK;
            constexpr int32_t offset_neg_1 = (k0_nope_loops - 1) % Traits::kNumPrefetchK;
            constexpr int32_t offset_neg_2 = (k0_nope_loops - 2) % Traits::kNumPrefetchK;

            auto k_lds_nope_sliced_0 = ck_tile::get_slice_tile(
                k_lds_ld_window,
                ck_tile::sequence<offset_neg_2 * Traits::kBlockN0, 0>{},
                ck_tile::sequence<offset_neg_1 * Traits::kBlockN0, Traits::kBlockK0>{});
            auto k_lds_nope_sliced_1 = ck_tile::get_slice_tile(
                k_lds_ld_window,
                ck_tile::sequence<offset_neg_1 * Traits::kBlockN0, 0>{},
                ck_tile::sequence<offset_neg_0 * Traits::kBlockN0, Traits::kBlockK0>{});

            if constexpr(!kIsRopeSeparate)
            {
                ck_tile::block_sync_lds();
                gemm_0(s_acc, q_regs[(k0_nope_loops - 2) % 2], k_lds_nope_sliced_0);
                ck_tile::block_sync_lds(); // TODO: do we still need this?
                gemm_0(s_acc, q_regs[(k0_nope_loops - 1) % 2], k_lds_nope_sliced_1);
            }
            else
            {
                ...
                ck_tile::async_load_tile_raw(
                    k_lds_st_windows.at(k0_nope_loops % Traits::kNumPrefetchK),
                    k_dram_window_rope,
                    ck_tile::number<-1>{},
                    k_oob_ck,
                    k_pre_np);
                ...
                if constexpr(k0_rope_loops > 2)
                {
                    ck_tile::static_for<k0_nope_loops, k0_loops - 2, 1>{}([&](auto k0_id) {
                        ck_tile::block_sync_lds();
                        gemm_0(s_acc, q_regs[k0_id % 2], k_lds_window);
                        ck_tile::block_sync_lds();
                        // pre-load q
                        ck_tile::move_tile_window(q_dram_window_rope, qk_direction);
                        q_regs[k0_id % 2] = ck_tile::load_tile(q_dram_window_rope);
                        // pre-load k
                        ck_tile::move_tile_window(k_dram_window_rope, qk_direction);
                        ck_tile::store_tile(k_lds_window, k_block_tile);
                        k_block_tile = ck_tile::load_tile(k_dram_window_rope);
                ...
                // Tailing 2 tiles of QK rope GEMM_0
                ck_tile::block_sync_lds();
        ...
        if(Policy::HandleGemm0())
        {
            ...
            const auto k_origin = k_dram_window_nope_origin.get_window_origin();
            const bool need_perpixel_check =
                mask.IsEdgeTile(__builtin_amdgcn_readfirstlane(q_origin_m),
            ...
            {
                ck_tile::set_tile_if(s_acc, -ck_tile::numeric<acc_t>::infinity(), [&](auto ids) {
                    const auto row = q_origin_m + ids.at(ck_tile::number<0>{});
                    const auto col =
                        k_origin.at(ck_tile::number<0>{}) + ids.at(ck_tile::number<1>{});
                    return mask.IsOutOfBound(row, col);
                });
            }
        }

        s_acc_shuffled = ShuffleSacc<Policy>(s_acc, p_smem + Policy::GetPSmemStart());

        // Get max of row
        auto m_local = block_reduce_2d(
            s_acc_shuffled, reduce_max_func.GetIdentityValue<acc_t>(), reduce_max_func);
        ...
        // Compute exp(x_i - m)
        auto p_intermedia =
            ck_tile::make_static_distributed_tensor<acc_t>(s_acc_shuffled.get_tile_distribution());
        const auto p_spans = decltype(p_intermedia)::get_distributed_spans();
        ck_tile::sweep_tile_span(p_spans[ck_tile::number<0>{}], [&](auto id0) {
            constexpr auto i = ck_tile::make_tuple(id0);
            auto row_max     = GetValidatedMax<Mask::IsMasking>(m[i]);
            ck_tile::sweep_tile_span(p_spans[ck_tile::number<1>{}], [&](auto id1) {
                constexpr auto ij = ck_tile::make_tuple(id0, id1);
#if FMLA_FWD_FAST_EXP2
        ...
        auto rowsum_p = block_reduce_2d(
            p_intermedia, reduce_sum_func.GetIdentityValue<acc_t>(), reduce_sum_func);
        block_reduce_2d_sync(rowsum_p, reduce_sum_func);

        // Calculate new l and adjust old output acc
        constexpr auto o_spans =
            ck_tile::remove_cvref_t<decltype(o_acc[0])>::get_distributed_spans();
        ck_tile::sweep_tile_span(o_spans[ck_tile::number<0>{}], [&](auto id0) {
            constexpr auto i   = ck_tile::make_tuple(id0);
        ...
        ck_tile::static_for<0, n1_loops, 1>{}([&](auto n1_id) {
            ...
            if constexpr(k1_loops > 1)
            {
                ck_tile::static_for<0, k1_loops - 1, 1>{}([&](auto k1_id) {
                    ...
                    ck_tile::static_for<0, kVNumRepeat, 1>{}([&](auto rid) {
                        const int32_t seqlen_idx = seqlen_v_base_idx + rid.value +
                                                   loop_idx * Traits::kBlockN0 +
                                                   (k1_id + 1) * Traits::kBlockK1;
                        const int32_t page_idx   = seqlen_idx / page_block_size;
                        const int32_t inside_idx = seqlen_idx % page_block_size;
                        v_offsets[rid] =
                            (p_block_table[page_idx] * page_block_size + inside_idx) * stride_s_v;
                        v_valids[rid] = (seqlen_idx < seqlen_k_end);
                    });
            ...
            if constexpr(n1_id < (n1_loops - 1))
            {
                ...
                ck_tile::set_tile_if(v_prefetch, ck_tile::numeric<scalar_t>::zero(), [&](auto ids) {
        ...
        if((loop_idx + 1) < num_total_loop)
        {
            ...
            ck_tile::array<int32_t, 2> next_qk_origin_rope = {
                0, is_even_loop ? Traits::kBlockK0 * (k0_rope_loops - 1) : 0};
            ck_tile::move_tile_window(k_dram_window_nope_origin, {Traits::kBlockN0, 0});
            k_dram_window_nope.set_window_origin(k_dram_window_nope_origin.get_window_origin() +
                                                 next_qk_origin_nope);
            // Recalculate offsets
            ck_tile::static_for<0, kKNumRepeat, 1>{}([&](auto rid) {
            ...
            if constexpr(kIsRopeSeparate)
            {
                ck_tile::move_tile_window(k_dram_window_rope_origin, {Traits::kBlockN0, 0});
                k_dram_window_rope.set_window_origin(k_dram_window_rope_origin.get_window_origin() +
                                                     next_qk_origin_rope);
            ...
            ck_tile::static_for<0, n1_loops, 1>{}([&](auto n1_id) {
                ...
                // (k1_loops - 1)});
                ck_tile::static_for<0, kVNumRepeat, 1>{}([&](auto rid) {
                    const int32_t seqlen_idx =
                        seqlen_v_base_idx + rid.value + (loop_idx + 1) * Traits::kBlockN0;
                    const int32_t page_idx   = seqlen_idx / page_block_size;
                    const int32_t inside_idx = seqlen_idx % page_block_size;
                    v_offsets[rid] =
                        (p_block_table[page_idx] * page_block_size + inside_idx) * stride_s_v;
    ...
    kn_fmla_fwd_splitkv_prefill_tile_epilogue<Traits, scalar_t, acc_t, out_t>(
        lse_dram_window_, out_dram_window_, o_acc, m, l, mask);
}
...
template <typename Traits,
          ...
          typename KNopeDramTileWindow,
          typename KRopeDramTileWindow,
          typename LseDramBlockWindow,
          typename OutDramBlockWindow,
          typename Mask>
CK_TILE_DEVICE static void
kn_fmla_fwd_splitkv_prefill_load_once_tile(const QNopeDramRegBlockWindow& q_nope_dram_window_,
                                           const QRopeDramRegBlockWindow& q_rope_dram_window_,
                                           const KNopeDramTileWindow k_nope_dram_window_,
                                           const KRopeDramTileWindow k_rope_dram_window_,
                                           LseDramBlockWindow& lse_dram_window_,
                                           OutDramBlockWindow& out_dram_window_,
                                           const int32_t* p_block_table,
                                           const int32_t page_block_size,
                                           ...
                                           uint8_t* p_smem)
{
    ...
    // 1. Allocate LDS
    //
    const auto k_rope_smem_offset =
        (Traits::kNumPrefetchK *
         Policy::template GetSingleKSpaceSize<Traits::kKNopeLdsBlkSize,
    ...
    auto k_nope_ld_lds_view = ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
        k_nope_lds_ptr,
        Policy::template MakeKLdsLoadBlockDescriptor<Traits::kKNopeLdsBlkSize,
                                                     Traits::kKNopeLdsIterations>());
    auto k_nope_ld_lds_window =
        ck_tile::make_tile_window(k_nope_ld_lds_view,
    ...
    auto k_rope_ld_lds_view = ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
        k_rope_lds_ptr, Policy::template MakeKLdsLoadBlockDescriptor<Traits::kSizeRope, 1>());
    auto k_rope_ld_lds_window =
        ck_tile::make_tile_window(k_rope_ld_lds_view,
                                  ck_tile::make_tuple(ck_tile::number<Traits::kBlockN0>{},
                                                      ck_tile::number<Traits::kSizeRope>{}),
                                  {0, 0});

    auto vt_lds_ptr =
        reinterpret_cast<scalar_t*>(p_smem + Traits::kNumPrefetchK * Policy::GetSmemSizeK());
    ...
    auto vt_st_lds_window = ck_tile::make_tile_window(
        ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(
            vt_lds_ptr, Policy::template MakeVLdsStoreBlockDescriptor<Traits::kSizeDV>()),
        Policy::template MakeVLdsStoreBlockDescriptor<Traits::kSizeDV>().get_lengths(),
        {0, 0});
    ...
    auto [origin_start, origin_end]     = mask.GetTileRangeAlongX(q_origin.at(ck_tile::number<0>{}),
                                                              ck_tile::number<Traits::kBlockM>{},
                                                              ck_tile::number<Traits::kBlockN0>{});
    auto [seqlen_k_start, seqlen_k_end] = GetSeqlenRange(seqlen_k,
                                                         Traits::kBlockN0,
    ...
    constexpr auto kKNopeNumRepeat =
        Policy::template GetNumRepeatOfKDramTileDistribution<Traits::kKNopeLdsBlkSize>();
    ...
    ck_tile::static_for<0, kKNopeNumRepeat, 1>{}([&](auto rid) {
        const int32_t seqlen_idx =
            seqlen_k_nope_base_idx + Traits::kBlockN0 / kKNopeNumRepeat * rid.value;
        const int32_t page_idx   = seqlen_idx / page_block_size;
        const int32_t inside_idx = seqlen_idx % page_block_size;
        k_nope_offsets[rid] =
            (p_block_table[page_idx] * page_block_size + inside_idx) * stride_s_k_nope;
        k_nope_valids[rid] = (seqlen_idx < seqlen_k_end);
    });
    auto k_nope_dram_window =
        ck_tile::make_tile_scatter_gather(k_nope_dram_window_raw.get_bottom_tensor_view(),
                                          k_nope_dram_window_raw.get_window_lengths(),
                                          k_nope_dram_window_raw.get_window_origin(),
                                          k_nope_dist,
                                          k_nope_offsets,
                                          k_nope_valids,
                                          kKPageIdxDim);
    ...
    ck_tile::static_for<0, Traits::kKNopeLdsIterations, 1>{}([&](auto rid) {
        ck_tile::async_load_tile_raw(k_nope_st_lds_windows.at(ck_tile::number<0>{}),
                                     k_nope_dram_window,
                                     ck_tile::number<-1>{},
                                     k_oob_ck,
                                     k_pre_np,
    ...
    const auto k_rope_origin_k = k_rope_dram_window_.get_window_origin().at(ck_tile::number<1>{});
    auto k_rope_dram_window_raw =
        ck_tile::make_tile_window(k_rope_dram_window_.get_bottom_tensor_view(),
                                  k_rope_dram_window_.get_window_lengths(),
                                  {seqlen_k_start, k_rope_origin_k});

    auto k_rope_dist  = Policy::template MakeKDramTileDistribution<Traits::kSizeRope>();
    auto k_rope_coord = k_rope_dist.calculate_index();
    constexpr auto kKRopeNumRepeat =
        Policy::template GetNumRepeatOfKDramTileDistribution<Traits::kSizeRope>();
    ...
    ck_tile::static_for<0, kKRopeNumRepeat, 1>{}([&](auto rid) {
        const int32_t seqlen_idx =
            seqlen_k_rope_base_idx + Traits::kBlockN0 / kKRopeNumRepeat * rid.value;
        const int32_t page_idx   = seqlen_idx / page_block_size;
        const int32_t inside_idx = seqlen_idx % page_block_size;
        k_rope_offsets[rid] =
            (p_block_table[page_idx] * page_block_size + inside_idx) * stride_s_k_rope;
        k_rope_valids[rid] = (seqlen_idx < seqlen_k_end);
    });
    auto k_rope_dram_window =
        ck_tile::make_tile_scatter_gather(k_rope_dram_window_raw.get_bottom_tensor_view(),
                                          k_rope_dram_window_raw.get_window_lengths(),
                                          k_rope_dram_window_raw.get_window_origin(),
                                          k_rope_dist,
                                          k_rope_offsets,
                                          k_rope_valids,
                                          kKPageIdxDim);
    ...
    ck_tile::async_load_tile_raw(k_rope_st_lds_windows(ck_tile::number<0>{}),
                                 k_rope_dram_window,
                                 ck_tile::number<-1>{},
                                 k_oob_ck,
                                 k_pre_np,
    ...
    auto main_loop = [&]<bool IsEvenLoop>(int32_t loop_idx) {
        ...
        constexpr auto k_ld_begin =
            IsEvenLoop ? ck_tile::number<0>{} : ck_tile::number<Traits::kBlockN0>{};
        constexpr auto k_ld_end = ck_tile::number<k_ld_begin + Traits::kBlockN0>{};
        constexpr ck_tile::array<int32_t, 2> v_lds_direction = {
            IsEvenLoop ? Traits::kBlockN0 : -Traits::kBlockN0, -Traits::kSizeNope};
        ...
        auto vt_tile =
            ck_tile::make_static_distributed_tensor<scalar_t>(Policy::MakeVTTileDistribution());
#pragma unroll 2
        for(int32_t vidx = 0; vidx < Traits::kKNopeLdsIterations; ++vidx)
        {
            ...
            ck_tile::static_for<0, VKRepeats, 1>{}([&](auto rid) {
                ...
                reinterpret_cast<uint32_t*>(vt_thread_buffer)[rid] =
                    __builtin_amdgcn_perm(reinterpret_cast<uint32_t*>(k_thread_buffer)[dw_offset_1],
                                          reinterpret_cast<uint32_t*>(k_thread_buffer)[dw_offset],
                                          s_perm1);
                reinterpret_cast<uint32_t*>(vt_thread_buffer)[vt_dw_offset] =
                    __builtin_amdgcn_perm(reinterpret_cast<uint32_t*>(k_thread_buffer)[dw_offset_1],
                                          reinterpret_cast<uint32_t*>(k_thread_buffer)[dw_offset],
                                          s_perm0);
            });
        ...
        if(Policy::HandleGemm0())
        {
            ...
            if constexpr(k0_loops > 1)
            {
                ...
                ck_tile::static_for<0, k0_loops, 1>{}([&](auto k0_id) {
                    ck_tile::block_sync_lds();
                    gemm_0(
                        s_acc,
                        ck_tile::get_slice_tile(
        ...
        const bool need_perpixel_check =
            mask.IsEdgeTile(__builtin_amdgcn_readfirstlane(q_origin.at(ck_tile::number<0>{})),
                            __builtin_amdgcn_readfirstlane(k_origin.at(ck_tile::number<0>{})),
                            ck_tile::number<Traits::kBlockM>{},
                            ck_tile::number<Traits::kBlockN0>{});
        ...
        if(loop_idx != num_total_loop - 1)
        {
            ...
            // Recalculate offsets
            ck_tile::static_for<0, kKNopeNumRepeat, 1>{}([&](auto rid) {
                const int32_t seqlen_idx = seqlen_k_nope_base_idx +
                                           (loop_idx + 1) * Traits::kBlockN0 +
            ...
            ck_tile::static_for<0, kKRopeNumRepeat, 1>{}([&](auto rid) {
                const int32_t seqlen_idx = seqlen_k_rope_base_idx +
                                           (loop_idx + 1) * Traits::kBlockN0 +
            ...
            ck_tile::static_for<0, 4, 1>{}([&](auto rid) {
                ck_tile::async_load_tile_raw(k_nope_st_lds_windows.at(k_st_idx{}),
                                             k_nope_dram_window,
                                             ck_tile::number<-1>{},
                                             k_oob_ck,
                                             k_pre_np,
            ...
            ck_tile::async_load_tile_raw(k_rope_st_lds_windows.at(k_st_idx{}),
                                         k_rope_dram_window,
                                         ck_tile::number<-1>{},
                                         k_oob_ck,
                                         k_pre_np,
        ...
        if constexpr(k1_loops == 1)
        {
            ...
            ck_tile::static_for<0, n1_loops, 1>{}([&](auto n1_id) {
                ck_tile::block_sync_lds();
                gemm_1(o_acc[n1_id], p, vt_ld_lds_window);
            });
        }
        else
        {
