...
#pragma unroll
    ...
    else
    {
        ...
        const int block_idx =
            (global_token_idx < context_len) ? global_token_idx / BLOCK_SIZE : last_ctx_block;
        ...
        if constexpr(ALIBI_ENABLED)
        {
            for(int h = 0; h < QHLOOP; h++)
            {
                const int qhead_idx = h * 4 + lane4id;
                alibi_slope[h] =
                    (qhead_idx < GQA_RATIO) ? alibi_slopes[wg_start_head_idx + qhead_idx] : 0.f;
            }
        }
...
#define QK_mfma(x)                                                                             \
    ...
    {                                                                                          \
        dout[h] =                                                                              \
            gcn_mfma4x4x4_instr<scalar_t, 4, x, 0>(Qlocal[h].xy[0], Klocal[x].xy[0], dout[h]); \
        dout[h] =                                                                              \
            gcn_mfma4x4x4_instr<scalar_t, 4, x, 0>(Qlocal[h].xy[1], Klocal[x].xy[1], dout[h]); \
    }
...
#undef QK_mfma
        ...
        for(int h = 0; h < QHLOOP; h++)
        {
            ...
            auto tmp =
                __builtin_amdgcn_ds_bpermute(bpermute_mask, *reinterpret_cast<int*>(&qk_max[h]));
        ...
        for(int h = 0; h < QHLOOP; h++)
        {
            ...
            auto tmp =
                __builtin_amdgcn_ds_bpermute(bpermute_mask, *reinterpret_cast<int*>(&exp_sum[h]));
...
#define SV_mfma(x)                                                                                \
    ...
    {                                                                                             \
        acc[qh] =                                                                                 \
            gcn_mfma4x4x4_instr<scalar_t, 4, 2 * x, 0>(logits[qh], Vlocal[vh][x].xy[0], acc[qh]); \
...
#undef SV_mfma
    ...
    if(warpid == 0)
    {
        ...
        scalar_t* out_ptr =
            out + seq_idx * num_heads * max_num_partitions * HEAD_SIZE + partition_idx * HEAD_SIZE;
