                ...
                int64_t reg_inp_ptr, int64_t reg_inp_bytes)
{
    ...
    // reg_inp_ptr == 0 (graph mode): input is IPC-registered, write-mode kernel
    // writes directly to peers' IPC output buffers. reg_inp_ptr != 0 (eager):
    // output not registered, kernel uses temp-buffer path.
                                             ...
                                             bool transpose_scale)
{
    ...
    // bf16_out_ptr: opaque ptr (0 = not requested). Non-zero -> fused kernel also
    // writes the pre-quant bf16/fp16 normed output, so GDN-style callers get an
    // unquantized view without a separate per-group quant kernel.
