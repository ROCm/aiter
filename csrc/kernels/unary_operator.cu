...
torch::Tensor binary_operation(torch::Tensor &input)
{
  int dim = input.dim();
  ...
  if (is_support)
  {
    ...
    VLLM_DISPATCH_FLOATING_TYPES(
        ...
        { vllm::unary_operator_tile_kernel<scalar_t, rows, vec, Operation>
...
{
  return binary_operation<vllm::SigmoidOp>(input);
}

torch::Tensor ater_tanh(torch::Tensor &input)
{
  return binary_operation<vllm::TanhOp>(input);
}
