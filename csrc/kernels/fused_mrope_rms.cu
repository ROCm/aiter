...
__global__ void fused_mrope_rms_neox_kernel(T* qkv,
                                            const T* q_w,
                                            const T* k_w,
                                            const T* cos_sin,
                                            const int64_t* positions,
                                            ...
                                            int64_t total_warps)
{
    ...
    auto qkv_ = qkv + token_id * (num_heads_q + num_heads_k + num_heads_v) * HEAD_SIZE +
                head_id_in_token * HEAD_SIZE;
...
template <typename T, int HEAD_SIZE, bool IS_MROPE, bool IS_INTERLEAVED, int M>
__global__ void fused_mrope_rms_noneox_kernel(T* qkv,
                                              const T* q_w,
                                              const T* k_w,
                                              const T* cos_sin,
                                              const int64_t* positions,
                                              ...
                                              int64_t total_warps)
{
    ...
    if constexpr(IS_MROPE)
    {
        mrope_load_cos_sin_vec<T, VEC_SIZE, HEAD_SIZE, IS_INTERLEAVED, M>(cos_vec,
                                                                          ...
                                                                          access_id_in_head / 2,
                                                                          mrope_section);
        mrope_load_cos_sin_vec<T, VEC_SIZE, HEAD_SIZE, IS_INTERLEAVED, M>(sin_vec,
                                                                          cos_sin,
                                                                          positions,
                                                                          ps0,
                                                                          ps1,
...
#define DISPATCH_NEOX(HEAD_SIZE, IS_INTERLEAVED)                             \
    if(is_neox_style)                                                        \
    {                                                                        \
        fused_mrope_rms_neox_kernel<T, HEAD_SIZE, true, IS_INTERLEAVED, M>   \
            <<<numBlocks, threadsPerBlock, 0, stream>>>(qkv,                 \
                                                        q_w,                 \
                                                        k_w,                 \
                                                        cos_sin,             \
    ...
    {                                                                        \
        fused_mrope_rms_noneox_kernel<T, HEAD_SIZE, true, IS_INTERLEAVED, M> \
            <<<numBlocks, threadsPerBlock, 0, stream>>>(qkv,                 \
                                                        q_w,                 \
                                                        k_w,                 \
                                                        cos_sin,             \
                        ...
                        double eps)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, qkv.scalar_type(), "fused_mrope_3d_rms", [&] {
        using T = KernelElementType<scalar_t>::type;
        rope_rms::fused_mrope_rms<T, 3>((T*)qkv.data_ptr<scalar_t>(),
                    ...
                    double eps)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, qkv.scalar_type(), "fused_rope_rms", [&] {
        using T = KernelElementType<scalar_t>::type;
        rope_rms::fused_rope_rms<T>((T*)qkv.data_ptr<scalar_t>(),
                                    (T*)qw.data_ptr<scalar_t>(),
                                    (T*)kw.data_ptr<scalar_t>(),
                                    (T*)cos_sin.data_ptr<scalar_t>(),
                                    positions.data_ptr<int64_t>(),
                           ...
                           int64_t x)
{
    ...
    auto qkv_dtype      = qkv.scalar_type();
    TORCH_CHECK(pos_strides.size() == 1);
    AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, qkv_dtype, "fused_rope_rms_set_kv", [&] {
        using T = KernelElementType<scalar_t>::type;
        if(kv_cache_dtype == qkv_dtype)
        {
            T* k_out_ptr =
            ...
            T* v_out_ptr =
                (return_kv && v_out.has_value()) ? (T*)v_out.value().data_ptr<scalar_t>() : nullptr;
            rope_rms::fused_rope_rms_set_kv<T, T>((T*)qkv.data_ptr<scalar_t>(),
                                                  (T*)qw.data_ptr<scalar_t>(),
                                                  (T*)kw.data_ptr<scalar_t>(),
                                                  (T*)cos_sin.data_ptr<scalar_t>(),
                                                  positions.data_ptr<int64_t>(),
