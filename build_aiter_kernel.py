...
SRC_BIG = Path("artemis_kernels/gdn_decode_m128_m256.py")
DST = Path(sys.argv[2] if len(sys.argv) > 2 else "fused_gdn_decode_qkvz_kernel.py")

text = SRC.read_text()
sha = hashlib.sha256(text.encode()).hexdigest()
big_text = SRC_BIG.read_text()
sha_big = hashlib.sha256(big_text.encode()).hexdigest()
...
start = next(i for i, line in enumerate(lines) if line.startswith("@gluon.jit"))
    ...
    if line.startswith("def gdn_fused_decode_group_fp8_quant")
)
...
), "quant epilogue lost its indentation"
assert (
    "\n        values = normalized.to(gl.float32)\n" not in body
), "quant epilogue is dead code"
...
sub("@gluon.jit\ndef _fused_decode(", "@gluon.jit(repr=_small_repr)\ndef _fused_decode(")
sub(
...
# --- large-batch band, from gdn_decode_m128_m256 -----------------------------
# A separate decomposition, not a tuning of the above: it keeps the whole [V, K]
# state tile in registers across both matrix-vector products. At batch 256 that
# reaches 75% of peak HBM bandwidth against this file's 65%, with identical byte
# counts -- see notes/gdn-perf-matrix.md. Shares no helpers with m16, so the two
# coexist without collision.
big_lines = big_text.splitlines()
b0 = next(i for i, line in enumerate(big_lines) if line.startswith("@gluon.jit"))
b1 = next(
    i
    for i, line in enumerate(big_lines)
    if line.startswith("def gdn_fused_decode_group_fp8_quant")
)
big = "\n".join(big_lines[b0:b1]).rstrip() + "\n"


def sub_big(old, new, count=1):
    global big
    found = big.count(old)
    assert found == count, f"expected {count} of {old!r} in big band, found {found}"
    big = big.replace(old, new)
...
sub_big("@gluon.jit\ndef _decode_group(", "@gluon.jit(repr=_group_repr)\ndef _decode_group(")
sub_big(
...
# The quant epilogue is the tail of _decode_group; guard it and indent.
marker = (
    "    # FP8 scales must consume the rounded BF16 output, not the FP32 precursor."
)
assert big.count(marker) == 1
head_big, tail_big = big.split(marker, 1)
tail_big = marker + tail_big
big = (
    head_big
    + "    if HAS_FP8:\n"
    + "\n".join(
        ("    " + ln) if ln.strip() else ln for ln in tail_big.rstrip().splitlines()
    )
    + "\n"
)
assert "\n        values = normalized.to(gl.float32)\n" in big, "epilogue not guarded"
...
guard and the optional FP8 epilogue. Do not hand-edit; see build_aiter_kernel.py.
  ...
  source: {SRC.name}
  sha256: {sha}
  source: {SRC_BIG.name}   (large-batch band)
  sha256: {sha_big}
"""
...
_small_repr = make_kernel_repr(
    "_fused_decode", ["BATCH", "INDEX64", "PAD_SLOT_ID", "HAS_FP8"]
)
_tiled_repr = make_kernel_repr(
    "_fused_decode_tiled", ["B32", "INDEX64", "PAD_SLOT_ID", "HAS_FP8"]
)
_group_repr = make_kernel_repr(
    "_decode_group", ["NW", "KL", "KS", "PAD_SLOT_ID", "HAS_FP8"]
)
...
print(
    f"wrote {DST} ({len((header + body).splitlines())} lines) from {SRC.name} sha256={sha[:16]}"
)
