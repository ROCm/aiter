...
BENCH_ROW_RE = re.compile(
    r"^\s*(?P<M>\d+)\s+\|\s+" r"(?P<mx_us>[\d.]+)\s+\|\s+" r"(?P<fly_us>[\d.]+)\s+\|"
)
...
def classify_kernel(name: str) -> str:
    """Categorize a kernel by name into gemm1 / gemm2 / sort.

    Classify by name, not by row position: the mxfp4 gemm1/gemm2 device rows
    can swap order within a block at large M, so a position-based split
    (last row = gemm1, second-last = gemm2) mislabels them. Both the HIP
    kernels (``...::gemm1::kernel``) and the FlyDSL kernels (``gemm1_kernel_0``)
    carry the gemm1/gemm2 token in their names.
    """
    if "gemm1" in name:
        return "gemm1"
    if "gemm2" in name:
...
def summarize_blocks(
    ...
    for block_idx, block in enumerate(blocks):
        ...
        gemm1_rows = [r for r in valid_rows if classify_kernel(r.name) == "gemm1"]
        gemm2_rows = [r for r in valid_rows if classify_kernel(r.name) == "gemm2"]
        sort_rows = [r for r in valid_rows if classify_kernel(r.name) == "sort"]
...
def compare_totals(rows: list[ComparisonRow]) -> list[TotalComparisonRow]:
    ...
    for pair in sorted(rows_by_pair):
        pair_rows = [row for row in rows_by_pair[pair] if row.name in CATEGORY_ORDER]
        if not pair_rows:
            continue
...
def compute_delta_contributions(
    rows: list[ComparisonRow],
) -> list[DeltaContributionRow]:
...
def format_comparison_kernel_names(
    row: ComparisonRow, lhs_label: str, rhs_label: str
) -> str:
    return (
        f"{lhs_label}={format_kernel_names(row.mx_kernel_names)}; "
        f"{rhs_label}={format_kernel_names(row.fly_kernel_names)}"
    )


def format_total_kernel_names(
    row: TotalComparisonRow, lhs_label: str, rhs_label: str
) -> str:
    return (
        f"{lhs_label}={format_kernel_names(row.mx_kernel_names)}; "
        f"{rhs_label}={format_kernel_names(row.fly_kernel_names)}"
...
def print_comparison_table(
    rows: Iterable[ComparisonRow], lhs_label: str, rhs_label: str
) -> None:
    rows = list(rows)
    ...
    for name in CATEGORY_ORDER:
        ...
        for row in table_rows:
            values = [
                ...
                format_comparison_kernel_names(row, lhs_label, rhs_label),
            ]
            body.append(values)
            widths = [max(width, len(value)) for width, value in zip(widths, values)]
    ...
    if totals:
        print()
        print_total_comparison_table(totals, lhs_label, rhs_label)

    contributions = compute_delta_contributions(rows)
    if contributions:
        print()
        print_delta_contribution_table(contributions, lhs_label)


def print_total_comparison_table(
    rows: Iterable[TotalComparisonRow], lhs_label: str, rhs_label: str
) -> None:
    headers = [
        "pair",
        "M",
    ...
    for row in rows:
        values = [
            ...
            format_num(row.delta_avg_time),
            f"{row.fly_over_mx:.2f}x",
            format_total_kernel_names(row, lhs_label, rhs_label),
        ]
        body.append(values)
        widths = [max(width, len(value)) for width, value in zip(widths, values)]
...
def print_delta_contribution_table(
    rows: Iterable[DeltaContributionRow], lhs_label: str
) -> None:
    headers = [
        "pair",
        "M",
        f"total_{lhs_label}_avg",
        "total_pct",
        "gemm1_pct",
        "gemm2_pct",
        "sort_pct",
...
def print_comparison_csv(
    ...
    writer.writerow(
        [
            ...
            "kernel_names",
            f"{lhs_label}_cnt",
            f"{lhs_label}_device_time_sum",
            f"{lhs_label}_avg_time",
            f"{rhs_label}_cnt",
            f"{rhs_label}_device_time_sum",
            f"{rhs_label}_avg_time",
            "delta_avg_time",
            f"{rhs_label}_over_{lhs_label}",
        ]
    )
    for row in rows:
        writer.writerow(
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=["mx", "fly"],
        help=(
            "operator labels for each timing block within a pair, in run order; "
            "accepts space- or comma-separated values, default: mx fly"
        ),
    )
    parser.add_argument(
        "--csv",
    ...
    lhs_label, rhs_label = labels
    if args.csv:
        print_comparison_csv(comparisons, lhs_label, rhs_label)
    else:
        print_comparison_table(comparisons, lhs_label, rhs_label)
